...
import { migrateUserSubagentMarkdown, migrateSubagentStateFile } from "@codez/shared/node";
import {
  parseAgentProfileFromMarkdown,
  type AgentProfile,
  type AgentProfileParseDiagnostic,
} from "@codez/core";
import type { Logger, PluginMetadata } from "@codez/contracts";
import {
  createAgentStateId,
...
interface LoadCodezAgentProfilesInput {
  logger?: Logger;
  storageRoot: string;
  workingDirectory: string;
}

interface LoadCodezAgentProfilesResult {
...
export async function loadCodezAgentProfiles(
  input: LoadCodezAgentProfilesInput,
): Promise<LoadCodezAgentProfilesResult> {
  ...
  const roots = [
    ...
    { path: join(input.workingDirectory, ".codez", "agents"), source: "project" as const },
  ];
...
function sanitizeProjectAgentProfile(profile: AgentProfile): AgentProfile {
  ...
  // 项目级 .codez/agents/*.md 是仓库内容，不能通过 frontmatter
