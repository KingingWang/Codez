...
import { openStartupSqliteSessionStore } from "@codez/adapters/storage";
import type { SessionInfo } from "@codez/contracts";
import type { ListCodezSessionsOptions, ResolveLatestSessionOptions } from "./app/types.js";
...
export async function listCodezSessions(
  options: ListCodezSessionsOptions = {},
): Promise<SessionInfo[]> {
  const ownsStore = options.sessionStore === undefined;
  let sessionStore = options.sessionStore;
  if (!sessionStore) {
