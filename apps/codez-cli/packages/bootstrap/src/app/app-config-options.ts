...
import { detectLocale, resolveLocale } from "@codez/i18n";
import type { RuntimeConfigPatch, SupportedLocale, UiLocale } from "@codez/contracts";
import type { CodezAppOptions } from "./types.js";

export function isMessageEnabled(env: NodeJS.ProcessEnv): boolean {
  return env.CODEZ_MESSAGE_ENABLED === "1" || env.CODEZ_MESSAGE_ENABLED === "true";
}

export function createConfigCliOverrides(options: CodezAppOptions): RuntimeConfigPatch | undefined {
...
function detectAppLocale(options: CodezAppOptions): SupportedLocale | undefined {
