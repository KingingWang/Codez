...
import type { PluginLoadOutcome } from "@codez/contracts";
...
import type {
  CodezApp,
  CodezAppOptions,
  CodezPluginSetResult,
  CodezPluginUninstallResult,
} from "./types.js";
...
function createPluginFacade(options: CreatePluginFacadeOptions): PluginFacade {
  ...
  return {
    listPlugins: async (): Promise<PluginLoadOutcome> => listCodezPlugins(commonOptions()),
    setPluginEnabled: async (
      plugin: string,
      enabled: boolean,
    ): Promise<CodezPluginSetResult> => {
    ...
    uninstallPlugin: async (plugin: string): Promise<CodezPluginUninstallResult> => {
      ...
      if (removed) {
        ...
        if (removed.marketplace === CODEZ_OFFICIAL_PLUGIN_MARKETPLACE) {
