...
} from "@codez/adapters/storage";
import { createNodeLoggerFactory } from "@codez/adapters/logging";
import { createConfig, resolvePath } from "@codez/adapters/config";
import {
  createNodeExecutionAdapter,
  resolveEffectiveBashShellSelection,
} from "@codez/adapters/exec";
import { createNodeFileSystemAdapter } from "@codez/adapters/fs";
import { createNodeWebFetchHttpClientAdapter } from "@codez/adapters/http";
import { createJimpImageProcessorAdapter } from "@codez/adapters/image";
import { createPopplerPdfDocumentAdapter } from "@codez/adapters/pdf";
import { createNodeSessionMailboxAdapter } from "@codez/adapters/mailbox";
import { createNodeContextSourceAdapter } from "@codez/adapters/context";
import { createNodeSkillAdapter } from "@codez/adapters/skills";
import { createMcpAdapter } from "@codez/adapters/mcp";
import {
  AgentRuntime,
  PermissionService,
...
import { createModelTelemetry } from "@codez/telemetry";
import {
...
import { isRemoteWorkspaceIdentity, resolveCodezRuntimeEnv } from "@codez/shared";
import {
...
import { resolveCodezCustomCommandPrompt } from "../custom-command-prompt.js";
import { resolveCodezBuiltinPromptCommand } from "../builtin-prompt-command.js";
import { collectDisabledPaths } from "../skill-command-overrides.js";
import { loadPluginAgentProfiles, loadCodezAgentProfiles } from "../subagents.js";
...
export async function createCodezApp(options: CodezAppOptions): Promise<CodezApp> {
  if (!options?.providerRegistry) {
    throw new Error("createCodezApp requires a Provider Registry");
  }
  ...
  const modelLogger = loggerFactory.createLogger("codez").child({
    ...traceContextToLogContext(traceContext),
  ...
  try {
    ...
    const modelIoDir = getModelIoDir(
      cliStorageRoot,
      resolveCodezRuntimeEnv(options.env ?? process.env) === "development",
    );
    const codezSubagentProfileOutcome = await loadCodezAgentProfiles({
      logger,
      storageRoot,
      workingDirectory,
    });
    const codezSubagentProfiles = codezSubagentProfileOutcome.profiles;
    ...
    const pluginSubagentProfiles = loadPluginAgentProfiles({
      ...
      reservedProfileNames: codezSubagentProfiles.map((profile) => profile.name),
      modelSelectionOverrides: codezSubagentProfileOutcome.pluginAgentModelSelectionOverrides,
    }).profiles;
    ...
    const subagentProfiles = [...codezSubagentProfiles, ...pluginSubagentProfiles];
    ...
    let { configuredMcpServers, runtimeConfig, untrustedProjectMcpServers } =
      resolveAppRuntimeConfig({
        ...
        builtInSubagentModelSelectionOverrides:
          codezSubagentProfileOutcome.builtInModelSelectionOverrides,
    ...
    startupTimer.mark("Codez runtime configuration resolved", {
      ...
      ...(options.workspaceHookReviewHost
        ? {
            emitReviewEvent: async (event) => {
              if (!runtime) throw new Error("Codez runtime is not initialized yet.");
    ...
    const sessionMailboxPort =
      ...
      (messageEnabled
        ? createNodeSessionMailboxAdapter({
            rootDir: resolvePath(
              (options.env ?? process.env).CODEZ_MAILBOX_ROOT ?? "~/.codez/mailbox",
            ),
          })
        : undefined);
    markStorageAdaptersInitialized({
    ...
    const inputFacade = createInputFacade({
      ...
      customCommandPromptResolver: async (text, resolverOptions) => {
        const builtinPrompt = resolveCodezBuiltinPromptCommand(text, {
          workingDirectory,
        });
        if (builtinPrompt !== undefined) {
          return builtinPrompt;
        }
        return await resolveCodezCustomCommandPrompt(text, {
      ...
      if (input.messageId && input.attachmentIndex !== undefined) {
        ...
        if (persistedAttachment?.type === "file") {
          ...
          artifactUri =
            ...
            (persistedAttachment.url.startsWith("codez-artifact://")
    ...
    return {
      ...
      readPromptAttachment: async (input) => {
        ...
        if (ref.startsWith("codez-artifact://")) {
      ...
      statPromptAttachment: async (input) => {
        ...
        if (artifactUri) {
          if (!artifactStore.statToolResultArtifact) {
            throw new CodezAttachmentFaultError(CODEZ_ATTACHMENT_FAULT_CODES.statUnsupported);
          }
        ...
        if (result.kind !== "file") {
          ...
          throw new CodezAttachmentFaultError(CODEZ_ATTACHMENT_FAULT_CODES.statNotFile);
        }
        return {
    ...
    startupTimer.fail("Codez app startup failed", error, {
