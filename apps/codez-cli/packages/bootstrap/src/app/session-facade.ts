import { updateUiLocaleInFileConfig, type ConfigResult } from "@codez/adapters/config";
import type { AgentRuntime } from "@codez/core";
import { resolveLocale } from "@codez/i18n";
import { normalizeModelSelection, type ModelSelection } from "@codez/provider";
import {
...
import type { PrepareUserExecutionBoundary, CodezApp } from "./types.js";
...
export function createSessionFacade(deps: CreateSessionFacadeDeps): SessionFacade {
  ...
  return {
    ...
    setTarget: async (input) =>
      (await setTargetStatus("set", input)) as Awaited<ReturnType<CodezApp["setTarget"]>>,
    updateTargetStatus: async (status) =>
      (await setTargetStatus("status_updated", { status })) as Awaited<
        ReturnType<CodezApp["updateTargetStatus"]>
      >,
