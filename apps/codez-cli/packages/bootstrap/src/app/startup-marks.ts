import type { ConfigResult } from "@codez/adapters/config";
import { discoverNodePluginsSync } from "@codez/adapters/plugins";
import type { AgentRuntimeConfig } from "@codez/core";
import type { Logger, McpServerConfig, PluginLoadOutcome } from "@codez/contracts";
...
import type { CodezAppOptions } from "./types.js";
...
export function resolveStartupPlugins(input: {
  ...
  logger?: Logger;
  options: Pick<CodezAppOptions, "officialPluginRoots" | "pluginStorageRoot">;
  ...
  const pluginOutcome = discoverNodePluginsSync({
    ...
    // (resolveCodezPlugins) 和应用启动路径都要传，否则 `/skill skill-creator` 在会话里
    // 报 "Skill not found: skill-creator"，但 `codez plugins list` 却显示它是 enabled。
    officialPluginsEnabledByDefault: DEFAULT_ENABLED_OFFICIAL_PLUGIN_IDS,
    storageRoot: pluginStorageRoot,
    workingDirectory: input.workingDirectory,
  });
  input.startupTimer.mark("Codez plugins resolved", {
    context: {
...
export function startAppStartup(input: {
  ...
  input.startupTimer.start("Codez app startup started", {
    context: {
...
export function markConfigurationLoaded(input: {
  ...
  input.startupTimer.mark("Codez app configuration loaded", {
    context: {
...
export function markStorageAdaptersInitialized(input: {
  ...
  input.startupTimer.mark("Codez storage adapters initialized", {
    context: {
...
export function markMcpAdapterInitialized(input: {
  ...
  input.startupTimer.mark("Codez MCP adapter initialized", {
    context: {
...
export function markRuntimeConstructed(input: {
  ...
  input.startupTimer.mark("Codez runtime constructed", {
    context: {
      hasInjectedModelAdapter: input.hasInjectedModelAdapter,
...
export function completeAppStartup(input: {
  ...
  input.startupTimer.complete("Codez app startup completed", {
    context: {
      sessionId: input.sessionId,
      workingDirectory: input.workingDirectory,
    },
