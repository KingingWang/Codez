import type { CodezToolExecResource, BackgroundBashOutputResult } from "@codez/shared";
import type { AiSdkModelAdapter } from "@codez/adapters/model";
import type {
  AgentRuntime,
  AgentRuntimeConfig,
  ExecuteTurnOptions,
...
import type { CodezModelOption } from "@codez/shared";
import type { EffectiveModelSelectionResult } from "@codez/shared/model-selection";
export type { CodezModelOption } from "@codez/shared";
import type { ModelProviderSourceTitle } from "../model-config.js";
import type { CodezInstalledPluginData } from "../plugins.js";
import type {
  AutomationPort,
  OffPeakPort,
...
import type { AgentTelemetryRuntimeOwner, WorkspaceHookPolicy } from "@codez/contracts";
...
export type CodezAppRuntimeConfigInput = AgentRuntimeConfig;

export interface CodezAppOptions {
  sessionId?: SessionId;
  resume?: boolean;
  version?: string;
  traceContext?: TraceContext;
  runtimeConfig?: CodezAppRuntimeConfigInput;
  /**
  ...
  onToolExecResource?: (sample: CodezToolExecResource) => void;
...
export type SubmitPromptOptions = SubmitPromptOptionsBase &
  import("@codez/contracts").TurnBackgroundAttribution;
...
export interface CodezPluginSetResult {
  enabled: boolean;
  path: string;
  plugin: PluginMetadata;
}

export interface CodezPluginUninstallResult {
  // null 表示该 plugin id 当前未安装（幂等 no-op），调用方据此提示"未安装"。
  removed: CodezInstalledPluginData | null;
}
...
export interface CodezApp {
  ...
  getCurrentModelOption?(): CodezModelOption | undefined;
  /** 只读 Registry 元数据；不要求选项完整，也不绑定执行模型。 */
  getModelOption?(selection: ModelSelection): CodezModelOption | undefined;
  ...
  readSubagents(input?: {
    endedCursor?: string;
    endedLimit?: number;
  }): Promise<import("@codez/shared").CodezSessionSubagentsResult>;
  ...
  listModels(): CodezModelOption[];
  listThoughtLevels(): string[];
  listPlugins(): Promise<PluginLoadOutcome>;
  setPluginEnabled(plugin: string, enabled: boolean): Promise<CodezPluginSetResult>;
  uninstallPlugin(plugin: string): Promise<CodezPluginUninstallResult>;
  /**
...
export interface RunCodezProtocolAgentOptions {
...
export interface ListCodezSessionsOptions {
  directory?: string;
  env?: NodeJS.ProcessEnv;
  limit?: number;
  sessionStore?: SessionStorePort;
