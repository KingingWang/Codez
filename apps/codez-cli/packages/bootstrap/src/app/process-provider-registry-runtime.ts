...
export interface ProcessProviderRegistryRuntimeOptions {
  ...
  readonly standalone?: {
    ...
    readonly onBuiltinRefreshResult?: (event: CodezBuiltinRefreshEvent) => void;
  };
}
...
export async function startProcessProviderRegistryRuntime(
  ...
  if (!paths) {
    throw new Error("缺少进程 Provider Registry 的 Codez Built-in / Personal Config 路径");
  }

  const accountSource = new MutableAccountProviderConfigSource();
  const credentialStore = options.standalone
    ? (options.standalone.credentialStore ?? createSharedCodezCredentialStore({ env: { ...env } }))
    : undefined;
  let standaloneAccount: AccountProviderService | undefined;
  const bundledFile = options.standalone
    ? env[CODEZ_BUILTIN_PROVIDER_BUNDLED_CONFIG_FILE_ENV]?.trim()
    : undefined;
  const runtime = new NodeProviderRegistryRuntime({
    ...paths,
    ...(bundledFile
      ? {
          codezBuiltinFilePath: bundledFile,
          codezBuiltinActiveFilePath: paths.codezBuiltinFilePath,
          codezBuiltinRemote: {
            controlFilePath: join(
              dirname(paths.codezBuiltinFilePath),
              "codez-builtin-refresh.json",
            ),
            resolveEndpointKey: () => resolveRuntimeCodezEndpointOrigin(env),
            fetchRelease: (endpointOrigin, signal) =>
              downloadCodezBuiltinRelease({
                endpointOrigin,
                signal,
                appVersion: CODEZ_VERSION,
                platform: resolveCodezBuiltinClientPlatform(),
      ...
      : {}),
    onCodezBuiltinRefreshError: options.standalone?.onBuiltinRefreshError,
    accountSource,
    ...(credentialStore
      ? {
  ...
  const disposeRecovery = standaloneAccount
    ? runtime.onDidCheckCodezBuiltin(async () => {
        const [config, account] = await Promise.all([
          runtime.configService.read(),
          standaloneAccount!.read(),
        ]);
        if (config.codezBuiltinRevision !== account.basedOnCodezBuiltinRevision)
...
export function parseProcessAccountProviderConfigSnapshot(input: {
  ...
  readonly basedOnCodezBuiltinRevision: string;
  ...
  const basedOnCodezBuiltinRevision = input.basedOnCodezBuiltinRevision.trim();
  if (!basedOnCodezBuiltinRevision) {
