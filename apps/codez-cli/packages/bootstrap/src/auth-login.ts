import {
  ...
  createSharedCodezCredentialStore,
  createCliOAuthClient,
  createCliOAuthPollToken,
  openUrlInBrowser,
  SHARED_CODEZ_CREDENTIAL_KEYS,
  type BrowserOpenResult,
  type SharedCodezCredentialStore,
  type CliOAuthClient,
...
import { createNodeHttpClientAdapter } from "@codez/adapters/http";
import type { EnvRecord } from "@codez/adapters/model";
import { buildCodezEndpointUrls, resolveRuntimeCodezEndpointOrigin } from "@codez/shared";
import {
...
export interface LoginCodezCliOptions {
  providerId?: CodingPlanProviderId;
  abortSignal?: AbortSignal;
  apiKeyResolver?: ReturnType<typeof createCodingPlanApiKeyResolver>;
  baseUrl?: string;
  credentialStore?: SharedCodezCredentialStore;
  env?: EnvRecord;
...
export interface LoginCodezCliResult {
...
export type LoginBigmodelCodingPlanOptions = Omit<LoginCodezCliOptions, "providerId">;
export type LoginBigmodelCodingPlanResult = LoginCodezCliResult & { providerId: "bigmodel" };
...
export interface LogoutCodezCliOptions {
  credentialStore?: SharedCodezCredentialStore;
  env?: EnvRecord;
}

export interface LogoutCodezCliResult {
  credentialsPath: string;
}
...
export async function hasConfiguredStandaloneCodingPlan(
  ...
  const credentialStore =
    options.credentialStore ?? createSharedCodezCredentialStore({ env: options.env });
  return hasStandaloneCodingPlanAccess(credentialStore, options.env ?? process.env);
}

export class CodezCliLoginError extends Error {
  readonly code:
    | "auth_failed"
    | "auth_timeout"
  ...
  constructor(
    code: CodezCliLoginError["code"],
    message: string,
    options: { cause?: unknown } = {},
  ) {
    super(message, options);
    this.name = "CodezCliLoginError";
    this.code = code;
  }
}

export async function loginCodezCli(
  options: LoginCodezCliOptions = {},
): Promise<LoginCodezCliResult> {
  ...
  const timeoutError = () =>
    new CodezCliLoginError("auth_timeout", "Authorization timed out. Please retry login.");
  ...
  try {
    ...
    const credentialStore = options.credentialStore ?? createSharedCodezCredentialStore({ env });
    ...
    const readyData = await pollUntilReady({
      ...
      createError: (code) =>
        code === "auth_timeout"
          ? timeoutError()
          : new CodezCliLoginError(code, "Authorization failed. Please retry login."),
    });
    ...
    try {
      if (providerId === "zai") {
        ...
        await credentialStore.saveMany({
          [SHARED_CODEZ_CREDENTIAL_KEYS.activeProvider]: providerId,
          [SHARED_CODEZ_CREDENTIAL_KEYS.codezJwtToken]: readyData.token,
          [SHARED_CODEZ_CREDENTIAL_KEYS.bigmodelAccessToken]: readyData.accessToken,
          ...(readyData.refreshToken
            ? { [SHARED_CODEZ_CREDENTIAL_KEYS.bigmodelRefreshToken]: readyData.refreshToken }
            : {}),
          [SHARED_CODEZ_CREDENTIAL_KEYS.bigmodelUserInfo]: JSON.stringify({
    ...
    try {
      ...
      throw new CodezCliLoginError(
        "config_update_failed",
        "Login succeeded but updating Codez config failed.",
        { cause: error },
      );
    }
    return {
...
export async function loginBigmodelCodingPlan(
  ...
  return {
    ...(await loginCodezCli({ ...options, providerId: "bigmodel" })),
    providerId: "bigmodel",
  };
}
...
export async function configureCodingPlanApiKey(
  ...
  if (!apiKey) {
    throw new CodezCliLoginError("config_update_failed", "API key must not be empty.");
  }
  const credentialStore =
    options.credentialStore ?? createSharedCodezCredentialStore({ env: options.env });
...
export async function logoutCodezCli(
  options: LogoutCodezCliOptions = {},
): Promise<LogoutCodezCliResult> {
  const credentialStore =
    options.credentialStore ?? createSharedCodezCredentialStore({ env: options.env });
  ...
  const keys = [
    ...Object.values(SHARED_CODEZ_CREDENTIAL_KEYS),
    ...identityKeys,
    ...dynamicApiKeyKeys,
  ];
...
async function persistStandaloneCodingPlanConnection(input: {
  ...
  readonly credentialStore: SharedCodezCredentialStore;
  ...
  const path =
    ...
    input.env[CODEZ_PERSONAL_PROVIDER_CONFIG_FILE_ENV]?.trim() ??
...
function createOAuthClient(options: LoginCodezCliOptions, env: EnvRecord): CliOAuthClient {
  return createCliOAuthClient({
    baseUrl:
      options.baseUrl ?? buildCodezEndpointUrls(resolveCliCodezEndpointOrigin(env)).apiBaseUrl,
    providerId: options.providerId ?? "zai",
    httpClient: options.httpClient ?? createDefaultHttpClient(env),
  });
}

function resolveCliCodezEndpointOrigin(env: EnvRecord): string {
  return resolveRuntimeCodezEndpointOrigin(env);
}
