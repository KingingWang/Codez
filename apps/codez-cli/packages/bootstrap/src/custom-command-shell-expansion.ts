...
function createShellExpansionEnv(input: {
  ...
  set.CODEZ_PROJECT_DIR = input.workingDirectory;
  if (input.sessionId) {
    set.CLAUDE_CODE_SESSION_ID = input.sessionId;
    set.CLAUDE_SESSION_ID = input.sessionId;
    set.CODEZ_SESSION_ID = input.sessionId;
  }
  if (input.plugin) {
    set.CLAUDE_PLUGIN_DATA = input.plugin.dataPath;
    set.CLAUDE_PLUGIN_ROOT = input.plugin.rootPath;
    set.CODEZ_PLUGIN_DATA = input.plugin.dataPath;
    set.CODEZ_PLUGIN_ID = input.plugin.id;
    set.CODEZ_PLUGIN_NAME = input.plugin.name;
    set.CODEZ_PLUGIN_ROOT = input.plugin.rootPath;
  }
...
function assertShellExpansionContextAvailable(input: {
  ...
  for (const match of input.shellCommand.matchAll(SHELL_CONTEXT_VARIABLE_PATTERN)) {
    const name = match[1];
    if (!name) continue;
    if (name === "CLAUDE_SKILL_DIR" || name === "CODEZ_SKILL_DIR") {
      throw new Error(
    ...
    if (
      ...
      (name === "CLAUDE_PLUGIN_DATA" ||
        ...
        name === "CODEZ_PLUGIN_DATA" ||
        name === "CODEZ_PLUGIN_ROOT")
    ) {
      throw new Error(
