...
import { getModelUsageTotalTokens, SessionEventType } from "@codez/contracts";
import { parseAutomationRunId } from "@codez/shared";
...
export function streamingParentToolCallId(payload: Record<string, unknown>): string | undefined {
  ...
  const codez = recordValue(meta.codez);
  return (
    ...
    optionalString(codez.parentToolCallId) ??
    optionalString(codez.parentToolUseId)
  );
}
