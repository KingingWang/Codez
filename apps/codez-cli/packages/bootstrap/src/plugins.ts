...
import { CODEZ_OFFICIAL_PLUGIN_MARKETPLACE, isOfficialMarketplaceId } from "@codez/contracts";
import { CODEZ_CUA_OFFICIAL_PLUGIN_ID, isCodezCuaInternalFeatureEnabled } from "@codez/shared";
...
export interface ResolveCodezPluginsOptions {
...
export interface ListCodezPluginsOptions extends ResolveCodezPluginsOptions {}

export interface SetCodezPluginEnabledOptions extends ResolveCodezPluginsOptions {
  enabled: boolean;
  plugin: string;
  scope?: "user" | "workspace";
}

export interface SetCodezPluginEnabledResult {
  enabled: boolean;
  path: string;
  plugin: PluginMetadata;
}

export interface CodezMarketplaceSummaryData {
  id: string;
  name: string;
...
export interface CodezAvailablePluginData {
  id: string;
  name: string;
  marketplace: string;
  description?: string;
...
export interface CodezInstalledPluginData {
  id: string;
  name: string;
  marketplace: string;
  description?: string;
...
export interface CodezPluginsOverviewData {
  marketplaces: CodezMarketplaceSummaryData[];
  availablePlugins: CodezAvailablePluginData[];
  installedPlugins: CodezInstalledPluginData[];
  restorableBuiltins: CodezAvailablePluginData[];
  diagnostics: PluginLoadOutcome["diagnostics"];
}

export interface CodezMarketplaceUpdateData {
  diagnostics: PluginLoadOutcome["diagnostics"];
  marketplaces: CodezMarketplaceSummaryData[];
}

export interface AddCodezMarketplaceOptions extends ResolveCodezPluginsOptions {
...
export interface RemoveCodezMarketplaceOptions extends ResolveCodezPluginsOptions {
  marketplace: string;
}

export interface UpdateCodezMarketplaceOptions extends ResolveCodezPluginsOptions {
  abortSignal?: AbortSignal;
  marketplace?: string;
}

export interface InstallCodezMarketplacePluginOptions extends ResolveCodezPluginsOptions {
  abortSignal?: AbortSignal;
  dryRun?: boolean;
  marketplace: string;
  pluginName: string;
...
export interface UninstallCodezMarketplacePluginOptions extends ResolveCodezPluginsOptions {
  pluginId?: string;
  pluginName?: string;
  marketplace?: string;
  removeCache?: boolean;
  /** 保留 data/<plugin-id> 用户数据目录（`codez plugins uninstall --keep-data`）。 */
  keepData?: boolean;
}

export interface UpdateCodezMarketplacePluginOptions extends ResolveCodezPluginsOptions {
  abortSignal?: AbortSignal;
  pluginId: string;
}

export interface ValidateCodezPluginPathOptions extends ResolveCodezPluginsOptions {
  abortSignal?: AbortSignal;
  path: string;
}

export interface CodezPluginUpdateData extends CodezPluginInstallData {
  previousVersion: string;
}

interface RestoreBuiltinPluginOptions extends ResolveCodezPluginsOptions {
  pluginId: string;
}

interface ConfigureCodezPluginOptions extends ResolveCodezPluginsOptions {
...
interface ResetCodezPluginConfigOptions extends ResolveCodezPluginsOptions {
  pluginId: string;
  scope?: "user" | "workspace";
}

interface ValidateCodezPluginOptions extends ResolveCodezPluginsOptions {
  marketplace?: string;
  pluginName?: string;
  source?: string;
}

interface DescribeCodezPluginOptions extends ResolveCodezPluginsOptions {
  marketplace: string;
  pluginName: string;
}

export interface CodezPluginInstallData {
...
function countVisibleMarketplacePlugins(
  ...
  if (marketplaceId !== CODEZ_OFFICIAL_PLUGIN_MARKETPLACE) return plugins.length;
  return plugins.filter((entry) => entry.name !== OFFICIAL_NODE_REPL_HOST_PLUGIN_NAME).length;
}

export function resolveCodezPlugins(options: ResolveCodezPluginsOptions = {}): PluginLoadOutcome {
...
export function getCodezPluginsOverview(
  options: ResolveCodezPluginsOptions = {},
): CodezPluginsOverviewData {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
  const outcome = resolveCodezPlugins({
    ...options,
    configResult,
    pluginStorageRoot,
  });
  ...
  // codez-plugins-official 的内置与 CDN 分片已在 adapter 层合并为唯一 canonical manifest。
  const catalogs: Array<{
    summary: CodezMarketplaceSummaryData;
  ...
  const restorableBuiltins: CodezAvailablePluginData[] = OFFICIAL_PLUGIN_DEFINITIONS.filter(
    (def) =>
      suppressed.has(`${def.name}@${CODEZ_OFFICIAL_PLUGIN_MARKETPLACE}`) &&
      // computer-use 的恢复入口需要 internal 特性开启（与 restoreBuiltinPluginCore 同口径）。
      (def.name !== "computer-use" || isCodezCuaInternalFeatureEnabled(options.env ?? process.env)),
  ).map((def) => {
    const listing = def.listing
      ? parseEntryStoreListing({ name: def.name, ...def.listing })
      : undefined;
    return {
      id: `${def.name}@${CODEZ_OFFICIAL_PLUGIN_MARKETPLACE}`,
      name: def.name,
...
export function listCodezPlugins(options: ListCodezPluginsOptions = {}): PluginLoadOutcome {
  const outcome = resolveCodezPlugins(options);
...
function loadPluginListingsById(storageRoot: string): Record<string, PluginStoreListing> {
  ...
  for (const definition of OFFICIAL_PLUGIN_DEFINITIONS) {
    ...
    if (listing) {
      listings.set(`${definition.name}@${CODEZ_OFFICIAL_PLUGIN_MARKETPLACE}`, listing);
    }
  }
...
export async function setCodezPluginEnabled(
  options: SetCodezPluginEnabledOptions,
): Promise<SetCodezPluginEnabledResult> {
...
export async function addCodezPluginMarketplace(
  options: AddCodezMarketplaceOptions,
): Promise<CodezMarketplaceSummaryData> {
  const { pluginStorageRoot } = resolvePluginContext(options);
...
export async function removeCodezPluginMarketplace(
  options: RemoveCodezMarketplaceOptions,
): Promise<void> {
  const { pluginStorageRoot } = resolvePluginContext(options);
...
export async function updateCodezPluginMarketplace(
  options: UpdateCodezMarketplaceOptions,
): Promise<CodezMarketplaceUpdateData> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
...
export async function installCodezMarketplacePlugin(
  options: InstallCodezMarketplacePluginOptions,
): Promise<CodezPluginInstallData> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
  if (options.dryRun === true) {
  ...
  const isSuppressedBundledOfficial =
    options.marketplace === CODEZ_OFFICIAL_PLUGIN_MARKETPLACE &&
  ...
  if (options.marketplace === CODEZ_OFFICIAL_PLUGIN_MARKETPLACE) {
...
export async function uninstallCodezMarketplacePlugin(
  options: UninstallCodezMarketplacePluginOptions,
): Promise<CodezInstalledPluginData | null> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  return withPluginStorageLock(pluginStorageRoot, async () => {
    const pluginId = resolvePluginIdForMutation(options);

    // 官方 CDN marketplace 与内置插件共享 codez-plugins-official id 空间，且其缓存
...
/**
 * `codez plugins update <plugin>`：先刷新所属 marketplace 目录，再按同一条目重装。
 * cacheMarketplacePlugin 对已存在的安装记录做原地覆盖并保留 installedAt；启用态只会给
 * 用户配置里尚未显式声明的 id 补默认值，因此更新不会改变用户已经做过的开关选择。
 */
export async function updateCodezMarketplacePlugin(
  options: UpdateCodezMarketplacePluginOptions,
): Promise<CodezPluginUpdateData> {
  const { pluginStorageRoot } = resolvePluginContext(options);
  ...
  const refreshed = await updateCodezPluginMarketplace({
    ...options,
  ...
  const installed = await installCodezMarketplacePlugin({
    ...options,
    marketplace: record.marketplace,
...
/** `codez plugins validate <path>`：只读校验本地插件目录或 marketplace 目录。 */
export async function validateCodezPluginPath(
  options: ValidateCodezPluginPathOptions,
...
async function restoreBuiltinPluginCore(options: RestoreBuiltinPluginOptions): Promise<void> {
  const codezCuaPluginId = CODEZ_CUA_OFFICIAL_PLUGIN_ID;
  if (
    options.pluginId === codezCuaPluginId &&
    !isCodezCuaInternalFeatureEnabled(options.env ?? process.env)
  ) {
    // overview 虽然隐藏了恢复入口，但协议调用仍可绕过 UI 写用户配置。
    // 功能开关关闭时在写盘前失败，确保用户配置与插件缓存都保持零痕迹。
    throw new Error("computer-use built-in plugin requires CODEZ_CUA_PRODUCT_HELPER to be enabled");
  }
...
export async function configureCodezPlugin(options: ConfigureCodezPluginOptions): Promise<void> {
...
export async function resetCodezPluginConfig(
  options: ResetCodezPluginConfigOptions,
...
export async function validateCodezPlugin(
  options: ValidateCodezPluginOptions,
): Promise<PluginLoadOutcome["diagnostics"]> {
  const { pluginStorageRoot } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
  if (options.source) {
...
export async function describeCodezPlugin(
  options: DescribeCodezPluginOptions,
...
function resolvePluginContext(options: ResolveCodezPluginsOptions): {
  configResult: ConfigResult;
  pluginStorageRoot: string;
  workingDirectory: string;
} {
...
function resolvePluginIdForMutation(options: UninstallCodezMarketplacePluginOptions): string {
...
function resolvePluginConfigPath(
  options: ResolveCodezPluginsOptions & { scope?: "user" | "workspace" },
  configResult: ConfigResult,
  workingDirectory: string,
): string {
  if (options.scope !== "workspace") {
  ...
  // Workspace Plugin 配置固定落在当前 `<workspace>/.codez/config.json`。嵌套 workspace
  // 可能同时发现仓库根与自身的配置，读取端 innermost 优先；写入端也必须锁定当前
  // workspace，不能用 project discovery 的第一个 outermost 文件。
  const workspaceConfigPath = join(workingDirectory, ".codez", "config.json");
