...
import {
  CODEZ_CLI_RESOURCE_SAMPLE_INTERVAL_MS,
  type CodezProcessResourceSample,
} from "@codez/shared";

/** 采样周期与 app 侧聚合共用 shared 的同一个常量，避免两侧节拍各自漂移。 */
const CODEZ_PROCESS_RESOURCE_SAMPLE_INTERVAL_MS = CODEZ_CLI_RESOURCE_SAMPLE_INTERVAL_MS;
...
interface CreateCodezProcessResourceSamplerOptions {
  /**
   * 第二个参数是本周期完整的内存快照，供进程内本地诊断日志使用；
   * 协议样本自身只带 rss 与 heapUsed 两项内存字段。
   */
  onSample(sample: CodezProcessResourceSample, memoryUsage: ProcessMemoryUsageSnapshot): void;
  platform?: CodezProcessResourceSample["platform"];
  arch?: CodezProcessResourceSample["arch"];
...
export interface CodezProcessResourceSampler {
  start(): void;
  stop(): void;
}
...
export function createCodezProcessResourceSampler(
  options: CreateCodezProcessResourceSamplerOptions,
): CodezProcessResourceSampler {
  const platform = options.platform ?? (process.platform as CodezProcessResourceSample["platform"]);
  const arch = options.arch ?? (process.arch as CodezProcessResourceSample["arch"]);
  ...
  const sample = (): void => {
    try {
      ...
      baseline = nextBaseline;
      const resourceSample: CodezProcessResourceSample = {
        platform,
        arch,
        logicalCpuCount,
        intervalMs,
  ...
  return {
    start() {
      ...
      try {
        timerHandle = timer.setInterval(sample, CODEZ_PROCESS_RESOURCE_SAMPLE_INTERVAL_MS);
      } catch {
