...
import { codezProtocolMessageSchema, codezProtocolMethods } from "@codez/shared";
import type { Logger } from "@codez/contracts";

type CodezProtocolOutgoingMessage =
  | CodezProtocolError
  | CodezProtocolNotification
  | CodezProtocolRequest
  | CodezProtocolResponse;

type CodezProtocolMessageHandler = (
  message: CodezProtocolMessage,
) => Promise<CodezProtocolOutgoingMessage | undefined>;

const PROTOCOL_EOF_DRAIN_MS = 100;

interface CodezProtocolNdjsonConnectionOptions {
  signal?: AbortSignal;
  clearPostResponseMessages?: () => void;
  handleMessage: CodezProtocolMessageHandler;
  ...
  ) => readonly CodezProtocolOutgoingMessage[];
  takePostResponseBatch?: (requestId: CodezProtocolRequestId) => {
    readonly messages: readonly CodezProtocolOutgoingMessage[];
    commit(): boolean;
  } | null;
}

export class CodezProtocolNdjsonConnection {
  private buffer = "";
  ...
  constructor(private readonly options: CodezProtocolNdjsonConnectionOptions) {
  ...
  send(message: CodezProtocolOutgoingMessage): void {
  ...
  private readonly onClose = (): void => {
    ...
    this.draining = true;
    this.notifyTransportClosed(new Error("Codez Protocol client connection closed"));
  ...
  private readonly onAbort = (): void => {
    this.notifyTransportClosed(new Error("Codez Protocol runtime stopping"));
    this.finish();
  };
  ...
  private decodeLine(line: string): CodezProtocolMessage | null {
    let decoded: unknown;
    try {
      decoded = JSON.parse(line);
    } catch (error) {
      this.options.logger?.warn("Codez Protocol JSON parse failed", {
        error: error instanceof Error ? error.message : String(error),
        event: "codez_protocol.parse.failed",
    ...
    const parsed = codezProtocolMessageSchema.safeParse(decoded);
    if (!parsed.success) {
      this.sendError("invalid-message", -32600, "Invalid Codez Protocol message", {
  ...
  private shouldBypassProcessingQueue(message: CodezProtocolMessage): boolean {
    // 模型任务占住串行队列时，停止/取消请求必须仍能进入 server，
    // 才能把底层 AbortSignal 传给真实模型请求。控制面只旁路当前执行，普通请求仍保持串行。
    return (
      "id" in message &&
      "method" in message &&
      (message.method === codezProtocolMethods.sessionStop ||
        message.method === codezProtocolMethods.workspaceCancelGenerateText)
    );
  }

  private async handleMessage(message: CodezProtocolMessage): Promise<void> {
    if (this.terminal) return;
