import type { Logger, McpPort } from "@codez/contracts";
import type { McpConnectionPool, McpTelemetryTracker } from "@codez/adapters/mcp";
import type { SqliteSessionStore } from "@codez/adapters/storage";
import { shutdownPreparedModelTelemetry } from "@codez/telemetry";
...
import type { CodezProtocolAgentServer } from "./server.js";
...
export async function cleanupProtocolRuntime(options: {
  logger: Logger;
  deadlineAt?: number;
  server?: Pick<CodezProtocolAgentServer, "shutdown" | "disposeProjections">;
  processResourceSampler?: Pick<CodezProcessResourceSampler, "stop">;
  ...
  const step = async (resource: string, cleanup: () => unknown | Promise<unknown>) => {
    ...
    try {
      ...
      options.logger.warn(`Codez Protocol ${resource} shutdown failed`, {
        errorType: error instanceof Error ? error.name : typeof error,
        event: `codez_protocol.${resource}.shutdown.failed`,
