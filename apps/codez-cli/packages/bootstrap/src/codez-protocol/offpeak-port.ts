import type { OffPeakPort, OffPeakTaskSummary } from "@codez/contracts";
import {
  codezOffPeakCreateResultSchema,
  codezOffPeakListResultSchema,
  codezProtocolMethods,
  type CodezOffPeakTaskProtocolSnapshot,
} from "@codez/shared";
import type {
  ...
  return {
    async create(input, createContext) {
      ...
      if (boundSessionId) {
        ...
        try {
          const listed = await context.requestClient(
            codezProtocolMethods.offPeakList,
            {},
            codezOffPeakListResultSchema,
          );
      ...
      const result = await context.requestClient(
        codezProtocolMethods.offPeakCreate,
        {
          title: input.title,
          prompt: input.prompt,
...
function toOffPeakTaskSummary(input: CodezOffPeakTaskProtocolSnapshot): OffPeakTaskSummary {
  return {
