...
import type { Logger, McpConnectionSnapshot, McpPort, McpServerStatus } from "@codez/contracts";
import {
  codezMcpListParamsSchema,
  codezMcpListResultSchema,
  type CodezMcpListResult,
} from "@codez/shared";
import {
...
export async function listMcpServers(
  ...
  rawParams: unknown,
): Promise<CodezMcpListResult> {
  const params = parseParams(codezMcpListParamsSchema, rawParams);
  ...
  const startupTimer = new StartupTimer(
    ...
    {
      module: "bootstrap.codez_protocol.mcp",
  ...
  const configuredMcpServers = {
    ...
    // 设置页的本地 MCP 列表由 desktop main 解析 `.codez` / `.agents` fallback，
  ...
  if (mcpPort) {
    ...
    if (pendingAuthorizationSnapshot) {
      ...
      return codezMcpListResultSchema.parse({
