...
import {
  ...
  codezPluginsReferenceCatalogParamsSchema,
  codezPluginsResolveSuggestedReferenceParamsSchema,
  type CodezPluginReferenceCatalogEntry,
  type CodezPluginDiagnostic as SharedPluginDiagnostic,
  type CodezPluginsReferenceCatalogResult,
  type CodezPluginsResolveSuggestedReferenceResult,
} from "@codez/shared";
import type { PluginReferenceCatalogEntry } from "@codez/contracts";
import { buildPluginReferenceCatalog } from "@codez/core";
import {
...
export async function getPluginReferenceCatalog(
  ...
  const params = parseParams(codezPluginsReferenceCatalogParamsSchema, rawParams);
  if (params.sessionId) {
...
const SUGGESTED_PLUGIN_MARKETPLACE = CODEZ_OFFICIAL_PLUGIN_MARKETPLACE_ID;
...
export async function resolveSuggestedPluginReference(
  ...
  rawParams: unknown,
  signal?: AbortSignal,
): Promise<CodezPluginsResolveSuggestedReferenceResult> {
  const params = parseParams(codezPluginsResolveSuggestedReferenceParamsSchema, rawParams);
  ...
  if (
    ...
    return unavailable(
      ...
      "推荐插件不是受信任的官方 codez-plugins-official 来源",
    );
  }

  const workingDirectory = params.workspace.workspacePath;
  const readState = () => {
    const outcome = resolveCodezPlugins({ logger: context.logger, workingDirectory });
  ...
  context.notify({
    method: codezProtocolNotifications.pluginOperationProgress,
  ...
  try {
    const refreshRequest = updateCodezPluginMarketplace({
    ...
    const refreshTimeoutRequest = new Promise<never>((_, reject) => {
      ...
      refreshTimeout = setTimeout(() => {
        ...
        const timeoutError = new Error("刷新 codez-plugins-official 超时（10000 ms）");
  ...
  const overview = getCodezPluginsOverview({ logger: context.logger, workingDirectory });
...
function resolveReferenceListingDisplayByPluginId(
  ...
  const overview = getCodezPluginsOverview({ workingDirectory: workspacePath });
...
function toReferenceCatalogEntry(
  ...
  includeCategory = false,
): CodezPluginReferenceCatalogEntry {
