...
import {
  createCodezProcessResourceSampler,
  type CodezProcessResourceSampler,
...
export function startProtocolResourceSampler(
  server: CodezProtocolAgentServer,
  send: (message: CodezProtocolNotification) => void,
  logger: Logger,
): CodezProcessResourceSampler | undefined {
  try {
    // 本地内存诊断日志：复用同一 60s 节拍，
    // 变化/心跳门控后才写一行，避免与消息流同频刷盘。
    const memoryDiagnosticsGate = createMemorySampleWriteGate();
    const sampler = createCodezProcessResourceSampler({
      onSample: (sample, memoryUsage) => {
        send({
          method: codezProtocolNotifications.processResourceSample,
          params: sample,
        });
        ...
        try {
          ...
          if (reason) {
            ...
            logger.info("Process memory sample", {
              event: "codez_protocol.process.memory_sample",
              reason,
              ...memoryFields,
              counters,
            });
