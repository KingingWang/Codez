...
import { workspaceHookPolicySchema } from "@codez/contracts";
import type { WorkspaceHookPolicyProvider } from "@codez/core";
import {
  codezWorkspaceHookTrustGrantParamsSchema,
  codezWorkspaceHookTrustGrantReasonCodeSchema,
  codezWorkspaceHookTrustGrantResultSchema,
  type CodezWorkspaceHookTrustGrantReasonCode,
  type CodezWorkspaceHookTrustGrantResult,
} from "@codez/shared";
import {
...
export async function grantWorkspaceHookTrustForProtocol(
  ...
  const params = codezWorkspaceHookTrustGrantParamsSchema.parse(rawParams);
  ...
  if (policyRejection) {
    return codezWorkspaceHookTrustGrantResultSchema.parse({
      accepted: false,
...
): CodezWorkspaceHookTrustGrantReasonCode | undefined {
  try {
...
function toPublicReasonCode(error: unknown): CodezWorkspaceHookTrustGrantReasonCode {
  const candidate = error instanceof Error ? error.message : error;
  const parsed = codezWorkspaceHookTrustGrantReasonCodeSchema.safeParse(candidate);
