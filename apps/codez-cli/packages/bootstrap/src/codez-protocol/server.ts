...
import {
  codezPluginsCancelOperationParamsSchema,
  codezProtocolMethods,
  codezWorkspaceCancelGenerateTextParamsSchema,
  codezWorkspaceHookTrustGrantParamsSchema,
} from "@codez/shared";
import type { BrowserControlPort } from "@codez/contracts";
import { InMemoryWorkspaceHookPolicyProvider } from "@codez/core";
import {
  V4_METHODS,
  V4_NOTIFICATIONS,
...
import type {
  CodezProtocolError,
  CodezProtocolMessage,
  CodezProtocolMethod,
  CodezProtocolNotification,
  CodezProtocolRequest,
  CodezProtocolRequestId,
  CodezProtocolResponse,
} from "@codez/shared";
import {
...
} from "../codez-protocol-v4/interaction-registry.js";
...
import {
  ...
  type CodezProtocolAgentDependencies,
  type CodezProtocolAgentServerContext,
  type CodezProtocolSessionRecord,
} from "./server-types.js";
import { createInMemorySessionEventStore } from "@codez/contracts";

export type { CodezProtocolAgentDependencies, CodezProtocolSessionRecord };

const MAX_CLIENT_REQUEST_REANNOUNCE_INTERVAL_MS = 10_000;

type CodezProtocolOutboundMessage = CodezProtocolNotification | CodezProtocolRequest;
...
async function notifyWorkspaceHookTrustGrantSessions(input: {
  ...
  sessions: Map<string, CodezProtocolSessionRecord>;
}): Promise<void> {
...
interface CodezProtocolPostResponseBatch {
  readonly messages: readonly CodezProtocolOutboundMessage[];
  commit(): boolean;
}
...
export class CodezProtocolAgentServer {
   ...
   */
  get officialMcpAuthRequestContext(): Pick<CodezProtocolAgentServerContext, "requestClient"> {
    return this.context;
  }

  private messageSink?: (message: CodezProtocolOutboundMessage) => void;
  private clientDisconnectError?: Error;
  private readonly context: CodezProtocolAgentServerContext;
  ...
  private readonly postResponseOutbox = new Map<
    CodezProtocolRequestId,
    CodezProtocolPostResponseBatch
  >();
  private nextClientRequestId = 1;

  constructor(deps: CodezProtocolAgentDependencies) {
    this.runtimeResources = new ProtocolRuntimeResources(deps.createCodezApp);
    const resolvedDeps = {
      ...deps,
      createCodezApp: this.runtimeResources.create,
      // 默认 turn 窗口保留策略。
    ...
    this.context = {
      ...
      sessions: new Map<string, CodezProtocolSessionRecord>(),
  ...
  setNotificationSink(sink: (message: CodezProtocolOutboundMessage) => void): void {
  ...
  shutdown(): Promise<void> {
    ...
    const error = new Error("Codez Protocol runtime stopping");
    ...
    for (const record of this.context.sessions.values()) {
      ...
      try {
        ...
        this.logger?.warn("Session unsubscribe failed during protocol shutdown", {
          event: "codez_protocol.session.unsubscribe.failed",
        });
      }
    }
  ...
  takePostResponseMessages(requestId: CodezProtocolRequestId): CodezProtocolOutboundMessage[] {
  ...
  takePostResponseBatch(requestId: CodezProtocolRequestId): CodezProtocolPostResponseBatch | null {
  ...
  async handleMessage(
    message: CodezProtocolMessage,
  ): Promise<CodezProtocolError | CodezProtocolResponse | undefined> {
    this.runtimeResources.assertServing();
    ...
    if (isNotification(message)) {
      this.logger?.debug("Codez Protocol notification ignored", {
        event: "codez_protocol.notification.ignored",
  ...
  private async handleRequest(
    request: CodezProtocolRequest,
  ): Promise<CodezProtocolError | CodezProtocolResponse> {
  ...
  private async dispatchRequest(request: CodezProtocolRequest) {
    switch (request.method) {
      ...
      case codezProtocolMethods.workspaceHookTrustGrant: {
        ...
        if (grantResult.accepted) {
          await notifyWorkspaceHookTrustGrantSessions({
            ...
            grantedWorkspaceKey: codezWorkspaceHookTrustGrantParamsSchema.safeParse(request.params)
              .success
              ? codezWorkspaceHookTrustGrantParamsSchema.parse(request.params).workspace
                  .workspaceKey
              : undefined,
      ...
      case codezProtocolMethods.providerUpdateAccountConfig:
        return await updateAccountProviderConfig(this.context, request.params);
      case codezProtocolMethods.workspaceUpdateInteractionPreferences:
  ...
  private cancelPluginOperation(rawParams: unknown) {
    const params = parseParams(codezPluginsCancelOperationParamsSchema, rawParams);
  ...
  private cancelWorkspaceGenerateText(rawParams: unknown) {
    const params = parseParams(codezWorkspaceCancelGenerateTextParamsSchema, rawParams);
  ...
  private ok(id: CodezProtocolRequestId, result: unknown): CodezProtocolResponse {
    return { id, result };
  }

  private fail(
    id: CodezProtocolRequestId,
    code: number,
    message: string,
    data?: unknown,
  ): CodezProtocolError {
  ...
  private requestClient<T>(
    method: CodezProtocolMethod,
    params: unknown,
    resultSchema: ParamsSchema<T>,
    options?: CodezProtocolClientRequestOptions,
  ): Promise<T> {
    if (this.clientDisconnectError) {
      throw this.clientDisconnectError;
    }
    if (!this.messageSink) {
      throw new ProtocolRequestError(-32020, `No Codez Protocol client is attached for ${method}`);
    }
  ...
  private resolveClientRequest(id: CodezProtocolRequestId, result: unknown): void {
    const key = String(id);
  ...
  private rejectClientRequest(id: CodezProtocolRequestId, error: Error): void {
    const key = String(id);
    const pending = this.pendingClientRequests.get(key);
    if (!pending) {
      return;
