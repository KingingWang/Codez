import {
  codezPluginsConfigureParamsSchema,
  codezPluginsResetConfigParamsSchema,
  codezPluginsInstallParamsSchema,
  codezPluginsMarketplaceAddParamsSchema,
  codezPluginsMarketplaceRemoveParamsSchema,
  codezPluginsMarketplaceUpdateParamsSchema,
  codezPluginsOverviewParamsSchema,
  codezPluginsListParamsSchema,
  codezPluginsSetEnabledParamsSchema,
  codezPluginsUninstallParamsSchema,
  codezPluginsUpdateParamsSchema,
  codezPluginsValidateParamsSchema,
  codezPluginsDescribeParamsSchema,
  codezPluginsRestoreBuiltinParamsSchema,
  type CodezAvailablePluginSummary,
  type CodezInstalledPluginSummary,
  type CodezPluginComponentGroup,
  type CodezPluginDiagnostic,
  type CodezPluginInfo,
  type CodezPluginMarketplaceSummary,
  type CodezPluginsConfigureResult,
  type CodezPluginsDescribeResult,
  type CodezPluginsInstallResult,
  type CodezPluginsListResult,
  type CodezPluginsMarketplaceMutationResult,
  type CodezPluginsOverviewResult,
  type CodezPluginsRestoreBuiltinResult,
  type CodezPluginsSetEnabledResult,
  type CodezPluginsUninstallResult,
  type CodezPluginsValidateResult,
} from "@codez/shared";
import type { PluginDiagnostic, PluginMetadata } from "@codez/contracts";
import {
  addCodezPluginMarketplace,
  configureCodezPlugin,
  describeCodezPlugin,
  getCodezPluginsOverview,
  installCodezMarketplacePlugin,
  removeCodezPluginMarketplace,
  resolveCodezPlugins,
  resetCodezPluginConfig,
...
import { listInstalledPluginRecords } from "@codez/adapters/plugins";
...
import { createConfig, resolvePath, type ConfigResult } from "@codez/adapters/config";
import { parseParams, type CodezProtocolAgentServerContext } from "./server-types.js";

// 把 CLI 的 PluginMetadata 投影成协议可序列化的 CodezPluginInfo (只保留 UI 需要的字段)。
function toPluginInfo(plugin: PluginMetadata, configResult?: ConfigResult): CodezPluginInfo {
...
function toPluginDiagnostic(diagnostic: PluginDiagnostic): CodezPluginDiagnostic {
  return {
...
export async function listPlugins(
  context: CodezProtocolAgentServerContext,
  rawParams: unknown,
): Promise<CodezPluginsListResult> {
  const params = parseParams(codezPluginsListParamsSchema, rawParams);
...
): Promise<CodezPluginsSetEnabledResult> {
  const params = parseParams(codezPluginsSetEnabledParamsSchema, rawParams);
  abortSignal?.throwIfAborted();
  const result = await setCodezPluginEnabled({
...
export async function getPluginsOverview(
  context: CodezProtocolAgentServerContext,
  rawParams: unknown,
): Promise<CodezPluginsOverviewResult> {
  const params = parseParams(codezPluginsOverviewParamsSchema, rawParams);
  const overview = getCodezPluginsOverview({
...
export async function addPluginMarketplace(
  ...
  const params = parseParams(codezPluginsMarketplaceAddParamsSchema, rawParams);
...
export async function removePluginMarketplace(
  ...
  const params = parseParams(codezPluginsMarketplaceRemoveParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
...
export async function updatePluginMarketplace(
  ...
  const params = parseParams(codezPluginsMarketplaceUpdateParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
...
export async function installPlugin(
  ...
  rawParams: unknown,
  abortSignal?: AbortSignal,
): Promise<CodezPluginsInstallResult> {
  const params = parseParams(codezPluginsInstallParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
  const result = await withPluginStorageLock(pluginStorageRoot, async () =>
    installCodezMarketplacePlugin({
      abortSignal,
...
export async function uninstallPlugin(
  context: CodezProtocolAgentServerContext,
  rawParams: unknown,
): Promise<CodezPluginsUninstallResult> {
  const params = parseParams(codezPluginsUninstallParamsSchema, rawParams);
  const removed = await uninstallCodezMarketplacePlugin({
    logger: context.logger,
    marketplace: params.marketplace,
...
export async function updatePlugin(
  ...
  const params = parseParams(codezPluginsUpdateParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
  ...
  return withPluginStorageLock(pluginStorageRoot, async () => {
    const installedPlugins: CodezInstalledPluginSummary[] = [];
    const dependencyClosure: string[] = [];
    // 聚合每条记录重装产生的诊断：installCodezMarketplacePlugin 失败时不抛错，而是返回
    // CLI 形态的 PluginDiagnostic（见其错误分支的 toMarketplaceInstallDiagnostic），
    // 这里逐条经协议侧 toPluginDiagnostic 投影成 CodezPluginDiagnostic 回传，
    // 让失败的重装显式暴露，而不是静默"成功"。
    const diagnostics: CodezPluginDiagnostic[] = [];
    for (const record of installed) {
      const result = await installCodezMarketplacePlugin({
        logger: context.logger,
...
export async function restoreBuiltinPlugin(
  ...
  const params = parseParams(codezPluginsRestoreBuiltinParamsSchema, rawParams);
...
export async function configurePlugin(
  ...
  const params = parseParams(codezPluginsConfigureParamsSchema, rawParams);
  await configureCodezPlugin({
...
export async function resetPluginConfig(
  ...
  const params = parseParams(codezPluginsResetConfigParamsSchema, rawParams);
  await resetCodezPluginConfig({
    logger: context.logger,
    pluginId: params.pluginId,
    scope: params.scope,
    workingDirectory: params.workspace.workspacePath,
...
export async function validatePlugin(
  context: CodezProtocolAgentServerContext,
  rawParams: unknown,
): Promise<CodezPluginsValidateResult> {
  const params = parseParams(codezPluginsValidateParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
...
export async function describePlugin(
  context: CodezProtocolAgentServerContext,
  rawParams: unknown,
): Promise<CodezPluginsDescribeResult> {
  const params = parseParams(codezPluginsDescribeParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
  const result = await withPluginStorageLock(pluginStorageRoot, async () =>
    describeCodezPlugin({
      logger: context.logger,
  ...
  const components: CodezPluginComponentGroup[] = result.components.map((group) => ({
    kind: group.kind,
    items: group.items.map((item) => ({
      name: item.name,
      ...(item.description ? { description: item.description } : {}),
...
function toMarketplaceSummary(input: {
  ...
  isOfficial?: boolean;
  featured?: string[];
  refreshFailure?: CodezPluginMarketplaceSummary["refreshFailure"];
...
function toAvailablePluginSummary(input: {
  ...
  listing?: CodezAvailablePluginSummary["listing"];
}): CodezAvailablePluginSummary {
  return {
    id: input.id,
    name: input.name,
...
function toInstalledPluginSummary(input: {
  ...
  installedAt?: string;
  componentTypes?: string[];
  hookDetails?: CodezInstalledPluginSummary["hookDetails"];
  updateStatus?: "none" | "update-available" | "version-changed";
  latestVersion?: string;
  listing?: CodezInstalledPluginSummary["listing"];
}): CodezInstalledPluginSummary {
  return {
    id: input.id,
    name: input.name,
    marketplace: input.marketplace,
