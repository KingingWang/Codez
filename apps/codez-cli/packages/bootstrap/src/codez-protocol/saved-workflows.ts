...
// 解析器与序列化器只从 @codez/core 取：这里不解析 frontmatter，也不拼 YAML。
//
// 全局作用域：五个方法的 params 收可选 `scope`（缺省
// `project`）。`global` 时改按本机全局根（`~/.codez/workflows/`）操作，`workspace` 只是**载体**——
// 处理器对全局档不读它的路径。`workflows/move` 把全局档搬回 `workspace` 项目（只此一向）。
import { unlink, writeFile } from "node:fs/promises";
import { SavedWorkflowMetaSchema, isValidSavedWorkflowName } from "@codez/contracts";
import {
  listSavedWorkflows,
  moveSavedWorkflow,
...
import {
  CODEZ_WORKFLOWS_RUNS_MAX_LIMIT,
  codezWorkflowsDeleteParamsSchema,
  codezWorkflowsGetParamsSchema,
  codezWorkflowsListParamsSchema,
  codezWorkflowsMoveParamsSchema,
  codezWorkflowsRunsParamsSchema,
  codezWorkflowsUpdateMetaParamsSchema,
  type CodezSavedWorkflowRun,
  type CodezSavedWorkflowScope,
  type CodezWorkflowsDeleteResult,
  type CodezWorkflowsGetResult,
  type CodezWorkflowsListResult,
  type CodezWorkflowsMoveResult,
  type CodezWorkflowsRunsResult,
  type CodezWorkflowsUpdateMetaResult,
} from "@codez/shared";
import type { JournalStorePort } from "@codez/dynamic-workflow";
...
function scopeOf(params: { scope?: CodezSavedWorkflowScope }): CodezSavedWorkflowScope {
...
export async function listSavedWorkflowsOp(
  ...
  rawParams: unknown,
): Promise<CodezWorkflowsListResult> {
  const params = parseParams(codezWorkflowsListParamsSchema, rawParams);
...
export async function getSavedWorkflowOp(
  _context: CodezProtocolAgentServerContext,
  rawParams: unknown,
): Promise<CodezWorkflowsGetResult> {
  const params = parseParams(codezWorkflowsGetParamsSchema, rawParams);
...
export async function updateSavedWorkflowMetaOp(
  ...
  const params = parseParams(codezWorkflowsUpdateMetaParamsSchema, rawParams);
...
export async function deleteSavedWorkflowOp(
  ...
  const params = parseParams(codezWorkflowsDeleteParamsSchema, rawParams);
...
export async function listSavedWorkflowRunsOp(
  ...
  const params = parseParams(codezWorkflowsRunsParamsSchema, rawParams);
  ...
  const limit = Math.min(CODEZ_WORKFLOWS_RUNS_MAX_LIMIT, params.limit);
  ...
  const runs: CodezSavedWorkflowRun[] = page.map((row) => ({
    runId: row.runId,
...
): Map<string, CodezSavedWorkflowRun["artifacts"]> {
  const byRun = new Map<string, CodezSavedWorkflowRun["artifacts"]>();
...
export async function moveSavedWorkflowOp(
  _context: CodezProtocolAgentServerContext,
  rawParams: unknown,
): Promise<CodezWorkflowsMoveResult> {
  const params = parseParams(codezWorkflowsMoveParamsSchema, rawParams);
