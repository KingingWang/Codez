import {
  ...
  type CodezActiveToolCall,
  type CodezContextUsageBreakdownItem,
  type CodezDeliveryKind,
  type CodezPendingPermission,
  type CodezSessionContextUsage,
  type CodezSessionEvent,
  type CodezSessionGoal,
  type CodezSessionGoalVerification,
  type CodezSessionGoalVerificationTimeline,
  type CodezSessionGoalStats,
  type CodezSessionInfo,
  type CodezSessionKind,
  type CodezSessionProjection,
  type CodezSessionRuntimeState,
  type CodezSessionSettingsState,
  type CodezSessionStateSnapshot,
  type CodezSessionTodoGroup,
...
export async function buildSessionSnapshot(input: {
  ...
  return {
    ...
    protocol: {
      name: CODEZ_PROTOCOL_NAME,
      version: CODEZ_PROTOCOL_VERSION,
    },
...
async function mapSnapshotMessages(
  app: Pick<CodezApp, "readToolResultArtifact">,
...
async function hydrateSnapshotFilePartUrl(
  ...
  // 历史图片附件持久化后只剩 codez-artifact:// 引用，UI/手机端不能直接渲染。
...
function snapshotFilePartArtifactUri(
  ...
  return artifactUri.startsWith("codez-artifact://") ? artifactUri : undefined;
}
...
export function mapSessionInfo(input: {
  app?: Pick<CodezApp, "getMode" | "getModel" | "sessionId" | "traceId">;
  fallbackCreatedAt?: number;
  fallbackUpdatedAt?: number;
  ...
  return {
    ...
    sessionId,
    sessionKind: (input.session?.taskType ?? input.taskType ?? "interactive") as CodezSessionKind,
...
export function mapSessionEvents(
  ...
  return events
    ...
    .filter((event): event is CodezSessionEvent => event !== null);
}
...
function mapModelNetworkStatusPayload(payload: unknown): Record<string, unknown> {
  const record = asRecord(payload);
  const apiRetry = codezApiRetryFromModelNetworkStatusPayload(record);
  if (apiRetry === undefined) {
    return record;
  }
  const meta = asRecord(record._meta);
  const codezMeta = asRecord(meta.codez);
  return {
    ...record,
    _meta: {
      ...meta,
...
function mapStreamRecoveryPayload(payload: unknown): Record<string, unknown> {
  const record = asRecord(payload);
  const apiRetry = codezApiRetryFromStreamRecoveryPayload(record);
  if (apiRetry === undefined) {
    return record;
  }
  const meta = asRecord(record._meta);
...
function mapSessionProjection(projection: SessionProjection): CodezSessionProjection {
  return {
...
function mapRuntimeState(input: {
  activeTurn?: ReturnType<CodezApp["runtime"]["getActiveTurnInfo"]>;
  deliveryKind?: CodezDeliveryKind;
  eventSeq: number;
  messages: MessageWithParts[];
...
export function resolveSessionContextUsage(input: {
  ...
  persistedContextUsageBreakdownEvents?: readonly SessionEvent[];
  projection: SessionProjection;
}): CodezSessionContextUsage | undefined {
...
): CodezSessionGoalVerificationTimeline[] {
...
function contextUsageFromProjection(
  projection: SessionProjection,
  cache: CodezSessionContextUsage["cache"] | undefined,
): CodezSessionContextUsage | undefined {
...
function contextCacheUsageFromMessages(
  messages: readonly MessageWithParts[],
): CodezSessionContextUsage["cache"] | undefined {
  let inputTokens = 0;
...
function mapPendingPermission(permission: PendingPermission): CodezPendingPermission {
  // display / optionsPolicy 刻意不进 legacy v3 输出。
  // 根因不是"扩 schema 只能单向兼容"，而是 strict schema 随 packages/shared 打进每个桌面端
  // 的产物：今天把 codezPendingPermissionSchema（shared/src/codez-protocol/index.ts:1139）和
  // codezPermissionRequestedEventPayloadSchema（同文件:1536）改成可选，也保护不了已经装出去
...
function mapPermissionRequestedPayload(payload: unknown): Record<string, unknown> {
  ...
  // 剔除，否则会直接漏进 strict 的 codezPermissionRequestedEventPayloadSchema。
...
function mapActiveToolCall(toolCall: ActiveToolCall): CodezActiveToolCall {
  return {
...
function mapSessionGoal(goal: SessionGoal | null | undefined): CodezSessionGoal | null | undefined {
...
function buildGoalStats(
  projection: SessionProjection,
  messages: readonly MessageWithParts[],
): CodezSessionGoalStats | undefined {
  const target = projection.target;
  if (!target) {
    return undefined;
  }
...
function buildTodoGroups(
  ...
  projection: SessionProjection,
): CodezSessionTodoGroup[] {
  const target = projection.target;
  const timeline = getTargetGoalVerificationTimeline(projection, target);
  const groups = new Map<string, CodezSessionTodoGroup>();
...
function ensureTodoGroup(
  groups: Map<string, CodezSessionTodoGroup>,
  input: {
  ...
  const group: CodezSessionTodoGroup = {
    id: input.groupId,
...
function mapSessionEventType(type: SessionEvent["type"]): CodezSessionEvent["type"] {
  switch (type) {
