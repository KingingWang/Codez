...
import {
  ...
  codezAutomationCheckTaskBindingResultSchema,
  codezAutomationCreateResultSchema,
  codezAutomationDeleteResultSchema,
  codezAutomationListResultSchema,
  codezAutomationUpdateResultSchema,
  codezProtocolMethods,
  type CodezAutomationProtocol,
} from "@codez/shared";
import {
...
export function createProtocolAutomationPort(
  ...
  resolveOwnSession?: () => CodezProtocolSessionRecord | undefined,
): AutomationPort {
  return {
    async create(input, createContext) {
        ...
        try {
          try {
            const result = await context.requestClient(
              codezProtocolMethods.automationCheckTaskBinding,
            ...
            const legacyResult = await context.requestClient(
              codezProtocolMethods.automationList,
              {},
              codezAutomationListResultSchema,
            );
      ...
      try {
        result = await context.requestClient(
          codezProtocolMethods.automationCreate,
          {
    ...
    async update(input) {
      const hasIntervalCarrier = input.intervalUnit !== undefined && input.interval !== undefined;
      const result = await context.requestClient(
        codezProtocolMethods.automationUpdate,
        {
    ...
    async delete(input) {
      const result = await context.requestClient(
        codezProtocolMethods.automationDelete,
        { automationId: input.id },
        codezAutomationDeleteResultSchema,
      );
...
function normalizeCronAutomationMode(
  mode: CodezAutomationProtocol["mode"],
): CronAutomation["mode"] {
  switch (mode) {
    case undefined:
      return undefined;
...
function toCronAutomation(input: CodezAutomationProtocol): CronAutomation {
  return {
