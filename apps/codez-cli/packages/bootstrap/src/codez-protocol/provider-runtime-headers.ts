...
import {
  ...
  codezProviderRuntimeHeadersResponseSchema,
  type CodezWorkspaceRef,
} from "@codez/shared";
import type { CodezAppOptions } from "../app/types.js";
import {
...
): NonNullable<CodezAppOptions["providerRuntimeHeadersPort"]> {
  return {
    ...
    async refreshBeforeModelRequest(input) {
      ...
      try {
        result = await context.requestClient(
          codezProtocolMethods.interactionRequestProviderRuntimeHeaders,
          {
        ...
        if (input.abortSignal?.aborted || timedOut) {
          context.notify({
            method: codezProtocolNotifications.providerRuntimeHeadersCancelled,
            trace: protocolTraceFromTraceContext(input.traceContext),
      ...
      // codez-plan 的账号鉴权材料是每次请求刷新的运行时配置，
