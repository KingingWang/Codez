...
import type { CodezSessionEndedSubagent, CodezSessionRunningSubagent } from "@codez/shared";
...
interface SessionSubagentProjection {
  revision: number;
  running: CodezSessionRunningSubagent[];
  ended: CodezSessionEndedSubagent[];
}
...
function runningStatus(input: {
  ...
  parentProjection?: SessionProjection;
}): CodezSessionRunningSubagent["status"] | undefined {
...
): CodezSessionEndedSubagent["status"] | undefined {
  switch (task?.status) {
    case "completed":
      return "success";
    case "cancelled":
...
function endedStatus(input: {
  ...
  childOutcome: ReturnType<typeof lastChildOutcome>;
  childProjection?: SessionProjection;
}): CodezSessionEndedSubagent["status"] {
...
export function projectSessionSubagents(
  ...
  const running: CodezSessionRunningSubagent[] = [];
  const ended: CodezSessionEndedSubagent[] = [];
...
function encodeCursor(item: CodezSessionEndedSubagent): string {
  return Buffer.from(
...
export function paginateEndedSubagents(
  ended: readonly CodezSessionEndedSubagent[],
  options: { cursor?: string; limit: number },
): { items: CodezSessionEndedSubagent[]; nextCursor?: string } {
