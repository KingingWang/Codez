/* eslint-disable max-lines -- Codez Protocol 的 session/workspace 方法共享同一个 server context 与 snapshot helpers，迁移期先集中维护。 */
import { observeSessionDebug } from "./session-debug.js";
import {
  TASK_LIST_SESSION_TYPES,
  isTaskListSessionType,
} from "../codez-protocol-v4/task-list-session-membership.js";
import { resolveEffectiveBashShellSelection } from "@codez/adapters/exec";
import { inputIntentMetadata } from "../codez-protocol-v4/commands/input-intent.js";
import { createModelExecutionContext } from "./model-execution.js";
import type { SendInputOptions } from "../app/types.js";
import { repairPersistedRemoteSessionPaths, type TurnAttachment } from "@codez/core";
import {
  CoreErrorType,
...
import {
  ...
  codezProtocolErrorCodes,
  codezProtocolMethods,
  codezSessionCancelBackgroundTaskParamsSchema,
  codezSessionCompactParamsSchema,
  codezSessionCloseParamsSchema,
  codezSessionCreateParamsSchema,
  codezSessionEventsParamsSchema,
  codezSessionForkParamsSchema,
  codezSessionGoalParamsSchema,
  codezSessionListParamsSchema,
  codezSessionMessagesParamsSchema,
  codezSessionReadParamsSchema,
  codezSessionRuntimePreferencesResultSchema,
  codezSessionResumeParamsSchema,
  codezSessionSendParamsSchema,
  codezSessionSetModeParamsSchema,
  codezSessionSetModelParamsSchema,
  codezSessionSetThoughtLevelParamsSchema,
  codezSessionStopParamsSchema,
  codezSessionSubscribeParamsSchema,
  codezSessionSubagentsParamsSchema,
  codezTaskTokenUsageParamsSchema,
  codezUsageStatsParamsSchema,
  ...
  type CodezSessionCreateParams,
  type CodezDeliveryKind,
  type IntegratedTerminalShellSelection,
  type CodezSessionRuntimePreferencesScope,
  type CodezSessionRuntimePreferencesResult,
  type CodezModelContextBudgetStrategy,
  type CodezProtocolTrace,
  type CodezSessionEvent,
  type CodezSessionHistoryTarget,
  type CodezSessionResumeParams,
  type CodezSessionPersistence,
...
import { runSessionModelConfigMutation } from "../codez-protocol-v4/model-config-mutation.js";
...
type ProtocolGoalTarget = NonNullable<
  Awaited<ReturnType<NonNullable<CodezProtocolSessionRecord["app"]["readTarget"]>>>
>;

type CodezSessionRecordParams = (
  | CodezSessionCreateParams
  | (CodezSessionResumeParams & {
      mode?: CodezSessionCreateParams["mode"];
      model?: CodezSessionCreateParams["model"];
      parentSessionId?: CodezSessionCreateParams["parentSessionId"];
      thoughtLevel?: CodezSessionCreateParams["thoughtLevel"];
      // 自动化会话会在创建期关闭标题二次生成，而 create/resume
      // 共用 record 初始化函数；resume 兼容分支也必须声明该策略字段。
      titleGenerationEnabled?: CodezSessionCreateParams["titleGenerationEnabled"];
    })
...
type SessionStartupPreferencesSource =
  | { kind: "host" }
  | { kind: "inherit"; parent: CodezProtocolSessionRecord };

function resolveSupportedAppThoughtLevel(
  app: Pick<CodezProtocolSessionRecord["app"], "listThoughtLevels">,
...
const liveProtocolStreamingDeltaBatches = new WeakMap<
  CodezProtocolSessionRecord,
  Map<string, LiveProtocolStreamingDeltaBatch>
>();

function protocolEventSequenceKey(deliveryKind?: CodezDeliveryKind): string {
...
function getProtocolToolInputTransmissionState(
  record: CodezProtocolSessionRecord,
  deliveryKind?: CodezDeliveryKind,
): CodezProtocolToolInputTransmissionState {
  const key = protocolEventSequenceKey(deliveryKind);
  const existing = record.protocolToolInputTransmissions.get(key);
  if (existing) {
    return existing;
  }
  const created: CodezProtocolToolInputTransmissionState = {
...
function readProtocolStreamingParentToolUseId(
  ...
  const codez = asRecord(meta.codez);
  return (
    stringValue(meta.parentToolUseId) ??
    stringValue(meta.parentToolCallId) ??
    stringValue(codez.parentToolUseId) ??
    stringValue(codez.parentToolCallId)
  );
}
...
function markStreamedToolInputIfPresent(
  mappedEvent: CodezSessionEvent,
  toolInputTransmissions: CodezProtocolToolInputTransmissionState,
): void {
...
function mapProtocolSessionEvent(
...
): CodezSessionEvent | null {
  if (shouldHideProtocolSessionEvent(record, event)) {
    return null;
  }
  if (!shouldExposeSessionEventToProtocol(event)) {
    const payload = asRecord(event.payload);
    context.logger?.debug("Codez Protocol session event filtered", {
      event: "codez_protocol.session_event.filtered",
  ...
  // tool_input_delta 等高频模型中间态。Codez Protocol 的 seq 是 UI/replay 的恢复键，
...
): Promise<CodezSessionEvent[]> {
  const events = await record.eventStore.getEvents(record.app.sessionId as SessionId);
  const visibleEvents: CodezSessionEvent[] = [];
  const toolInputTransmissions: CodezProtocolToolInputTransmissionState = {
    streamedToolCallIdsWithInput: new Set(),
  };
...
function logProtocolSessionEventSent(
  ...
  context.logger?.debug("Codez Protocol session event sent", {
    deliveryKind: mappedEvent.deliveryKind,
    event: "codez_protocol.session_event.sent",
...
function isImportedHistoryMessage(
  ...
  source: NonNullable<CodezSessionCreateParams["importedHistory"]>["source"],
): boolean {
...
async function removePreviousImportedSessionHistory(params: {
  sessionStore: NonNullable<CodezProtocolAgentServerContext["deps"]["sessionStore"]>;
  sessionId: SessionId;
  source: NonNullable<CodezSessionCreateParams["importedHistory"]>["source"];
}): Promise<number> {
...
async function persistImportedSessionHistory(params: {
  ...
  record: CodezProtocolSessionRecord;
  sessionId: SessionId;
  createParams: CodezSessionCreateParams;
}): Promise<void> {
  ...
  params.context.logger?.info("Codez Protocol imported history persisted", {
    event: "codez_protocol.session_import.persisted",
...
async function createSessionWithProjection<T>(
  context: CodezProtocolAgentServerContext,
  rawParams: unknown,
  trace: CodezProtocolTrace | undefined,
  project: (
    record: CodezProtocolSessionRecord,
  ) => Promise<{ value: T; phaseDurationsMs?: SnapshotPhaseDurationsMs; messageCount?: number }>,
): Promise<T> {
  const params = parseParams(codezSessionCreateParamsSchema, rawParams);
  ...
  context.logger?.info("Codez Protocol session/create started", {
    event: "codez_protocol.session_create.started",
  ...
  try {
    await runSessionModelConfigMutation(record.app, async () => {
      ...
      if (supportedInitialThoughtLevel) {
        ...
        context.logger?.warn("Codez Protocol session/create skipped unsupported thought level", {
          event: "codez_protocol.session_create.thought_level_skipped",
          module: "bootstrap.codez_protocol",
    ...
    if (params.importedHistory) {
      ...
      // 让导入结果从第一刻起就是可续聊、可切模型的 Codez session。
    ...
    context.logger?.info("Codez Protocol session/create completed", {
      durationMs: Date.now() - startedAt,
      event: "codez_protocol.session_create.completed",
    ...
    context.logger?.warn("Codez Protocol session/create failed", {
      durationMs: Date.now() - startedAt,
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "codez_protocol.session_create.failed",
      module: "bootstrap.codez_protocol",
      persistence: params.persistence,
      recordCreateDurationMs,
      rootTraceId: record.traceContext.traceId,
...
export async function activateSessionForResume(
  context: CodezProtocolAgentServerContext,
  params: CodezSessionResumeParams,
  ...
  context.logger?.debug("Codez Protocol session resume started", {
    activeBeforeWait,
    event: "codez_protocol.session.resume_started",
  ...
  if (!session) {
    ...
    context.logger?.warn("Codez Protocol session resume found no persisted record", {
      activeBeforeWait,
      activeSessionCount: context.sessions.size,
      durationMs: Math.max(0, Date.now() - resumeStartedAt),
      event: "codez_protocol.session.resume_persisted_missing",
      hasSessionStore: Boolean(context.deps.sessionStore),
      module: "bootstrap.codez_protocol",
      sessionId: params.sessionId,
    });
    throw new ProtocolRequestError(
      codezProtocolErrorCodes.sessionUnavailable,
  ...
  context.logger?.info("Codez Protocol session resume completed", {
    activeSessionCount: context.sessions.size,
    durationMs: Math.max(0, Date.now() - resumeStartedAt),
    event: "codez_protocol.session.resume_completed",
    module: "bootstrap.codez_protocol",
...
export async function resumeSession(context: CodezProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(codezSessionResumeParamsSchema, rawParams);
...
async function repairLegacyRemoteSessionWorkspaceForResume(
  ...
  if (!repairPersisted) {
    context.logger?.warn("legacy remote session workspace repair was not persisted", {
      event: "codez_protocol.session_resume.legacy_remote_workspace_repair_rejected",
      legacyWorkspaceDirectory,
  ...
  if (repaired) {
    context.logger?.info("legacy remote session workspace repaired", {
      event: "codez_protocol.session_resume.legacy_remote_workspace_repaired",
      legacyWorkspaceDirectory,
      legacyWorkspaceIdentity: legacyRemote.workspaceIdentity,
      module: "bootstrap.codez_protocol",
      sessionId: session.id,
...
export async function listSessions(context: CodezProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(codezSessionListParamsSchema, rawParams ?? {});
...
export async function listSessionSubagents(
  ...
  const params = parseParams(codezSessionSubagentsParamsSchema, rawParams ?? {});
  const store = context.deps.sessionStore;
  ...
  if (!parentSession) {
    ...
    context.logger?.warn("Codez Protocol session subagents has no persisted parent", {
      activeSessionCount: context.sessions.size,
      activeSession: Boolean(liveParent),
      event: "codez_protocol.session.persisted_missing",
      module: "bootstrap.codez_protocol",
...
export async function getUsageStats(context: CodezProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(codezUsageStatsParamsSchema, rawParams ?? {});
...
export async function readSession(context: CodezProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(codezSessionReadParamsSchema, rawParams);
...
export async function readMessages(context: CodezProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(codezSessionMessagesParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
export async function readEvents(context: CodezProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(codezSessionEventsParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  return {
...
export async function subscribeSession(
  ...
  const params = parseParams(codezSessionSubscribeParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
export async function sendPrompt(context: CodezProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(codezSessionSendParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  ...
  if (record.activeAbortController) {
    context.logger?.warn("Codez Protocol session/send rejected: active prompt exists", {
  ...
  context.logger?.info("Codez Protocol session/send accepted", {
...
export async function compactSession(context: CodezProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(codezSessionCompactParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
async function runCompactTurnInBackground(
  ...
  context.logger?.info("Codez Protocol background compact started", {
    inputId: params.inputId,
    sessionId: record.app.sessionId,
    workspacePath: record.workspace.workspacePath,
  });
  try {
    ...
    context.logger?.info("Codez Protocol background compact completed", {
      durationMs: Date.now() - startedAt,
      inputId: params.inputId,
      sessionId: record.app.sessionId,
      workspacePath: record.workspace.workspacePath,
    ...
    context.logger?.warn("Codez Protocol background compact failed", {
      durationMs: Date.now() - startedAt,
...
export async function goalSession(context: CodezProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(codezSessionGoalParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
  ...
  if (params.action === "pause") {
    ...
    if (target && activeAbortController) {
      context.logger?.info("Codez Protocol goal pause aborting active turn", {
        inputId: params.inputId,
        sessionId: params.sessionId,
        targetId: target.targetID,
        workspacePath: record.workspace.workspacePath,
      });
      activeAbortController.abort(new Error("Codez Protocol goal paused"));
    }
...
export async function forkSession(context: CodezProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(codezSessionForkParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
export async function registerForkedSession(
  context: CodezProtocolAgentServerContext,
  record: CodezProtocolSessionRecord,
  fork: Awaited<ReturnType<CodezProtocolSessionRecord["app"]["forkFromCheckpoint"]>>,
  options: {
    runtimeConfig: {
...
async function inheritForkedSessionTarget(
  context: CodezProtocolAgentServerContext,
  parentRecord: CodezProtocolSessionRecord,
  forkRecord: CodezProtocolSessionRecord,
...
async function runPromptTurnInBackground(
  ...
  const startedAt = Date.now();
  context.logger?.info("Codez Protocol background turn started", {
    inputId: params.inputId,
    queryId: params.queryId,
    sessionId: record.app.sessionId,
    textLength: params.content.length,
  ...
  try {
    ...
    context.logger?.info("Codez Protocol background turn completed", {
      durationMs: Date.now() - startedAt,
      inputId: params.inputId,
      queryId: params.queryId,
      sessionId: record.app.sessionId,
    ...
    context.logger?.warn("Codez Protocol background turn failed", {
      durationMs: Date.now() - startedAt,
      error: error instanceof Error ? error.message : String(error),
      inputId: params.inputId,
      sessionId: record.app.sessionId,
    ...
    if (record.activeAbortController === params.abortController) {
      ...
      record.activeAbortController = undefined;
      context.logger?.info("Codez Protocol background turn cleared active controller", {
        durationMs: Date.now() - startedAt,
        inputId: params.inputId,
        sessionId: record.app.sessionId,
        workspacePath: record.workspace.workspacePath,
...
export async function stopSession(context: CodezProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(codezSessionStopParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  const hadActivePrompt = Boolean(record.activeAbortController);
  let pausedTarget: ProtocolGoalTarget | null = null;
  context.logger?.info("Codez Protocol session/stop received", {
    hadActivePrompt,
    sessionId: params.sessionId,
    workspacePath: record.workspace.workspacePath,
  });
  ...
  record.activeAbortController?.abort(new Error("Codez Protocol session stopped"));
  if (pausedTarget) {
...
async function pauseActiveGoalForSessionStop(
  ...
  try {
    ...
    if (pausedTarget) {
      context.logger?.info("Codez Protocol session/stop paused active goal", {
        sessionId: params.sessionId,
    ...
    context.logger?.warn("Codez Protocol session/stop failed to pause active goal", {
      error: error instanceof Error ? error.message : String(error),
      sessionId: params.sessionId,
      targetId: target.targetID,
      workspacePath: record.workspace.workspacePath,
...
export async function cancelBackgroundTask(
  ...
  const params = parseParams(codezSessionCancelBackgroundTaskParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  context.logger?.info("Codez Protocol session/cancelBackgroundTask received", {
    sessionId: params.sessionId,
...
export async function setModel(context: CodezProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(codezSessionSetModelParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
  await runSessionModelConfigMutation(record.app, async () => {
...
export async function setThoughtLevel(
  ...
  const params = parseParams(codezSessionSetThoughtLevelParamsSchema, rawParams);
...
export async function setMode(context: CodezProtocolAgentServerContext, rawParams: unknown) {
...
export async function closeSession(context: CodezProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(codezSessionCloseParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
function shouldCloseSessionForExpectedPersistence(
  currentPersistence: CodezSessionPersistence,
  expectedPersistence?: CodezSessionPersistence,
): boolean {
...
export async function generateWorkspaceText(
  ...
  const params = parseParams(codezWorkspaceGenerateTextParamsSchema, rawParams);
  ...
  const app =
    active?.app ??
    (await createWorkspaceCodezApp(context, params.workspace, {
...
function ensureNoActiveTurn(record: CodezProtocolSessionRecord, message: string): void {
  if (!record.activeAbortController) {
    return;
  }
...
function assertExpectedProviderRevision(
  _context: CodezProtocolAgentServerContext,
  _record: CodezProtocolSessionRecord,
...
async function resolveForkTarget(
  context: CodezProtocolAgentServerContext,
  record: CodezProtocolSessionRecord,
  target: CodezSessionHistoryTarget,
...
export async function getTaskTokenUsage(
  ...
  const params = parseParams(codezTaskTokenUsageParamsSchema, rawParams ?? {});
  const usageStore = context.deps.sessionStore as Partial<UsageStorePort> | undefined;
 ...
 */
function reconcileRecordPersistence(record: CodezProtocolSessionRecord): void {
...
export function onSessionEvent(
  ...
  if (computerUseOperationEvent) {
    try {
      ...
      context.notify({
        method: codezProtocolMethods.computerUseOperationEvent,
      ...
      context.logger?.debug("Computer Use operation lifecycle notification sent", {
        event: "codez_protocol.computer-use.operation-event.sent",
      ...
      context.logger?.warn("Computer Use operation lifecycle notification failed", {
        error: error instanceof Error ? error.message : String(error),
        event: "codez_protocol.computer-use.operation-event.failed",
        eventId: computerUseOperationEvent.eventId,
        sessionId: computerUseOperationEvent.sessionId,
        turnId:
          "turnId" in computerUseOperationEvent ? computerUseOperationEvent.turnId : undefined,
...
async function requestSessionRuntimePreferences(
  ...
  const startedAt = Date.now();
  // CodezProtocolTrace.traceId 是协议层的 string；LogContext 需要 branded TraceId，
  // 按本文件既有惯例断言收口，避免多处日志构造重复转换。
  const traceId = trace?.traceId as TraceId | undefined;
  context.logger?.debug("Codez Protocol runtime preferences request started", {
    event: "codez_protocol.runtime_preferences.request_started",
    module: "bootstrap.codez_protocol",
    scope,
    sessionId,
    traceId,
  ...
  try {
    const result = await context.requestClient(
      codezProtocolMethods.sessionRequestRuntimePreferences,
      { sessionId, scope },
      codezSessionRuntimePreferencesResultSchema,
      {
    ...
    context.logger?.debug("Codez Protocol runtime preferences response received", {
      durationMs: Math.max(0, Date.now() - startedAt),
      event: "codez_protocol.runtime_preferences.response_received",
      module: "bootstrap.codez_protocol",
      scope,
      sessionId,
      traceId,
    ...
    const diagnostic = {
      ...
      errorCode,
      errorMessage,
      event: "codez_protocol.runtime_preferences.request_failed",
      module: "bootstrap.codez_protocol",
      scope,
      sessionId,
      traceId,
    ...
    if (errorCode === -32022) {
      ...
      context.logger?.warn("Codez Protocol runtime preferences request timed out", diagnostic);
    } else if (errorCode !== -32601 && errorCode !== -32020) {
      context.logger?.warn("Codez Protocol runtime preferences request failed", diagnostic);
    } else {
      context.logger?.debug(
        "Codez Protocol runtime preferences compatibility fallback",
        diagnostic,
      );
    }
...
async function materializeSessionRecord(
  context: CodezProtocolAgentServerContext,
  params: CodezSessionRecordParams,
  sessionId: SessionId,
  resume: boolean,
  source: SessionStartupPreferencesSource,
  trace?: CodezProtocolTrace,
): Promise<CodezProtocolSessionRecord> {
...
async function createRecord(
  ...
  context.logger?.info("Codez Protocol createRecord MCP config", {
    event: "codez_protocol.create_record.mcp_config",
  ...
  const app = await createWorkspaceCodezApp(context, workspace, {
    ...
    runtimeConfig: {
      ...
      // Codez Protocol app-server 创建的 session（desktop/web/mobile）没有传，导致
    ...
    // Codez Protocol app-server 以前没有注入可等待的交互 broker，
...
): CodezSessionCreateParams["mode"] | undefined {
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    const info = messages[index]?.info;
    if (info?.role === "assistant" && isCodezSessionMode(info.mode)) return info.mode;
  }
  return undefined;
}

function isCodezSessionMode(
  value: unknown,
): value is NonNullable<CodezSessionCreateParams["mode"]> {
  return (
    value === "plan" ||
    value === "build" ||
    value === "edit" ||
...
async function snapshotWithDiagnostics(
  ...
  if (totalDurationMs >= SLOW_SNAPSHOT_LOG_THRESHOLD_MS) {
    ...
    context.logger?.warn("Codez Protocol session snapshot slow", {
      durationMs: totalDurationMs,
      event: "codez_protocol.session_snapshot.slow",
...
async function readSnapshotTarget(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted session goal for protocol snapshot", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "codez_protocol.snapshot.target_read_failed",
      sessionId: record.app.sessionId,
    });
    return undefined;
  }
...
async function readSnapshotTodos(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted session todos for protocol snapshot", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "codez_protocol.snapshot.todos_read_failed",
      sessionId: record.app.sessionId,
    });
    return [];
  }
...
async function readSessionTargetCompletionVerificationEvents(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted goal verification entries", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "codez_protocol.snapshot.goal_verification_entries_read_failed",
      sessionId: record.app.sessionId,
    });
    return [];
  }
  ...
  return await repairPersistedRemoteSessionPaths(sessionStore, session, {
    onPersistenceFailure: (error) => {
      context.logger?.warn("Session path repair persistence failed; using in-memory repair", {
        errorMessage: error instanceof Error ? error.message : String(error),
        event: "codez_protocol.session.path_repair.persist_failed",
        sessionId,
      });
    },
  });
...
function emitStateUpdated(
  ...
  const notification: CodezStateUpdatedNotification = {
    patch,
    reason,
