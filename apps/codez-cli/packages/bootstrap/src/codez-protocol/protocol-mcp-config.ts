import type { McpServerConfig } from "@codez/contracts";
import type { CodezProtocolMcpServer } from "@codez/shared";

export function protocolMcpServersToRuntimeMcpConfig(
  servers: CodezProtocolMcpServer[] | undefined,
  ...
  for (const server of servers) {
    if ("command" in server) {
      runtimeServers[server.name] = {
        ...
        // Codez Protocol 的 mcpServers 是 session/runtime 和状态查询的完整覆盖配置。
