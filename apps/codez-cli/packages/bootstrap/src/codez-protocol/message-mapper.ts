import type { CodezMessagePart, CodezMessageWithParts, CodezToolState } from "@codez/shared";
import type { MessagePart, MessageWithParts, ToolState } from "@codez/contracts";
import { shouldHideInvalidToolCallFromProduct } from "../tool-call-product-visibility.js";

export function mapMessageWithParts(message: MessageWithParts): CodezMessageWithParts {
  return {
    info:
...
function mapMessagePart(part: MessagePart): CodezMessagePart {
  const base = {
...
function mapToolState(state: ToolState): CodezToolState {
  switch (state.status) {
    case "pending":
...
function mapCompletedToolStateMetadata(
  ...
  // Codez protocol 的 completed tool state 要求 metadata 必须是 object。
  return visibleMetadata;
}
