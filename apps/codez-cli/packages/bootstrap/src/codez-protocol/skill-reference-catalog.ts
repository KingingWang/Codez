...
import {
  codezSkillsReferenceCatalogParamsSchema,
  type CodezSkillReferenceCatalogEntry,
  type CodezSkillsReferenceCatalogResult,
} from "@codez/shared";
import type { SkillLoadOutcome, SkillMetadata } from "@codez/contracts";
import { listCodezSkills } from "../skills.js";
import {
  parseParams,
  requireSession,
...
export async function getSkillReferenceCatalog(
  ...
  const params = parseParams(codezSkillsReferenceCatalogParamsSchema, rawParams);
  if (params.sessionId) {
  ...
  const outcome = await listCodezSkills({
...
function toResult(
  authority: CodezSkillsReferenceCatalogResult["authority"],
...
function toReferenceCatalogEntry(skill: SkillMetadata): CodezSkillReferenceCatalogEntry {
  const scope =
