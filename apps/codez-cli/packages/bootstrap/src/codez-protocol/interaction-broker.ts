...
import {
  ...
  codezUserInputResponseSchema,
  type CodezPermissionOption,
  type CodezPermissionResponse,
  type CodezUserInputQuestion,
...
import type { CodezProtocolAgentServerContext } from "./server-types.js";
import {
  ...
  const response = await raceClientRequestWithV4Interaction(
    ...
    (signal) =>
      context.requestClient(
        codezProtocolMethods.interactionRequestPermission,
        {
          input: request.input,
    ...
    // v4 answer → CodezPermissionResponse：optionId 语义来自 v4 reducer 合成的
...
function v4AnswerToPermissionResponse(
  ...
  permissionOptions: CodezPermissionOption[],
  toolName: string,
  ...
  if (exact) {
    ...
    // option.response——wire 上 codezPermissionUpdateSchema 是 strict，旧桌面多一个字段就丢事件。
...
async function requestUserInput(
  ...
  const response = await raceClientRequestWithV4Interaction(
    ...
    (signal) =>
      context.requestClient(
        codezProtocolMethods.interactionRequestUserInput,
        {
          input: request.input,
...
function v4AnswerToUserInputResponse(answer: V4InteractionAnswer): CodezUserInputResponse {
...
function v4AnswerToPlanApprovalResponse(answer: V4InteractionAnswer): CodezUserInputResponse {
...
function createExitPlanModeApprovalQuestion(): CodezUserInputQuestion {
  return {
    header: "Plan",
    options: [
      {
...
function mapAskUserQuestion(question: AskUserQuestion): CodezUserInputQuestion {
  return {
...
async function readPersistedAutoResolution(
  ...
  try {
    ...
    context.logger?.warn("Failed to restore user input auto-resolution state", {
      ...
      event: "codez_protocol.user_input_auto_resolution_restore_failed",
      interactionId: request.requestId,
