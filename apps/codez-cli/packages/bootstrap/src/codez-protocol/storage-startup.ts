...
import { SqliteSessionStore, type SqliteMigrationProgress } from "@codez/adapters/storage";
import {
  classifyDatabaseStartupError,
  databaseStartupErrorDetails,
  codezProtocolNotifications,
  codezStorageStartupStateSchema,
  type CodezStorageStartupState,
} from "@codez/shared";

export async function openProtocolStartupStorage(options: {
  dbPath: string;
  output: NodeJS.WritableStream;
  onProgress?: (progress: CodezStorageStartupState) => void;
  ...
  const report = async (progress: SqliteMigrationProgress) => {
    const params = codezStorageStartupStateSchema.parse({
      schemaVersion: 1,
      attemptId,
      databaseId,
    ...
    await new Promise<void>((resolve, reject) => {
      options.output.write(
        `${JSON.stringify({ method: codezProtocolNotifications.storageStartup, params })}\n`,
...
export async function prepareProtocolStartupStorage(options: {
  ...
  const { codezStoragePathReadySchema, classifyDatabaseStartupError } =
    await import("@codez/shared");
  ...
  const acknowledgement = new Promise<boolean>((resolve, reject) => {
    ...
    lines.once("line", (line) => {
      try {
        ...
        const ack = codezStoragePathReadySchema.parse(JSON.parse(line));
