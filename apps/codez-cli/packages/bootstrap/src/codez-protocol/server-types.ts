...
import type { McpTelemetryTracker } from "@codez/adapters";
import type { WorkspaceHookPolicyProvider } from "@codez/core";
import type { AccountProviderConfigSnapshot } from "@codez/provider";
import {
  ...
  type CodezProtocolMethod,
  type CodezProtocolNotification,
  type CodezProtocolRequest,
  type CodezProtocolRequestId,
  type CodezProtocolTrace,
  type CodezSessionMode,
  type CodezSessionPersistence,
  type CodezWorkspaceRef,
} from "@codez/shared";
import type { CodezApp, CodezAppOptions } from "../app/types.js";
import type { V4InteractionRegistry } from "../codez-protocol-v4/interaction-registry.js";
import type { ConversationV4Gateway } from "../codez-protocol-v4/v4-gateway.js";
...
export interface CodezProtocolAgentDependencies {
  createCodezApp(options?: Omit<CodezAppOptions, "providerRegistry">): CodezApp | Promise<CodezApp>;
  /**
...
export type CodezProtocolAgentResolvedDependencies = CodezProtocolAgentDependencies & {
  createSessionEventStore(sessionId: string): SessionEventStorePort;
  workspaceHookPolicyProvider: WorkspaceHookPolicyProvider;
};

export interface CodezProtocolEventSequenceState {
  lastSeq: number;
  seqBySourceEventKey: Map<string, number>;
}

export interface CodezProtocolToolInputTransmissionState {
...
export interface CodezProtocolSessionRecord {
  ...
  protocolEventSequences: Map<string, CodezProtocolEventSequenceState>;
  protocolToolInputTransmissions: Map<string, CodezProtocolToolInputTransmissionState>;
...
export interface CodezProtocolAgentServerContext {
  /** 进程退出期间，迟到的异步物化不能重新登记 session。 */
  assertServing?: () => void;
  deps: CodezProtocolAgentResolvedDependencies;
  logger?: Logger;
  ...
  notify(notification: CodezProtocolNotification): void;
  requestClient<T>(
...
export function isRequest(message: CodezProtocolMessage): message is CodezProtocolRequest {
  return "method" in message && "id" in message;
}

export function isNotification(
  message: CodezProtocolMessage,
): message is CodezProtocolNotification {
  return "method" in message && !("id" in message);
}

export function isResponse(
  message: CodezProtocolMessage,
): message is { id: CodezProtocolRequestId; result: unknown } {
  return "id" in message && "result" in message;
}

export function isErrorResponse(message: CodezProtocolMessage): message is {
...
export function protocolTraceFromTraceContext(context: TraceContext): CodezProtocolTrace {
  return {
...
export function requireSession(
  ...
  sessionId: string,
  options: { deliveryKind?: CodezDeliveryKind; operation?: string } = {},
  ...
  if (!record) {
    ...
    context.logger?.warn("Codez Protocol session runtime missing", {
      activeSessionCount: context.sessions.size,
      event: "codez_protocol.session.require_missing",
...
export function createProtocolLogger(deps: CodezProtocolAgentDependencies): Logger | undefined {
  return deps.loggerFactory?.createLogger("codez").child({
    module: "bootstrap.codez_protocol",
  });
}
