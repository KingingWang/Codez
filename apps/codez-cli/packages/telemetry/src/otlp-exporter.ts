...
export function createOwnedAgentTelemetryRuntime(
  ...
  const tracer = provider.getTracer("@codez/cli-agent-telemetry", String(TELEMETRY_SCHEMA_VERSION));
  const meter = meterProvider.getMeter(
    "@codez/cli-agent-telemetry",
...
function metricViews(): ViewOptions[] {
  return [
    histogramView("codez.agent.turn.duration", [1, 5, 10, 30, 60, 300, 600]),
    histogramView("codez.model.*.duration", [0.25, 1, 2, 5, 10, 30, 60, 120]),
    histogramView(
      "codez.context.compaction.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView(
      "codez.detached.operation.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView("codez.agent.step.duration", [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600]),
    histogramView("codez.tool.execution.duration", [0.01, 0.1, 0.5, 1, 5, 30, 120, 300]),
    histogramView(
      "codez.command.execution.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView("codez.*.time_to_first_*", [0.25, 0.5, 1, 2, 5, 10, 30, 60]),
    histogramView("codez.model.call.attempts", [1, 2, 3, 5, 8]),
    cardinalityView("codez.model.attempt.tokens"),
    cardinalityView("codez.model.attempt.stream_stall.count"),
    cardinalityView("codez.telemetry.creation_drop.count"),
    cardinalityView("codez.telemetry.abandoned.count"),
  ];
}
...
function telemetryResourceAttributes(
  ...
  return compactResourceAttributes({
    ...
    "codez.build.commit_id": resource.buildCommitId,
    "codez.device.installation_id": options.includeInstallationId
      ? resource.installationId
      : undefined,
    "codez.product.version": resource.productVersion,
    "codez.runtime.distribution": resource.runtimeDistribution,
    "codez.runtime.surface": resource.runtimeSurface,
    "codez.telemetry.schema_owner": "cli",
    "codez.telemetry.schema_version": TELEMETRY_SCHEMA_VERSION,
  });
}
