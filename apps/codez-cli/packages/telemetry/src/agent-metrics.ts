...
export class OtelAgentTelemetryMetrics implements AgentTelemetryMetricRecorder {
  ...
  constructor(meter: Meter) {
    this.durations = {
      agent_turn: duration(meter, "codez.agent.turn.duration"),
      agent_step: duration(meter, "codez.agent.step.duration"),
      tool_execution: duration(meter, "codez.tool.execution.duration"),
      command_execution: duration(meter, "codez.command.execution.duration"),
      context_compaction: duration(meter, "codez.context.compaction.duration"),
      detached_operation: duration(meter, "codez.detached.operation.duration"),
      model_call: duration(meter, "codez.model.call.duration"),
      model_attempt: duration(meter, "codez.model.attempt.duration"),
    };
    this.modelCallAttempts = meter.createHistogram("codez.model.call.attempts", {
      description: "Physical provider attempts per logical model call",
      unit: "{attempt}",
    });
    this.modelTokens = meter.createCounter("codez.model.attempt.tokens", {
      description: "Observed provider token delta by token type",
      unit: "{token}",
    });
    this.modelAttemptFirstProviderEvent = duration(
      meter,
      "codez.model.attempt.time_to_first_provider_event",
    );
    this.modelAttemptFirstContent = duration(meter, "codez.model.attempt.time_to_first_content");
    this.modelAttemptFirstText = duration(meter, "codez.model.attempt.time_to_first_text");
    this.modelAttemptStreamStalls = meter.createCounter("codez.model.attempt.stream_stall.count", {
      description: "Observed stream stalls",
      unit: "{stall}",
    });
    this.modelAttemptStreamMaxIdle = duration(meter, "codez.model.attempt.stream_max_idle");
    this.commandFirstOutput = duration(meter, "codez.command.execution.time_to_first_output");
    this.creationDrop = meter.createCounter("codez.telemetry.creation_drop.count", {
      description: "Spans rejected before creation by runtime capacity protection",
      unit: "{span}",
    });
    this.abandoned = meter.createCounter("codez.telemetry.abandoned.count", {
