...
import type {
  ...
  ToolTraceStart,
} from "@codez/contracts/telemetry";
import {
  activeWriterContext,
  BaseSpanWriter,
  compactAttributes,
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "codez.agent_turn.turn_number": integer(input.turnNumber),
            "codez.agent_turn.input_source": safeEnum(input.inputSource),
          }),
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "codez.agent_step.step_id": safeId(input.stepId),
            "codez.agent_step.step_index": integer(input.stepIndex),
          }),
          context: parentContext,
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "codez.execution.tool_call_id": safeId(input.toolCallId),
            "codez.tool_execution.tool_name": toolName,
            ...toolCompatibilityAttributes({
    ...
    return this.safeCreate(
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...executionProjection(parent?.correlation),
            "codez.context_compaction.trigger": safeEnum(input.trigger),
            "codez.context_compaction.phase": safeEnum(input.phase),
            "codez.context_compaction.model_mode": safeEnum(input.modelMode),
            "codez.context_compaction.outer_attempt": integer(input.outerAttempt),
            "codez.context_compaction.max_attempts": integer(input.maxAttempts),
            "codez.context_compaction.triggering_step_index": integer(input.triggeringStepIndex),
            "codez.context_compaction.policy_context_window_tokens": finiteNonNegative(
              input.policyContextWindowTokens,
            ),
            "codez.context_compaction.threshold_tokens": finiteNonNegative(input.thresholdTokens),
            "codez.context_compaction.token_source": safeEnum(input.tokenSource),
            "codez.context_compaction.recovered_from_logical_call_id": safeId(
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "codez.detached_operation.operation": safeEnum(input.operation),
            "codez.detached_operation.execution_kind": safeEnum(input.executionKind),
            "codez.detached_operation.trigger": safeEnum(input.trigger),
            "codez.detached_operation.target_kind": safeEnum(input.targetKind),
            "codez.detached_operation.goal_iteration": integer(input.goalIteration),
            "codez.detached_operation.chunk_index": integer(input.chunkIndex),
            "codez.detached_operation.chunk_count": integer(input.chunkCount),
          }),
          context: parentContext,
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "codez.execution.logical_call_id": safeId(input.logicalCallId),
            "codez.model_call.operation": safeEnum(input.operation),
            "codez.model_call.streaming": input.streaming,
            "codez.model_call.model_role": safeEnum(input.modelRole),
            "codez.model_call.requested_provider_id": safeString(input.requested.providerId, 128),
            "codez.model_call.requested_model": safeString(input.requested.requestedModel, 128),
            "codez.model_call.reasoning_capability": safeEnum(input.requested.reasoning.capability),
            "codez.model_call.reasoning_requested_state": safeEnum(
              input.requested.reasoning.requestedState,
            ),
            "codez.model_call.reasoning_requested_control": safeEnum(
              input.requested.reasoning.requestedControl,
            ),
            "codez.model_call.reasoning_requested_level": safeString(
              input.requested.reasoning.requestedLevel,
              128,
            ),
            "codez.model_call.reasoning_requested_budget_tokens": integer(
              input.requested.reasoning.requestedBudgetTokens,
            ),
            "codez.model_call.call_cause": safeEnum(input.callCause),
            "codez.model_call.previous_logical_call_id": safeId(input.previousLogicalCallId),
          }),
          context: parentContext,
          correlation: parent?.correlation,
          parent,
    ...
    return this.safeCreate(
      ...
      () => {
        const span = this.startSpan({
          attributes: compactAttributes({
            ...executionProjection(parent.correlation),
            "codez.execution.tool_call_id": safeId(parent.toolCallId),
            "codez.command_execution.safe_name": safeString(input.safeName, 128),
            "codez.command_execution.category": safeEnum(input.category),
            "codez.command_execution.command_count": integer(input.commandCount),
            "codez.command_execution.shell_kind": safeEnum(input.shellKind),
            "codez.command_execution.sandboxed": input.sandboxed,
          }),
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "codez.execution.tool_call_id": safeId(parent.toolCallId),
            "codez.execution.logical_call_id": parentWriter.logicalCallId,
            "codez.execution.model_operation": parentWriter.operation,
            "codez.execution.model_role": parentWriter.modelRole,
            "codez.model_attempt.request_id": safeId(input.requestId),
            "codez.model_attempt.attempt_number": integer(input.attemptNumber),
            "codez.model_attempt.max_attempts": integer(input.maxAttempts),
            "codez.model_attempt.attempt_cause": safeEnum(input.attemptCause),
            "codez.model_attempt.previous_request_id": safeId(input.previousRequestId),
            "codez.model_attempt.retry_delay_ms": finiteNonNegative(input.retryDelayMs),
            "codez.model_attempt.provider_id": safeString(target.providerId, 128),
            "codez.model_attempt.provider_kind": safeEnum(target.providerKind),
            "codez.model_attempt.provider_origin": safeString(target.providerOrigin),
            "codez.model_attempt.provider_route": safeString(target.providerRoute),
            "codez.model_attempt.requested_model": safeString(target.requestedModel, 128),
            "codez.model_attempt.transport": safeEnum(input.transport),
            "codez.model_attempt.api_operation": safeEnum(input.apiOperation),
            "codez.model_attempt.reasoning_capability": safeEnum(target.reasoning.capability),
            "codez.model_attempt.reasoning_requested_state": safeEnum(
              target.reasoning.requestedState,
            ),
            "codez.model_attempt.reasoning_requested_control": safeEnum(
              target.reasoning.requestedControl,
            ),
            "codez.model_attempt.reasoning_requested_level": safeString(
              target.reasoning.requestedLevel,
              128,
            ),
            "codez.model_attempt.reasoning_requested_budget_tokens": integer(
...
class TurnWriter extends TrackedBaseWriter implements AgentTurnSpanWriter {
  ...
  finishCompleted(
    ...
    this.finishCompletedIfOpen(() => this.setAttribute("codez.agent_turn.result_type", resultType));
  }
...
class StepWriter extends TrackedBaseWriter implements AgentStepSpanWriter {
  ...
  finishCompleted(
    ...
    this.finishCompletedIfOpen(() =>
      this.setAttribute("codez.agent_step.terminal_reason", terminalReason),
    );
  }
...
class ToolWriter extends TrackedBaseWriter implements ToolExecutionSpanWriter {
  ...
  setPermissionDecision(decision: "granted" | "denied" | "not_required"): void {
    this.setAttribute("codez.tool_execution.permission_decision", decision);
  ...
  setOutputBytes(bytes: number): void {
    this.setAttribute("codez.tool_execution.output_bytes", finiteNonNegative(bytes));
  }

  setOutputTruncated(truncated: boolean): void {
    this.setAttribute("codez.tool_execution.output_truncated", truncated);
  }
  ...
  finishDenied(reason: "user_denied" | "policy_denied" | "unavailable" | "unknown"): void {
    this.finishDomainOutcomeIfOpen("denied", () =>
      this.setAttribute("codez.tool_execution.permission_denial_reason", reason),
    );
  }
...
class CommandWriter extends TrackedBaseWriter implements CommandExecutionSpanWriter {
  ...
  markFirstOutput(): void {
    ...
    this.setAttribute("codez.command_execution.first_output_ms", elapsed);
  ...
  setExitCode(exitCode: number): void {
    ...
    this.setAttribute("codez.command_execution.exit_code", normalized);
  ...
  setSignal(signal: string): void {
    ...
    this.setAttribute("codez.command_execution.signal", normalized);
    this.setAttributes(commandCompatibilityAttributes({ signal: normalized }));
  }

  setOutputBytes(bytes: number): void {
    this.setAttribute("codez.command_execution.output_bytes", finiteNonNegative(bytes));
  }

  setTimedOut(timedOut: boolean): void {
    this.setAttribute("codez.command_execution.timed_out", timedOut);
  }
...
class CompactionWriter extends TrackedBaseWriter implements ContextCompactionSpanWriter {
  ...
  setInputTokens(tokens: number): void {
    this.setAttribute("codez.context_compaction.input_tokens", finiteNonNegative(tokens));
  }

  setOutputTokens(tokens: number): void {
    this.setAttribute("codez.context_compaction.output_tokens", finiteNonNegative(tokens));
  }
...
class DetachedWriter extends TrackedBaseWriter implements DetachedOperationSpanWriter {
  ...
  setResultType(resultType: "text" | "boolean" | "metadata" | "other"): void {
    this.setAttribute("codez.detached_operation.result_type", resultType);
  }
...
class ModelAttemptWriter extends TrackedBaseWriter implements ModelAttemptSpanWriter {
  ...
  setProviderRequestId(requestId: string): void {
    this.setAttribute("codez.model_attempt.provider_request_id", safeString(requestId, 256));
  }

  setResponseModel(model: ResponseModelTelemetryDescriptor): void {
    const value = safeString(model.model, 128);
    this.setAttribute("codez.model_attempt.response_model", value);
  ...
  setEffectiveReasoningState(state: ModelReasoningState): void {
    this.setAttribute("codez.model_attempt.reasoning_effective_state", safeEnum(state));
  }

  setEffectiveReasoningControl(control: ModelReasoningControlType): void {
    this.setAttribute("codez.model_attempt.reasoning_effective_control", safeEnum(control));
  }

  setEffectiveReasoningLevel(level: string): void {
    this.setAttribute("codez.model_attempt.reasoning_effective_level", safeString(level, 128));
  }

  setEffectiveReasoningBudgetTokens(tokens: number): void {
    this.setAttribute(
      "codez.model_attempt.reasoning_effective_budget_tokens",
  ...
  setFinishReason(reason: ModelFinishReason): void {
    ...
    this.setAttribute("codez.model_attempt.finish_reason", value);
    this.setAttributes(modelResponseCompatibilityAttributes({ finishReason: value }));
  }

  setInputTokens(tokens: number): void {
    this.setUsage("inputTokens", "input", tokens, "codez.model_attempt.input_tokens");
    this.setAttributes(modelResponseCompatibilityAttributes({ inputTokens: tokens }));
  }

  setOutputTokens(tokens: number): void {
    this.setUsage("outputTokens", "output", tokens, "codez.model_attempt.output_tokens");
    this.setAttributes(modelResponseCompatibilityAttributes({ outputTokens: tokens }));
  }

  setReasoningTokens(tokens: number): void {
    this.setUsage("reasoningTokens", "reasoning", tokens, "codez.model_attempt.reasoning_tokens");
  }

  setCacheReadTokens(tokens: number): void {
    this.setUsage("cacheReadTokens", "cache_read", tokens, "codez.model_attempt.cache_read_tokens");
  }
  ...
  setCacheWriteTokens(tokens: number): void {
    this.setUsage(
      ...
      "cache_write",
      tokens,
      "codez.model_attempt.cache_write_tokens",
    );
  }

  setStreamOutputCommitted(committed: boolean): void {
    this.setAttribute("codez.model_attempt.stream_output_committed", committed);
  }

  setHttpStatusCode(statusCode: number): void {
    const normalized = integer(statusCode);
    this.setAttribute("codez.model_attempt.http_status_code", normalized);
    this.setAttributes(httpResponseCompatibilityAttributes(statusCode));
  }

  setProviderErrorCode(code: string): void {
    this.setAttribute("codez.model_attempt.provider_error_code", safeString(code, 128));
  }

  setProviderErrorMessage(message: string): void {
    this.setAttribute("codez.model_attempt.provider_error_message", sanitizeErrorMessage(message));
  }

  setRetryAfterMs(delayMs: number): void {
    this.setAttribute("codez.model_attempt.retry_after_ms", finiteNonNegative(delayMs));
  }
  ...
  markFirstProviderEvent(): void {
    ...
    this.setAttribute("codez.model_attempt.time_to_first_provider_event_ms", elapsed);
  ...
  markFirstContent(): void {
    ...
    this.setAttribute("codez.model_attempt.time_to_first_content_ms", elapsed);
  ...
  markFirstText(): void {
    ...
    this.setAttribute("codez.model_attempt.time_to_first_text_ms", elapsed);
