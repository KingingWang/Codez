...
import type { ModelStatusSink } from "@codez/contracts/model";
...
export async function prepareModelTelemetryEnv(
  ...
  if (!resolveOtlpTraceEndpoint(env) || isExplicitlyDisabled(env.CODEZ_MODEL_TELEMETRY_ENABLED)) {
    return env;
  }
  const existingInstallationId = normalizeTelemetryDeviceMid(env.CODEZ_TELEMETRY_DEVICE_MID);
  const installationId =
    existingInstallationId ?? (await resolveStandaloneDeviceMid(env.CODEZ_HOME?.trim()));
  const preparedEnv = installationId ? { ...env, CODEZ_TELEMETRY_DEVICE_MID: installationId } : env;
...
async function createPreparedOwner(
  ...
  const resource: TelemetryResourceContext = {
    buildCommitId: normalizeBuildCommitId(options.buildCommitId ?? env.CODEZ_BUILD_COMMIT_ID),
    cliVersion: normalizeVersion(options.cliVersion),
    deploymentEnvironment:
      env.CODEZ_ENV ??
      env.CODEZ_RUNTIME_ENV ??
      resourceAttributesFromEnv(env)["deployment.environment.name"],
    installationId: normalizeTelemetryDeviceMid(env.CODEZ_TELEMETRY_DEVICE_MID),
    productVersion: normalizeVersion(options.productVersion ?? env.CODEZ_APP_VERSION),
    runtimeDistribution:
      options.runtimeDistribution ??
      resolveRuntimeDistribution(env.CODEZ_TELEMETRY_RUNTIME_DISTRIBUTION),
    runtimeSurface:
      options.runtimeSurface ?? resolveRuntimeSurface(env.CODEZ_TELEMETRY_RUNTIME_SURFACE),
    serviceInstanceId: randomUUID(),
    serviceName: env.OTEL_SERVICE_NAME?.trim() || "codez-cli-agent",
  };
  try {
...
function resolveTelemetryIdentity(env: EnvRecord): TelemetryIdentitySnapshot {
  const userSubjectId = normalizeTelemetryId(env.CODEZ_TELEMETRY_USER_SUBJECT_ID);
  const configuredState = env.CODEZ_TELEMETRY_IDENTITY_STATE?.trim();
  const identityState =
    ...
    configuredState === "unknown"
      ...
      : userSubjectId
        ? "authenticated"
        : normalizeTelemetryDeviceMid(env.CODEZ_TELEMETRY_DEVICE_MID)
          ? "anonymous"
          : "unknown";
...
async function resolveStandaloneDeviceMid(
  codezHome: string | undefined,
): Promise<string | undefined> {
  const stateFile = codezHome
    ? join(codezHome, "v2", "telemetry-state.json")
    : join(homedir(), ".codez", "v2", "telemetry-state.json");
