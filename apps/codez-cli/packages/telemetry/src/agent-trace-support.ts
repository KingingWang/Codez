...
const ACTIVE_WRITER_CONTEXT_KEY = createContextKey("@codez/telemetry/active-writer-v4");
...
export function contextFromCausation(causation: AgentTelemetryCausation): Context {
  ...
  // 为底，否则会把另一个异步任务的 Baggage 或 Codez Writer 私有状态夹带进来。
...
export function executionProjection(
  ...
  return {
    "codez.execution.session_id": options.includeSession
      ? safeId(correlation.sessionId)
      : undefined,
    "codez.execution.parent_session_id": options.includeSession
      ? safeId(correlation.parentSessionId)
      : undefined,
    "codez.execution.turn_id": safeId(correlation.turnId),
    "codez.execution.parent_turn_id": options.includeSession
      ? safeId(correlation.parentTurnId)
      : undefined,
    "codez.execution.query_id": options.includeQuery ? safeId(correlation.queryId) : undefined,
    "codez.execution.actor_kind": options.includeActor
      ? safeEnum(correlation.actorKind)
      : undefined,
    "codez.execution.agent_name": options.includeAgent
      ? safeIdentifier(correlation.agentName)
      : undefined,
    "codez.execution.identity_state": options.includeIdentity
      ? safeEnum(correlation.identityState)
      : undefined,
    "codez.execution.user_subject_id": options.includeIdentity
      ? safeId(correlation.userSubjectId)
      : undefined,
    "codez.execution.launch_surface": safeEnum(correlation.launchSurface),
  };
}
