...
const traceHeaderNames = ["x-codez-trace-id", "x-trace-id", "traceparent"];
...
export class NetworkCaptureService {
  ...
  getStatus(): NetworkCaptureStatus {
    ...
    const env: Record<string, string> = url
      ? {
          CODEZ_HTTP_PROXY: url,
          CODEZ_AGENT_CA_CERT: caCertPath,
        }
      : {};
  ...
  private recordForContext(ctx: IContext): NetworkRequestRecord | undefined {
    const id = stringValue(ctx.tags?.codezCaptureId) ?? ctx.uuid;
...
export function createNetworkCaptureServiceFromEnv(
  ...
  if (isDisabled(env.CODEZ_DEBUG_NETWORK_CAPTURE)) return undefined;
  return new NetworkCaptureService({
    host: env.CODEZ_DEBUG_NETWORK_HOST || undefined,
    port: parsePositiveInteger(env.CODEZ_DEBUG_NETWORK_PORT) ?? DEFAULT_PORT,
    caDir: env.CODEZ_DEBUG_NETWORK_CA_DIR || undefined,
    maxEntries: parsePositiveInteger(env.CODEZ_DEBUG_NETWORK_MAX_ENTRIES) ?? DEFAULT_MAX_ENTRIES,
  });
}
