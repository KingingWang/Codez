...
import { resolveWorkspaceHookTimeoutMs } from "@codez/shared/workspace-hook-discovery";
import {
  CoreErrorType,
  HookEventName,
  createCoreError,
...
export async function createCompatibleHookStdin(input: HookInput): Promise<{
  ...
  const tempDir = await mkdtemp(join(tmpdir(), "codez-hook-"));
  ...
  if ("toolName" in input) {
    ...
    // 这里只补无损 alias，继续保留 Codez camelCase 字段作为内部主契约。
...
export function createPluginEnvOverlay(
  ...
  const set: Record<string, string> = {
    ...
    CODEZ_PROJECT_DIR: input.cwd || workingDirectory,
    CODEZ_SESSION_ID: input.sessionId,
  };
  ...
  return {
    set: {
      ...
      CODEZ_PLUGIN_DATA: plugin.dataPath,
      CODEZ_PLUGIN_ID: plugin.id,
      CODEZ_PLUGIN_NAME: plugin.name,
      CODEZ_PLUGIN_ROOT: plugin.rootPath,
    },
  };
}
...
export function expandPluginVariables(
  ...
  if (plugin) {
    ...
    replacements.CODEZ_PLUGIN_DATA = plugin.dataPath;
    replacements.CODEZ_PLUGIN_ROOT = plugin.rootPath;
  }
  return value.replace(
    /\$\{(CLAUDE_CODE_SESSION_ID|CLAUDE_PLUGIN_DATA|CLAUDE_PLUGIN_ROOT|CLAUDE_PROJECT_DIR|CLAUDE_SESSION_ID|CLAUDE_SKILL_DIR|CODEZ_PLUGIN_DATA|CODEZ_PLUGIN_ROOT|CODEZ_PROJECT_DIR|CODEZ_SESSION_ID|CODEZ_SKILL_DIR)\}/gu,
    (_match, key: string) => {
      if (key === "CLAUDE_SKILL_DIR" || key === "CODEZ_SKILL_DIR") {
