...
import type { ModelCatalogEntry, ModelSelection } from "@codez/contracts";
import {
  CODEZ_MODEL_REASONING_SEPARATOR,
  formatModelPickerValue,
  parseModelPickerValue,
} from "@codez/shared/model-selection";
...
function splitReasoningLevel(text: string): { reference: string; level?: string } {
  ...
  const index = text.indexOf(CODEZ_MODEL_REASONING_SEPARATOR, searchFrom);
