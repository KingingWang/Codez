...
} from "@codez/codez-cua/frame-contract";
...
import { preserveOfficialCuaFrameResult } from "@codez/codez-cua/frame-contract";
...
function readBrowserScreenshotContentIndices(result: McpToolCallResult): Set<number> {
  const value = result._meta?.[CODEZ_MCP_BROWSER_SCREENSHOT_CONTENT_INDICES_META_KEY];
