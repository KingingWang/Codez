...
import type { FileSystemPort, Logger, TraceContext } from "@codez/contracts";
...
function resolvePersistentAgentMemoryRoot(input: {
  ...
  return input.scope === "project"
    ? join(workspace, ".codez", "agent-memory", agentKey)
    : join(workspace, ".codez", "agent-memory-local", agentKey);
}
