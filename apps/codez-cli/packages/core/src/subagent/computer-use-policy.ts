...
import {
  CODEZ_CUA_OFFICIAL_MCP_NAMESPACE_NAME as CODEZ_CUA_OFFICIAL_MCP_SERVER_NAME,
  CODEZ_CUA_OFFICIAL_PLUGIN_ID,
} from "@codez/shared";
import type { McpToolDescriptor, PluginReferenceCatalog, SkillMetadata } from "@codez/contracts";
...
const OFFICIAL_CUA_SERVER_PARTS = new Set([
  "computer-use",
  "computer_use",
  toModelVisibleMcpNamePart(CODEZ_CUA_OFFICIAL_MCP_SERVER_NAME),
]);
...
export function createOfficialCuaPolicy(
  ...
  for (const descriptor of descriptors) {
    ...
    if (descriptor.serverName === CODEZ_CUA_OFFICIAL_MCP_SERVER_NAME) {
  ...
  if (officialServerNames.has(CODEZ_CUA_OFFICIAL_MCP_SERVER_NAME)) {
  ...
  for (const serverName of officialServerNames) {
    if (serverName === CODEZ_CUA_OFFICIAL_MCP_SERVER_NAME) {
  ...
  const officialPlugin = catalog?.plugins.find(
    (plugin) => plugin.pluginId === CODEZ_CUA_OFFICIAL_PLUGIN_ID,
  );
