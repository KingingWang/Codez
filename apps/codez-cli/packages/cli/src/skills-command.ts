...
type BootstrapModule = typeof import("@codez/bootstrap");
type CliSkillListOutcome = Awaited<ReturnType<typeof listCodezSkills>>;
...
const SKILLS_COMMAND_USAGE = "Usage: codez skills [list|inspect <name>]";
...
async function runSkillsListCommand(
  ...
  try {
    ...
    const listSkills = deps.listSkills ?? (await bootstrap()).listCodezSkills;
...
async function runSkillsInspectCommand(
  ...
  try {
    ...
    const workingDirectory = (deps.cwd ?? process.cwd)();
    const bootstrap = deps.loadBootstrapModule ?? (() => import("@codez/bootstrap"));
    const inspectSkill = deps.inspectSkill ?? (await bootstrap()).inspectCodezSkill;
...
function formatHumanSkillInspection(
  inspection: CodezSkillInspection,
  options: GlobalOptions,
): string {
...
const formatSkillInspectionJson = (inspection: CodezSkillInspection, cwd: string): string =>
  formatJson({
    cwd,
