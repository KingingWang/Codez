...
import { getRuntimeToolRuntime, type RuntimeToolId } from "@codez/shared/runtime-tool-runtime";
...
const assetPrefix = "codez-runtime-tools/";
const manifestAssetKey = `${assetPrefix}manifest.json`;
const markerFileName = ".codez-runtime-tool.json";
...
export async function ensureSeaRuntimeTools(
  ...
  const configuredStorageRoot = options.storageRoot ?? env.CODEZ_STORAGE_DIR?.trim();
  const storageRoot = configuredStorageRoot || join(homedir(), ".codez");
