...
type CustomCommandListOutcome = Awaited<ReturnType<typeof listCodezCustomCommands>>;
...
const COMMANDS_COMMAND_USAGE = "Usage: codez commands [list|inspect <name>]";
...
async function resolveListCustomCommands(deps: CommandsCommandDependencies) {
  ...
  return (await bootstrap()).listCodezCustomCommands;
}
...
async function resolveInspectCustomCommand(deps: CommandsCommandDependencies) {
  ...
  const bootstrap = deps.loadBootstrapModule ?? (() => import("@codez/bootstrap"));
  return (await bootstrap()).inspectCodezCustomCommand;
}
...
function formatHumanCommandInspection(
  inspection: CodezCustomCommandInspection,
  options: GlobalOptions,
): string {
