...
import type { SetCodezPluginEnabledResult, CodezPluginInstallData } from "@codez/bootstrap";
import {
...
const PLUGINS_COMMAND_USAGE = `Usage: codez plugins <command> [options]
...
Scopes: user (default), project. \`codez plugin\` is an alias of \`codez plugins\`.`;
...
async function runPluginsUpdateCommand(
  ...
  ctx.stdout.write(
    version && version === result.previousVersion
      ...
      : `Updated plugin ${pluginId} from ${result.previousVersion || "unknown"} to ${version || "unknown"}. Restart codez to apply.\n`,
  );
...
async function runPluginsDisableCommand(
  ...
  const entries: Array<{ plugin: PluginListItem; result?: SetCodezPluginEnabledResult; error?: string }> = [];
