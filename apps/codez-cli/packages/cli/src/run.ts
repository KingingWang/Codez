...
import { createNodeLoggerFactory } from "@codez/adapters";
import { getRuntimeInfo, type PresentationSurface } from "@codez/core";
import { color, formatJson, supportsColor } from "@codez/core";
import { getCodezCopy, isUiLocale, type UiLocale } from "@codez/i18n";
import type { RunContext, GlobalOptions, GlobalOutputFormat } from "@codez/shared-types";
import {
...
const normalizeLocaleOption = (value: string | undefined): UiLocale | undefined => {
  ...
  throw new Error(getCodezCopy().cli.errors.localeUnsupported(value));
};
...
const normalizePresentationSurface = (value: string | undefined): PresentationSurface => {
  ...
  if (value.toLowerCase() === "desktop") return "codez_desktop";
...
const runDoctor = (ctx: RunContext, options: GlobalOptions, workingDirectory: string): number => {
  ...
  ctx.stdout.write(`${color.bold("codez doctor", colors)}\n`);
...
const runCodezProtocolCommand = async (
  ctx: RunContext,
  ...
  try {
    ...
    // 直接退出，外层只能看到 Codez agent transport closed。
    ...
    const runProtocolAgent =
      deps.runCodezProtocolAgent ?? (await loadBootstrapModule()).runCodezProtocolAgent;
...
export const run = async (ctx: RunContext, deps: RunDependencies = {}): Promise<number> => {
  ...
  const commandDeps: RunDependencies = {
    ...
    logger:
      deps.logger ??
      createNodeLoggerFactory({ env }).createLogger("codez").child({ module: "cli" }),
  ...
  switch (commandName(parsed.positionals)) {
    ...
    case "agent-server":
    case "app-server":
      return await runCodezProtocolCommand(
        ctx,
        options,
        commandDeps,
        presentationSurface,
