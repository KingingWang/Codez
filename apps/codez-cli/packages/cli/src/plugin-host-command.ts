...
import { CODEZ_PLUGIN_HOST_COMMAND } from "@codez/contracts/plugins";
import {
  getCapturedCodezCuaBrokerCredentials,
  CODEZ_CUA_BROKER_SOCKET_ENV_KEY,
  CODEZ_CUA_NODE_REPL_HOST_ENV_KEY,
} from "@codez/shared/runtime-env";
import { CODEZ_CUA_OFFICIAL_PLUGIN_ID, CODEZ_PLUGIN_ID_ENV_KEY } from "@codez/shared/mcp";
import type { RunContext } from "@codez/shared-types";

const HOST_USAGE = `${CODEZ_PLUGIN_HOST_COMMAND} <server-path> [-- <server-arg>...]`;
...
export function isPluginHostInvocation(argv: readonly string[]): boolean {
  return argv[0] === CODEZ_PLUGIN_HOST_COMMAND;
}

// __codez-plugin-host 在 agent 子进程里运行 official plugin 的 MCP server（server.js）。
...
// Agent 进程曾收到过 Helper 凭据，不能证明当前传入的 server 就是官方 codez-cua：
...
export async function runPluginHostCommand(ctx: RunContext, argv: string[]): Promise<number> {
  ...
  try {
    ...
    const capturedBrokerCredentials = getCapturedCodezCuaBrokerCredentials();
    ...
    const originalBrokerSocket = process.env[CODEZ_CUA_BROKER_SOCKET_ENV_KEY];
    // shared node_repl 把同一凭据组恢复到环境，由 broker bridge 读取；旧的独立 CUA
    // MCP 不再拥有执行入口。
    process.argv = [process.execPath, serverPath, ...serverArgs];
    if (capturedBrokerCredentials.socket && process.env[CODEZ_CUA_NODE_REPL_HOST_ENV_KEY] === "1") {
      process.env[CODEZ_CUA_BROKER_SOCKET_ENV_KEY] = capturedBrokerCredentials.socket;
    }
    try {
      await module.main();
    } finally {
      ...
      if (originalBrokerSocket === undefined) {
        delete process.env[CODEZ_CUA_BROKER_SOCKET_ENV_KEY];
      } else {
        process.env[CODEZ_CUA_BROKER_SOCKET_ENV_KEY] = originalBrokerSocket;
      }
    }
...
type CapturedBrokerCredentials = ReturnType<typeof getCapturedCodezCuaBrokerCredentials>;
...
function assertCapturedBrokerLaunchIsAuthorized(credentials: CapturedBrokerCredentials): void {
  ...
  const pluginId = process.env[CODEZ_PLUGIN_ID_ENV_KEY]?.trim().toLowerCase();
  ...
  // 残留读取方只能靠 CLI 自己的 typecheck 发现（根 `pnpm typecheck` 不含 apps/codez-cli）。
  ...
  if (
    ...
    pluginId !== CODEZ_CUA_OFFICIAL_PLUGIN_ID ||
    process.env[CODEZ_CUA_NODE_REPL_HOST_ENV_KEY] !== "1"
  ) {
    throw new Error(
      "Captured Codez CUA broker credentials may only launch the trusted shared node_repl host",
    );
  }
}
