import type { TuiSelection, TuiSubmitPrompt } from "@codez/tui";
...
export function buildLoginSelection(locale?: string): TuiSelection {
  const copy = getCodezCopy(locale).tui.loginSetup;
  return {
...
export function loginSetupResponse(locale?: string): string {
  return getCodezCopy(locale).tui.loginSetup.response;
}
