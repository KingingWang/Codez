...
import type { listCodezPlugins } from "@codez/bootstrap";
import { createInterface } from "node:readline";
import type { GlobalOptions, RunContext } from "@codez/shared-types";
import type { CliEnv } from "./env.js";

export type BootstrapModule = typeof import("@codez/bootstrap");
export type PluginListOutcome = ReturnType<typeof listCodezPlugins>;
 ...
 * 避免 `codez plugins list` 这类轻命令把整个 bootstrap 图提前拉起来。
 */
export const BOOTSTRAP_EXPORTS = {
  addMarketplace: "addCodezPluginMarketplace",
  getPluginsOverview: "getCodezPluginsOverview",
  installPlugin: "installCodezMarketplacePlugin",
  listPlugins: "listCodezPlugins",
  removeMarketplace: "removeCodezPluginMarketplace",
  setPluginEnabled: "setCodezPluginEnabled",
  uninstallPlugin: "uninstallCodezMarketplacePlugin",
  updateMarketplace: "updateCodezPluginMarketplace",
  updatePlugin: "updateCodezMarketplacePlugin",
  validatePluginPath: "validateCodezPluginPath",
...
/** `codez plugins` 子命令专属旗标；由 run.ts 的全局解析器收集后原样透传。 */
...
export async function resolveDep<K extends PluginDepName>(
  ...
  const bootstrap = deps.loadBootstrapModule ?? (() => import("@codez/bootstrap"));
...
export function resolveScope(value: string | undefined): PluginScope | undefined {
  ...
  if (value === "local") {
    throw new PluginsUsageError("Scope 'local' is not supported by codez. Use: user, project");
  }
