...
import {
  ...
  normalizeCodezRuntimeEnv,
  sanitizeCodezRuntimeEnv,
  sanitizeCodezRuntimeEnvInPlace,
...
export function prepareCliRuntimeEnv(
  ...
  const prepared = {
    ...sanitizeCodezRuntimeEnv(env),
    ...buildCodezToolEnvPassthroughEnv(env),
  };
...
export function applyCliRuntimeEnvSanitization(
  ...
  const toolEnvPassthrough = buildCodezToolEnvPassthroughEnv(env);
  sanitizeCodezRuntimeEnvInPlace(env);
...
function applyCliRuntimeEnvDefaults(env: CliEnv, argv: readonly string[]): void {
  env[CODEZ_RUNTIME_ENV_KEY] = resolveCliRuntimeEnv(env, argv);
  applyBetaStorageDefault(env, argv);
}

function resolveCliRuntimeEnv(env: CliEnv, argv: readonly string[]): CodezRuntimeEnv {
  const explicit = normalizeCodezRuntimeEnv(env[CODEZ_RUNTIME_ENV_KEY]);
  if (explicit) {
    return explicit;
  }
...
function applyBetaStorageDefault(env: CliEnv, argv: readonly string[]): void {
  if (env.CODEZ_STORAGE_DIR?.trim()) return;
  const explicitBeta = env.CODEZ_BETA === "1" || env.CODEZ_ENV === "beta";
  const invokedAsBeta = argv.some((arg) => /(^|[/\\])codez-beta(?:$|\.)/u.test(arg));
  if (!explicitBeta && !invokedAsBeta) return;
  env.CODEZ_STORAGE_DIR = join(homedir(), ".codez-beta");
}
