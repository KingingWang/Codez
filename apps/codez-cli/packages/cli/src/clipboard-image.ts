...
import type { TuiClipboardImage, TuiImageMediaType, TuiReadClipboardImage } from "@codez/tui";
...
function resolveDefaultClipboardDirectory(processEnv: NodeJS.ProcessEnv = process.env): string {
  const storageRoot = processEnv.CODEZ_STORAGE_DIR?.trim() || join(homedir(), ".codez");
...
async function readMacClipboardImage(options: {
  ...
  const directory = await mkdtemp(join(options.tempDirectory, "codez-clipboard-"));
