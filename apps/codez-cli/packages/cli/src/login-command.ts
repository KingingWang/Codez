...
export async function runLoginCommand(
  ...
  try {
    ...
    if (args.length > 1 || (providerId !== "zai" && providerId !== "bigmodel")) {
      throw new Error("Usage: codez login [zai|bigmodel] [--no-browser]");
    }
    ...
    const login = deps.loginCodezCli ?? (await loadBootstrapModule()).loginCodezCli;
...
export async function runLogoutCommand(
  ...
  try {
    ...
    const logout = deps.logoutCodezCli ?? (await loadBootstrapModule()).logoutCodezCli;
