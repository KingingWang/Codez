import { getDefaultConfigPath, updateUiLocaleInFileConfig } from "@codez/adapters/config";
import type { SessionEvent } from "@codez/contracts";
import type { CodezAppOptions } from "@codez/bootstrap";
import { DEFAULT_LOCALE, type SupportedLocale } from "@codez/i18n";
import type { TuiRequestPermission } from "@codez/tui";
...
export function createTuiSubmitPrompt(
  ...
  let app: Awaited<ReturnType<NonNullable<RunDependencies["createCodezApp"]>>> | undefined;
  ...
  const permissionBroker: NonNullable<CodezAppOptions["permissionBroker"]> = {
  ...
  const replaceApp = async (
    factory: () => Promise<Awaited<ReturnType<NonNullable<RunDependencies["createCodezApp"]>>>>,
  ...
  const createApp = async (request: CliResumeRequest) => {
    ...
    let createdApp: Awaited<ReturnType<NonNullable<RunDependencies["createCodezApp"]>>>;
    try {
