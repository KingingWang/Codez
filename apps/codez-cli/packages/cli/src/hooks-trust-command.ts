...
const USAGE = `Usage:
  codez hooks trust status [--workspace <path-or-identity>] [--json]
  codez hooks trust review [--workspace <path-or-identity>] [--json]
  codez hooks trust grant --workspace <path-or-identity> --hook-digest <sha256> [--hook-digest <sha256> ...]
  codez hooks trust grant --workspace <path-or-identity> --all-current --bundle-digest <sha256>
  codez hooks trust revoke --workspace <path-or-identity> [--hook-digest <sha256> ... | --all]
`;
...
type HooksCommandDependencies = RunDependencies & {
  loadBootstrapModule?: () => Promise<typeof import("@codez/bootstrap")>;
...
function formatHuman(status: WorkspaceHookTrustCliStatus, action: string): string {
  ...
  if (status.reasonCode === "workspace_hooks_pending_trust" && status.bundleDigest) {
    lines.push(
      ...
      `  codez hooks trust grant --workspace ${quote(status.workspaceIdentity)} --hook-digest <sha256>`,
      "Or trust every currently enabled declaration in this exact bundle with:",
      `  codez hooks trust grant --workspace ${quote(status.workspaceIdentity)} --all-current --bundle-digest ${status.bundleDigest}`,
    );
  }
