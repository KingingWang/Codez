...
import { createDenyPermissionBroker } from "@codez/core";
...
export const createHeadlessPermissionBroker = (): NonNullable<
  CodezAppOptions["permissionBroker"]
> => {
 ...
 *
 * 刻意**不**进 `codezSessionEventTypeSchema`（`shared/src/codez-protocol/index.ts`）：
 * 那是个闭集 `z.enum` 且喂给 `codezSessionEventSchema` 的 discriminated union，加值等于让
 ...
 * `RunDependencies.createCodezApp` 注入点边界）。
 *
