...
import { formatJson, type PresentationSurface } from "@codez/core";
import type { RunContext, GlobalOptions } from "@codez/shared-types";
...
export const runPrompt = async (
  ...
  let app:
    | Awaited<ReturnType<Awaited<ReturnType<typeof loadBootstrapModule>>["createCodezApp"]>>
    | undefined;
  ...
  try {
    ...
    const bootstrapModule = deps.createCodezApp ? undefined : await loadBootstrapModule();
    const createApp = deps.createCodezApp ?? bootstrapModule?.createCodezApp;
    if (!createApp) throw new Error("Codez app factory is unavailable.");
    ...
    const prepareTelemetry =
      deps.prepareCodezTelemetryEnv ?? bootstrapModule?.prepareCodezTelemetryEnv;
    if (prepareTelemetry) {
      shutdownTelemetry = deps.shutdownCodezTelemetry ?? bootstrapModule?.shutdownCodezTelemetry;
    }
    const appEnv = prepareTelemetry
      ? await prepareTelemetry(env, {
          cliVersion: version,
          productVersion: env.CODEZ_APP_VERSION,
        })
      : env;
    ...
    // 回合。结果是 `codez -p "/workflow ..."` 在第一个回合后就退出，把在飞的 run 孤儿化
...
async function isResolvableCustomCommand(deps: RunDependencies, name: string): Promise<boolean> {
  ...
  //
  // 探测刻意用「保留名检查 + load」这一对，而不是直接调 resolveCodezCustomCommandPrompt：
...
async function isReservedSlashCommandName(deps: RunDependencies, name: string): Promise<boolean> {
  ...
  return bootstrap.isReservedCodezSlashCommandName(name);
}
...
async function listCustomCommandsForPrompt(deps: RunDependencies) {
  ...
  const bootstrap = await loadBootstrapModule();
  return await bootstrap.listCodezCustomCommands({ env, logger: deps.logger, workingDirectory });
}
...
async function loadCustomCommandForPrompt(deps: RunDependencies, name: string) {
  ...
  const bootstrap = await loadBootstrapModule();
  return await bootstrap.loadCodezCustomCommand({
    env,
    logger: deps.logger,
    name,
    workingDirectory,
...
function writeHeadlessWorkspaceHookTrustDiagnostic(
  ...
  ctx.stderr.write(
    [
      ...
      `Review with: codez hooks trust review --workspace ${JSON.stringify(status.workspaceIdentity)}`,
    ].join("\n") + "\n",
  );
}
