import type { TuiSessionMetadata } from "@codez/tui";
...
export async function listSkillsForTui(deps: RunDependencies) {
  const env = deps.env ?? process.env;
  const workingDirectory = (deps.cwd ?? process.cwd)();
  const listCodezSkillsForTui = deps.listSkills ?? (await loadBootstrapModule()).listCodezSkills;
  return await listCodezSkillsForTui({
    env,
    logger: deps.logger,
    workingDirectory,
  });
...
export async function listSessionsForTui(deps: RunDependencies) {
  const env = deps.env ?? process.env;
  const workingDirectory = (deps.cwd ?? process.cwd)();
  const listCodezSessionsForTui =
    deps.listSessions ?? (await loadBootstrapModule()).listCodezSessions;
  const sessions = await listCodezSessionsForTui({
