...
// 品牌类型（SessionId）和 createCodezApp 的联合签名收窄错。
export async function prepareTuiAppRuntime(
  ...
  const createAppFactory = deps.createCodezApp ?? bootstrapModule?.createCodezApp;
  if (!createAppFactory) throw new Error("Codez app factory is unavailable.");
  const prepareTelemetry =
    deps.prepareCodezTelemetryEnv ?? bootstrapModule?.prepareCodezTelemetryEnv;
  if (prepareTelemetry) {
    state.shutdownTelemetry =
      deps.shutdownCodezTelemetry ?? bootstrapModule?.shutdownCodezTelemetry;
  }
