...
const cliBundle = resolve(dist, "codez.cjs");
const seaBlobForTarget = (target) => resolve(dist, `codez-${target}.sea.blob`);
    ...
    JSON.stringify(
      {
        assets: {
          ...
          "codez-node-license": nodeLicensePath,
        },
        disableExperimentalSEAWarning: true,
        main: "dist/codez.cjs",
        output: `dist/codez-${target}.sea.blob`,
        useCodeCache: false,
        useSnapshot: false,
      },
      null,
...
const smokeTestHostTarget = async (target, binaryPath) => {
  if (isHostTarget(target)) {
    const storageRoot = await mkdtemp(join(tmpdir(), "codez-sea-smoke-"));
    try {
      const env = {
        ...process.env,
        CODEZ_STORAGE_DIR: storageRoot,
      };
      delete env.CODEZ_BFS_BINARY;
      delete env.CODEZ_RG_BINARY;
      delete env.CODEZ_UGREP_BINARY;
...
export const main = async (argv = process.argv.slice(2)) => {
  ...
  if (!existsSync(cliBundle)) {
    throw new Error("Missing dist/codez.cjs. Run `pnpm build` first.");
  }
