...
export const DEFAULT_DESTINATION_ROOT = resolve(DEFAULT_MOUNT_ROOT, "codez", "deps");
export const RELEASE_DIRECTORY_PREFIX = "codez-cli-";
...
const usage = `Usage:
  ...
  node packages/cli/scripts/upload-sea-smb.mjs --dest-root /Volumes/shared/codez/deps
...
Options:
  ...
  --dest-root <path>  Mounted SMB release root. Defaults to /Volumes/shared/codez/deps.
  ...
  if (missing.length > 0) {
    throw new Error(
      `Missing SEA binaries in ${distDir}: ${missing.join(", ")}. ` +
        "Run `pnpm --filter @codez/cli build:sea` first.",
    );
  }
