import type { CollaborationMode, ModelUsageSummary, TodoItem, TurnId } from "@codez/contracts";
...
export function useTuiApplyResult(input: TuiApplyResultInput) {
  return React.useCallback(
    (result: TuiSubmitPromptResult, preserveTurnState = false) => {
      const resultCopy = getCodezCopy(result.locale ?? input.fallback.locale).tui;
