...
import type { SkillRoot, SkillSource } from "@codez/contracts";
...
function skillRootsForBase(
  ...
  // 合并而不是 fallback：用户可能同时安装原生 `.codez` skill 和兼容 `.agents` skill。
  // 同一级别仍保持 `.codez` 优先，后续同名按 root 顺序解析。
  return [
    root(join(baseDirectory, CODEZ_DIR, SKILLS_DIR), scope, "codez", nextPriority()),
