...
function resolveProxyForRequestInternal(
  ...
  const candidates = [[`env:${CODEZ_HTTP_PROXY_ENV_KEY}`, env[CODEZ_HTTP_PROXY_ENV_KEY]]];
  ...
  if (capturedProxy) {
    return {
      noProxyMatched: false,
      proxySource: `env:${CODEZ_TOOL_ENV_PASSTHROUGH_ENV_KEY}.${capturedProxy.key}`,
...
export function resolveTlsCaCertFile(options: NetworkTlsOptions): string | undefined {
  const candidates = [
    options.caCertFile,
    options.env?.[CODEZ_AGENT_CA_CERT_ENV_KEY],
  ];
...
function readExplicitNoProxyValue(options: NetworkProxyOptions): string | undefined {
  return (
    ...
    normalizePathLike(options.env?.[CODEZ_NO_PROXY_ENV_KEY])
  );
}
