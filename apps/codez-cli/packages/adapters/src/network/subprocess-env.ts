import {
  ...
  CODEZ_TOOL_ENV_PASSTHROUGH_ENV_KEY,
  readCodezToolEnvPassthroughEnv,
} from "@codez/shared";
...
const EXPLICIT_CA_SOURCE_KEYS = [CODEZ_AGENT_CA_CERT_ENV_KEY] as const;
const EXPLICIT_NO_PROXY_SOURCE_KEYS = [CODEZ_NO_PROXY_ENV_KEY] as const;
...
export function applyNetworkEgressEnv(
  ...
  deleteEnvKey(env, CODEZ_TOOL_ENV_PASSTHROUGH_ENV_KEY, platform);
...
function applyToolEnvPassthroughEnv(
  ...
  const passthroughEnv = readCodezToolEnvPassthroughEnv(sourceEnv);
...
function applyProxyEnv(
  ...
  const codezProxy = normalizeProxyValue(
    getFirstEnvValue(sourceEnv, [CODEZ_HTTP_PROXY_ENV_KEY], platform),
  );
  if (codezProxy) {
    for (const key of ALL_PROXY_KEYS) {
      setEnvKey(env, key, codezProxy, platform);
    }
    return;
  }
}
