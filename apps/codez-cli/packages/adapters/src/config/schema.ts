...
export const LEGACY_CUA_PLUGIN_ID = "codez-cua@codez-plugins-official";
export const CANONICAL_CUA_PLUGIN_ID = "computer-use@codez-plugins-official";
...
export const CodezConfigFileSchema = z
  .object({
...
export type CodezConfigFile = z.infer<typeof CodezConfigFileSchema>;
...
function normalizeMcpServerConfigInput(value: unknown): unknown {
  ...
  // External MCP configs may carry provider-specific timeout fields. Codez does not migrate
  ...
  if (server.type === "remote") {
    // 外部 Agent 配置常把 HTTP MCP 标记为 remote；Codez 运行态协议类型是 http。
    server.type = "http";
...
export function parseConfigFileToRuntimePatchWithDiagnostics(
  ...
  const parsed = CodezConfigFileSchema.parse(normalized);
  return {
...
function parsedConfigFileToRuntimePatch(parsed: CodezConfigFile): RuntimeConfigPatch {
...
function normalizePluginConfig(
  plugins: CodezConfigFile["plugins"],
