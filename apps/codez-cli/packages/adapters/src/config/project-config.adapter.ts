...
import type { McpServerConfig, RuntimeConfigPatch } from "@codez/contracts";
import {
    ...
    ...(hooks
      ? {
          hookCandidate: createWorkspaceHookSourceInput({
            ...
            // hooks 字段已由 loadFileConfig 经 CodezConfigFileSchema（shared 单源
...
function getProjectConfigBaseDir(path: string): string {
  ...
  return basename(configDirectory) === ".codez" ? dirname(configDirectory) : configDirectory;
}
