// Env Config Adapter - Parse the intentionally small CODEZ_* environment surface.
...
const DEFAULT_PREFIX = "CODEZ_";

/**
 * Parse CODEZ_* environment variables into config
 */
...
export function getToolConcurrencyConfig(): { maxConcurrency: number } {
  return {
    maxConcurrency: normalizeNumber(process.env.CODEZ_MAX_TOOL_CONCURRENCY ?? "10"),
  };
}
