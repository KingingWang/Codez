...
import type { CustomCommandRoot, CustomCommandSource } from "@codez/contracts";

const COMMANDS_DIR = "commands";
const GIT_MARKER = ".git";
const HOME_PREFIX = "~/";
const PRIORITY_STEP = 10;
const CODEZ_DIR = ".codez";
...
function commandRootsForBase(
  ...
  // 合并而不是 fallback：兼容 `.agents` 命令和原生 `.codez` 命令需要同时可见。
  // 同一级别 `.codez` 先扫描，命令同名时仍按“先到先赢”处理。
  return [
    root(join(baseDirectory, CODEZ_DIR, COMMANDS_DIR), scope, "codez", nextPriority()),
