...
const CREDENTIAL_SECRET_ENV_KEY = "CODEZ_CREDENTIAL_SECRET";

export interface CodezCredentialCipher {
  decrypt(value: string): string;
  encrypt(value: string): string;
}

export interface CodezCredentialCipherOptions {
  env?: Record<string, string | undefined>;
}

export function createCodezCredentialCipher(
  options: CodezCredentialCipherOptions = {},
): CodezCredentialCipher {
  const key = deriveCipherKey(resolveCredentialSecret(options.env ?? process.env));

  return {
    decrypt(value: string): string {
      if (!isEncryptedCodezCredentialValue(value)) {
        return value;
      }
...
export function isEncryptedCodezCredentialValue(value: string): boolean {
...
function resolveCredentialSecret(env: Record<string, string | undefined>): string {
  ...
  return `codez-credential-fallback:${platform()}:${homedir()}:${username}`;
}
