...
import { atomicWritePrivateTextFile, backupCorruptFile, withFileLock } from "@codez/shared/node";
import { createCodezCredentialCipher, type CodezCredentialCipher } from "./credential-cipher.js";

const CODEZ_DATA_BASE_DIR_ENV_KEY = "CODEZ_DATA_BASE_DIR";
...
export const SHARED_CODEZ_CREDENTIAL_KEYS = {
  ...
  codezJwtToken: "codezjwttoken",
} as const;

export interface SharedCodezCredentialStoreOptions {
  baseDir?: string;
  cipher?: CodezCredentialCipher;
...
export interface SharedCodezCredentialStore {
...
export function createSharedCodezCredentialStore(
  options: SharedCodezCredentialStoreOptions = {},
): SharedCodezCredentialStore {
  const env = options.env ?? process.env;
  const filePath = resolveSharedCodezCredentialsPath(options);
  const cipher = options.cipher ?? createCodezCredentialCipher({ env });
  ...
  return {
    ...
    async clearZaiLoginCredentials(): Promise<void> {
      await mutateRawCredentialRecord(filePath, async (rawCredentials) => {
        const activeProviderRaw = rawCredentials[SHARED_CODEZ_CREDENTIAL_KEYS.activeProvider];
        const activeProvider = activeProviderRaw ? cipher.decrypt(activeProviderRaw) : null;
        delete rawCredentials[SHARED_CODEZ_CREDENTIAL_KEYS.zaiAccessToken];
        delete rawCredentials[SHARED_CODEZ_CREDENTIAL_KEYS.zaiRefreshToken];
        delete rawCredentials[SHARED_CODEZ_CREDENTIAL_KEYS.zaiUserInfo];
        delete rawCredentials[SHARED_CODEZ_CREDENTIAL_KEYS.codezJwtToken];
        if (activeProvider === ZAI_PROVIDER_ID) {
          delete rawCredentials[SHARED_CODEZ_CREDENTIAL_KEYS.activeProvider];
        }
      });
    },
    ...
    async saveZaiLoginCredentials(payload: ZaiLoginCredentialPayload): Promise<void> {
      ...
      await mutateRawCredentialRecord(filePath, (rawCredentials) => {
        rawCredentials[SHARED_CODEZ_CREDENTIAL_KEYS.activeProvider] =
          encryptedCredentials.activeProvider;
        rawCredentials[SHARED_CODEZ_CREDENTIAL_KEYS.zaiAccessToken] =
          encryptedCredentials.accessToken;
        rawCredentials[SHARED_CODEZ_CREDENTIAL_KEYS.codezJwtToken] = encryptedCredentials.jwtToken;
        rawCredentials[SHARED_CODEZ_CREDENTIAL_KEYS.zaiUserInfo] = encryptedCredentials.userInfo;
      });
    },
  };
}

export function loadSharedCodezCredentialSync(
  key: string,
  options: SharedCodezCredentialStoreOptions = {},
): string | undefined {
  const filePath = resolveSharedCodezCredentialsPath(options);
  ...
  try {
    ...
    const cipher = options.cipher ?? createCodezCredentialCipher({ env: options.env });
...
export function resolveSharedCodezCredentialsPath(
  options: SharedCodezCredentialStoreOptions = {},
): string {
  if (options.filePath) {
  ...
  const env = options.env ?? process.env;
  const baseDir = options.baseDir ?? env[CODEZ_DATA_BASE_DIR_ENV_KEY] ?? homedir();
  return join(resolveUserPath(baseDir), ".codez", "v2", "credentials.json");
}
...
async function readRawCredentialRecord(filePath: string): Promise<Record<string, string>> {
  ...
  try {
    ...
    throw new Error(`Unable to read shared Codez credentials: ${filePath}`, { cause: error });
  }
  ...
  try {
    ...
    throw new Error(`Shared Codez credentials are corrupt: ${filePath}.${evidence}`, {
      cause: error,
    });
  }
}
