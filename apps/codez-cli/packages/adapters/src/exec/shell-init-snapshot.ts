...
import { windowsPathToGitBashPath } from "@codez/contracts";
import type { ExecutionShellDialect } from "@codez/contracts";
...
function pathExportLines(pathValue: string): string[] {
  return [
    ...
    "cat >> \"$SNAPSHOT_FILE\" << 'PATH_END_CODEZ_SHELL_INIT_SNAPSHOT'",
    `export PATH=${shellQuoteAlways(pathValue)}`,
    "PATH_END_CODEZ_SHELL_INIT_SNAPSHOT",
  ];
}
