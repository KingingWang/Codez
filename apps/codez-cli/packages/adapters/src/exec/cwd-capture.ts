...
export function createCwdCapturePlan(
  ...
  const cwdFilePath = join(cwdCaptureDir, `codez-${crypto.randomUUID()}-cwd`);
...
function createPosixCwdCaptureCommand(command: string, cwdFilePath: string): string {
  return [
    command,
    "__codez_status=$?",
    `if [ "$__codez_status" -eq 0 ]; then pwd -P > ${shellQuote(cwdFilePath)}; fi`,
    'exit "$__codez_status"',
  ].join("\n");
}
...
function createWindowsCmdCwdCaptureCommand(command: string, cwdFilePath: string): string {
  return [
    command,
    'set "__codez_status=%ERRORLEVEL%"',
    `if "%__codez_status%"=="0" cd > ${cmdQuote(cwdFilePath)}`,
    "exit /b %__codez_status%",
  ].join("\r\n");
}
