...
import type { WorkspaceHookTrustRecord, WorkspaceHookTrustStoreFile } from "@codez/contracts";
import {
...
export async function resolveWorkspaceHookTrustStorePath(
  ...
  const userConfigPath = resolve(
    options.userConfigPath ?? join(home, ".codez", "cli", "config.json"),
  );
  ...
  const storageRoot = configured ? resolveTrustedUserPath(configured, home) : join(home, ".codez");
