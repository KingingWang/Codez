...
import { acquireFileLock } from "@codez/shared/node";
import { CODEZ_FILE_LOCK_TIMEOUT_ERROR_CODE } from "@codez/shared";
import type { SharedCodezCredentialStore } from "../auth/shared-credentials.js";
...
export async function tryAcquireAuthorizationLease(input: {
  ...
  try {
    ...
    if (getErrorCode(error) === CODEZ_FILE_LOCK_TIMEOUT_ERROR_CODE) return undefined;
