...
import { CODEZ_MCP_RESOURCE_SAMPLE_INTERVAL_MS, type CodezMcpResourceSample } from "@codez/shared";
import {
  createProcessProbe,
  type ProcessProbe,
...
export interface McpResourceTelemetryOptions {
  arch: CodezMcpResourceSample["arch"];
  platform: CodezMcpResourceSample["platform"];
  now(): number;
  getProcesses(): McpResourceProcess[];
  onResourceSamples?(samples: CodezMcpResourceSample[]): void;
...
export function createMcpResourceTelemetry(options: McpResourceTelemetryOptions) {
  ...
  const sampleNow = async (): Promise<void> => {
    ...
    try {
      ...
      const intervalMs =
        previousAt === undefined
          ? CODEZ_MCP_RESOURCE_SAMPLE_INTERVAL_MS
          : Math.max(1, sampledAt - previousAt);
      const next = new Map<string, Map<number, number>>();
      const groups = new Map<string, CodezMcpResourceSample>();
