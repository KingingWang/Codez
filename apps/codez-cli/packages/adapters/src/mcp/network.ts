...
import { sanitizeCodezRuntimeEnv } from "@codez/shared";
...
export function buildMcpStdioEnv(options: {
  ...
  return prependRunningNodeDirectory(
    applyNetworkEgressEnv(sanitizeCodezRuntimeEnv(filterStringEnv(sourceEnv)), {
...
function prependRunningNodeDirectory(
  ...
  // remote Agent 由 ~/.codez/server/node 启动，但登录环境 PATH 不含该目录，
