...
import type { CodezMcpTelemetryEvent } from "@codez/shared";
import {
...
type McpTelemetryEvent = CodezMcpTelemetryEvent;
...
interface CreateMcpTelemetryTrackerOptions {
  arch?: CodezMcpTelemetryEvent["arch"];
  idSalt: string;
  now?: () => number;
  onEvent(event: McpTelemetryEvent): void;
  platform?: CodezMcpTelemetryEvent["platform"];
  randomId?: () => string;
...
export function createMcpTelemetryTracker(
  ...
  const arch = options.arch ?? (process.arch as CodezMcpTelemetryEvent["arch"]);
  const now = options.now ?? Date.now;
  const platform = options.platform ?? (process.platform as CodezMcpTelemetryEvent["platform"]);
