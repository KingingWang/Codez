...
import type { LogContext, LogEntry, Logger, LoggerFactory, LogRedactor } from "@codez/contracts";
import { LogLevel, LogLevelName } from "@codez/contracts";
...
export function createNodeLoggerFactory(options: NodeLoggerFactoryOptions = {}): NodeLoggerFactory {
  ...
  const logDir = options.logDir ?? options.env?.CODEZ_LOG_DIR ?? getDefaultLogDir();
  const consoleStream =
    typeof options.console === "object"
      ? options.console.stream
      : options.console === true || options.env?.CODEZ_LOG_CONSOLE === "1"
        ? process.stderr
        : undefined;
...
export function getDefaultLogDir(): string {
  return join(homedir(), ".codez", "cli", "log");
}
...
function isDevelopmentMode(env: NodeJS.ProcessEnv): boolean {
  const runtimeEnv = normalizeCodezRuntimeEnv(env[CODEZ_RUNTIME_ENV_KEY]);
...
function getLogFileName(): string {
  return `codez-${formatLocalLogDate(new Date())}.jsonl`;
}
