...
interface CodezBootstrapModule {
  createModelAdapter(input: unknown): unknown;
  createCodezApp(input: unknown): Promise<{
    close?: () => Promise<void>;
...
interface CodezContractsModule {
...
export async function recordPromptTrajectoryFromFixture(input: {
  ...
  try {
    const { bootstrap, contracts, provider } = await loadCodezModules();
    ...
    const app = await bootstrap.createCodezApp({
...
async function loadCodezModules(): Promise<{
  bootstrap: CodezBootstrapModule;
  contracts: CodezContractsModule;
  provider: CodezProviderModule;
}> {
  ...
  try {
    const [bootstrap, contracts, providerConfig, providerRegistry] = await Promise.all([
      import(bootstrapPath) as Promise<CodezBootstrapModule>,
      import(contractsPath) as Promise<CodezContractsModule>,
      import(providerConfigPath) as Promise<Omit<CodezProviderModule, "ProviderRegistry">>,
      import(providerRegistryPath) as Promise<Pick<CodezProviderModule, "ProviderRegistry">>,
    ]);
    return {
      bootstrap,
      contracts,
    ...
    throw new Error(
      "Missing built codez-cli packages. Run `pnpm --filter @codez/bootstrap^... build && pnpm --filter @codez/bootstrap build` before `record`.",
      { cause: error },
    );
  }
}
