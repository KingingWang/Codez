# @codez/prompt-trajectory

OpenAI protocol trajectory recorder for inspecting codez-cli prompt assembly.
...
```bash
pnpm --filter @codez/bootstrap^... build
pnpm --filter @codez/bootstrap build

pnpm --filter @codez/prompt-trajectory record -- \
  --fixture /path/to/recording.json \
  --out /tmp/codez-prompt-trajectory/basic-live

pnpm --filter @codez/prompt-trajectory record:prompt -- \
  --prompt "Say hello in one short sentence."

pnpm --filter @codez/prompt-trajectory derive -- \
  --out /tmp/codez-prompt-trajectory/basic-live

pnpm --filter @codez/prompt-trajectory model-io -- \
  --input ~/.codez/cli/debug/model-io-<session>.jsonl \
  --out /tmp/codez-prompt-trajectory/model-io-session
```
...
uses the same codez model config resolution as the CLI. The upstream request is
...
`model-io` reads a real Codez `model-io-*.jsonl` file and turns the main
