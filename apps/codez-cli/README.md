...
- `npm run build` produces `dist/codez.cjs`, which works anywhere Node.js 24.14.0 is installed.
...
codez plugins are local bundles that can contribute skills, custom commands, and MCP servers.

Plugin state lives under `~/.codez/cli/plugins`:
...
- `marketplaces/codez-plugins-official/`: bundled and CDN partitions plus the merged metadata for the single official marketplace.

This repository also ships built-in official plugins as workspace packages. The bundled Browser Use, Document Skills, Skill Creator, and Codez Guide content plugins are default-enabled and appear as `browser-use@codez-plugins-official`, `document-skills@codez-plugins-official`, `skill-creator@codez-plugins-official`, and `codez-guide@codez-plugins-official`. Runtime-heavy official plugins, and local-data migration plugins such as `ios-simulator@codez-plugins-official`, `android-emulator@codez-plugins-official`, and `restore-legacy-sessions@codez-plugins-official`, are discovered by codez but stay disabled until the user enables them.

```sh
codez plugins list
codez plugins enable ios-simulator
codez plugins disable browser-use
codez plugins enable restore-legacy-sessions
codez plugins disable ios-simulator
```
...
MCP config can live directly in `.codez-plugin/plugin.json` through `mcpServers`. A plugin may provide both `.mcp.json` and manifest `mcpServers`; when the same server name appears in both places, `mcpServers` from the selected manifest wins.

Supported fields in the current codez plugin surface:
...
Example `.codez-plugin/plugin.json` with inline MCP config:
...
```json
{
  ...
  "mcpServers": {
    "ios-simulator": {
      "command": "node",
      "args": ["${CODEZ_PLUGIN_ROOT}/dist/mcp/server.js"],
      "cwd": "${CODEZ_PROJECT_DIR}",
      "env": {
        "PLUGIN_DATA": "${CODEZ_PLUGIN_DATA}",
...
Only environment variables with the `CODEZ_` prefix are expanded. Missing variables disable the affected MCP server and produce a plugin diagnostic.

### Recommended Layout

```txt
my-plugin/
  .codez-plugin/plugin.json
  .mcp.json
  skills/
    my-skill/SKILL.md
  commands/
...
For MCP servers, prefer Node's normal package build and `bin` output when targeting codez-cli, and keep all process/file/network side effects inside the MCP server boundary.

## MCP Configuration

codez reads MCP servers from the main JSON config. The default user config path is `~/.codez/cli/config.json`; MCP entries live under `mcp.servers`. MCP is enabled by default, so `features.mcp` only needs to be set when you want an explicit on/off switch. The current CLI does not auto-discover standalone `mcp.json` or `.mcp.json` files outside enabled plugins.
...
- `stdio`: requires `command`; accepts `args`, `cwd`, `env`, `enabled`, and `timeoutMs`. `cwd` is resolved from the active working directory, and the server process inherits codez's environment plus any `env` overrides.
...
codez reads hooks from the same main JSON config file as MCP, usually `~/.codez/cli/config.json`. Hooks are disabled by default; set `hooks.enabled` to `true` and add process hooks under `hooks.events`.
