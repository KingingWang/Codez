...
import { atomicWritePrivateTextFile, withFileLock } from "@codez/shared/node";
import {
  decodeCodezBuiltinRelease,
  encodeCodezBuiltinRelease,
  serializeCodezBuiltinRelease,
  type CodezBuiltinRelease,
} from "./codez-builtin-release.js";

export interface NodeCodezBuiltinProviderConfigSourceOptions {
  readonly bundledFilePath: string;
  readonly activeFilePath?: string;
  readonly watch?: boolean;
}

export type ApplyCodezBuiltinReleaseResult = "updated" | "unchanged" | "stale";

/** Bundled、Active/LKG 与 Remote 共用同一 Release，并最终发布为现有 Config Snapshot。 */
export class NodeCodezBuiltinProviderConfigSource implements ProviderSource<ProviderConfigLayerSnapshot> {
  readonly #bundledFilePath: string;
  readonly #activeFilePath: string;
  ...
  constructor(options: NodeCodezBuiltinProviderConfigSourceOptions) {
    const bundledFilePath = options.bundledFilePath.trim();
    if (!bundledFilePath) throw new Error("Codez Built-in bundledFilePath 不能为空");
  ...
  async read(): Promise<ProviderConfigLayerSnapshot> {
    ...
    let release: CodezBuiltinRelease;
    try {
  ...
  async applyRemoteRelease(release: CodezBuiltinRelease): Promise<ApplyCodezBuiltinReleaseResult> {
    this.#assertNotDisposed();
    await this.#ensureWatcher();
    const result = await withFileLock(this.#activeFilePath, async () => {
      ...
      if (release.revision === current.revision) {
        if (serializeCodezBuiltinRelease(release) === serializeCodezBuiltinRelease(current)) {
          return "unchanged" as const;
        }
        throw new Error(`Codez Built-in 相同 revision ${release.revision} 对应不同内容`);
      }
  ...
  async #readAndMaterializeLocked(): Promise<CodezBuiltinRelease> {
  ...
  async #writeActiveLocked(release: CodezBuiltinRelease): Promise<void> {
    this.#assertNotDisposed();
    await atomicWritePrivateTextFile(
      this.#activeFilePath,
      JSON.stringify(encodeCodezBuiltinRelease(release), null, 2),
    );
  }
  ...
  #assertNotDisposed(): void {
    if (this.#disposed) throw new Error("NodeCodezBuiltinProviderConfigSource 已 dispose");
  }
}

export function createNodeCodezBuiltinProviderConfigSource(
  options: NodeCodezBuiltinProviderConfigSourceOptions,
): NodeCodezBuiltinProviderConfigSource {
  return new NodeCodezBuiltinProviderConfigSource(options);
}

interface ReleaseCandidate {
  readonly release?: CodezBuiltinRelease;
  readonly error?: unknown;
}

async function readReleaseCandidate(filePath: string): Promise<ReleaseCandidate | null> {
  try {
    return { release: decodeCodezBuiltinRelease(JSON.parse(await readFile(filePath, "utf8"))) };
  } catch (error) {
...
function selectReleaseCandidate(
  ...
  if (
    ...
    serializeCodezBuiltinRelease(bundled.release) !== serializeCodezBuiltinRelease(active.release)
  ) {
  ...
  const valid = [bundled?.release, active?.release].filter(
    (candidate): candidate is CodezBuiltinRelease => candidate !== undefined,
  );
  if (valid.length === 0) {
    throw new AggregateError(
      [bundled?.error, active?.error].filter((error) => error !== undefined),
      "Bundled 与 Active Codez Built-in Release 均不可用",
    );
  }
...
function snapshotFromRelease(
  release: CodezBuiltinRelease,
  sourceKey: string,
): ProviderConfigLayerSnapshot {
  return Object.freeze({
    revision: `codez-builtin:${release.revision}:${sourceKey}`,
...
function signatureOf(release: CodezBuiltinRelease): string {
  return `${release.revision}:${serializeCodezBuiltinRelease(release)}`;
}
