import { z } from "zod";
import { decodeCodezBuiltinRelease, type CodezBuiltinRelease } from "./codez-builtin-release.js";
...
export interface CodezBuiltinDownloadOptions {
...
export async function downloadCodezBuiltinRelease(
  options: CodezBuiltinDownloadOptions,
  ...
  try {
    ...
    stage = "cdn";
    return decodeCodezBuiltinRelease(await readJson(new URL(downloadUrl)));
  } catch (error) {
    ...
    const reason = signal.aborted
      ...
      : error instanceof DownloadBoundaryError
        ...
        : error instanceof z.ZodError
          ...
          : "invalid response";
    throw new Error(`Codez Built-in ${stage}: ${reason}`);
  } finally {
    clearTimeout(timer);
  }
