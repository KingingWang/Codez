...
import type { NodeCodezBuiltinProviderConfigSource } from "./codez-builtin-provider-config-source.js";
import type { CodezBuiltinRelease } from "./codez-builtin-release.js";

const HOUR_MS = 60 * 60 * 1_000;

export type CodezBuiltinRefreshResult =
  | "updated"
  | "unchanged"
  | "stale"
  | "missing"
...
export interface CodezBuiltinRemoteSynchronizerOptions {
  ...
  readonly onRefreshResult?: (event: CodezBuiltinRefreshEvent) => void;
  readonly now?: () => number;
...
export interface CodezBuiltinRefreshEvent {
  readonly result: CodezBuiltinRefreshResult;
...
export class CodezBuiltinRemoteSynchronizer {
  readonly #options: CodezBuiltinRemoteSynchronizerOptions;
  readonly #now: () => number;
  #inFlight: Promise<CodezBuiltinRefreshResult> | null = null;
  #disposed = false;
  #abortController: AbortController | null = null;

  constructor(options: CodezBuiltinRemoteSynchronizerOptions) {
    this.#options = options;
    this.#now = options.now ?? Date.now;
  }

  refresh(options: { readonly force?: boolean } = {}): Promise<CodezBuiltinRefreshResult> {
  ...
  async #refresh(force: boolean, signal: AbortSignal): Promise<CodezBuiltinRefreshResult> {
    const endpointKey = (await this.#options.resolveEndpointKey()).trim();
    if (this.#disposed) return "disposed";
    if (!endpointKey) throw new Error("Codez Built-in 远端 Endpoint 不能为空");
  ...
  #report(event: CodezBuiltinRefreshEvent): void {
