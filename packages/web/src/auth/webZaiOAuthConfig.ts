...
function createWebZaiOAuthConfig(env: WebImportMetaEnv = {}): WebZaiOAuthConfig {
  ...
  const codezEndpointUrls = buildCodezEndpointUrls(
    env.VITE_CODEZ_BASE_URL?.trim() ||
      env.VITE_CODEZ_ENDPOINT_ORIGIN?.trim() ||
      DEFAULT_CODEZ_ENDPOINT_ORIGIN,
  );
  ...
  return {
    ...
    // BigModel 用 appId 而不是 client_id，且默认值就是桌面端在用的 "codez"。
    bigmodelAppId: env.VITE_BIGMODEL_OAUTH_APP_ID?.trim() || "codez",
    redirectUri: codezEndpointUrls.webShareCallbackUrl,
    shareRedirectUri: codezEndpointUrls.webShareCallbackUrl,
    ...(devOrigin ? { devOrigin } : {}),
