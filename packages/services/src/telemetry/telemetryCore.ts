...
function sessionCreateEventId(userId: string, sessionId: string): string {
  const bytes = createHash("sha256")
    .update(JSON.stringify(["codez:session_create:v1", userId, sessionId]))
    .digest()
    .subarray(0, 16);
...
function resolveTelemetryStateFile(homeDir?: string): string {
  if (homeDir) {
    return join(homeDir, ".codez", "v2", "telemetry-state.json");
  }
...
function resolveTelemetryLockFile(homeDir?: string): string {
  if (homeDir) {
    return join(homeDir, ".codez", "v2", "telemetry-state.lock");
  }
...
export function createTelemetryCore(dependencies: TelemetryCoreDependencies = {}) {
  ...
  const appVersion = dependencies.appVersion ?? CODEZ_VERSION;
  ...
  async function sendReport(
    ...
    if (!CODEZ_TELEMETRY_ENABLED || !CODEZ_TELEMETRY_REPORT_ENDPOINT) {
      return;
    }
    ...
    const endpoint = String(
      rewriteCodezEndpointUrl(
        CODEZ_TELEMETRY_REPORT_ENDPOINT,
        (await dependencies.resolveCodezEndpointOrigin?.()) ?? CODEZ_TELEMETRY_REPORT_ENDPOINT,
      ),
    );

    const headers = buildCodezSourceHeadersFromContext({
      appVersion,
      platform,
      arch: dependencies.arch ?? process.arch,
      osVersion,
      releaseChannel: dependencies.releaseChannel ?? CODEZ_ENV,
