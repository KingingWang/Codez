...
import { DEFAULT_ENABLED_OFFICIAL_PLUGIN_IDS } from "@codez/shared";
...
const DEFAULT_COMMAND_AGENT_SOURCE: CommandAgentSource = CODEZ_COMMAND_AGENT_SOURCE;
const COMMAND_AGENT_SOURCE_ORDER: readonly CommandAgentSource[] = CODEZ_COMMAND_AGENT_SOURCES;
const ENABLE_OVERRIDE_KEY = "enable";
const HOME_PREFIX = "~/";
const CODEZ_OFFICIAL_PLUGIN_MARKETPLACE = "codez-plugins-official";
const CODEZ_INLINE_PLUGIN_MARKETPLACE = "inline";
const CODEZ_PLUGIN_MANIFEST_PATH = join(".codez-plugin", "plugin.json");
...
const CODEZ_COMMAND_DESCRIPTOR: CommandAgentSourceDescriptor = {
  agentSource: "codezAgent",
  directorySource: "codez",
  userDirectorySegments: [".codez", "commands"],
  workspaceDirectorySegments: [".codez", "commands"],
  fileExtension: ".md",
  format: "markdown",
  namespaceSeparator: "/",
...
async function scanOfficialPluginCacheRoots(pluginStorageRoot: string): Promise<string[]> {
  const cacheRoot = join(pluginStorageRoot, "cache", CODEZ_OFFICIAL_PLUGIN_MARKETPLACE);
...
async function findPluginManifestPath(rootPath: string): Promise<string | null> {
  for (const manifestPath of [
    join(rootPath, CODEZ_PLUGIN_MANIFEST_PATH),
...
async function resolvePluginCommandRootDescriptors(): Promise<PluginCommandRootDescriptor[]> {
  ...
  const candidates: PluginRootCandidate[] = [
    ...config.dirs.map((dir) => ({
      defaultEnabled: true,
      marketplace: CODEZ_INLINE_PLUGIN_MARKETPLACE,
  ...
  for (const candidate of candidates) {
    ...
    // 仍会从 cache 贡献命令。
    if (
      candidate.marketplace === CODEZ_OFFICIAL_PLUGIN_MARKETPLACE &&
...
export function createCommandsService(_options?: CommandsServiceOptions): ICommandsService {
  async function list(params: {
    ...
    // Codez Agent 需要先合并所有 workspace 目录，再合并所有 user 目录；
    // 按每个目录交错读取 project/user 会让 user .codez 抢在 workspace .agents 前面。
    for (const agentSource of agentSources) {
      const descriptors =
        agentSource === CODEZ_COMMAND_AGENT_SOURCE
    ...
    const pluginCommands =
      !params.agentSource || params.agentSource === CODEZ_COMMAND_AGENT_SOURCE
        ? await discoverPluginCommands(enabledOverrides)
        : [];

    return {
      commands: [...dedupedUserCommands, ...pluginCommands] as CodezCommand[],
...
async function discoverPluginCommandsRecursive(
  ...
  for (const entry of entries) {
    ...
    if (!entry.toLowerCase().endsWith(CODEZ_COMMAND_DESCRIPTOR.fileExtension)) {
      continue;
    }
    ...
    try {
      ...
      const parsed = CommandFileParser.parseCommandFile(
        content,
        fullPath,
        CODEZ_COMMAND_DESCRIPTOR.format,
      );
      if (!parsed) {
        continue;
      }
      const name = getCommandName(rootDir, fullPath, CODEZ_COMMAND_DESCRIPTOR);
...
async function discoverCommandsFromDirectorySources(params: {
  ...
  for (const descriptor of params.descriptors) {
    ...
    // `.codez` 是强优先级来源；只要读到有效命令，同 scope 的 `.agents` 就不再参与。
    if (descriptor.directorySource === "codez" && discoveredCount > 0) {
      break;
    }
  }
}
