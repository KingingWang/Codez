...
import { buildCodezToolEnvPassthroughEnv, sanitizeCodezRuntimeEnvInPlace } from "@codez/shared";
...
function buildLoginShellEnvPatch(
  ...
  return {
    ...patch,
    ...buildCodezToolEnvPassthroughEnv(snapshot),
  };
}
...
export function buildRuntimeProcessEnvPatch(
  ...
  const toolEnvPassthroughPatch = buildCodezToolEnvPassthroughEnv({
    ...normalizedBaseEnv,
    ...loginShellEnvPatch,
  });
  ...
  const envPatch: Record<string, string> = {
    ...
    // encoding is set. Codez/Bash tool output is consumed as UTF-8, so force Python
...
export function initializeRuntimeProcessEnv(
  ...
  // 网络变量会先封存为 CODEZ_TOOL_ENV_PASSTHROUGH_JSON，只有 Bash/tool 子进程边界才恢复原名。
  sanitizeCodezRuntimeEnvInPlace(process.env);
