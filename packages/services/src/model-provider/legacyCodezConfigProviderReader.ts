/* oxlint-disable eslint(max-lines) -- 已发布旧 Codez config.json 的多版 Provider 结构读取集中在同一边界。 */
...
function normalizeBigModelCodingPlanAnthropicBaseUrlForEnv(
  ...
  const fallbackBaseUrl =
    resolveRuntimeCodezEnv(env) === "production"
  ...
  if (resolveRuntimeCodezEnv(env) === "production") {
...
function getCodezConfigFilePath(): string {
...
interface CodezConfigFile {
  $schema?: string;
  provider?: Record<string, CodezOpenCodeProviderConfig>;
  [key: string]: unknown;
}

interface CodezOpenCodeProviderConfig {
  api?: string;
  name?: string;
  env?: string[];
  id?: string;
  ...
  createdAt?: number;
  updatedAt?: number;
  models?: Record<string, CodezOpenCodeModelConfig>;
  codez?: CodezProviderConfigExtension;
  [key: string]: unknown;
}

interface CodezOpenCodeModelConfig {
  id?: string;
  name?: string;
  family?: string;
  release_date?: string;
  attachment?: boolean;
  reasoning?: boolean | CodezReasoningConfigExtension;
  temperature?: boolean;
  tool_call?: boolean;
  interleaved?: unknown;
  cost?: JsonObject;
  ...
  modified?: boolean;
  deleted?: boolean;
  codez?: CodezModelConfigExtension;
  [key: string]: unknown;
}

interface CodezReasoningConfigExtension {
  enabled?: boolean;
  variants?: string[];
...
interface CodezProviderConfigExtension {
  enabled?: boolean;
  systemDisabledReason?: ModelProviderSystemDisabledReason;
  endpoints?: ModelProviderEndpoints;
  apiFormat?: ModelProviderApiFormat;
...
interface CodezModelConfigExtension {
  kinds?: ModelProviderKind[];
  defaultKind?: ModelProviderKind;
  modelIdByKind?: Partial<Record<ModelProviderKind, string>>;
  disabledReason?: string;
...
const codezModelProviderModalitySchema = z.enum(["text", "image", "video", "audio", "pdf"]);
const codezProviderModelMappingsSchema = z.record(z.string(), z.unknown());

const codezReasoningConfigExtensionSchema = z
  .object({
...
const codezModelConfigExtensionSchema = z
  .object({
...
const codezOpenCodeModelConfigSchema: z.ZodType<CodezOpenCodeModelConfig> = z
  .object({
    id: z.string().optional(),
    name: z.string().optional(),
    reasoning: z.union([z.boolean(), codezReasoningConfigExtensionSchema]).optional(),
    limit: z
      .object({
    ...
    modalities: z
      .object({
        input: z.array(codezModelProviderModalitySchema).optional(),
        output: z.array(codezModelProviderModalitySchema).optional(),
      })
      .optional(),
    ...
    modified: z.boolean().optional(),
    deleted: z.boolean().optional(),
    codez: codezModelConfigExtensionSchema.optional(),
  })
  .passthrough();

const codezProviderConfigExtensionSchema = z
  .object({
    enabled: z.boolean().optional(),
    ...
    defaultKind: modelProviderKindSchema.optional(),
    providerMappings: codezProviderModelMappingsSchema.optional(),
...
const codezOpenCodeProviderConfigSchema: z.ZodType<CodezOpenCodeProviderConfig> = z
  .object({
    ...
    createdAt: z.number().optional(),
    updatedAt: z.number().optional(),
    models: z.record(z.string(), codezOpenCodeModelConfigSchema).optional(),
    codez: codezProviderConfigExtensionSchema.optional(),
  })
  .passthrough();

const codezConfigFileSchema = z
  .object({
    $schema: z.string().optional(),
    provider: z.record(z.string(), codezOpenCodeProviderConfigSchema).optional(),
  })
  .passthrough();
...
function inferModelProviderKindFromOpenCodeProvider(
  provider: CodezOpenCodeProviderConfig,
): ModelProviderKind {
  if (provider.kind) {
...
function hasOpenCodeProviderRuntimeFields(provider: CodezOpenCodeProviderConfig): boolean {
  // config.json 的权威运行态字段是 kind/options.baseURL。
  // 旧 endpoints/apiFormat/defaultKind/codez 可能是历史迁移残留，若继续优先读取会覆盖新配置。
  return Boolean(
    provider.kind ||
...
function resolvePresetProviderKindFromRuntimeBaseUrl(
  ...
  if (path.includes("/api/anthropic") || path.includes("/codez-plan/anthropic")) {
    return "anthropic";
  }
  return undefined;
}
...
function resolveOpenCodeProviderEndpoints(
  ...
  return provider.endpoints ?? provider.codez?.endpoints ?? {};
}

function stripCodezPrefixedProviderMappings(
  ...
  const entries = Object.entries(providerMappings ?? {}).filter(
    ([key]) => !key.toLowerCase().startsWith("codez"),
  );
  return Object.fromEntries(entries);
}

function openCodeReasoningToModelReasoning(
  model: CodezOpenCodeModelConfig,
  ...
  if (!options.preferOpenCodeFields && model.codez?.reasoning) {
    return stripModelProviderReasoningPatches(model.codez.reasoning);
  }
...
function openCodeModelToModelProviderModel(
  ...
  const codez = options.preferOpenCodeFields ? undefined : model.codez;
  const codezExtensions = model.codez;
  const contextWindow =
  ...
  return createModelProviderModelConfig({
    ...
    priority: readFiniteNumber(model.priority) ?? readFiniteNumber(codezExtensions?.priority),
    disabledReason: model.disabledReason ?? codez?.disabledReason,
    supportsTools: model.supportsTools ?? codez?.supportsTools,
    supportsStructuredOutput: model.supportsStructuredOutput ?? codez?.supportsStructuredOutput,
    modified: model.modified ?? codezExtensions?.modified,
    deleted: model.deleted ?? codezExtensions?.deleted,
  });
}
...
function openCodeProviderToModelProviderConfig(
  ...
  for (const deletedModelId of normalizeDeletedModelIds(codez?.deletedModels)) {
  ...
  const apiKeyRequired =
    provider.apiKeyRequired ?? codez?.apiKeyRequired ?? readBoolean(options.apiKeyRequired);
  const providerMappings = preferOpenCodeFields
    ? undefined
    : stripCodezPrefixedProviderMappings(provider.providerMappings ?? codez?.providerMappings);
  return normalizeProviderForStore({
    ...
    ...(provider.enabled !== undefined
      ...
      : codez?.enabled !== undefined
        ? { enabled: codez.enabled }
        : {}),
    ...((provider.systemDisabledReason ?? codez?.systemDisabledReason)
      ? {
          systemDisabledReason: provider.systemDisabledReason ?? codez?.systemDisabledReason,
        }
      : {}),
    endpoints,
    apiFormat: preferOpenCodeFields
      ? resolveModelProviderKindApiFormat(defaultKind)
      : (provider.apiFormat ?? codez?.apiFormat ?? resolveModelProviderKindApiFormat(defaultKind)),
    ...((provider.source ?? codez?.source)
      ? { source: provider.source ?? codez?.source }
      : { source: "custom" }),
    ...((provider.catalogSourceId ?? codez?.catalogSourceId)
      ? { catalogSourceId: provider.catalogSourceId ?? codez?.catalogSourceId }
      : {}),
    ...((provider.catalogProviderId ?? codez?.catalogProviderId)
      ? {
          catalogProviderId: provider.catalogProviderId ?? codez?.catalogProviderId,
        }
      : {}),
    ...((provider.modelsDevProviderId ?? codez?.modelsDevProviderId)
      ? {
          modelsDevProviderId: provider.modelsDevProviderId ?? codez?.modelsDevProviderId,
        }
      : {}),
    ...(apiKeyRequired !== undefined ? { apiKeyRequired } : {}),
    ...((provider.headers ?? codez?.headers ?? readStringRecord(options.headers))
      ? {
          headers: provider.headers ?? codez?.headers ?? readStringRecord(options.headers),
        }
      : {}),
    ...((provider.logoUrl ?? codez?.logoUrl)
      ? { logoUrl: provider.logoUrl ?? codez?.logoUrl }
      : {}),
    apiKey,
    ...((provider.apiKeyUrl ?? codez?.apiKeyUrl)
      ? { apiKeyUrl: provider.apiKeyUrl ?? codez?.apiKeyUrl }
      : {}),
    defaultKind,
    models,
    ...(providerMappings ? { providerMappings } : {}),
    createdAt:
      (!preferOpenCodeFields ? (provider.createdAt ?? codez?.createdAt) : undefined) ?? now,
    updatedAt:
      (!preferOpenCodeFields ? (provider.updatedAt ?? codez?.updatedAt) : undefined) ?? now,
  });
}

async function readRawCodezConfigFile(): Promise<CodezConfigFile | null> {
  const filePath = getCodezConfigFilePath();
  let raw: string;
  try {
  ...
  const parsed = codezConfigFileSchema.safeParse(json);
  if (!parsed.success) {
...
async function readCodezConfigProviders(): Promise<ModelProviderConfig[] | null> {
  const config = await readRawCodezConfigFile();
  if (!config?.provider) {
    return null;
  }
...
function normalizeProviderForStore(provider: ModelProviderConfig): ModelProviderConfig {
  ...
  return {
    ...
    providerMappings: stripCodezPrefixedProviderMappings(
...
function applyProviderStoreMigrations(providers: ModelProviderConfig[]): ModelProviderConfig[] {
  ...
  // reasoning mapping 由 Codez Built-in/Personal ModelConfigRules 在 Registry 中解析；
...
export async function readLegacyCodezConfigProviders(): Promise<ModelProviderConfig[]> {
  const configProviders = await readCodezConfigProviders();
