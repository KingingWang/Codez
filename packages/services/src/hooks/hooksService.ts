...
} from "@codez/shared/workspace-hook-discovery";
import { parseWorkspaceHookTrustStoreContent } from "@codez/shared/workspace-hook-trust-store-file";
...
const CODEZ_CONFIG_FILE = "config.json";
...
function getRootDir(source: SettingsDirectorySource, workspacePath?: string): string {
  ...
  if (source === "codez") {
    return workspacePath ? join(baseDir, ".codez") : join(baseDir, ".codez", "cli");
  }
...
function getConfigPath(source: SettingsDirectorySource, workspacePath?: string): string {
  return join(
    ...
    source === "codez" ? CODEZ_CONFIG_FILE : SETTINGS_FILE,
  );
}
...
async function readUserCodezSource(): Promise<WorkspaceHookSourceInput | undefined> {
  const path = getConfigPath("codez");
...
async function readPersistentWorkspaceHookTrustDigests(
  ...
  const userConfig = (await readJsonFile<Record<string, unknown>>(getConfigPath("codez"))) ?? {};
...
async function writeCodezHooksConfig(
  workspacePath: string | undefined,
  hooks: Hook[],
): Promise<void> {
  const configPath = getConfigPath("codez", workspacePath);
  const existingConfig = (await readJsonFile<CodezConfigFile>(configPath)) ?? {};
  ...
  await atomicWriteWorkspaceHookConfig(configPath, {
    ...
    hooks: {
      ...
      events: toCodezHooksEvents(hooks),
    },
  });
}
...
async function saveHooksImpl(params: {
  ...
  const currentProjectConfigPath = resolve(params.workspacePath, ".codez", "config.json");
  const userHooks = params.hooks.filter(
    (hook) =>
      hook.editable !== false &&
      (!hook.location || (hook.location.source === "codez" && hook.location.scope === "user")),
  );
  const projectHooks = params.hooks.filter(
    (hook) =>
      hook.editable !== false &&
      hook.location?.source === "codez" &&
      hook.location.scope === "project" &&
      (!hook.configuredState ||
        resolve(hook.configuredState.sourcePath) === currentProjectConfigPath),
  );
  await writeCodezHooksConfig(undefined, userHooks);
  await writeCodezHooksConfig(params.workspacePath, projectHooks);
}
