import { BIGMODEL_PROVIDER_ID, buildBigModelApiUrl } from "@codez/shared";
...
const BIGMODEL_OAUTH_PROVIDER_CONFIG: Omit<OAuthProviderRuntimeConfig, "appSecret"> = {
  ...
  userinfoUrl: buildBigModelApiUrl({ CODEZ_ENV: "production" }, BIGMODEL_USERINFO_PATH),
  appId: "codez",
...
export function createBigModelProviderRuntimeConfig(
  ...
  return {
    ...
    tokenUrl:
      ...
      buildCodezApiUrlFromEnv(env, "/api/v1/oauth/token"),
    ...
    // 当前 BigModel callback 只消费 codez OAuth token 路由，显式 appSecret 仅保留给
