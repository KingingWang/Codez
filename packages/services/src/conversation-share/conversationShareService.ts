...
import {
  ...
  readCodezAttachmentFaultCode,
} from "@codez/shared/codez-protocol-v4";
import { Emitter } from "@codez/rpc";

import type { ICodezAgentService } from "../codez-agent/codezAgent.js";
import type { ICodezSessionService } from "#src/codez-session/codezSession.js";
...
interface ConversationShareServiceOptions {
  codezAgentService: ConversationShareAgentService;
  ...
  codezSessionService?: Pick<ICodezSessionService, "createSession" | "listSessions">;
...
function hasUnsupportedArtifactReference(value: unknown): boolean {
  if (typeof value === "string") return /^codez-artifact:\/\//iu.test(value);
 ...
 * 仅在 fault 码缺席时保留一条 errno 文本兜底，用于尚未带结构化码的旧 codez-cli。
 */
function isDefiniteMissingAttachment(error: unknown): boolean {
  const faultCode = readCodezAttachmentFaultCode(error);
  if (faultCode) {
    return (
      faultCode === CODEZ_ATTACHMENT_FAULT_CODES.shareStatNotFound ||
      faultCode === CODEZ_ATTACHMENT_FAULT_CODES.statNotFile
    );
  }
...
function isAttachmentAuthorizationError(error: unknown): boolean {
  ...
  if (faultCode) {
    return (
      faultCode === CODEZ_ATTACHMENT_FAULT_CODES.shareStatNotAuthorized ||
      faultCode === CODEZ_ATTACHMENT_FAULT_CODES.shareReadNotAuthorized ||
      faultCode === CODEZ_ATTACHMENT_FAULT_CODES.shareStatConnectionUntrusted ||
      faultCode === CODEZ_ATTACHMENT_FAULT_CODES.shareReadConnectionUntrusted
    );
  }
  const message = error instanceof Error ? error.message : String(error);
...
function isAttachmentTooLargeError(error: unknown): boolean {
  const faultCode = readCodezAttachmentFaultCode(error);
  return (
    faultCode === CODEZ_ATTACHMENT_FAULT_CODES.shareStatTooLarge ||
    faultCode === CODEZ_ATTACHMENT_FAULT_CODES.previewTooLarge
  );
}
...
export class ConversationShareService implements IConversationShareService {
  private readonly codezAgentService: ConversationShareAgentService;
  ...
  >();
  private readonly codezSessionService?: Pick<
  ...
  constructor(options: ConversationShareServiceOptions) {
    this.codezAgentService = options.codezAgentService;
    ...
    this.codezSessionService = options.codezSessionService;
    ...
    // 配置的 Codez origin）导入后回链仍指向生产站，点分割线打开的是另一个环境的分享。
    // 改用与 API base 同一个环境解析器（buildRuntimeCodezApiUrl 也走它），保证同环境。
    // 优先级不变：显式 option > CODEZ_CONVERSATION_SHARE_WEB_URL > 按环境推导。
    this.shareWebUrl = (
      options.shareWebUrl ??
      process.env.CODEZ_CONVERSATION_SHARE_WEB_URL ??
      `${resolveRuntimeCodezEndpointOrigin(process.env)}/cn/share`
    ).replace(/\/+$/u, "");
    this.importIndexPath = join(this.conversationWorkspaceRoot, ".codez-share-imports.json");
  ...
  async preflight(
    ...
    try {
      return await this.preflightWithAgent(input, this.codezAgentService);
    } catch (error) {
  ...
  private async importShareInternal(
    ...
    const shareRoot = join(workspacePath, ".codez-share");
    const importRoot = join(shareRoot, sanitizeFileSegment(continuation.share.share_id));
    const markerPath = join(importRoot, ".codez-share-import.json");
    ...
    try {
      ...
      if (
        ...
        const sessions = await this.codezSessionService.listSessions({ workspacePath, limit: 100 });
    ...
    try {
      ...
      for (const artifact of continuation.artifacts) {
        ...
        installedArtifacts.push({
          ...
          workspaceRelativePath: `.codez-share/${sanitizeFileSegment(continuation.share.share_id)}/shared-artifacts/${fileName}`,
      ...
      const snapshot = await this.codezSessionService.createSession({
        workspacePath,
        ...(workspaceIdentity ? { workspaceIdentity } : {}),
        sessionId,
   ...
   *
   * 目录名用的是 share_id 而不是 contextId（二者不等价），所以扫 .codez-share/ 下各
  ...
  async getImportedConversation(input: {
    ...
    const shareRoot = join(input.workspacePath, ".codez-share");
    let entries: Dirent[];
    try {
  ...
  async publish(input: PublishTextConversationInput, operationId: string) {
    return this.publishWithAgent(input, operationId, this.codezAgentService);
  }
  ...
  private async publishInternal(
    input: PublishTextConversationInput,
    operationId?: string,
    agentService: ConversationShareAgentService = this.codezAgentService,
    ...
    const artifactSnapshot = await buildConversationShareArtifactSnapshot({
      codezAgentService: agentService,
  ...
  private async cleanupAbandonedImports(): Promise<void> {
    ...
    for (const entry of imports) {
      ...
      if (typeof marker.sessionId !== "string" || !marker.sessionId.startsWith("share-import-"))
        continue;
      try {
        const sessions = await this.codezSessionService.listSessions({
