...
export function usesCodexBridgeRuntime(
  ...
  if (env.ZCODE_AGENT_SERVER_COMMAND?.trim() || options.customCommandResolver) return false;
  if (env.ZCODE_DESKTOP_RUNTIME?.trim() === "legacy") return false;
  return (
    Boolean(env.ZCODE_CODEX_BRIDGE_PATH?.trim()) ||
    env.ZCODE_DESKTOP_RUNTIME?.trim() === "codex" ||
...
export function resolveCodexBridgeCommand(
  ...
  const deployedBridge = env.ZCODE_CODEX_BRIDGE_PATH?.trim();
  if (deployedBridge && (!isAbsolute(deployedBridge) || !existsSync(deployedBridge))) {
    throw new Error(
      `ZCODE_CODEX_BRIDGE_PATH must point to an existing absolute bridge path: ${deployedBridge}`,
    );
  }
  let bridge =
  ...
  if (!bridge) {
    throw new Error(
      "ZCode Codex bridge missing. Run node scripts/build-codex-bridge.mjs or reinstall the packaged app.",
    );
  }
  const nativeOverride = env.ZCODE_CODEX_COMMAND?.trim();
  const native =
    nativeOverride ||
