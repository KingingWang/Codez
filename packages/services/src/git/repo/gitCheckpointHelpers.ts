...
import type { GitCheckpointDiff, GitCheckpointFileDiff } from "@codez/shared";
...
export function getCheckpointRefName(workspacePath: string, checkpointId: string): string {
  ...
  return `refs/codez/checkpoints/${workspaceHash}/${checkpointId}`;
}
...
export function buildCheckpointEnv(tempIndexPath: string): NodeJS.ProcessEnv {
  return {
    ...
    GIT_AUTHOR_NAME: "Codez Checkpoint",
    GIT_AUTHOR_EMAIL: "checkpoint@codez.local",
    GIT_COMMITTER_NAME: "Codez Checkpoint",
    GIT_COMMITTER_EMAIL: "checkpoint@codez.local",
  };
}
