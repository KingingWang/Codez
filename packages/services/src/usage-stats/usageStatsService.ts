...
import { isCodingPlanModelProviderId } from "@codez/shared";
...
import type { OfficialMcpCredentialSource } from "./providers/codezMcpQuotaProvider.js";
...
export function createUsageStatsService(
  ...
  return {
    async getAppUsageSnapshot(request: AppUsageRequest): Promise<AppUsageSnapshot> {
      ...
      // 经 Codez Protocol usage/stats 取回。不再读本地 session JSON 估算。
      return dependencies.codezAgentService.getAppUsageStats({
        range: request.range,
