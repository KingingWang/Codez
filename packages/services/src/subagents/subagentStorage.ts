...
export async function resolveUserSubagentRoot(options?: SubagentStorageOptions): Promise<string> {
  return join(await resolveCodezStorageRoot(options), "agents");
}

export function resolveWorkspaceSubagentRoot(workspacePath: string): string {
  return join(workspacePath, ".codez", "agents");
}

export async function resolveSubagentStateFile(options?: SubagentStorageOptions): Promise<string> {
  return join(await resolveCodezStorageRoot(options), "v2", "agents-state.json");
}

export async function resolveCodezStorageRoot(options?: SubagentStorageOptions): Promise<string> {
...
async function readUserCliConfig(
  ...
  try {
    const raw = await readFile(
      join(resolveUserHomeDir(options), ".codez", "cli", "config.json"),
      "utf8",
    );
