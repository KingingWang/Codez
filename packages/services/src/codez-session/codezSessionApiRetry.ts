import {
  ...
  resolveWorkspaceKey,
  type CodezSessionApiRetryStatus,
  type CodezSessionStateSnapshot,
  codezApiRetryFromModelNetworkStatusPayload,
  codezApiRetryFromStreamRecoveryPayload,
} from "@codez/shared";
import type { CodezSessionServiceEvent, CodezTaskTarget } from "#src/codez-session/codezSession.js";

export function createCodezSessionApiRetryRuntimeTracker(): {
  trackApiRetryFromSessionEvent: (
    params: CodezTaskTarget,
    event: CodezSessionServiceEvent,
  ) => CodezSessionServiceEvent;
  withApiRetryRuntime: (snapshot: CodezSessionStateSnapshot) => CodezSessionStateSnapshot;
} {
  const apiRetryBySessionKey = new Map<string, CodezSessionApiRetryStatus | null>();
  ...
  function trackApiRetryFromSessionEvent(
    ...
    if (
      ...
      if (apiRetry !== undefined) {
        ...
        isCodezModelRetryRecoveryProgressPayload(asRecord(event.event.payload))
      ) {
  ...
  function withApiRetryRuntime(snapshot: CodezSessionStateSnapshot): CodezSessionStateSnapshot {
  ...
  function sessionKey(params: CodezTaskTarget): string {
...
): CodezSessionApiRetryStatus | null | undefined {
