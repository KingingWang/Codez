/* eslint-disable max-lines -- codezSessionService 聚合 desktop-continuous session 操作、draft lifecycle 和 task index 同步边界，拆分需要单独设计。 */
import type { ICodezAgentService } from "#src/codez-agent/codezAgent.js";
import type { CodezTaskIndexSyncer } from "#src/codez-agent/codezTaskIndexSyncer.js";
...
import type {
  ...
  CodezSessionReadWorkspacePresentationParams,
...
import { formatModelPickerValue } from "#src/codez-agent/codezConfigOptions.js";
import { createCodezSessionApiRetryRuntimeTracker } from "#src/codez-session/codezSessionApiRetry.js";
import { appendWorkspaceToFilesystemMcpServers } from "#src/session/mcpWorkspaceScope.js";
import { repairEmptyImportedClaudeSessionSnapshot } from "#src/codez-session/importedClaudeSessionRepair.js";
import { createCodezDeferredDraftRegistry } from "#src/codez-session/codezSessionDraftRegistry.js";
...
interface CreateCodezSessionServiceOptions {
...
export function createCodezSessionService({
  agentService,
  taskIndexSyncer,
  cuaProductMcpServerResolver,
}: CreateCodezSessionServiceOptions): ICodezSessionService {
  const { withApiRetryRuntime } = createCodezSessionApiRetryRuntimeTracker();
  const deferredDraftSessions = createCodezDeferredDraftRegistry();

  function notifySyncer(
    target: CodezTaskTarget,
    options?: {
  ...
  function getSessionSnapshotDiagnostics(snapshot: CodezSessionStateSnapshot) {
  ...
  // codezSessionService 是 desktop-continuous 主路径，状态变更后必须
  ...
  async function broadcastSnapshot(
    ...
    try {
      ...
      logger.warn(
        undefined,
        `[codez-session-service] ${tag} syncSnapshotAndBroadcast 失败 taskId=${snapshot.session.sessionId}`,
        error,
      );
    }
  }
  ...
  async function repairEmptyImportedClaudeSession(
    snapshot: CodezSessionStateSnapshot,
    params: CodezSessionResumeParams | CodezSessionReadParams,
  ...
  async function withResolvedMcpServers<
    T extends CodezSessionCreateParams | CodezSessionResumeParams,
  >(params: T): Promise<T> {
  ...
  return {
    async initializeWorkspace(params: CodezSessionWorkspaceTarget) {
    ...
    getWorkspaceRuntimeIdentity(params: CodezSessionWorkspaceTarget) {
      return agentService.getWorkspaceRuntimeIdentity(params);
    },

    readWorkspacePresentation(params: CodezSessionReadWorkspacePresentationParams) {
      return agentService.readWorkspacePresentation(params);
    },

    async createSession(params: CodezSessionCreateParams) {
      ...
      logger.info(sessionTraceId, "[codez-session-service] createSession 分配 session trace", {
      ...
      logger.info(sessionTraceId, "[codez-session-service] createSession agent 返回", {
      ...
      // desktop-continuous 路径不会经过 Codez task adapter，sqlite 的 task index 全靠
      ...
      logger.info(sessionTraceId, "[codez-session-service] createSession task index 同步完成", {
    ...
    async resumeSession(params: CodezSessionResumeParams) {
      const startedAt = Date.now();
      ...
      if (
        ...
        if (
          ...
          logger.warn(
            undefined,
            "[codez-session-service] resumeSession 跳过不支持的 task 思考强度",
            {
      ...
      if (thoughtLevelOverride && snapshot.settings.thoughtLevel.current !== thoughtLevelOverride) {
        logger.info(undefined, "[codez-session-service] resumeSession 重放 task 思考强度", {
      ...
      logger.info(undefined, "[codez-session-service] resumeSession 历史恢复完成", {
        agentDurationMs,
    ...
    listSessions(params: CodezSessionListParams) {
      return agentService.listSessions(params);
    },

    async readSession(params: CodezSessionReadParams) {
      const startedAt = Date.now();
      const snapshot = await repairEmptyImportedClaudeSession(
        withApiRetryRuntime(await agentService.readSession(params)),
        params,
      );
      logger.info(undefined, "[codez-session-service] readSession 历史快照读取完成", {
    ...
    readSessionMessages(params: CodezSessionMessagesParams) {
      return agentService.readSessionMessages(params);
    },

    readSessionEvents(params: CodezSessionEventsParams) {
      return agentService.readSessionEvents(params);
    },

    promoteDeferredDraftSession(params: CodezTaskTarget) {
      ...
      notifySyncer(params);
      logger.info(undefined, "[codez-session-service] deferred draft session 已提升为 task", {
        sessionId: params.sessionId,
        workspaceIdentity: params.workspaceIdentity ?? null,
        workspacePath: params.workspacePath,
      });
    ...
    closeSession(params: CodezTaskTarget) {
      deferredDraftSessions.forget(params);
      return agentService.closeSession(params).then(() => undefined);
    },

    async closeDeferredDraftSession(params: CodezTaskTarget) {
      try {
        ...
        logger.warn(
          undefined,
          "[codez-session-service] 条件关闭 deferred draft 失败，保留旧 session",
          {
    ...
    async setModel(params: CodezSessionSetModelParams) {
    ...
    async setThoughtLevel(params: CodezSessionSetThoughtLevelParams) {
      return withApiRetryRuntime(await agentService.setThoughtLevel(params));
    },

    async setMode(params: CodezSessionSetModeParams) {
