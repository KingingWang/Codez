...
export interface IOffPeakTaskService {
  ...
  createTask(params: CodezOffPeakTaskCreateParams): Promise<OffPeakTaskCreateResult>;
  cancelTask(offPeakTaskId: string): Promise<CodezOffPeakTask | null>;
  pauseTask(offPeakTaskId: string): Promise<CodezOffPeakTask | null>;
  continueTask(offPeakTaskId: string): Promise<CodezOffPeakTask | null>;
  deleteTask(offPeakTaskId: string): Promise<void>;
  /** 仅隐藏本地 History 行；不删除 task/session/执行字段。 */
  deleteHistory(offPeakTaskId: string): Promise<CodezOffPeakTask | null>;
  updateTask(
    offPeakTaskId: string,
    params: OffPeakUpdateTaskParams,
  ): Promise<CodezOffPeakTask | null>;
  list(): Promise<CodezOffPeakTask[]>;
  get(offPeakTaskId: string): Promise<CodezOffPeakTask | null>;
}
