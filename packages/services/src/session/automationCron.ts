...
import type { CodezAutomation, CodezAutomationScheduleRule } from "@codez/shared";
...
export function scheduleRuleDefinition(rule: CodezAutomationScheduleRule): string {
...
export function isOneShotAutomation(
  automation: Pick<CodezAutomation, "recurring" | "maxRuns">,
): boolean {
...
export function buildRelativeDelaySchedule(
  delayMinutes: number,
  from = Date.now(),
): Pick<CodezAutomation, "cronExpr" | "scheduleRule"> {
...
export function computeInitialAutomationNextRunAt(
  automation: Pick<CodezAutomation, "cronExpr" | "recurring" | "scheduleRule">,
  from = Date.now(),
): number | null {
...
export function computeScheduleRuleNextRunAt(
  rule: CodezAutomationScheduleRule,
  from = Date.now(),
): number | null {
...
export function inferMinuteIntervalScheduleRule(
  cronExpr: string,
  anchorAt: number,
): CodezAutomationScheduleRule | undefined {
...
export function buildIntervalScheduleRule(
  intervalUnit: CodezAutomationScheduleRule["unit"],
  interval: number,
  cronExpr: string,
  anchorAt: number,
...
export function computeAutomationNextRunAt(
  automation: Pick<CodezAutomation, "cronExpr" | "scheduleRule">,
  from?: number,
): number | null {
