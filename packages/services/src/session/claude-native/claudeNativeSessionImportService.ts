...
import type { CodezImportSessionsResult, CodezTaskMeta } from "@codez/shared";
...
export async function importClaudeNativeSessions(params: {
  ...
  sessionIds: string[];
  createImportedSession?: (source: ClaudeNativeImportedSessionSource) => Promise<CodezTaskMeta>;
  onTaskImported: (meta: CodezTaskMeta) => void;
}): Promise<CodezImportSessionsResult> {
  const normalizedSessionIds = [
    ...new Set(params.sessionIds.map((item) => item.trim()).filter(Boolean)),
  ];
  const result: CodezImportSessionsResult = {
    imported: [],
    skipped: [],
    failed: [],
  };
