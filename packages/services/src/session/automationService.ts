import type {
  CodezAutomation,
  CodezAutomationCreateParams,
  CodezAutomationRun,
  CodezAutomationScheduleRule,
  CodezAutomationUpdateParams,
} from "@codez/shared";
import { resolveWorkspaceKey } from "@codez/shared";
...
function assertValidAutomationScheduleRule(rule: CodezAutomationScheduleRule): void {
...
export class AutomationService {
  ...
  async create(params: CodezAutomationCreateParams): Promise<CodezAutomation> {
  ...
  async update(
    ...
    // 同时顺手修复历史上已经存在的同类脏数据。
    const normalizedParams: CodezAutomationUpdateParams = hasIntervalCarrier
    ...
    const options: {
      ...
      lifecycleStatus?: CodezAutomation["lifecycleStatus"];
      resetRetry?: boolean;
    } = {};
    ...
    const effectiveDirectScheduleRule = hasDirectScheduleRule
      ? directScheduleRule
      : carrierScheduleRule;
    const normalizedWithSchedule: CodezAutomationUpdateParams =
  ...
  async runNow(
    automationId: string,
    scope?: AutomationWorkspaceScope,
  ): Promise<{ automation: CodezAutomation; run: CodezAutomationRun } | null> {
    const workspaceKey = resolveScopeKey(scope);
    const existing = await this.repo.get(automationId, workspaceKey);
    if (!existing) return null;
