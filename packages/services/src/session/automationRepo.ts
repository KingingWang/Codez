...
import {
  ...
  type CodezAutomationCreateParams,
  type CodezAutomationDispatchStatus,
  type CodezAutomationLifecycleStatus,
  type ModelSelection,
  type CodezAutomationRun,
  type CodezAutomationRunDispatchStatus,
  type CodezAutomationRunOutcome,
  type CodezAutomationTrigger,
  type CodezAutomationUpdateParams,
} from "@codez/shared";
...
function rowToAutomation(row: AutomationRow): CodezAutomation {
  ...
  return {
    ...
    scheduleRule: row.schedule_rule
      ? (JSON.parse(row.schedule_rule) as CodezAutomation["scheduleRule"])
      : undefined,
    ...(row.schedule_edited_by_user === 1 ? { scheduleEditedByUser: true } : {}),
    runCount: row.run_count,
    enabled: row.enabled === 1,
    lifecycleStatus: row.lifecycle_status as CodezAutomationLifecycleStatus,
    nextRunAt: row.next_run_at ?? undefined,
    lastRunAt: row.last_run_at ?? undefined,
    dispatchStatus: row.dispatch_status as CodezAutomationDispatchStatus,
...
function readAutomationModelSelection(row: AutomationRow): CodezAutomation["modelSelection"] {
...
function serializeAutomationModelSelection(
  selection: CodezAutomation["modelSelection"],
): string | null {
...
function normalizeAutomationMode(mode: string | null): CodezAutomation["mode"] | undefined {
  const parsed = codezTaskModeSchema.safeParse(mode);
  return parsed.success ? parsed.data : undefined;
}

function assertValidAutomationMode(mode: unknown): void {
  if (mode === undefined || mode === null) return;
  if (!codezTaskModeSchema.safeParse(mode).success) {
    // 读取兼容历史脏数据不代表允许继续写脏数据；Repo 是绕过 RPC 时的最终持久化边界。
    throw new Error(`Invalid automation mode: ${String(mode)}`);
  }
}

function rowToRun(row: AutomationRunRow): CodezAutomationRun {
  ...
  return {
    ...
    trigger: row.trigger as CodezAutomationTrigger,
    ...(modelSelection ? { modelSelection } : {}),
    dispatchStatus: row.dispatch_status as CodezAutomationRunDispatchStatus,
    outcome: (row.outcome as CodezAutomationRunOutcome | null) ?? undefined,
...
export class AutomationRepo {
  ...
  async create(
    params: CodezAutomationCreateParams,
    options: { nextRunAt: number | null; lifecycleStatus?: CodezAutomationLifecycleStatus },
  ): Promise<CodezAutomation> {
  ...
  async get(automationId: string, workspaceKey?: string): Promise<CodezAutomation | null> {
    await this.ensureReady();
    const row = this.getRow(automationId, workspaceKey);
  ...
  async update(
    automationId: string,
    params: CodezAutomationUpdateParams,
    options?: {
      nextRunAt?: number | null;
      lifecycleStatus?: CodezAutomationLifecycleStatus;
      resetRetry?: boolean;
    },
    workspaceKey?: string,
  ): Promise<CodezAutomation | null> {
    assertValidAutomationMode(params.mode);
    await this.ensureReady();
   ...
   */
  async claimDue(now: number): Promise<CodezAutomation[]> {
    await this.ensureReady();
    const db = this.getDatabase();
    db.exec("BEGIN IMMEDIATE");
    try {
  ...
  async ensureRunClaimed(params: {
    ...
    workspaceKey: string;
    scheduledAt: number | null;
    trigger: CodezAutomationTrigger;
  }): Promise<void> {
    await this.ensureReady();
    const now = Date.now();
    this.getDatabase()
  ...
  async markRunDispatch(params: {
    runId: string;
    dispatchStatus: CodezAutomationRunDispatchStatus;
  ...
  async listRuns(automationId: string, workspaceKey?: string): Promise<CodezAutomationRun[]> {
    await this.ensureReady();
    const rows = this.getDatabase()
      .prepare(
  ...
  async getRun(runId: string): Promise<CodezAutomationRun | null> {
    await this.ensureReady();
    const row = this.getDatabase()
