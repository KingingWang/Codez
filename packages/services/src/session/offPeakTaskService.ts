...
import {
  ...
  type CodezOffPeakTaskCreateParams,
} from "@codez/shared";
...
import type { ModelSelection, ModelSelectionValidation } from "@codez/provider";
...
function isValidCreateParams(params: CodezOffPeakTaskCreateParams): boolean {
  return (
...
export class OffPeakTaskService implements IOffPeakTaskService {
  ...
  async createTask(params: CodezOffPeakTaskCreateParams): Promise<OffPeakTaskCreateResult> {
    let providerName = "";
    try {
    ...
    const normalizedParams: CodezOffPeakTaskCreateParams = {
      ...params,
   ...
   */
  async cancelTask(offPeakTaskId: string): Promise<CodezOffPeakTask | null> {
  ...
  async pauseTask(offPeakTaskId: string): Promise<CodezOffPeakTask | null> {
   ...
   */
  async continueTask(offPeakTaskId: string): Promise<CodezOffPeakTask | null> {
  ...
  async deleteHistory(offPeakTaskId: string): Promise<CodezOffPeakTask | null> {
  ...
  async list(): Promise<CodezOffPeakTask[]> {
    return this.projectModelSelectionIssues(await this.deps.repo.list());
  }

  async get(offPeakTaskId: string): Promise<CodezOffPeakTask | null> {
  ...
  ): Promise<CodezOffPeakTask[]> {
    const repaired: CodezOffPeakTask[] = [];
  ...
  private async settleOne(task: CodezOffPeakTask): Promise<void> {
