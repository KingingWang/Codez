/* eslint-disable max-lines -- Codez task wrapper service 接口集中承载 app/runtime API，拆散会让替换阶段更难追踪。 */
import type { Event } from "@codez/rpc";
import { ServiceChannels } from "@codez/shared";
import type { CommandPayloadMap } from "@codez/shared/codez-protocol-v4";
import { createServiceDescriptor } from "#src/descriptors.js";
import type {
  CodezImportSessionsResult,
  CodezImportableSessionCandidate,
  CodezSessionCompactResult,
  CodezSessionGoalAction,
  CodezSessionGoalResult,
  CodezTaskCreateResult,
  CodezTaskMeta,
  CodezStreamEvent,
  CodezTaskMode,
  CodezConfigOption,
  CodezError,
  CodezAssistantMessageFeedback,
  CodezPromptAttachment,
  CodezProvider,
  CodezWorkspaceEvent,
  TraceId,
  CodezSessionFile,
  CodezAgentMcpServer,
  CodezTaskSnapshot,
  CodezTaskSnapshotBody,
  CodezTaskSnapshotToolCallsSlice,
  CodezTaskSnapshotRefContent,
  CodezEnqueueTaskCommandResult,
  CodezCancelTaskCommandResult,
  CodezTaskClientMode,
  CodezTaskTokenUsageResult,
...
export interface CodezTaskSnapshotWithEtagResult {
  snapshot: CodezTaskSnapshot | null;
  etag?: string;
  notModified?: boolean;
}

/** 同一 workspace 的固定归档删除集合；每个去重后的目标恰好属于一种结果。 */
export interface CodezArchivedTaskDeletionResult {
...
export type CodezModelTrajectoryContentPart =
...
export interface CodezModelTrajectoryMessage {
  role: string;
  parts: CodezModelTrajectoryContentPart[];
}

/** model_io 记录里归一化出来的 token 用量。 */
export interface CodezModelTrajectoryUsage {
  inputTokens?: number;
  outputTokens?: number;
  totalTokens?: number;
...
export type CodezModelTrajectoryCallSourceKind =
  | "main"
  | "sidecar"
  | "subagent"
  | "compact"
...
export interface CodezModelTrajectoryCallSource {
  kind: CodezModelTrajectoryCallSourceKind;
  querySource?: string;
}
 ...
 */
export interface CodezModelTrajectoryRecord {
  requestId: string;
  attempt: number;
  startedAt: string;
  completedAt?: string;
  ...
  turnId?: string;
  traceId?: string;
  callSource?: CodezModelTrajectoryCallSource;
  model: {
    modelId?: string;
    providerId?: string;
    role?: string;
  ...
  request: {
    messages: CodezModelTrajectoryMessage[];
    toolNames: string[];
  };
  response?: {
    finishReason?: string;
    ...
    toolCalls: CodezModelTrajectoryContentPart[];
    usage?: CodezModelTrajectoryUsage;
    responseId?: string;
    modelId?: string;
  };
  error?: {
...
export interface CodezModelTrajectory {
  taskId: string;
  /** runtime 是否支持读取 model-io 轨迹（仅 Codez Agent 落盘 model-io）。 */
  available: boolean;
  records: CodezModelTrajectoryRecord[];
  /** 命中的源文件绝对路径，便于排查。 */
  sourceFiles: string[];
...
export interface CodezTaskTerminalOutcome {
  taskId: string;
...
export interface CodezTaskReadyOutcome {
  taskId: string;
  reason: "prompt_completed" | "prompt_failed";
}

/**
 * ICodezTaskService — Codez task wrapper API 服务接口
 *
 * UI、remote controller 通过这层访问 task wrapper 状态；核心 session 状态由
 * Codez Agent server 维护，新功能应优先走 ICodezSessionService。
 */
export interface ICodezTaskService {
  // ---- 生命周期 ----
  ...
  enqueueTaskCommand(params: {
    ...
    automationId?: string;
    ownerRunId?: TraceId;
  }): Promise<CodezEnqueueTaskCommandResult>;
  ...
  cancelTaskCommand(params: {
    ...
    ownerRunId?: TraceId;
    clientMode: CodezTaskClientMode;
  }): Promise<CodezCancelTaskCommandResult>;
  ...
  }): Promise<CodezSessionCompactResult>;
  ...
  goalSession(params: {
    ...
    objective?: string;
    expectedRevision?: number;
  }): Promise<CodezSessionGoalResult>;
   ...
   */
  listTaskList(params: CodezTaskListQuery): Promise<CodezTaskListResult>;
  ...
  applyGroupedTaskViewOrder(params: CodezGroupedTaskViewOrderInput): Promise<CodezGroupedTaskView>;
  ...
  getTaskSnapshot(params: {
    ...
    model?: string;
    thoughtLevel?: string;
  }): Promise<CodezTaskSnapshot | null>;
  ...
  getTaskSnapshotWithEtag(params: {
    ...
    model?: string;
    thoughtLevel?: string;
  }): Promise<CodezTaskSnapshotWithEtagResult>;
  ...
  getTaskSnapshotBody(params: {
    ...
    workspaceIdentity?: string;
    refId: string;
  }): Promise<CodezTaskSnapshotBody | null>;
  ...
  getTaskSnapshotRef(params: {
    ...
    workspaceIdentity?: string;
    refId: string;
  }): Promise<CodezTaskSnapshotRefContent | null>;
  ...
  getTaskSnapshotToolCallsSlice(params: {
    ...
    limit: number;
  }): Promise<CodezTaskSnapshotToolCallsSlice | null>;
  ...
  getTaskConfigOptions(params: { taskId: string }): Promise<CodezConfigOption[]>;
  ...
  setAssistantMessageFeedback(params: {
    ...
    workspaceIdentity?: string;
    turnIndex: number;
    feedback: CodezAssistantMessageFeedback | null;
  }): Promise<CodezSessionFile>;
  ...
  /** 切换 task 模式 */
  setMode(params: { taskId: string; mode: CodezTaskMode }): Promise<void>;
  ...
  /** 获取 Codez Agent 当前结构化日志文件路径。 */
  getTaskNativeSessionLogFile(params: {
    taskId: string;
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<{
    provider: CodezProvider | null;
    path: string | null;
    exists: boolean;
  }>;

  /**
   * 读取 task 对应的模型调用轨迹（来自 ~/.codez/cli/{debug,rollout} 的 model-io JSONL）。
   * taskId 即 Codez Agent 的 sessionId，按 sessionId 匹配 model-io 记录。
   */
  getModelTrajectory(params: {
    taskId: string;
    /** 最多返回的调用条数（按时间倒序保留最近 N 条），默认 200。 */
    limit?: number;
  }): Promise<CodezModelTrajectory>;
  ...
  /**
   * 重启指定 workspace 的 Codez Agent 共享进程。
   * 配置变更需要重新读取进程环境时使用。
  ...
  deleteArchivedTasks(params: {
    ...
    workspaceIdentity?: string;
    taskIds: string[];
  }): Promise<CodezArchivedTaskDeletionResult>;
  ...
  onDynamicWorkspaceEvent(
    workspace: string | CodezWorkspaceEventSubscriptionParams,
  ): Event<CodezWorkspaceEvent>;

  /** 全局错误事件 */
  onError: Event<CodezError>;
}

export const ICodezTaskService = createServiceDescriptor<ICodezTaskService>(
  ServiceChannels.CodezTask,
);
