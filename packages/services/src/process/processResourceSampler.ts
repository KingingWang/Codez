...
export interface HostResourceUsageAgent {
  ...
  children: readonly CodezProcessChildProcess[];
}
...
type Owner =
  ...
  | { kind: "mcp"; child: CodezProcessChildProcess }
...
function ownerToRow(
  ...
  if (owner.kind === "agent") {
    return {
      pid: sample.pid,
      name: isOwnerRoot
        ? formatCodezAgentProcessName(owner.agent.provider, owner.agent.workspacePath)
