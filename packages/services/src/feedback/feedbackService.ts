...
import type { ApiClient, FeedbackDeviceInfo } from "@codez/shared";
import {
...
function resolveApiBaseUrl(explicit?: string): string {
  return (
    explicit?.trim() ||
    process.env.CODEZ_FEEDBACK_API_BASE?.trim() ||
    buildRuntimeCodezApiUrl(process.env, "/api/v1")
  );
}
...
function buildDeviceSnapshot(): FeedbackDeviceInfo {
  return {
    ...
    buildCommitId: CODEZ_COMMIT,
    buildTime: CODEZ_BUILD_TIME,
...
export function createFeedbackService(options: CreateFeedbackServiceOptions): IFeedbackService {
  ...
  async function getZcodeJwtToken(): Promise<string | undefined> {
    return (await options.credentialService.load(CODEZ_JWT_TOKEN_KEY))?.trim() || undefined;
  }
