...
import { type ICodezTaskService as ICodezTaskServiceShape } from "../session/codezTaskService.js";
...
export function createBotRemoteWorkspaceService(params: {
  ...
  return {
    ...
    async getCodezTaskService(target: {
      workspacePath: string;
      workspaceIdentity: string;
    }): Promise<ICodezTaskServiceShape | null> {
      return (await getRuntimeServices(target))?.codezTaskService ?? null;
    },
    ...
    async syncAppRuntimePreferences(preferences: CodezAgentAppRuntimePreferences): Promise<void> {
      ...
      await Promise.all(
        Array.from(runtimeServicesByWorkspaceKey.values()).map((services) =>
          services.codezAgentService.syncAppRuntimePreferences(preferences),
        ),
      );
    },
    dispose(): void {
  ...
  async function getRuntimeServices(target: {
    ...
    // task wrapper 命令走 ICodezTaskService，session 主状态走 Codez session facade。
    ...
    while (true) {
      ...
      const preferences: CodezAgentAppRuntimePreferences = cachedPreferences
        ? cachedPreferences
      ...
      await services.codezAgentService.syncAppRuntimePreferences(preferences);
