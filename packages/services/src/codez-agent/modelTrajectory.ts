...
} from "#src/codez-agent/modelTrajectoryFileTail.js";
import type { TrajectoryFileTail } from "#src/codez-agent/modelTrajectoryFileTail.js";
...
/**
 * 解析 ~/.codez/cli/{debug,rollout} 下的 model-io JSONL，按 sessionId 还原某个 task 的模型调用轨迹。
 *
 * 设计说明：
 * - model-io 由 adapters/model/runner-debug.ts 落盘；一个 session 一个
 *   `model-io-<sanitizedSessionId>.jsonl`。
 * - Codez Agent 把 taskId 当作 sessionId（见 codezTaskServiceAdapter），所以这里只读取
...
function mapRecord(record: Record<string, unknown>): CodezModelTrajectoryRecord {
  ...
  const mapped: CodezModelTrajectoryRecord = {
...
function mapMessages(value: unknown): CodezModelTrajectoryMessage[] {
...
function mapResponseToolCall(rawToolCall: unknown): CodezModelTrajectoryContentPart {
...
function mapUsage(value: unknown): CodezModelTrajectoryUsage | undefined {
