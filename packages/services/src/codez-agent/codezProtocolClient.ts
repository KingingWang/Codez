import { CodezStorageStartupGate } from "#src/codez-agent/codezStorageStartupGate.js";
...
import type { V4Method } from "@codez/shared/codez-protocol-v4";
import type { z } from "zod";
import type { CodezProtocolTransport } from "./codezProtocolTransport.js";

/** 客户端可发的方法名：旧 codezProtocolMethods + v4/*（并存，收敛为 v4）。 */
type CodezProtocolClientMethod = CodezProtocolMethod | V4Method;

interface CodezProtocolClientOptions {
  requireStorageStartup?: boolean;
  requestTimeoutMs?: number;
}

interface CodezProtocolRequestTimeoutEvent {
  method: CodezProtocolClientMethod;
  requestId: CodezProtocolRequestId;
  timeoutMs: number;
}
...
interface CodezProtocolClientRequestOptions {
...
const DEFAULT_CODEZ_PROTOCOL_REQUEST_TIMEOUT_MS = 3 * 60_000;

class CodezProtocolClientError extends Error {
  constructor(
    message: string,
    ...
    super(message);
    this.name = "CodezProtocolClientError";
  }
}

export class CodezProtocolRequestTimeoutError extends Error {
  constructor(
    readonly method: CodezProtocolClientMethod,
    readonly requestId: CodezProtocolRequestId,
    readonly timeoutMs: number,
  ) {
    super(`Codez Protocol request timed out: ${method}`);
    this.name = "CodezProtocolRequestTimeoutError";
  }
}

export class CodezProtocolClient implements IDisposable {
  readonly storageStartup: CodezStorageStartupGate;
  private readonly pending = new Map<string, PendingRequest<unknown>>();
  private readonly notificationEmitter = new Emitter<CodezProtocolNotification>();
  private readonly requestEmitter = new Emitter<CodezProtocolRequest>();
  private readonly requestTimeoutEmitter = new Emitter<CodezProtocolRequestTimeoutEvent>();
   ...
   * 否则会立即抛 "Codez Protocol client is disposed"。
   */
  ...
  constructor(
    private readonly transport: CodezProtocolTransport,
    options?: CodezProtocolClientOptions,
  ) {
    this.storageStartup = new CodezStorageStartupGate(options?.requireStorageStartup ?? false);
    this.requestTimeoutMs = options?.requestTimeoutMs ?? DEFAULT_CODEZ_PROTOCOL_REQUEST_TIMEOUT_MS;
    this.disposables.push(
      transport.onMessage((message) => this.handleMessage(message)),
      transport.onClose((event) => {
        this.storageStartup.dispose();
        const suffix = event.reason ? `: ${event.reason}` : "";
        this.rejectAll(new Error(`Codez agent transport closed${suffix}`));
  ...
  async request<T = unknown>(
    ...
    const resultPromise = new Promise<T>((resolve, reject) => {
      const expire = () => {
        ...
        const error = new CodezProtocolRequestTimeoutError(method, id, requestTimeoutMs);
  ...
  async notify(method: CodezProtocolClientMethod, params?: unknown): Promise<void> {
    this.assertNotDisposed();
    await this.transport.send({ method, params });
  }

  async respond(id: CodezProtocolRequestId, result: unknown): Promise<void> {
    this.assertNotDisposed();
  ...
  private handleMessage(message: unknown): void {
    ...
    if ("result" in message && "id" in message) {
      this.resolveResponse(
        (message as { id: CodezProtocolRequestId; result: unknown }).id,
    ...
    if ("method" in message && "id" in message) {
      this.requestEmitter.fire(message as CodezProtocolRequest);
      return;
    }
    ...
    if ("method" in message) {
      if (
        ...
        this.storageStartup.accept((message as CodezProtocolNotification).params)
      ) {
      ...
      this.notificationEmitter.fire(message as CodezProtocolNotification);
    }
  }

  private resolveResponse(id: CodezProtocolRequestId, result: unknown): void {
    const requestKey = String(id);
    const pending = this.pending.get(requestKey);
    if (!pending) {
      return;
    ...
    try {
      ...
      pending.reject(
        error instanceof Error
          ? error
          : new Error(`Codez Protocol response parse failed: ${pending.method}`),
      );
    }
  }

  private rejectResponse(id: CodezProtocolRequestId, error: Error): void {
    const requestKey = String(id);
    const pending = this.pending.get(requestKey);
    if (!pending) {
      return;
  ...
  private disposeLocalResources(): void {
    this.rejectAll(new Error("Codez Protocol client disposed"));
  ...
  private assertNotDisposed(): void {
    if (this.disposed) {
      throw new Error("Codez Protocol client is disposed");
    }
  }
}
