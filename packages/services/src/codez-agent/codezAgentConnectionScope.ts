...
import { Emitter, Event as RpcEvent, type Event, type IDisposable } from "@codez/rpc";
import {
...
export type CodezAgentV4ClientMode = "desktop-continuous" | "web-remote-replayable";

export interface CodezAgentV4ConnectionContext {
  connectionId: string;
  clientMode: CodezAgentV4ClientMode;
  role?: "terminal-client" | "trusted-host-relay";
}

const TRUSTED_CONNECTION_FIELD = "__codezTrustedV4Connection";
const TRUSTED_UNSUBSCRIBE_ROUTE_FIELD = "__codezTrustedV4UnsubscribeRoute";

type TrustedConnectionCarrier = {
  [TRUSTED_CONNECTION_FIELD]?: CodezAgentV4ConnectionContext;
};

interface TrustedCodezAgentV4UnsubscribeRoute {
  topic: string;
  connectionId: string;
}

type TrustedUnsubscribeRouteCarrier = {
  [TRUSTED_UNSUBSCRIBE_ROUTE_FIELD]?: TrustedCodezAgentV4UnsubscribeRoute;
};

export function readTrustedCodezAgentV4UnsubscribeRoute(
  value: unknown,
): TrustedCodezAgentV4UnsubscribeRoute | null {
 ...
 */
export function readTrustedCodezAgentV4Connection(
  value: unknown,
): CodezAgentV4ConnectionContext | null {
  if (typeof value !== "object" || value === null) return null;
...
function withTrustedConnection<T extends object>(
  value: T,
  context: CodezAgentV4ConnectionContext,
): T {
...
function withTrustedUnsubscribeRoute<T extends object>(
  value: T,
  route: TrustedCodezAgentV4UnsubscribeRoute,
): T {
  const forwarded = {
    ...(value as unknown as Record<string, unknown>),
  };
...
function workspaceKey(target: CodezAgentWorkspaceTarget): string {
  return target.workspaceIdentity?.trim() || target.workspacePath;
}

function workspaceTarget(target: CodezAgentWorkspaceTarget): CodezAgentWorkspaceTarget {
  return {
...
interface ConnectionFlowRoute {
  key: string;
  target: CodezAgentWorkspaceTarget;
  connection: CodezAgentV4ConnectionContext;
}
...
function deliveryProfileFor(clientMode: CodezAgentV4ClientMode): HelloMessage["deliveryProfile"] {
  return clientMode === "desktop-continuous" ? "continuous" : "replayable";
}

function createHello(context: CodezAgentV4ConnectionContext): HelloMessage {
...
export interface CodezAgentConnectionScope {
  readonly service: ICodezAgentService;
...
export function createCodezAgentConnectionScope(
  base: ICodezAgentService,
  context: CodezAgentV4ConnectionContext,
  ...
  const forwardedConnection = (params: unknown): CodezAgentV4ConnectionContext => {
    const downstream = readTrustedCodezAgentV4Connection(params);
  ...
  const ownershipKey = (kind: SubscriptionKind, target: CodezAgentWorkspaceTarget, topic: string) =>
    `${kind}\0${workspaceKey(target)}\0${topic}`;
  const routedEventKey = (kind: SubscriptionKind, target: CodezAgentWorkspaceTarget) =>
    `${kind}\0${workspaceKey(target)}`;
  const flowRouteKey = (target: CodezAgentWorkspaceTarget, connectionId: string) =>
  ...
  const flowRouteForEntry = (entry: OwnedSubscription): ConnectionFlowRoute => ({
    ...
    } satisfies CodezAgentV4ConnectionContext,
  });
  ...
  const findBySubscription = (
    ...
    target: CodezAgentWorkspaceTarget,
    subscriptionId: string,
    expectedRoute: TrustedCodezAgentV4UnsubscribeRoute | null,
  ...
  const forwardedUnsubscribeRoute = (
    ...
    const downstream = readTrustedCodezAgentV4UnsubscribeRoute(params);
  ...
  const unsubscribeBase = (
    ...
    const params: CodezAgentConversationUnsubscribeParams = {
      ...entry.target,
  ...
  const resyncBase = (entry: OwnedSubscription, params: CodezAgentConversationResyncParams) => {
    const forwarded = withTrustedUnsubscribeRoute(params, {
      topic: entry.topic,
      connectionId: entry.connectionId,
    });
  ...
  const resyncOwned = (kind: SubscriptionKind, params: CodezAgentConversationResyncParams) => {
    assertReady();
  ...
  const overrides: Partial<ICodezAgentService> = {
