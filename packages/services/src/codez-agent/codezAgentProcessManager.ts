import { resolveCodezAgentSpawnCwd } from "#src/codez-agent/codezAgentSpawnCwd.js";
import type { CodezAgentStorageStartupSnapshot } from "#src/codez-agent/codezAgent.js";
/* eslint-disable max-lines -- codezAgentProcessManager 集中维护 agent 子进程启动、复用、超时回收和 runtime identity，拆分会扩大进程生命周期状态同步面 */
...
import {
  parseCodezProcessDiagnostic,
  CODEZ_AGENT_LIFECYCLE_LOG_MARKER,
  CODEZ_PROCESS_DIAGNOSTIC_NAME_MAX_CHARS,
  CODEZ_PROCESS_DIAGNOSTIC_MESSAGE_MAX_CHARS,
  CODEZ_PROCESS_DIAGNOSTIC_STACK_MAX_CHARS,
} from "@codez/shared/process-diagnostic";
import {
  CODEZ_AGENT_RUNTIME,
  CODEZ_AGENT_PROVIDER,
  CODEZ_RUNTIME_ENV_KEY,
  resolveWorkspaceKey,
  resolveCodezRuntimeEnv,
  sanitizeCodezRuntimeEnv,
} from "@codez/shared";
import {
  findCodezAgentRuntimeBinary,
  findCodezAgentRuntimeNodeBundle,
...
import type { CodezAgentPresentationSurface } from "./codezAgentPresentationSurface.js";
...
export interface CodezAgentCommand {
...
export interface CodezAgentCommandResolverContext {
  presentationSurface?: CodezAgentPresentationSurface;
  workspacePath: string;
  workspaceIdentity?: string;
  workspaceKey: string;
}

export type CodezAgentCommandResolver = (
  context: CodezAgentCommandResolverContext,
) => Promise<CodezAgentCommand | null> | CodezAgentCommand | null;

export interface CodezAgentProcessManagerOptions {
  commandResolver?: CodezAgentCommandResolver;
  presentationSurface?: CodezAgentPresentationSurface;
...
interface ManagedCodezAgentProcess {
  ...
  runtimeIdentity: CodezAgentRuntimeIdentity;
...
function buildE2EAgentCoverageEnv(env: NodeJS.ProcessEnv = process.env): Record<string, string> {
  const artifactDir = env.CODEZ_E2E_ARTIFACT_DIR?.trim();
  if (env.CODEZ_E2E_COVERAGE !== "1" || !artifactDir) {
    return {};
  }
  const directory = resolve(artifactDir, "coverage", "raw", "cli");
  mkdirSync(directory, { recursive: true });
  const preloadPath = resolve(directory, "codez-e2e-coverage-preload.cjs");
 ...
 */
interface CodezAgentRuntimeRestartedEvent {
  workspaceKey: string;
  runtimeIdentity: CodezAgentRuntimeIdentity;
}

export interface CodezAgentRuntimeLifecycleEvent {
  workspacePath: string;
  workspaceIdentity?: string;
  workspaceKey: string;
  runtimeIdentity: CodezAgentRuntimeIdentity;
...
interface CodezAgentRuntimeIdentity {
  generation: number;
  identity: string;
  processId?: number;
  workspaceKey: string;
...
interface CodezAgentSpawnPreflight {
  command: string;
  args: string[];
  requestedCwd: string;
  cwd: string;
...
const serviceLog = createServiceLogger("codez-agent");
...
function parseArgsJson(raw: string | undefined): string[] | undefined {
  ...
  if (!Array.isArray(parsed) || parsed.some((item) => typeof item !== "string")) {
    throw new Error("CODEZ_AGENT_SERVER_ARGS_JSON must be a JSON string array");
  }
  return parsed;
}
...
async function buildCodezAgentSpawnPreflight(
  command: CodezAgentCommand,
  workspacePath: string,
  spawnFallbackCwd?: string,
): Promise<CodezAgentSpawnPreflight> {
  ...
  const {
    ...
    cwdExists,
  } = await resolveCodezAgentSpawnCwd({ requestedCwd, workspacePath, spawnFallbackCwd });
  return {
...
function resolveBundledWorkspaceCodezAgentCommand(
  context: CodezAgentCommandResolverContext,
): CodezAgentCommand | null {
  const distEntrypoint = findUpward("apps/codez-cli/packages/cli/dist/codez.cjs");
  if (distEntrypoint) {
    const useBytecode =
      process.versions.electron && process.env.CODEZ_DESKTOP_AGENT_BYTECODE === "1";
    const entrypoint = useBytecode
      ? join(dirname(distEntrypoint), "codez.bytecode.cjs")
      : distEntrypoint;
    ...
    return {
      ...
      // 这里显式启用 Node 运行模式，避免内置 codez-agent 被当成 Electron/Chromium 子进程启动并卡在 GPU 初始化。
      env: { ELECTRON_RUN_AS_NODE: "1" },
    };
  }

  const sourceEntrypoint = findUpward("apps/codez-cli/packages/cli/src/main.ts");
...
function resolveDeployedCodezAgentBinaryCommand(
  context: CodezAgentCommandResolverContext,
): CodezAgentCommand | null {
  // 旧 resolver 只识别 CODEZ_AGENT_SERVER_COMMAND env 和 monorepo 源码树。
  // SSH 远端把 codez-server.cjs 单文件部署到 ~/.codez/server/，宿主进程的 cwd 不在仓库内、
  // env 也不会被 ssh exec 继承，即使 codez-agent 已经部署到 ~/.codez/server/agents/glm/，
  // resolver 也找不到，第一次 getClient 就抛 "Codez agent server command is not configured"。
  // 这里复用 findCodezAgentRuntimeBinary 的候选链（含 GLM_BINARY_PATH env、
  // packagedResourcesPath、~/.codez/server/agents/glm、bundled-agents 等），
  // 把已部署的原生 binary 当成最终兜底，远端/桌面打包形态都能命中。
  const binaryPath = findCodezAgentRuntimeBinary();
  if (!binaryPath) {
    return null;
  }
  return {
    command: binaryPath,
    args: CODEZ_AGENT_RUNTIME.spawnArgs,
    cwd: context.workspacePath,
  };
}

function resolveElectronRuntimeCodezAgentCommand(
  context: CodezAgentCommandResolverContext,
): CodezAgentCommand | null {
  // 桌面打包态：host 跑在 Electron utility process 里，process.execPath 指向 Electron Helper，
  // 它内置的 Node runtime 与 codez-cli 目标版本一致（Electron 41 = Node 24.x）。
  // 这里直接用 app 自带的 Electron Node 执行打进 resources/glm 的 codez.cjs，
  ...
  const bundlePath = findCodezAgentRuntimeNodeBundle();
  if (!bundlePath) {
    return null;
  }
  return {
    command: process.execPath,
    args: [bundlePath, ...CODEZ_AGENT_RUNTIME.spawnArgs],
...
export function resolveDefaultCodezAgentCommand(
  context: CodezAgentCommandResolverContext,
): CodezAgentCommand | null {
  const command = process.env.CODEZ_AGENT_SERVER_COMMAND?.trim();
  if (command) {
    return applyPresentationSurfaceToCommand(
      {
        command,
        args: parseArgsJson(process.env.CODEZ_AGENT_SERVER_ARGS_JSON) ?? ["app-server", "--stdio"],
        cwd: process.env.CODEZ_AGENT_SERVER_CWD?.trim() || context.workspacePath,
      },
  ...
  // 抢先匹配）→ 桌面打包态 Electron Node runtime 跑 codez.cjs → 已部署 native binary（远端 SSH 兜底）。
  const bundled =
    resolveBundledWorkspaceCodezAgentCommand(context) ??
    resolveElectronRuntimeCodezAgentCommand(context);
  return applyPresentationSurfaceToCommand(
    bundled
      ? { ...bundled, supportsStorageStartup: true }
      : resolveDeployedCodezAgentBinaryCommand(context),
    context.presentationSurface,
  );
}

function applyPresentationSurfaceToCommand(
  command: CodezAgentCommand | null,
  presentationSurface: CodezAgentCommandResolverContext["presentationSurface"],
): CodezAgentCommand | null {
...
function wrapCodezAgentCommandWithStdioTapDevProxy(
  command: CodezAgentCommand,
  workspaceKey: string,
): CodezAgentCommand {
  const tapState = readCodezStdioTapDevState();
  if (!tapState.enabled) {
    return command;
  }

  const tapScript = findUpward("scripts/dev/codez-stdio-tap.mjs");
  if (!tapScript) {
    debugLog("Codez stdio tap proxy enabled but script not found");
    return command;
  }
...
export class CodezAgentProcessManager {
  private readonly processesByWorkspaceKey = new Map<string, ManagedCodezAgentProcess>();
  private readonly ownedProcesses = new Set<ManagedCodezAgentProcess>();
  private readonly startingByWorkspaceKey = new Map<string, Promise<CodezProtocolClient>>();
  ...
  getStorageStartupState(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): CodezAgentStorageStartupSnapshot | null {
  ...
  private readonly commandResolver: CodezAgentCommandResolver;
  private readonly presentationSurface: CodezAgentProcessManagerOptions["presentationSurface"];
  private readonly requestTimeoutMs: number | undefined;
  private readonly processLifecycleReporter: RuntimeProcessLifecycleReporter | undefined;
  private readonly resolveSpawnEnv: CodezAgentProcessManagerOptions["resolveSpawnEnv"];
  private readonly waitForSpawnAdmission: CodezAgentProcessManagerOptions["waitForSpawnAdmission"];
  ...
  private readonly runtimeRestartedEmitter = new Emitter<CodezAgentRuntimeRestartedEvent>();
  private readonly runtimeLifecycleEmitter = new Emitter<CodezAgentRuntimeLifecycleEvent>();
  private disposeAllInFlight: Promise<void> | undefined;
  private disposed = false;

  /** 进程换代通知（generation>1 时触发）；v4 订阅方据此重订，见 CodezAgentRuntimeRestartedEvent。 */
  ...
  constructor(options?: CodezAgentProcessManagerOptions) {
    this.commandResolver = options?.commandResolver ?? resolveDefaultCodezAgentCommand;
  ...
  private reportProcessLifecycle(
    ...
    try {
      ...
      warnLog("Codez agent process lifecycle reporter failed", error);
    }
  }

  private clearIdleTimer(managed: ManagedCodezAgentProcess): void {
   ...
   */
  private scheduleIdleReclaim(workspaceKey: string, managed: ManagedCodezAgentProcess): void {
    ...
    const timer = setTimeout(() => {
      ...
      log("Codez agent process idle timeout; reclaiming", {
        workspaceKey,
  ...
  private recordTerminationIntent(
    managed: ManagedCodezAgentProcess,
  ...
  private reportRuntimeUnavailable(managed: ManagedCodezAgentProcess): void {
  ...
  private reportRuntimeReady(managed: ManagedCodezAgentProcess): void {
    if (
      !managed.spawned ||
      managed.exited ||
  ...
  private cleanupManagedProcess(
    ...
    const cleanupPromise = managed.client
      ...
      .then(() => {
        ...
        log("Codez agent process cleanup completed", {
    ...
    if (options.reportError !== false) {
      void cleanupPromise.catch((error) => {
        errorLog("Codez agent process cleanup failed", {
          workspaceKey: managed.runtimeIdentity.workspaceKey,
          pid: managed.child.pid,
          runtimeIdentity: managed.runtimeIdentity.identity,
          reason,
  ...
  private async cleanupManagedProcessForShutdown(managed: ManagedCodezAgentProcess): Promise<void> {
  ...
  private async cleanupManagedProcessWithRetry(
    ...
    try {
      ...
      warnLog(`Codez agent process cleanup retrying during ${retryScope}`, {
        workspaceKey: managed.runtimeIdentity.workspaceKey,
        pid: managed.child.pid,
        runtimeIdentity: managed.runtimeIdentity.identity,
        cleanupStage: reason,
  ...
  async getClient(params: {
    ...
    if (starting) {
      ...
      log("Codez agent process start already in progress", {
        workspaceKey,
      });
      const client = await starting;
      log("Codez agent process start wait completed", {
        workspaceKey,
  ...
  private async startClient(
    ...
    if (!command) {
      throw new Error(
        "Codez agent server command is not configured. Set CODEZ_AGENT_SERVER_COMMAND before integration.",
      );
    }
    if (admissionSignal.aborted) {
      throw admissionSignal.reason ?? new Error("Codez agent process start was cancelled.");
    }
    const effectiveCommand = wrapCodezAgentCommandWithStdioTapDevProxy(command, workspaceKey);
    log("Codez agent command resolved", {
      workspaceKey,
      command: command.command,
    ...
    if ((this.restartGenerationByWorkspaceKey.get(workspaceKey) ?? 0) !== startGeneration) {
      ...
      throw new Error("Codez agent process start was cancelled.");
    }
    // cwd 探测也让出事件循环，必须放在最终 admission 与销毁/代际检查之前。
    const spawnPreflight = await buildCodezAgentSpawnPreflight(
      effectiveCommand,
    ...
    // app 以本地开发方式启动时，让 agent 子进程也带上 CODEZ_RUNTIME_ENV=development；
    // 不再传 NODE_ENV，避免用户 shell/runtime 变量影响 Codez 运行模式或泄漏到 Bash 工具。
    const runtimeEnv = resolveCodezRuntimeEnv(process.env);
    log("Codez agent spawn preflight", {
      workspaceKey,
      spawnPreflight,
    });
    ...
    const child = spawn(effectiveCommand.command, spawnPreflight.args, {
      ...
      env: {
        ...sanitizeCodezRuntimeEnv(process.env),
        [CODEZ_RUNTIME_ENV_KEY]: runtimeEnv,
        ...spawnEnv,
    ...
    const transport = new CodezStdioTransport(child, {
      onStderrLine: (line) => {
        const diagnostic = parseCodezProcessDiagnostic(line);
    ...
    const client = new CodezProtocolClient(transport, {
    ...
    const runtimeIdentity: CodezAgentRuntimeIdentity = {
    ...
    const managed: ManagedCodezAgentProcess = {
      child,
      client,
      exited: false,
      readyReported: false,
    ...
    child.once("spawn", () => {
      ...
      this.reportRuntimeReady(managed);
      log("Codez agent process started", {
        workspaceKey,
        command: effectiveCommand.command,
        cwd: spawnPreflight.cwd,
        pid: child.pid,
    ...
    child.once("error", (error) => {
      errorLog(
        `Codez agent process error${this.processLifecycleReporter?.onError ? ` ${CODEZ_AGENT_LIFECYCLE_LOG_MARKER}` : ""}`,
        {
          workspaceKey,
          pid: child.pid,
    ...
    child.once("exit", async (code, signal) => {
      ...
      log("Codez agent process exited", exitContext);
      if (terminationKind === "unexpected") {
        ...
        errorLog(
          `Codez agent process exited unexpectedly${this.processLifecycleReporter ? ` ${CODEZ_AGENT_LIFECYCLE_LOG_MARKER}` : ""}`,
          {
            ...exitContext,
            stderr,
          },
    ...
    client.onRequestTimeout((event) => {
      ...
      if (event.method === "workspace/cancelGenerateText") {
        warnLog(
          "Codez agent cancel notification timed out; keeping client (best-effort control plane)",
          {
            workspaceKey,
            method: event.method,
      ...
      warnLog("Codez agent request timed out; disposing stale protocol client", {
        workspaceKey,
        method: event.method,
        requestId: event.requestId,
        timeoutMs: event.timeoutMs,
    ...
    client.onClose(() => {
      ...
      log("Codez agent protocol client closed", {
        workspaceKey,
        pid: child.pid,
        runtimeIdentity: runtimeIdentity.identity,
        wasActiveClient,
  ...
  async getRuntimeIdentity(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<CodezAgentRuntimeIdentity> {
    const workspaceKey = resolveWorkspaceKey(params);
    ...
    if (!managed || managed.exited || managed.child.killed) {
      throw new Error("Codez agent runtime identity is unavailable.");
    }
    ...
    try {
      ...
      return command
        ...
        : {
            available: false,
            workspaceKey,
            reason: "CODEZ_AGENT_SERVER_COMMAND is not configured",
          };
    } catch (error) {
      return {
        available: false,
  ...
  private abortPendingStarts(
    workspaceKey: string,
    reason = new Error("Codez agent process start was cancelled."),
  ): void {
  ...
  private abortAllPendingStarts(
    reason = new Error("Codez agent process manager is disposed."),
  ): void {
  ...
  async disposeAllAndWait(): Promise<void> {
    ...
    // 依赖 unref timer，host 自己退出后 timer 不会再执行，codez-cli 会残留为孤儿进程。
