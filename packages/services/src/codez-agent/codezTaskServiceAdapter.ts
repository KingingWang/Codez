/* oxlint-disable eslint(max-lines) -- 迁移期需要在一个门面里集中维护旧 task projection 到 Codez session 的协议适配。 */
...
import {
  coalesceConsecutiveCodezAssistants,
  createSessionTraceId,
  decodeCustomModelValue,
  deriveCodezTaskStatusFromSessionSnapshot,
  extractPlanStepsFromToolInput,
  extractPlanStepsFromToolOutput,
  generateTraceId,
  getCodezGoalActiveIterationCount,
  getCodezGoalIterationByAssistantMessageId,
  getCodezUserVisibleMessages,
  isMainAgentToolProjectionSource,
  normalizeCodezApiRetryStatus,
  attachCodezBackgroundTaskNotificationToRaw,
  collectCodezBackgroundTaskNotificationsByToolUseId,
  mergeCodezBackgroundTaskControlItems,
  parseCodezBackgroundTaskControlItems,
  parseCodezBackgroundTaskNotificationText,
  parseModelPickerValue as parseSharedModelSelection,
  resolveWorkspaceKey,
  resolveCodezVisibleSessionTitle,
  textFromCodezMessageParts,
  CODEZ_AGENT_PROVIDER,
  codezBackgroundTaskNotificationToolUpdateStatus,
  appendCodezStreamingToolInputDelta,
  buildCodezStreamingToolInputPreview,
  createCodezToolProjectionMemory,
  finalizeCodezToolProjectionInput,
  forgetCodezToolProjectionMetadata,
  isCodezModelRetryRecoveryProgressPayload,
  markCodezStreamingToolInputPreviewMaterialized,
  resolveCodezToolProjectionMetadata,
  codezApiRetryFromModelNetworkStatusPayload,
  codezApiRetryFromStreamRecoveryPayload,
  codezTaskNetworkDebugStatusFromPayload,
  shouldMaterializeCodezStreamingToolInputPreview,
  type CodezApiRetryStatus,
  type CodezAssistantMessageFeedback,
  type CodezBackgroundTaskNotificationInfo,
  type CodezBackgroundTaskControlItem,
  type CodezBackgroundTurnAttribution,
  type CodezAutomationBotDeliveryTarget,
  type CodezCancelTaskCommandResult,
  type CodezConfigOption,
  type CodezEnqueueTaskCommandResult,
  type CodezError,
  type CodezGoalVerificationTimelineMeta,
  type CodezImportSessionsResult,
  type CodezImportableSessionCandidate,
  type CodezUsage,
  type CodezPersistedMessage,
  type CodezPersistedMessagePart,
  type CodezPersistedToolCall,
  type CodezPromptAttachment,
  type CodezProvider,
  type CodezSessionFile,
  type CodezTaskGoal,
  type CodezTaskGoalStats,
  type CodezTaskMode,
  type CodezPlanStep,
  type CodezSlashCommand,
  type CodezStreamEvent,
  type CodezTaskCreateResult,
  type CodezTaskMeta,
  type CodezTaskClientMode,
  type CodezTaskRuntimeCommand,
  type CodezTaskSnapshot,
  type CodezTaskSnapshotBody,
  type CodezTaskSnapshotRefContent,
  type CodezTaskSnapshotToolCallsSlice,
  type CodezTaskTokenUsageResult,
  type CodezTodoGroup,
  type CodezTurnSteerCommandKind,
  type CodezTurnSteerSource,
  type CodezWorkspaceEvent,
  type CodezWorkspaceTaskListChanged,
  type InputId,
  type TraceId,
  codezContextUsageBreakdownSchema,
  codezSessionSettingsStateSchema,
  type CodezDeliveryKind,
  type CodezMessagePart,
  type CodezMessageWithParts,
  type ModelSelection,
  type CodezPermissionOption,
  type CodezPermissionRequestParams,
  type CodezPermissionRequest,
  type CodezSessionEvent,
  type CodezSessionMode,
  type CodezSessionSettingsState,
  type CodezSessionStateSnapshot,
  type CodezStateUpdatedNotification,
  type CodezContextCompactionTimelineMeta,
  type CodezTimelineMeta,
  type CodezTimelineStatus,
  type CodezTimelineTrigger,
  type CodezToolProjectionMemory,
  type CodezUserInputRequestParams,
  type CodezUserInputResponse,
  type CodezAgentMcpServer,
} from "@codez/shared";
import type {
  CodezTaskListQuery,
  CodezTaskListResult,
  CodezWorkspaceEventSubscriptionParams,
  ICodezTaskService,
  CodezArchivedTaskDeletionResult,
  CodezTaskReadyOutcome,
  CodezTaskTerminalOutcome,
...
import type {
  CodezTaskIndexReadyEvent,
  CodezTaskIndexSyncer,
...
import {
  ...
  normalizeAvailableCodezMode,
  settingsToConfigOptions,
} from "./codezConfigOptions.js";
...
interface CreateCodezTaskServiceAdapterOptions {
  codezAgentService: ICodezAgentService;
...
type WorkspaceEventInput = string | CodezWorkspaceEventSubscriptionParams;
type CodezSendPromptRuntimeCommand = Extract<CodezTaskRuntimeCommand, { type: "send_prompt" }>;
type CodezTerminalStreamEvent =
  | Extract<CodezStreamEvent, { type: "task_complete" }>
  | Extract<CodezStreamEvent, { type: "task_error" }>;

const GLM_PROVIDER: CodezProvider = CODEZ_AGENT_PROVIDER;
const EMPTY_SLASH_COMMANDS: CodezSlashCommand[] = [];
const logger = createServiceLogger("codez-task-service");
...
function formatCodezAgentLogDate(now: Date): string {
  return [now.getFullYear(), padDatePart(now.getMonth() + 1), padDatePart(now.getDate())].join("-");
}

function resolveCodezAgentCurrentLogFilePath(now = new Date()): string {
...
export function createCodezTaskServiceAdapter(
  options: CreateCodezTaskServiceAdapterOptions,
): ICodezTaskService {
  const errorEmitter = new Emitter<CodezError>();
  const taskEmitters = new Map<string, Emitter<CodezStreamEvent>>();
  const globalTaskEmitters = new Map<string, Emitter<CodezStreamEvent>>();
  const overlays = new Map<string, TaskOverlay>();
  const taskTargets = new Map<string, TaskTarget>();
  const runtimeCommands = new Map<string, CodezTaskRuntimeCommand[]>();
  const runtimeCommandDrains = new Map<string, Promise<void>>();
  const apiRetryByTaskKey = new Map<string, CodezApiRetryStatus | null>();
  const backgroundTaskControlsByTaskKey = new Map<string, CodezBackgroundTaskControlItem[]>();
  const streamedTurnKeys = new Set<string>();
  const activePromptInputIds = new Map<string, InputId>();
  const toolProjectionMemoryByTaskKey = new Map<string, CodezToolProjectionMemory>();
  ...
  function unsupported(name: string): never {
    throw Object.assign(
      new Error(`Codez task service adapter does not support ICodezTaskService.${name} yet.`),
      {
        code: "CODEZ_AGENT_UNSUPPORTED_LEGACY_TASK_METHOD",
      },
    );
  }
  ...
  function turnAttributionOf(
    params: CodezBackgroundTurnAttribution,
  ...
  async function resolveProductMcpServers(
    servers: CodezAgentMcpServer[] | undefined,
  ): Promise<CodezAgentMcpServer[] | undefined> {
  ...
  async function sendPromptToAgent(
    ...
    // Codez task wrapper 的字段仍叫 traceId，但这里语义已经是单次输入 inputId。
    // 先记录 inputId，后续 Codez session 事件回投 Codez Agent 时才能让 UI 终态按输入轮次收口。
    activePromptInputIds.set(taskKey(target), params.traceId);
    logger.info(params.traceId, "Codez task facade sendPrompt 开始", {
    ...
    try {
      ...
      if (params.attachments?.length) {
        ...
        await options.codezAgentService.sendPrompt({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
      ...
      logger.info(params.traceId, "Codez task facade sendPrompt ACK", {
      ...
      logger.warn(params.traceId, "Codez task facade sendPrompt 失败", {
        durationMs: Date.now() - startedAt,
  ...
  function setRuntimeCommands(params: TaskTarget, commands: CodezTaskRuntimeCommand[]): void {
    const key = taskKey(params);
  ...
  function markRuntimeCommandRunning(
    params: TaskTarget,
    command: CodezSendPromptRuntimeCommand,
  ): CodezSendPromptRuntimeCommand {
    const key = taskKey(params);
    const runningCommand: CodezSendPromptRuntimeCommand = {
      ...command,
      status: "running",
      updatedAt: Date.now(),
    };
  ...
  function markRuntimeCommandFailed(
    ...
    const key = taskKey(params);
    const failedCommand: CodezSendPromptRuntimeCommand = {
      ...command,
      status: "failed",
      updatedAt: Date.now(),
      error: error instanceof Error ? error.message : String(error),
  ...
  function completeRuntimeCommandByInputId(
    ...
    logger.info(command.traceId, "Codez task command 终态收口", {
  ...
  function completeRuntimeCommandForTerminalEvent(
    params: TaskTarget,
    event: CodezTerminalStreamEvent,
  ): void {
  ...
  async function drainRuntimeCommands(params: TaskTarget, reason: string): Promise<void> {
    ...
    const command = commands.find(
      (candidate): candidate is CodezSendPromptRuntimeCommand =>
    ...
    logger.info(runningCommand.traceId, "Codez task command drain 开始", {
    ...
    try {
      ...
      logger.warn(runningCommand.traceId, "Codez task command drain 失败", {
        commandId: runningCommand.commandId,
        error: error instanceof Error ? error.message : String(error),
        reason,
        taskId: params.taskId,
  ...
  async function drainRuntimeCommandsSafely(params: TaskTarget, reason: string): Promise<void> {
    try {
      ...
      logger.warn(undefined, "Codez task command drain 调度失败", {
        error: error instanceof Error ? error.message : String(error),
        reason,
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
  ...
  function persistedStatusFromToolUpdate(
    status: Extract<CodezStreamEvent, { type: "tool_call_update" }>["status"],
  ): CodezPersistedToolCall["status"] | undefined {
  ...
  function rememberLiveToolProjection(params: TaskTarget, event: CodezStreamEvent): void {
    ...
    const nextTool: CodezPersistedToolCall =
  ...
  function persistedToolId(tool: CodezPersistedToolCall): string | undefined {
  ...
  function mergePersistedToolCall(
    snapshotTool: CodezPersistedToolCall | undefined,
    liveTool: CodezPersistedToolCall,
  ): CodezPersistedToolCall {
    if (!snapshotTool) {
      return liveTool;
    }
    return {
  ...
  function findToolLocation(
    messages: readonly CodezPersistedMessage[],
    toolId: string,
  ...
  function latestAssistantMessageIndex(messages: readonly CodezPersistedMessage[]): number {
  ...
  function upsertPersistedToolPart(
    parts: readonly CodezPersistedMessagePart[] | undefined,
    toolIndex: number,
  ): CodezPersistedMessagePart[] {
  ...
  function updateSnapshotMessageTool(
    message: CodezPersistedMessage,
    toolIndex: number,
    tool: CodezPersistedToolCall,
  ): CodezPersistedMessage {
  ...
  function mergeLiveToolProjectionIntoSnapshotMessages(
    ...
    if (changed) {
      logger.debug(undefined, "Codez snapshot 合并 live tool projection", {
        event: "codez_task.snapshot.live_tool_projection.merged",
  ...
  function getTaskTarget(taskId: string): TaskTarget {
    ...
    if (!target) {
      throw Object.assign(new Error(`Codez session target is not loaded: ${taskId}`), {
        code: "CODEZ_SESSION_TARGET_NOT_FOUND",
      });
    }
    return target;
  }
  ...
  function getToolProjectionMemory(params: TaskTarget): CodezToolProjectionMemory {
    const key = taskKey(params);
    let memory = toolProjectionMemoryByTaskKey.get(key);
    if (!memory) {
      memory = createCodezToolProjectionMemory();
  ...
  function getWorkspaceEmitter(workspace: WorkspaceEventInput): Emitter<CodezWorkspaceEvent> {
    return taskIndexSyncer.getWorkspaceEmitter(workspace);
  }

  function getTaskEmitter(params: TaskTarget): Emitter<CodezStreamEvent> {
    const key = taskKey(params);
    let emitter = taskEmitters.get(key);
    if (!emitter) {
      emitter = new Emitter<CodezStreamEvent>();
      taskEmitters.set(key, emitter);
    }
    return emitter;
  }

  function getGlobalTaskEmitter(taskId: string): Emitter<CodezStreamEvent> {
  ...
  function emitTaskEvent(params: TaskTarget, event: CodezStreamEvent): void {
  ...
  function emitWorkspaceTaskListChanged(
    ...
    taskMeta: CodezTaskMeta | undefined,
    reason: CodezWorkspaceTaskListChanged["reason"],
  ) {
  ...
  async function resumeSnapshot(
    ...
    return options.codezAgentService.resumeSession({
      workspacePath: params.workspacePath,
      workspaceIdentity: params.workspaceIdentity,
  ...
  async function sendConfigCasCommandV4<T extends "switchModelConfig" | "switchCollaborationMode">(
    ...
    await sendHostCasCommandV4({
      send: (envelope) =>
        options.codezAgentService.sendConversationCommandV4({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          envelope,
        }),
   ...
   * command.ts 裁决），而旧协议 CodezSessionMode 含 auto 且旧 op 接受它——为 UI 行为
  ...
  async function switchCollaborationModeViaProtocol(
    ...
    if (mode === "auto") {
      await options.codezAgentService.setMode({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: target.taskId,
        mode,
  ...
  async function repairEmptyImportedClaudeSnapshot(
    ...
    const repaired = await repairImportedClaudeSessionSnapshot({
      snapshot,
      target: params,
      createSession: (input) => options.codezAgentService.createSession(input),
      onRepair: (history) => {
        logger.warn(
          undefined,
  ...
  function snapshotToMeta(snapshot: CodezSessionStateSnapshot): CodezTaskMeta {
    const target = {
    ...
    return applyOverlayToMeta(
      {
        ...
        mode: fromCodezMode(snapshot.session.mode),
        ...
        status: deriveCodezTaskStatusFromSessionSnapshot(snapshot),
        ...
        target: snapshot.projection.target
          ? fromCodezGoal(snapshot.projection.target)
  ...
  function rememberIndexedTaskMeta(meta: CodezTaskMeta): CodezTaskMeta {
  ...
  async function resolveTaskIndexResumeHints(
    ...
    if (model || thoughtLevel) {
      ...
      logger.info(undefined, "从 task index 回填 Codez session resume 配置", {
        model: model || null,
  ...
  async function syncTaskIndexMeta(meta: CodezTaskMeta): Promise<CodezTaskMeta> {
  ...
  async function updateIndexedTaskState(
    ...
    try {
      ...
      // 旧 Codez session 可能还没有轻量 task index 行。
  ...
  function updateTaskIndexFromStreamEvent(params: TaskTarget, event: CodezStreamEvent): void {
  ...
  function updateTaskApiRetryFromStreamEvent(params: TaskTarget, event: CodezStreamEvent): void {
    const key = taskKey(params);
  ...
  function snapshotToCodez(
    ...
    const goalIterationByAssistantMessageId = getCodezGoalIterationByAssistantMessageId(
      snapshot.messages,
      {
    ...
    const backgroundTaskNotifications = collectCodezBackgroundTaskNotificationsByToolUseId(
      snapshot.messages,
    );
      ...
      normalizeGoalVerificationTimelineMessageOrder(
        addGoalVerificationTimelineSnapshotFallback(
          addSessionForkSnapshotFallback(
            coalesceConsecutiveCodezAssistants(
              getCodezUserVisibleMessages(snapshot.messages, {
                target: snapshot.projection.target,
              }).map((message) =>
                mapMessage(
                  message,
    ...
    const pendingPermissions: CodezPermissionRequest[] = snapshot.projection.pendingPermissions
    ...
    const pendingElicitations = snapshot.projection.pendingPermissions
      ...
      .filter(
        (event): event is Extract<CodezStreamEvent, { type: "elicitation_request" }> =>
          event !== null,
      );
    const backgroundTaskControls = parseCodezBackgroundTaskControlItems(
  ...
  function mapServiceEvent(params: TaskTarget, event: CodezAgentServiceEvent): void {
    if (event.type === "snapshot") {
      void syncTaskIndexSnapshot(event.snapshot).catch((error) => {
        logger.warn(undefined, "同步 Codez snapshot 到 task index 失败", error);
      });
      const snapshotEvent: CodezStreamEvent = {
        type: "task_snapshot_updated",
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        workspaceKey: workspaceKey(params),
  ...
  function handleTaskIndexTerminalEvent(event: CodezTaskIndexTerminalEvent): void {
  ...
  function handleTaskIndexReadyEvent(event: CodezTaskIndexReadyEvent): void {
    const params: TaskTarget = {
      taskId: event.target.sessionId,
      workspacePath: event.target.workspacePath,
      workspaceIdentity: event.target.workspaceIdentity,
  ...
  async function disposeCodezAgentServiceAndWait(): Promise<void> {
    const agentService = options.codezAgentService as ICodezAgentService & {
  ...
  const service: ICodezTaskService & {
    disposeAll(): void;
    disposeAllAndWait(): Promise<void>;
  } = {
    async initialize(params) {
      const result = await options.codezAgentService.initialize(params);
      return {
    ...
    async createTask(params): Promise<CodezTaskCreateResult> {
      ...
      const mcpServers = await resolveProductMcpServers(params.mcpServers);
      let snapshot: CodezSessionStateSnapshot | null = null;
      if (draftSessionId && !mcpServers) {
        try {
          snapshot = await options.codezAgentService.readSession({
            ...target,
          ...
          if (requestedSelection) {
            if (!sameModelSelection(snapshot.settings.model.current, requestedSelection)) {
              ...
              snapshot = await options.codezAgentService.setModel({
                ...target,
                sessionId: draftSessionId,
          ...
          if (
            ...
            snapshot = await options.codezAgentService.setThoughtLevel({
              ...target,
              sessionId: draftSessionId,
      ...
      if (!snapshot) {
        ...
        if (params.v4Create === true) {
          ...
          const ack = assertV4CommandAckOk(
            "createSession",
            await options.codezAgentService.sendConversationCommandV4({
              ...target,
              envelope: createHostCommandEnvelope({
                type: "createSession",
                sessionId: null,
                payload: {
                  ...
                  config: {
                    ...
                    ...(model?.options?.reasoningLevel
                      ...
                      : {}),
                    ...(params.mode ? { mode: toCodezMode(params.mode) } : {}),
                  },
                  ...(mcpServers ? { mcpServers } : {}),
                },
              }),
          ...
          snapshot = await options.codezAgentService.createSession({
            ...target,
            sessionTraceId: createSessionTraceId(),
            mode: toCodezMode(params.mode),
            model: requestedSelection,
            thoughtLevel: requestedSelection?.options?.reasoningLevel,
    ...
    async enqueueTaskCommand(params): Promise<CodezEnqueueTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const workspaceKeyValue = workspaceKey(params);
      const command: CodezTaskRuntimeCommand = {
    ...
    async promoteTaskCommand(params): Promise<CodezEnqueueTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const key = taskKey(params);
      const commands = runtimeCommands.get(key) ?? [];
    ...
    async cancelTaskCommand(params): Promise<CodezCancelTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const key = taskKey(params);
      const commands = runtimeCommands.get(key) ?? [];
      const command = commands.find((candidate) => candidate.commandId === params.commandId);
      if (!command) {
        logger.info(undefined, "Codez task command 取消时已不存在", {
          commandId: params.commandId,
          taskId: params.taskId,
          workspaceIdentity: params.workspaceIdentity ?? null,
          workspaceKey: resolveWorkspaceKey(params),
      ...
      if (command.status === "running") {
        logger.info(command.traceId, "Codez task command 已开始运行，跳过取消", {
          commandId: command.commandId,
          taskId: params.taskId,
          workspaceIdentity: params.workspaceIdentity ?? null,
          workspaceKey: resolveWorkspaceKey(params),
      ...
      logger.info(command.traceId, "Codez task command 已取消", {
        commandId: command.commandId,
        status: command.status,
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
    ...
    async stopGeneration(params): Promise<void> {
      ...
      logger.info(params.runId, "Codez task facade stopGeneration 开始", {
      ...
      logger.info(params.runId, "Codez task facade stopGeneration ACK", {
        durationMs: Date.now() - startedAt,
        taskId: params.taskId,
        workspaceIdentity: target.workspaceIdentity ?? null,
        workspaceKey: resolveWorkspaceKey(target),
    ...
    async compactSession(params) {
      ...
      try {
        const result = await options.codezAgentService.compactSession({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          sessionId: params.taskId,
    ...
    async goalSession(params) {
      ...
      logger.info(params.inputId, "[codez-task-service] goalSession start", {
      ...
      const result = await options.codezAgentService.goalSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
        inputId: params.inputId,
      ...
      logger.info(params.inputId, "[codez-task-service] goalSession agent 返回", {
        action: params.action,
        durationMs: Date.now() - startedAt,
      ...
      emitWorkspaceTaskListChanged(target, meta, "task_status_changed");
      logger.info(params.inputId, "[codez-task-service] goalSession task index 同步完成", {
        action: params.action,
        durationMs: Date.now() - startedAt,
        startedTurn: result.startedTurn,
        status: result.snapshot.session.status,
    ...
    async closeTask(params): Promise<void> {
      ...
      await options.codezAgentService.closeSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async resumeTask(params): Promise<CodezTaskMeta> {
    ...
    async listTasks(params): Promise<CodezTaskMeta[]> {
    ...
    async listPinnedTasks(params): Promise<CodezTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: GLM_PROVIDER,
    ...
    async listTaskList(params: CodezTaskListQuery): Promise<CodezTaskListResult> {
    ...
    async listArchivedTasks(params): Promise<CodezTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: GLM_PROVIDER,
    ...
    async archiveStaleTasks(params): Promise<CodezTaskMeta[]> {
    ...
    async archiveWorkspaceTasks(params): Promise<CodezTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: GLM_PROVIDER,
    ...
    async getTaskSnapshot(params): Promise<CodezTaskSnapshot | null> {
      const startedAt = Date.now();
      ...
      const codezSnapshot = limitTaskSnapshotMessages(
        snapshotToCodez(snapshot, {
      ...
      const indexedMeta = await syncTaskIndexMeta(codezSnapshot.meta);
      const indexDurationMs = Date.now() - indexStartedAt;
      logger.info(undefined, "[codez-task-service] 历史快照读取完成", {
        ...
        stats: getTaskSnapshotMessageDiagnostics(codezSnapshot),
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
        workspacePath: params.workspacePath,
      });
      return { ...codezSnapshot, meta: indexedMeta };
    },
    ...
    async getTaskSnapshotWithEtag(params) {
      ...
      if (!snapshot) {
        logger.info(undefined, "[codez-task-service] 历史快照 ETag 读取为空", {
          clientMode: params.clientMode ?? "unknown",
          durationMs: Date.now() - startedAt,
          messageLimit: params.messageLimit ?? null,
          taskId: params.taskId,
      ...
      if (params.ifNoneMatch && params.ifNoneMatch === etag) {
        logger.info(undefined, "[codez-task-service] 历史快照 ETag 命中缓存", {
          clientMode: params.clientMode ?? "unknown",
          durationMs: Date.now() - startedAt,
          etagDurationMs,
          messageLimit: params.messageLimit ?? null,
      ...
      logger.info(undefined, "[codez-task-service] 历史快照 ETag 生成完成", {
        clientMode: params.clientMode ?? "unknown",
        durationMs: Date.now() - startedAt,
        etagDurationMs,
        messageLimit: params.messageLimit ?? null,
    ...
    async getTaskSnapshotBody(): Promise<CodezTaskSnapshotBody | null> {
      return null;
    },

    async getTaskSnapshotRef(): Promise<CodezTaskSnapshotRefContent | null> {
      return null;
    },

    async getTaskSnapshotToolCallsSlice(): Promise<CodezTaskSnapshotToolCallsSlice | null> {
      return null;
    },

    async getTaskMeta(params): Promise<CodezTaskMeta | null> {
      const meta = await taskIndexRepo.getTaskMeta(params);
      return meta ? rememberIndexedTaskMeta(meta) : null;
    },

    async getTaskConfigOptions(params): Promise<CodezConfigOption[]> {
    ...
    async getTaskModelSelection(params): Promise<ModelSelection | null> {
      const target = getTaskTarget(params.taskId);
      const snapshot = await options.codezAgentService.readSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async setAssistantMessageFeedback(params): Promise<CodezSessionFile> {
      const snapshot = await service.getTaskSnapshot(params);
      if (!snapshot) {
      ...
      if (targetMessage) {
        targetMessage.feedback = params.feedback as CodezAssistantMessageFeedback | undefined;
      }
      return snapshot;
    },
    ...
    async scanImportableClaudeSessions(params: {
      ...
      limit?: number;
    }): Promise<CodezImportableSessionCandidate[]> {
    ...
    async importClaudeSessions(params: {
      ...
      workspaceIdentity?: string;
      sessionIds: string[];
    }): Promise<CodezImportSessionsResult> {
      return importClaudeNativeSessions({
        ...
        createImportedSession: async (source) => {
          const targetWorkspaceIdentity = params.workspacePath
            ...
            : undefined;
          const snapshot = await options.codezAgentService.createSession({
          ...
          // 导入后的任务必须是真实 Codez session，setModel/sendPrompt 才能继续命中 runtime。
    ...
    async setMode(params): Promise<void> {
      ...
      const target = getTaskTarget(params.taskId);
      await switchCollaborationModeViaProtocol(target, toCodezMode(params.mode) ?? "build");
      const snapshot = await resumeSnapshot(target);
      await syncTaskIndexSnapshot(snapshot);
    },

    async setConfigOption(params): Promise<CodezConfigOption[]> {
      const target = getTaskTarget(params.taskId);
      ...
      if (params.configId === THOUGHT_LEVEL_CONFIG_ID) {
        ...
        const current = await options.codezAgentService.readSession({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          sessionId: params.taskId,
        });
        ...
        await switchCollaborationModeViaProtocol(
          target,
          toCodezMode(params.value as CodezTaskMode) ?? "build",
        );
      }
      const snapshot = await resumeSnapshot(target);
      await syncTaskIndexSnapshot(snapshot);
    ...
    async setModel(params): Promise<CodezConfigOption[]> {
      ...
      const target = getTaskTarget(params.taskId);
      await options.codezAgentService.setModel({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
    ...
    async setAutomationSessionConfig(params): Promise<CodezConfigOption[]> {
      const target = getTaskTarget(params.taskId);
    ...
    async getModelTrajectory(params) {
      // Codez Agent 把 taskId 当作 sessionId 落盘 model-io（见本文件其它 sessionId: params.taskId 用法），
      // 这里按 sessionId 还原该 task 的模型调用轨迹，供 UI 侧边栏可视化。
      const trajectory = await readModelTrajectory(params.taskId, params.limit);
      logger.info(
        `[CodezTaskService] getModelTrajectory taskId=${params.taskId} records=${trajectory.records.length} files=${trajectory.sourceFiles.length} truncated=${trajectory.truncated}`,
      );
      return trajectory;
    },

    async getTaskTokenUsage(params): Promise<CodezTaskTokenUsageResult> {
      // 摘要面板需要展示 task 的累计模型消耗，不能复用 usage_update 的 context window。
      // 这里通过 Codez Protocol 读 agent SQLite 的 model_usage 聚合，保持桌面和远控同一事实源。
      return options.codezAgentService.getTaskTokenUsage({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async deleteArchivedTasks(params): Promise<CodezArchivedTaskDeletionResult> {
      const result: CodezArchivedTaskDeletionResult = {
        deletedTaskIds: [],
        skippedTaskIds: [],
        failedTaskIds: [],
      };
    ...
    async renameTask(params): Promise<CodezTaskMeta> {
      const renamedAt = Date.now();
      logger.info(undefined, "[CodezTaskService] renameTask start", {
        taskId: params.taskId,
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        workspaceKey: resolveWorkspaceKey(params),
      ...
      try {
        ...
        logger.info(undefined, "[CodezTaskService] renameTask overlay set", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        });
        ...
        logger.info(undefined, "[CodezTaskService] renameTask index updated", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        ...
        logger.info(undefined, "[CodezTaskService] renameTask event emitted", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        });
        return meta;
      } catch (error) {
        logger.error(undefined, "[CodezTaskService] renameTask failed", {
          taskId: params.taskId,
          workspacePath: params.workspacePath,
          workspaceIdentity: params.workspaceIdentity,
          workspaceKey: resolveWorkspaceKey(params),
    ...
    async setTaskPinned(params): Promise<CodezTaskMeta> {
    ...
    async setTaskUnread(params): Promise<CodezTaskMeta> {
    ...
    async archiveTask(params): Promise<CodezTaskMeta> {
    ...
    async unarchiveTask(params): Promise<CodezTaskMeta> {
    ...
    async branchTaskFromPrompt(): Promise<CodezTaskCreateResult> {
      unsupported("branchTaskFromPrompt");
    },

    onDynamicStreamEvent(taskId: string): Event<CodezStreamEvent> {
    ...
    onDynamicTaskReady(taskId: string): Event<CodezTaskReadyOutcome> {
      return (listener) =>
    ...
    onDynamicTaskEvent(params): Event<CodezStreamEvent> {
      const target = {
        taskId: params.taskId,
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
      ...
      return (listener) => {
        ...
        // 已收敛 v4 命令；读路径消费方是
        // host 镜像 taskRealtimePort（词表同为 CodezStreamEvent），镜像换 v4 帧 = relay
        // 协议与手机 store 整链重做。
        // 过渡归宿 = replayable 读路径 v4 store，与 host/index.ts 镜像、
        // mapStateUpdated/mapServiceEvent、agentService.onDynamicSessionEvent 同批摘除。
        const upstreamDisposable = options.codezAgentService.onDynamicSessionEvent({
          workspacePath: params.workspacePath,
          workspaceIdentity: params.workspaceIdentity,
          sessionId: params.taskId,
          deliveryKind: toCodezDeliveryKind(params.deliveryKind),
    ...
    onDynamicWorkspaceEvent(workspace): Event<CodezWorkspaceEvent> {
    ...
    disposeAll(): void {
      ...
      options.codezAgentService.disposeAll();
    ...
    async disposeAllAndWait(): Promise<void> {
      ...
      // 否则 host 退出时会把 codez-cli 的 SIGKILL 兜底 timer 一起带走。
      taskIndexSyncer.disposeAll();
      await disposeCodezAgentServiceAndWait();
      disposeLocalTaskState();
    },
  };
...
function applyOverlayToMeta(meta: CodezTaskMeta, overlay: TaskOverlay): CodezTaskMeta {
  return {
    ...meta,
...
function deriveTitleFromSnapshot(snapshot: CodezSessionStateSnapshot): string {
  return resolveCodezVisibleSessionTitle({
...
function buildSnapshotHistory(
  ...
  messageLimit: number,
): NonNullable<CodezTaskSnapshot["history"]> {
  return {
...
function getTaskSnapshotMessageDiagnostics(snapshot: CodezTaskSnapshot) {
...
function parseModelPickerValue(value: string): ModelSelection {
  ...
  if (customModel?.providerId && customModel.modelName) {
    // UI 下拉的 custom:provider:model 只是展示态，不能原样传给 codez-cli。
...
function toCodezMode(mode: CodezTaskMode | undefined): CodezSessionMode | undefined {
  switch (mode) {
    case "plan":
      return "plan";
    case "edit":
...
function addSessionForkSnapshotFallback(
  ...
  return [
    ...messages,
    {
      id: `codez-timeline-fork-${parentSessionId}-`,
      role: "user",
      content: "",
...
function addGoalVerificationTimelineSnapshotFallback(
  ...
  const timelineMessages = timeline
    ...
    .map<CodezPersistedMessage>((item) => ({
...
function goalVerificationTimelineIdentityKey(
  item: Extract<CodezGoalVerificationTimelineMeta, { type: "goal_verification" }>,
): string {
...
function goalVerificationTimelineMessageId(
  ...
  if (typeof item.goalIteration === "number") {
    return `codez-goal-verification-${item.targetId}-${item.goalIteration}`;
  }
  return `codez-goal-verification-${item.verificationId}`;
}

function insertGoalVerificationTimelineMessages(
  messages: CodezPersistedMessage[],
  timelineMessages: CodezPersistedMessage[],
): CodezPersistedMessage[] {
...
function getSnapshotGoalActiveIterationCount(snapshot: CodezSessionStateSnapshot): number {
  ...
  return getCodezGoalActiveIterationCount({
    targetStatus: snapshot.projection.target?.status ?? null,
    timeline,
  });
}

function toCodezDeliveryKind(
...
function backgroundTaskNotificationToolUpdateFromInput(params: {
  ...
  taskId: string;
  traceId: TraceId;
}): Extract<CodezStreamEvent, { type: "tool_call_update" }> | null {
  const parsed = parseCodezBackgroundTaskNotificationText(params.input);
  if (!parsed) {
    return null;
  }
  const status = codezBackgroundTaskNotificationToolUpdateStatus(parsed.notification.status);
  return {
    ...
    ...(status === "failed" && parsed.notification.error
      ...
      : {}),
    raw: attachCodezBackgroundTaskNotificationToRaw(
...
function mapMessage(
  message: CodezMessageWithParts,
  goalIteration?: number,
  backgroundTaskNotifications?: ReadonlyMap<string, CodezBackgroundTaskNotificationInfo>,
): CodezPersistedMessage {
  const tools: CodezPersistedToolCall[] = [];
  const parts: CodezPersistedMessagePart[] = [];
  const attachments =
    message.info.role === "user" ? mapPromptAttachmentsFromParts(message.parts) : undefined;
  let syntheticTimeline: CodezTimelineMeta | undefined;
...
function mapPromptAttachmentsFromParts(
  parts: readonly CodezMessagePart[],
): CodezPromptAttachment[] | undefined {
  const attachments = parts
    .filter((part): part is Extract<CodezMessagePart, { type: "file" }> => part.type === "file")
    .map(mapPromptAttachmentFromFilePart)
    .filter((attachment): attachment is CodezPromptAttachment => attachment !== undefined);
  return attachments.length > 0 ? attachments : undefined;
}

function mapPromptAttachmentFromFilePart(
  part: Extract<CodezMessagePart, { type: "file" }>,
): CodezPromptAttachment | undefined {
...
function extractSyntheticTimelineFromTextPart(
  part: Extract<CodezMessagePart, { type: "text" }>,
): CodezTimelineMeta | undefined {
...
function synthesizeCompactionTimeline(
  part: Extract<CodezMessagePart, { type: "compaction" }>,
): CodezTimelineMeta | undefined {
  const metadata = asRecord(part.metadata);
...
function mapToolPart(
  part: Extract<CodezMessagePart, { type: "tool" }>,
  backgroundTaskNotifications?: ReadonlyMap<string, CodezBackgroundTaskNotificationInfo>,
): CodezPersistedToolCall {
  const state = part.state;
  const taskNotification = backgroundTaskNotifications?.get(part.callId);
  // Codez Protocol 的 part.callId 是实时流和终态 snapshot 共同的工具身份。
  // 以前只保存 metadata 会丢掉 toolCallId，手机 replayable 里 result-only 临时工具就无法被终态快照覆盖。
  const raw = attachCodezBackgroundTaskNotificationToRaw(
  ...
  if (state.status === "completed") {
    ...
    const notificationStatus = taskNotification?.status
      ? codezBackgroundTaskNotificationToolUpdateStatus(taskNotification.status)
      : undefined;
...
function textFromParts(parts: readonly CodezMessagePart[]): string {
  return textFromCodezMessageParts(parts);
}

function reasoningFromParts(parts: readonly CodezMessagePart[]): string | undefined {
  const text = parts
    .filter(
      (part): part is Extract<CodezMessagePart, { type: "reasoning" }> => part.type === "reasoning",
    )
    .map((part) => part.text)
    .join("");
...
function fromCodezGoal(goal: unknown): CodezTaskGoal {
  ...
  return {
    ...
    status: isCodezTaskGoalStatus(status) ? status : "active",
...
function isCodezTaskGoalStatus(status: string | undefined): status is CodezTaskGoal["status"] {
  return (
    status === "active" ||
    status === "paused" ||
...
function sessionTodosToPlanSteps(
  todos: CodezSessionStateSnapshot["todos"],
): CodezPlanStep[] | null {
...
function sessionGoalStatsToRuntime(
  stats: CodezSessionStateSnapshot["goalStats"],
): CodezTaskGoalStats | null {
  return stats ? { ...stats } : null;
}

function sessionTodoGroupsToRuntime(
  groups: CodezSessionStateSnapshot["todoGroups"],
): CodezTodoGroup[] | null {
...
function mapStateUpdated(
  ...
  const parsedSettings = codezSessionSettingsStateSchema.safeParse(notification.patch);
  ...
  return [
    {
      ...
      taskId: params.taskId,
      traceId,
      currentModeId: normalizeAvailableCodezMode(parsedSettings.data.mode.current),
      availableModes: getCodezAgentAvailableModes(),
    },
    {
...
function mapSessionEvent(
  ...
  toolProjectionMemory?: CodezToolProjectionMemory,
  backgroundTaskControlsByTaskKey?: Map<string, CodezBackgroundTaskControlItem[]>,
  ...
  // Codez Protocol runtime trace 只在事件没有 inputId 时兜底，避免同一轮 chunk/tool/complete 被拆成不同 trace。
  ...
  if (eventInputId && eventInputId !== protocolTraceId) {
    logger.debug(
      eventInputId,
      `对齐 Codez prompt inputId eventType=${event.type} protocolTrace=${protocolTraceId}`,
    );
  }
  ...
  if (event.type === "turn.started") {
    ...
    const runStartedEvent: CodezStreamEvent = {
    ...
    if (stringValue(payload.turnPhase) === "compact") {
      return [
        compactFailureToTimelineEvent(
          ...
          traceId,
          eventInputId,
          stringValue(errorPayload.message) ?? "Codez compact failed",
        ),
      ];
    }
    ...
    return [
      {
        ...
        traceId,
        ...(eventInputId ? { inputId: eventInputId } : {}),
        error: stringValue(errorPayload.message) ?? "Codez session failed",
...
): Extract<CodezStreamEvent, { type: "session_info_update" }> | null {
  if (!hasActiveApiRetry || !isCodezModelRetryRecoveryProgressPayload(payload)) {
    return null;
  }
...
function mapToolInputStreaming(
  ...
  if (kind === "tool_input_delta") {
    const state = appendCodezStreamingToolInputDelta(
      streamingToolInputById?.get(toolId),
      stringValue(payload.delta) ?? "",
    );
    streamingToolInputById?.set(toolId, state);
    if (!shouldMaterializeCodezStreamingToolInputPreview(state, { toolName })) {
    ...
    const preview = buildCodezStreamingToolInputPreview(state.rawInput);
    markCodezStreamingToolInputPreviewMaterialized(state);
  ...
  if (kind === "tool_call") {
    ...
    const preview = buildCodezStreamingToolInputPreview(
      rawInput,
    ...
    if ((hasCompleteInput || preview.complete) && toolProjectionMemory) {
      finalizeCodezToolProjectionInput(toolId, preview.input, toolProjectionMemory);
    }
...
function logStreamingToolInputProjection(
  ...
  logger.debug(traceId, "Codez streaming tool input projected", {
    ...details,
    event: "codez.task.streaming_tool_input.projected",
  });
}
...
function mapCompactTimelinePayload(
  ...
  inputId: InputId | undefined,
  payload: Record<string, unknown>,
): Extract<CodezStreamEvent, { type: "agent_message_chunk" }> | null {
...
function mapSyntheticTimelinePartPayload(
  ...
  const timeline = extractSyntheticTimelineFromTextPart(
    part as Extract<CodezMessagePart, { type: "text" }>,
  );
  if (!timeline) {
    return null;
  }
...
function compactTimelineMetaFromPayload(
  payload: Record<string, unknown>,
  inputId: InputId | undefined,
): CodezContextCompactionTimelineMeta | null {
...
function compactFailureToTimelineEvent(
  ...
  inputId: InputId | undefined,
  reason: string,
): Extract<CodezStreamEvent, { type: "agent_message_chunk" }> {
  return {
    type: "agent_message_chunk",
    taskId,
    traceId,
...
function mapToolUpdated(
  ...
  const rememberedTool = resolveCodezToolProjectionMetadata(payload, toolId, memory);
  ...
  if ("input" in payload && "toolName" in payload) {
    ...
    toolNameById?.set(toolId, toolName);
    finalizeCodezToolProjectionInput(toolId, payload.input, memory);
    const toolEvent: CodezStreamEvent = {
      type: "tool_call",
      taskId,
      traceId,
      ...(inputId ? { inputId } : {}),
  ...
  if ("result" in payload) {
    ...
    // Codez Protocol 的 ToolCallResult 只有 toolCallId/result，不再重复带 toolName。
    // 去掉 Codez Agent 后如果不记住前序 ToolCallScheduled 的 TodoWrite 名称，result 里的 todos
    ...
    if (status !== "in_progress") {
      forgetCodezToolProjectionMetadata(toolId, memory);
    }
    const planSteps = isMainAgentToolProjectionSource(payload)
      ? extractPlanStepsFromToolOutput({
          title: toolName,
...
function parentToolUseIdFromToolPayload(payload: Record<string, unknown>): string | null {
  // Codez Protocol 发送的父级字段叫 parentToolCallId；
...
function permissionRequestToStreamEvent(
  taskId: string,
  request: CodezPermissionRequestParams,
...
function pendingPermissionToStreamEvent(
  taskId: string,
  permission: CodezSessionStateSnapshot["projection"]["pendingPermissions"][number],
): CodezPermissionRequest {
  return {
    type: "permission_request",
    taskId,
...
function userInputRequestToElicitationStreamEvent(
  taskId: string,
  request: CodezUserInputRequestParams,
): CodezStreamEvent {
  const questions =
...
): Extract<CodezStreamEvent, { type: "elicitation_response" }> {
  return {
...
function pendingUserInputBackedPermissionToElicitationEvent(
  taskId: string,
  permission: CodezSessionStateSnapshot["projection"]["pendingPermissions"][number],
): Extract<CodezStreamEvent, { type: "elicitation_request" }> | null {
...
function pendingExitPlanModeToElicitationEvent(
  taskId: string,
  permission: CodezSessionStateSnapshot["projection"]["pendingPermissions"][number],
): Extract<CodezStreamEvent, { type: "elicitation_request" }> {
...
function setTaskBackgroundTaskControlCache(
  cache: Map<string, CodezBackgroundTaskControlItem[]> | undefined,
  cacheKey: string,
  jobs: CodezBackgroundTaskControlItem[],
) {
...
function updateTaskBackgroundTaskControlCacheFromPayload(
  ...
  cacheKey: string,
  payload: Record<string, unknown>,
): CodezBackgroundTaskControlItem[] | null {
  const parsedJobs = parseCodezBackgroundTaskControlItems([payload]);
  ...
  const nextJobs = mergeCodezBackgroundTaskControlItems(cache.get(cacheKey) ?? [], parsedJobs);
...
function permissionOptionsFromPayload(payload: Record<string, unknown>): CodezPermissionOption[] {
  return Array.isArray(payload.options) ? (payload.options as CodezPermissionOption[]) : [];
}
...
function permissionResolvedPayloadToStreamEvents(
  ...
  } as Extract<CodezStreamEvent, { type: "permission_response" }>;
...
function mapSessionInfoLikePayload(
  ...
  const networkDebugStatus = codezTaskNetworkDebugStatusFromPayload({
    taskId,
    traceId,
    ...(inputId ? { inputId } : {}),
    ...(eventId ? { eventId } : {}),
  ...
  if ("target" in payload && ("action" in payload || "source" in payload)) {
    events.push({
      ...
      target: {
        ...
        target: payload.target ? fromCodezGoal(payload.target as never) : null,
        previousTarget: payload.previousTarget
          ? fromCodezGoal(payload.previousTarget as never)
          : undefined,
      },
    });
  }
  ...
  if (Array.isArray(projection.backgroundJobs)) {
    const jobs = parseCodezBackgroundTaskControlItems(projection.backgroundJobs);
    setTaskBackgroundTaskControlCache(
      backgroundTaskControlsByTaskKey,
...
function taskTokenUsageDeltaFromPayload(
  ...
  eventId: string | undefined,
  payload: Record<string, unknown>,
): Extract<CodezStreamEvent, { type: "task_token_usage_delta" }> | null {
...
function apiRetryFromSessionInfoPayload(
  payload: Record<string, unknown>,
): CodezApiRetryStatus | null | undefined {
  if ("apiRetry" in payload) {
    return normalizeCodezApiRetryStatus(payload.apiRetry);
  }

  const runtimeRetry = normalizeCodezApiRetryStatus(asRecord(payload.runtime).apiRetry);
  if (runtimeRetry !== undefined) {
    return runtimeRetry;
  }

  const metaRetry = normalizeCodezApiRetryStatus(asRecord(asRecord(payload._meta).codez).apiRetry);
  if (metaRetry !== undefined) {
    return metaRetry;
  }

  return (
    codezApiRetryFromStreamRecoveryPayload(payload) ??
    codezApiRetryFromModelNetworkStatusPayload(payload)
  );
}

function recordAgentModelNetworkTelemetry(event: CodezSessionEvent): void {
...
function buildAgentModelNetworkInterface(payload: Record<string, unknown>): string {
  ...
  return `codez_agent.model.${providerKind}.${transport}.${base}`;
}
...
type ContextUsageUpdate = Pick<
  Extract<CodezStreamEvent, { type: "usage_update" }>,
  "used" | "size" | "cost" | "cache" | "breakdown"
>;

function contextUsageFromProjection(
  projection: CodezSessionStateSnapshot["projection"],
...
function contextUsageFromRuntime(
  runtimeUsage: CodezSessionStateSnapshot["runtime"]["contextUsage"] | undefined,
): ContextUsageUpdate | null {
...
function contextUsageFromPayload(payload: Record<string, unknown>): ContextUsageUpdate | null {
  ...
  // Codez Protocol 的 session.updated 里 contextUsed/contextWindow 是 projection 事实源；
...
function optionalContextUsageBreakdownFromPayload(
  ...
  const parsed = codezContextUsageBreakdownSchema.safeParse(payload.contextUsageBreakdown);
...
function usageFromPayload(value: unknown): CodezUsage | undefined {
...
function timelineStatusValue(value: unknown): CodezTimelineStatus | undefined {
...
function timelineTriggerValue(value: unknown): CodezTimelineTrigger | undefined {
  return value === "manual" ||
    value === "auto" ||
    value === "reactive" ||
    value === "partial" ||
...
function turnSteerSourceValue(value: unknown): CodezTurnSteerSource | undefined {
  return value === "plan_approval_feedback" || value === "workflow_refine_feedback"
    ? value
    : undefined;
}

function turnSteerCommandKindValue(value: unknown): CodezTurnSteerCommandKind | undefined {
