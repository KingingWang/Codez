...
export function buildAgentTelemetrySpawnEnv(
  ...
  return {
    ...input.telemetryEnv,
    ...(deviceMid ? { CODEZ_TELEMETRY_DEVICE_MID: deviceMid } : {}),
    ...(userId
      ? {
          CODEZ_TELEMETRY_IDENTITY_STATE: "authenticated",
          // Desktop 原始账号只在 Host 凭据边界可见；Agent 仅收到不可读的 subject，
          // Trace 可以按用户关联，但不会上传账号、邮箱或登录名。
          CODEZ_TELEMETRY_USER_SUBJECT_ID: createHash("sha256").update(userId).digest("hex"),
        }
      : {
          CODEZ_TELEMETRY_IDENTITY_STATE: deviceMid ? "anonymous" : "unknown",
        }),
    CODEZ_TELEMETRY_RUNTIME_SURFACE: input.runtimeSurface,
  };
}
