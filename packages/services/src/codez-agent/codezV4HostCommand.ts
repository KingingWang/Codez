...
class CodezV4CommandRejectedError extends Error {
  readonly code = "CODEZ_V4_COMMAND_REJECTED";
  ...
  constructor(
    ...
    this.name = "CodezV4CommandRejectedError";
  }
}
...
export function assertV4CommandAckOk(
  ...
  throw new CodezV4CommandRejectedError(commandType, ack, contextMessage);
}
...
export async function sendHostCasCommandV4<T extends CommandType>(input: {
  ...
  throw new CodezV4CommandRejectedError(
    input.type,
    lastAck ?? {
      commandId: "",
      status: "stale",
