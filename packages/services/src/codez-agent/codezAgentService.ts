import { requestPluginReferenceCatalog } from "#src/codez-agent/pluginReferenceCatalogRequest.js";
import {
...
/* oxlint-disable eslint(max-lines) -- Codez Protocol transport、通知 wiring 和 app-facing session 方法必须共享同一个 client/emitter 上下文。 */
...
import {
  CODEZ_SESSION_RUNTIME_PREFERENCES_REQUEST_TIMEOUT_MS,
  formatLogPrefix,
  resolveWorkspaceKey,
  type TraceId,
  CODEZ_AGENT_PROVIDER,
  CODEZ_AGENT_PROVIDER_NOT_READY_CODE,
  CODEZ_AGENT_PROVIDER_NOT_READY_REASON,
  CODEZ_MODEL_REASONING_SEPARATOR,
  isRemoteWorkspaceIdentity,
  CODEZ_PROTOCOL_NAME,
  CODEZ_PROTOCOL_VERSION,
  codezMcpListResultSchema,
  codezPermissionRequestParamsSchema,
  codezBrowserListParamsSchema,
  codezBrowserExecuteParamsSchema,
  ...
  type CodezProcessChildProcess,
  codezSkillsReferenceCatalogResultSchema,
  codezWorkflowsDeleteResultSchema,
  codezWorkflowsGetResultSchema,
  codezWorkflowsListResultSchema,
  codezWorkflowsMoveResultSchema,
  codezWorkflowsRunsResultSchema,
  codezWorkflowsUpdateMetaResultSchema,
  codezPluginsResolveSuggestedReferenceResultSchema,
  codezPluginOperationProgressNotificationSchema,
  codezPluginsRestoreBuiltinResultSchema,
  codezPluginsSetEnabledResultSchema,
  codezPluginsCancelOperationResultSchema,
  codezPluginsUninstallResultSchema,
  codezPluginsValidateResultSchema,
  codezPluginsDescribeResultSchema,
  codezAutomationCheckTaskBindingParamsSchema,
  codezAutomationCreateParamsSchema,
  codezAutomationDeleteParamsSchema,
  codezAutomationListParamsSchema,
  codezAutomationUpdateParamsSchema,
  codezOffPeakCreateParamsSchema,
  codezOffPeakListParamsSchema,
  OFF_PEAK_PROVIDER_IDS,
  codezComputerUseOperationEventSchema,
  codezProviderRuntimeHeadersCancelledSchema,
  codezProviderRuntimeHeadersRequestParamsSchema,
  codezProviderTestModelConnectivityResultSchema,
  codezOfficialMcpAuthHeadersRequestParamsSchema,
  ...
  codezSessionCompactResultSchema,
  codezSessionEventsResultSchema,
  codezSessionGoalResultSchema,
  codezSessionListResultSchema,
  codezSessionSubagentsResultSchema,
  codezSessionMessagesResultSchema,
  codezSessionEventSchema,
  codezSessionSendResultSchema,
  codezSessionRequestRuntimePreferencesParamsSchema,
  codezSessionRuntimePreferencesResultSchema,
  codezSessionStateSnapshotSchema,
  codezSessionSubscribeResultSchema,
  codezStateUpdatedNotificationSchema,
  codezUserInputRequestParamsSchema,
  ...
  codezWorkspaceUpdateOffPeakToolPolicyResultSchema,
  codezWorkspaceUpdateDynamicWorkflowPolicyResultSchema,
  type DynamicWorkflowClientConfig,
  type AgentLaneResourceSample,
  type ProcessResourceCliLane,
  type CodezMcpTelemetryEvent,
  type CodezMcpResourceSample,
...
} from "#src/codez-agent/automationToolPolicy.js";
import { CODEZ_AGENT_RUNTIME_UNAVAILABLE_CODE } from "./codezAgent.js";
import type {
  CodezProtocolRequestId,
  ModelSelection,
  CodezProviderRuntimeHeadersRequestParams,
  CodezSessionEvent,
  CodezSessionRuntimePreferencesScope,
  CodezSavedWorkflowScope,
...
import type {
  ICodezAgentService,
  CodezAgentBackgroundBashOutputParams,
  CodezAgentAppRuntimePreferences,
  CodezAgentRuntimeLifecycleEvent,
  CodezAgentAddPluginMarketplaceParams,
  CodezAgentAppUsageParams,
  CodezAgentCancelPluginOperationParams,
  CodezAgentCompactParams,
  ...
  CodezAgentGenerateWorkspaceTextParams,
  CodezAgentTestModelConnectivityParams,
  CodezAgentGoalParams,
  CodezAgentGrantWorkspaceHookTrustParams,
  CodezAgentInitializeResult,
  CodezAgentListSessionsParams,
  CodezAgentListSessionSubagentsParams,
  CodezAgentReadWorkspacePresentationParams,
  CodezAgentReadSessionEventsParams,
  CodezAgentReadSessionMessagesParams,
  CodezAgentReadSessionParams,
  CodezAgentRemovePluginMarketplaceParams,
  CodezAgentRespondSessionRuntimePreferencesParams,
  ...
  CodezAgentSessionRuntimePreferencesRequest,
  CodezAgentTaskTokenUsageParams,
  ...
  CodezAgentAttachmentReadParams,
  CodezAgentConversationAttachmentReadParams,
  CodezAgentConversationAttachmentStatParams,
  CodezAgentAttachmentPreviewSourceParams,
  CodezAgentAttachmentTerminalParams,
  CodezAgentConversationCommandParams,
  ...
  CodezAgentConversationWorkflowRunArtifactDataParams,
  CodezAgentConversationWorkflowRunArtifactReadParams,
  CodezAgentConversationWorkflowRunArtifactsParams,
  ...
  CodezAgentConnectionFlowParams,
  CodezAgentSessionsIndexSubscribeParams,
  CodezAgentWorkspaceConfigSubscribeParams,
} from "./codezAgent.js";
import {
...
import {
  ...
  readTrustedCodezAgentV4UnsubscribeRoute,
  type CodezAgentV4ConnectionContext,
} from "./codezAgentConnectionScope.js";
import { createBackgroundSessionEventCoalescer } from "#src/codez-agent/codezSessionEventCoalescer.js";
...
import type { CodezAgentProcessManagerOptions } from "./codezAgentProcessManager.js";
import type { IOffPeakTaskService } from "#src/session/offPeakTask.js";
import {
  CodezProtocolRequestTimeoutError,
  type CodezProtocolClient,
} from "./codezProtocolClient.js";
import { getDataBaseDir } from "../paths.js";
import {
  collectBrowserAmbientContext,
  type BrowserAmbientContextExecutor,
} from "./codezAgentBrowserAmbientContext.js";
import {
...
const logger = createServiceLogger("codez-agent-service");
...
function isClosedStdioTransportError(error: unknown): boolean {
  return error instanceof Error && error.message === "Codez agent stdio transport is closed";
}
...
function buildWorkspaceRef(params: CodezAgentWorkspaceTarget): CodezWorkspaceRef {
  return {
...
function savedWorkflowTargetHasWorkspace(
  params: CodezAgentSavedWorkflowTarget,
): params is CodezAgentWorkspaceTarget & { scope?: CodezSavedWorkflowScope } {
  return typeof (params as Partial<CodezAgentWorkspaceTarget>).workspacePath === "string";
}

// 只在 `scope` 有定义时下推到 RPC params：不给 scope 的项目档保持与今天逐字一致的线上形状。
function savedWorkflowScopeParam(params: CodezAgentSavedWorkflowTarget): {
  scope?: CodezSavedWorkflowScope;
} {
  return params.scope === undefined ? {} : { scope: params.scope };
}

function ensurePluginManagementWorkspacePath(): string {
  const workspacePath = join(getDataBaseDir(), ".codez", PLUGIN_MANAGEMENT_WORKSPACE_DIR_NAME);
...
// NOTE: this counts ONLY the per-session MCP servers passed through the Codez Protocol
// session/create params (the app→protocol channel). It is deliberately independent of the
// CLI/bootstrap MCP servers configured in ~/.codez/cli/config.json (mcp.servers), which the agent
// runtime connects separately and reports via the `mcp.server.connected`/toolCount events. So a
// createSession log line with mcpServerCount:0 is EXPECTED when codez-cua is a CLI-config MCP server
...
// real kimi-k2.6 turns call mcp__codez-cua__* tools (get_app_state/type/open_application, status
...
function isProtocolRequestTimeout(error: unknown, method: string): boolean {
  if (error instanceof CodezProtocolRequestTimeoutError) {
    return error.method === method;
  }
  return error instanceof Error && error.message === `Codez Protocol request timed out: ${method}`;
}
...
function buildSessionCreateParams(
  params: CodezAgentCreateSessionParams & {
...
function buildSessionResumeParams(
  params: CodezAgentResumeSessionParams & {
    offPeakToolEnabled?: boolean;
    dynamicWorkflowEnabled?: boolean;
  },
...
function buildSessionSendParams(
  params: CodezAgentSendPromptParams,
...
function buildSessionCompactParams(params: CodezAgentCompactParams) {
  return {
    sessionId: params.sessionId,
    inputId: params.inputId,
...
function formatModelSelectionForLog(ref: ModelSelection | undefined): string | null {
  ...
  return reasoningLevel ? `${base}${CODEZ_MODEL_REASONING_SEPARATOR}${reasoningLevel}` : base;
}

function sessionEventKey(params: CodezAgentSessionTarget): string {
...
function permissionRequestKey(params: CodezAgentSessionTarget & { requestId: string }): string {
  return `${sessionEventKey(params)}\u0000${params.requestId}`;
}

function userInputRequestKey(params: CodezAgentSessionTarget & { requestId: string }): string {
  return `${sessionEventKey(params)}\u0000${params.requestId}`;
}

function providerRuntimeHeadersRequestKey(
  params: CodezAgentSessionTarget & { requestId: string },
): string {
  return `${sessionEventKey(params)}\u0000${params.requestId}`;
}
...
interface CodezAgentProviderReadinessSnapshot {
...
): CodezAgentProviderReadinessSnapshot {
...
function createRuntimeUnavailableError(params: CodezAgentWorkspaceTarget): Error & {
  code: typeof CODEZ_AGENT_RUNTIME_UNAVAILABLE_CODE;
  workspaceKey: string;
} {
  const error = new Error("Codez Agent runtime is not running.") as Error & {
    code: typeof CODEZ_AGENT_RUNTIME_UNAVAILABLE_CODE;
    workspaceKey: string;
  };
  error.code = CODEZ_AGENT_RUNTIME_UNAVAILABLE_CODE;
  error.workspaceKey = resolveWorkspaceKey(params);
  return error;
}

interface CreateCodezAgentServiceOptions extends Omit<
...
function toProtocolAutomation(automation: CodezAutomation) {
  return {
...
async function respondOffPeakInternalError(
  client: Pick<CodezProtocolClient, "respondError">,
  request: { id: CodezProtocolRequestId; method: string },
  workspace: CodezAgentWorkspaceTarget,
  error: unknown,
): Promise<void> {
...
export function createCodezAgentService(
  options?: CreateCodezAgentServiceOptions,
): ICodezAgentService & { disposeAllAndWait(): Promise<void> } {
  const processManager = new CodezAgentProcessManager(options);
  const usesDefaultCodexBridge = usesCodexBridgeRuntime({
    ...
    customCommandResolver: Boolean(
      options?.commandResolver && options.commandResolver !== resolveDefaultCodezAgentCommand,
    ),
  });
  ...
  const pluginProcessManager = new CodezAgentProcessManager({
  ...
  const mcpStatusProcessManager = new CodezAgentProcessManager({
    commandResolver: options?.commandResolver,
    presentationSurface: options?.presentationSurface,
  ...
  const sessionEmitters = new Map<string, Emitter<CodezAgentServiceEvent>>();
  /**
  ...
  const sessionRuntimePreferencesRequestEmitter =
    new Emitter<CodezAgentSessionRuntimePreferencesRequest>();
  const processResourceSampleEmitter = new Emitter<AgentLaneResourceSample>();
  const toolExecResourceEmitter = new Emitter<CodezToolExecResource>();
  const mcpResourceSamplesEmitter = new Emitter<CodezMcpResourceSample[]>();
  const mcpTelemetryEmitter = new Emitter<CodezMcpTelemetryEvent>();
  const pluginOperationProgressEmitters = new Map<
    string,
    Emitter<CodezPluginOperationProgressNotification>
  >();
  ...
  const wiredClients = new WeakSet<CodezProtocolClient>();
  const clientDisposables = new WeakMap<CodezProtocolClient, IDisposable[]>();
  ...
  let latestAppRuntimePreferences: CodezAgentAppRuntimePreferences | undefined;
  ...
  const accountConfigSyncByClient = new WeakMap<CodezProtocolClient, Promise<void>>();
  // 此缓存只去重已交付的账号快照，不表示 Worker 的 Registry 已应用该版本。
  const accountConfigReceivedRevisionByClient = new WeakMap<CodezProtocolClient, string>();
  ...
  function invalidateWorkspaceClient(workspaceKey: string, client: CodezProtocolClient): void {
  ...
  async function respondAccountRequestAuthWithoutInteraction(params: {
    ...
    try {
      ...
      logger.info(undefined, "Codez provider runtime headers 已应用", {
      ...
      if (pendingProviderRuntimeHeaders.get(params.key) !== params.pending) return;
      logger.warn(undefined, "Codez provider runtime headers 应用失败", {
        modelId: params.pending.request.modelSelection.modelId,
        providerId: params.pending.request.providerId,
        requestId: params.pending.request.requestId,
        sessionId: params.pending.request.sessionId,
  ...
  function expireSessionRuntimePreferencesRequest(requestId: string): void {
    ...
    logger.warn(undefined, "运行时偏好请求等待 Host 响应超时", {
      event: "codez_agent.runtime_preferences.host_response_timeout",
      module: "services.codez_agent",
      requestId,
      scope: pending.request.scope,
      sessionId: pending.request.sessionId,
      timeoutMs: CODEZ_SESSION_RUNTIME_PREFERENCES_REQUEST_TIMEOUT_MS,
    ...
    const current = previous
      ...
      .then(async () => {
        ...
        const result = await params.client.request(
          codezProtocolMethods.providerUpdateAccountConfig,
          {
            revision: snapshot.revision,
            basedOnCodezBuiltinRevision: snapshot.basedOnCodezBuiltinRevision,
        ...
        logger.info(undefined, "account provider config 已交付到 Codez agent", {
  ...
  function enqueueInteractionPreferenceSync(params: {
    client: CodezProtocolClient;
    preferences: CodezAgentAppRuntimePreferences;
    workspace: CodezAgentWorkspaceTarget;
  }): Promise<void> {
    ...
    const current = previous
      ...
      .then(async () => {
        await params.client.request(
          codezProtocolMethods.workspaceUpdateInteractionPreferences,
          {
        ...
        try {
          await params.client.request(
            codezProtocolMethods.workspaceUpdateModelIoPreferences,
            {
              workspace: buildWorkspaceRef(params.workspace),
              preferences: {
  ...
  async function handleProviderReadinessChanged(event: {
    reason: string;
    snapshot: CodezAgentProviderReadinessSnapshot;
  }): Promise<void> {
    if (event.snapshot.readiness.ready) {
      await Promise.allSettled(
        Array.from(waitingWorkspaceStartups.entries()).map(async ([workspaceKey, waiting]) => {
          ...
          logger.info(undefined, "provider/model 就绪后已启动等待中的 Codez agent", {
  ...
  function getSessionEmitter(params: CodezAgentSessionTarget) {
  ...
  function getPluginOperationProgressEmitter(operationId: string) {
    ...
    let created: Emitter<CodezPluginOperationProgressNotification>;
    created = new Emitter({
  ...
  function getConversationFrameEmitter(workspace: CodezAgentWorkspaceTarget) {
  ...
  function getSessionsIndexFrameEmitter(workspace: CodezAgentWorkspaceTarget) {
    const key = resolveWorkspaceKey(workspace);
  ...
  function getWorkspaceConfigFrameEmitter(workspace: CodezAgentWorkspaceTarget) {
    const key = resolveWorkspaceKey(workspace);
  ...
  function resolveV4UnsubscribeRoute(
    params: CodezAgentConversationUnsubscribeParams | CodezAgentConversationResyncParams,
    topicPrefix: string,
  ): V4SubscriptionRoute | null {
    const expectedWorkspaceKey = resolveWorkspaceKey(params);
    const trusted = readTrustedCodezAgentV4UnsubscribeRoute(params);
    if (trusted) {
  ...
  async function unsubscribeV4Route(
    params: CodezAgentConversationUnsubscribeParams,
    topicPrefix: string,
  ): Promise<void> {
  ...
  async function resyncV4Route(params: CodezAgentConversationResyncParams, topicPrefix: string) {
    const route = resolveV4UnsubscribeRoute(params, topicPrefix);
  ...
  function handleStateUpdated(
    workspace: CodezAgentWorkspaceTarget,
    notification: CodezStateUpdatedNotification,
  ): void {
  ...
  function wireClient(
    ...
    const disposables = [
      client.onNotification((message) => {
        if (message.method === codezProtocolNotifications.providerRuntimeHeadersCancelled) {
          const parsed = codezProviderRuntimeHeadersCancelledSchema.safeParse(message.params);
          if (
            !parsed.success ||
        ...
        if (message.method === codezProtocolNotifications.processResourceSample) {
          const parsed = codezProcessResourceSampleSchema.safeParse(message.params);
          if (parsed.success) {
            processResourceSampleEmitter.fire({ ...parsed.data, lane });
          } else {
            logger.debug(undefined, "丢弃无效 Codez CLI 资源样本", {
        ...
        if (message.method === codezProtocolNotifications.toolExecResource) {
          const parsed = codezToolExecResourceSchema.safeParse(message.params);
          if (parsed.success) toolExecResourceEmitter.fire(parsed.data);
          return;
        }
        if (message.method === codezProtocolNotifications.mcpResourceSamples) {
          const parsed = codezMcpResourceSamplesSchema.safeParse(message.params);
          if (parsed.success) mcpResourceSamplesEmitter.fire(parsed.data);
          else logger.debug(undefined, "丢弃无效 MCP 资源样本");
          return;
        }

        if (message.method === codezProtocolNotifications.mcpTelemetry) {
          const parsed = codezMcpTelemetryEventSchema.safeParse(message.params);
          if (parsed.success) {
            mcpTelemetryEmitter.fire(parsed.data);
          } else {
            logger.debug(undefined, "丢弃无效 Codez CLI MCP 遥测事件", {
              issues: parsed.error.issues.map((issue) => ({
                code: issue.code,
                path: issue.path.join("."),
              })),
        ...
        if (message.method === codezProtocolNotifications.pluginOperationProgress) {
          const parsed = codezPluginOperationProgressNotificationSchema.safeParse(message.params);
          if (parsed.success) {
            pluginOperationProgressEmitters.get(parsed.data.operationId)?.fire(parsed.data);
          } else {
            logger.warn(undefined, "丢弃无效 Codez Protocol 插件操作进度", {
              issues: parsed.error.issues.map((issue) => ({
                code: issue.code,
        ...
        if (message.method === codezProtocolMethods.computerUseOperationEvent) {
          const parsed = codezComputerUseOperationEventSchema.safeParse(message.params);
          if (parsed.success) {
            ...
            logger.warn(undefined, "丢弃无效 Codez Protocol Computer Use operation event", {
              issues: parsed.error.issues.map((issue) => ({
                code: issue.code,
                message: issue.message,
                path: issue.path.join("."),
        ...
        if (message.method === "session/event") {
          const parsed = codezSessionEventSchema.safeParse(message.params);
          if (parsed.success) {
            ...
            logger.warn(
              ...
              "丢弃无效 Codez Protocol session event",
              {
        ...
        if (message.method === "state.updated") {
          const parsed = codezStateUpdatedNotificationSchema.safeParse(message.params);
          if (parsed.success) {
      ...
      client.onRequest((request) => {
        if (request.method === codezProtocolMethods.sessionRequestRuntimePreferences) {
          ...
          const parsed = codezSessionRequestRuntimePreferencesParamsSchema.safeParse(
            request.params,
          );
          if (!parsed.success) {
            void client
          ...
          if (sessionRuntimePreferencesAuthority === "local") {
            void (async () => {
              let preferences: CodezSessionRuntimePreferencesResult;
              try {
                preferences = codezSessionRuntimePreferencesResultSchema.parse(
          ...
          const dynamicRequest: CodezAgentSessionRuntimePreferencesRequest = {
            ...parsed.data,
            requestId,
          };
          ...
          logger.info(undefined, "运行时偏好请求已转发给 Host", {
            event: "codez_agent.runtime_preferences.host_request_dispatched",
            module: "services.codez_agent",
            requestId,
        ...
        if (request.method === codezProtocolMethods.interactionRequestPermission) {
          const parsed = codezPermissionRequestParamsSchema.safeParse(request.params);
          if (!parsed.success) {
        ...
        if (request.method === codezProtocolMethods.interactionRequestUserInput) {
          const parsed = codezUserInputRequestParamsSchema.safeParse(request.params);
          if (!parsed.success) {
            void client.respondError(request.id, {
              code: -32602,
        ...
        if (request.method === codezProtocolMethods.interactionRequestProviderRuntimeHeaders) {
          const parsed = codezProviderRuntimeHeadersRequestParamsSchema.safeParse(request.params);
          if (!parsed.success) {
            void client.respondError(request.id, {
              code: -32602,
          ...
          logger.info(request.trace?.traceId, "收到 Codez provider runtime headers 请求", {
        ...
        if (request.method === codezProtocolMethods.interactionRequestOfficialMcpAuthHeaders) {
          const parsed = codezOfficialMcpAuthHeadersRequestParamsSchema.safeParse(request.params);
          if (!parsed.success) {
            void client.respondError(request.id, {
              code: -32602,
        ...
        if (request.method === codezProtocolMethods.interactionBrowserList) {
          const parsed = codezBrowserListParamsSchema.safeParse(request.params);
          if (!parsed.success) {
            void client.respondError(request.id, {
              code: -32602,
        ...
        if (request.method === codezProtocolMethods.interactionBrowserExecute) {
          const parsed = codezBrowserExecuteParamsSchema.safeParse(request.params);
          if (!parsed.success) {
            void client.respondError(request.id, {
              code: -32602,
        ...
        if (request.method === codezProtocolMethods.automationCreate) {
          const parsed = codezAutomationCreateParamsSchema.safeParse(request.params);
          if (!parsed.success) {
            void client.respondError(request.id, {
              code: -32602,
        ...
        if (request.method === codezProtocolMethods.offPeakCreate) {
          const parsed = codezOffPeakCreateParamsSchema.safeParse(request.params);
          if (!parsed.success) {
            void client.respondError(request.id, {
              code: -32602,
        ...
        if (request.method === codezProtocolMethods.offPeakList) {
          const parsed = codezOffPeakListParamsSchema.safeParse(request.params ?? {});
          if (!parsed.success) {
            void client.respondError(request.id, {
              code: -32602,
        ...
        if (request.method === codezProtocolMethods.automationList) {
          const parsed = codezAutomationListParamsSchema.safeParse(request.params ?? {});
          if (!parsed.success) {
            void client.respondError(request.id, {
              code: -32602,
        ...
        if (request.method === codezProtocolMethods.automationCheckTaskBinding) {
          const parsed = codezAutomationCheckTaskBindingParamsSchema.safeParse(request.params);
          if (!parsed.success) {
            void client.respondError(request.id, {
              code: -32602,
        ...
        if (request.method === codezProtocolMethods.automationUpdate) {
          const parsed = codezAutomationUpdateParamsSchema.safeParse(request.params);
          if (!parsed.success) {
            void client.respondError(request.id, {
              code: -32602,
        ...
        if (request.method === codezProtocolMethods.automationDelete) {
          const parsed = codezAutomationDeleteParamsSchema.safeParse(request.params);
          if (!parsed.success) {
            void client.respondError(request.id, {
              code: -32602,
        ...
        void client.respondError(request.id, {
          code: -32601,
          message: `Unsupported Codez Protocol request: ${request.method}`,
        });
      }),
      client.onClose(() => {
  ...
  function getSessionTraceId(target: CodezAgentSessionTarget): TraceId | undefined {
  ...
  function createProviderNotReadyError(params: {
    ...
    code: typeof CODEZ_AGENT_PROVIDER_NOT_READY_CODE;
    data: {
      providerCount: number;
      reason: typeof CODEZ_AGENT_PROVIDER_NOT_READY_REASON;
    ...
    error.code = CODEZ_AGENT_PROVIDER_NOT_READY_CODE;
    error.data = {
      providerCount: params.snapshot?.providerCount ?? 0,
      reason: CODEZ_AGENT_PROVIDER_NOT_READY_REASON,
  ...
  function isProviderNotReadyError(
    error: unknown,
  ): error is Error & { code: typeof CODEZ_AGENT_PROVIDER_NOT_READY_CODE } {
    return (
      error instanceof Error &&
      (error as { code?: unknown }).code === CODEZ_AGENT_PROVIDER_NOT_READY_CODE
    );
  }

  async function resolveStartupReadiness(): Promise<
    CodezAgentProviderReadinessSnapshot | undefined
  > {
  ...
  function isReusableActiveClientEntry(
    ...
    activeClientsByWorkspaceKey.delete(workspaceKey);
    interactionPreferenceSyncByWorkspaceKey.delete(workspaceKey);
    logger.warn(undefined, "复用的 Codez Protocol client 已 disposed，清理 stale entry", {
      workspaceKey,
      workspacePath: params.workspacePath,
    });
    return false;
  ...
  async function getOrStartReadOnlyClient(
    ...
    const interactionPreferencesReady = (async () => {
      let appliedSnapshot: CodezAgentAppRuntimePreferences | undefined;
    ...
    const offPeakToolPolicyReady = (async () => {
      ...
      try {
        await client.request(
          codezProtocolMethods.workspaceUpdateOffPeakToolPolicy,
    ...
    const dynamicWorkflowPolicyReady = (async () => {
      ...
      try {
        await client.request(
          codezProtocolMethods.workspaceUpdateDynamicWorkflowPolicy,
          { workspace: buildWorkspaceRef(params), enabled: true },
          codezWorkspaceUpdateDynamicWorkflowPolicyResultSchema,
        );
      } catch (error) {
    ...
    logger.info(undefined, "为只读会话控制面启动 Codez agent", {
      workspaceKey,
      workspacePath: params.workspacePath,
    });
    return entry;
  ...
  async function getClient(params: CodezAgentWorkspaceTarget) {
    ...
    if (!readinessSnapshot || !readiness?.ready) {
      ...
      if (waiting.lastLoggedRevision !== revision) {
        ...
        logger.info(
          undefined,
          active
            ? "provider/model 尚未就绪，Codez agent 保持只读"
            : "provider/model 尚未就绪，Codez agent 保持未启动",
          {
    ...
    waitingWorkspaceStartups.delete(workspaceKey);
    logger.info(undefined, "provider/model 就绪，允许 Codez agent 模型执行", {
  ...
  // homedir() 即用户家目录，全局根 `~/.codez/workflows/` 因此解析到真实目录。
  // 远程 runtime（SSH/WSL identity 或带 remoteSessionId）的 home 不是本机，绝不选它当载体。
  function isLocalActiveWorkspaceClient(workspace: CodezAgentWorkspaceTarget): boolean {
    return (
      !workspace.remoteSessionId &&
      !(workspace.workspaceIdentity && isRemoteWorkspaceIdentity(workspace.workspaceIdentity))
    );
  ...
  async function resolveSavedWorkflowCarrier(
    params: CodezAgentSavedWorkflowTarget,
  ): Promise<{ client: CodezProtocolClient; workspace: CodezWorkspaceRef }> {
  ...
  async function getMcpStatusClient(): Promise<CodezProtocolClient> {
    const workspace = { workspacePath: ensurePluginManagementWorkspacePath() };
  ...
  return {
    async codexRequest(params) {
      ...
      return client.request(codezProtocolMethods.codexRequest, request);
    },
    ...
    async initialize(params: CodezAgentWorkspaceTarget): Promise<CodezAgentInitializeResult> {
      const workspaceKey = resolveWorkspaceKey(params);
      const startedAt = Date.now();
      logger.info(undefined, "开始初始化 Codez agent", {
        workspaceKey,
        workspacePath: params.workspacePath,
      });
      try {
        const client = await getClient(params);
        logger.info(undefined, "Codez agent 初始化完成", {
        ...
        return {
          available: true,
          workspaceKey,
          protocolName: CODEZ_PROTOCOL_NAME,
          protocolVersion: CODEZ_PROTOCOL_VERSION,
          transportKind: client.transportKind === "websocket" ? "websocket" : "stdio",
        };
      } catch (error) {
        ...
        logger[providerNotReady ? "info" : "warn"](
          undefined,
          providerNotReady ? "Codez agent 等待 provider/model 就绪" : "Codez agent 初始化失败",
          {
            durationMs: Date.now() - startedAt,
            message: error instanceof Error ? error.message : String(error),
            workspaceKey,
        ...
        return {
          ...
          ...(providerNotReady ? { reasonCode: CODEZ_AGENT_PROVIDER_NOT_READY_REASON } : {}),
        };
      }
    },

    async syncAppRuntimePreferences(preferences: CodezAgentAppRuntimePreferences): Promise<void> {
      const normalizedPreferences: CodezAgentAppRuntimePreferences = {
        ...preferences,
    ...
    async getWorkspaceRuntimeIdentity(params: CodezAgentWorkspaceTarget) {
    ...
    async createSession(params: CodezAgentCreateSessionParams) {
      const startedAt = Date.now();
      const client = await getClient(params);
      await ensureAccountProviderConfigSynced({
        client,
      ...
      logger.info(sessionTraceId, "开始请求 Codez Protocol session/create", {
      ...
      try {
        const snapshot = await client.request(
          codezProtocolMethods.sessionCreate,
        ...
        logger.info(sessionTraceId, "Codez Protocol session/create 完成", {
          durationMs: Date.now() - startedAt,
        ...
        if (compatFields.length === 0) {
          logger.warn(sessionTraceId, "Codez Protocol session/create 失败", {
            durationMs: Date.now() - startedAt,
            message: error instanceof Error ? error.message : String(error),
            persistence: params.persistence,
            workspaceKey: resolveWorkspaceKey(params),
        ...
        logger.warn(sessionTraceId, "Codez Protocol session/create 命中新旧协议兼容重试", {
          compatFields,
          durationMs: Date.now() - startedAt,
          workspaceKey: resolveWorkspaceKey(params),
          workspacePath: params.workspacePath,
        ...
        if (!compatFields.includes("thoughtLevel") || !params.thoughtLevel) {
          logger.info(sessionTraceId, "Codez Protocol session/create 兼容重试完成", {
            durationMs: Date.now() - startedAt,
            sessionId: snapshot.session.sessionId,
            snapshotTraceId: snapshot.session.traceId ?? null,
            workspaceKey: resolveWorkspaceKey(params),
        ...
        const snapshotWithThoughtLevel = await client.request(
          codezProtocolMethods.sessionSetThoughtLevel,
          {
            sessionId: snapshot.session.sessionId,
            thoughtLevel: params.thoughtLevel,
        ...
        logger.info(
          sessionTraceId,
          "Codez Protocol session/create 兼容重试后设置 thoughtLevel 完成",
          {
            durationMs: Date.now() - startedAt,
            sessionId: snapshot.session.sessionId,
    ...
    async resumeSession(params: CodezAgentResumeSessionParams) {
      const startedAt = Date.now();
      const client = await getClient(params);
      await ensureAccountProviderConfigSynced({
        client,
      ...
      const dynamicWorkflowEnabled = await resolveDynamicWorkflowGate();
      logger.info(cachedTraceId, "开始请求 Codez Protocol session/resume", {
        mcpServerCount: getMcpServerCount(params),
        mcpServerNames: getMcpServerNames(params),
      ...
      try {
        const snapshot = await client.request(
          codezProtocolMethods.sessionResume,
        ...
        logger.info(sessionTraceId, "Codez Protocol session/resume 完成", {
          durationMs: Date.now() - startedAt,
          messageCount: snapshot.messages.length,
          modelCurrent: formatModelSelectionForLog(snapshot.settings.model.current),
          mcpServerCount: getMcpServerCount(params),
        ...
        if (compatFields.length === 0) {
          logger.warn(cachedTraceId, "Codez Protocol session/resume 失败", {
            durationMs: Date.now() - startedAt,
            message: error instanceof Error ? error.message : String(error),
            sessionId: params.sessionId,
            workspaceKey: resolveWorkspaceKey(params),
        ...
        logger.warn(cachedTraceId, "Codez Protocol session/resume 命中新旧协议兼容重试", {
          compatFields,
          durationMs: Date.now() - startedAt,
          sessionId: params.sessionId,
          workspaceKey: resolveWorkspaceKey(params),
        ...
        const sessionTraceId = rememberSessionTrace(params, snapshot) ?? cachedTraceId;
        logger.info(sessionTraceId, "Codez Protocol session/resume 兼容重试完成", {
          durationMs: Date.now() - startedAt,
          sessionId: params.sessionId,
          snapshotTraceId: snapshot.session.traceId ?? null,
          workspaceKey: resolveWorkspaceKey(params),
    ...
    async listSessions(params: CodezAgentListSessionsParams) {
      const client = await getReadOnlyClient(params, params.runtimePolicy);
      const result = await client.request(
        codezProtocolMethods.sessionList,
        {
          workspace: buildWorkspaceRef(params),
    ...
    async listSessionSubagents(params: CodezAgentListSessionSubagentsParams) {
      const client = await getReadOnlyClient(params);
      return client.request(
        codezProtocolMethods.sessionSubagents,
        {
          sessionId: params.sessionId,
    ...
    async getAppUsageStats(params: CodezAgentAppUsageParams) {
    ...
    async readSession(params: CodezAgentReadSessionParams) {
      const client = await getReadOnlyClient(params, params.runtimePolicy);
      ...
      const snapshot = await client.request(
        codezProtocolMethods.sessionRead,
        {
          sessionId: params.sessionId,
    ...
    async readSessionMessages(params: CodezAgentReadSessionMessagesParams) {
      const client = await getReadOnlyClient(params);
      const result = await client.request(
        codezProtocolMethods.sessionMessages,
        {
          sessionId: params.sessionId,
    ...
    async readSessionDebug(params) {
      ...
      return client.request(
        codezProtocolMethods.sessionDebug,
        { sessionId: params.sessionId },
        sessionDebugSnapshotSchema,
      );
    },

    async readSessionEvents(params: CodezAgentReadSessionEventsParams) {
      const client = await getReadOnlyClient(params);
      const result = await client.request(
        codezProtocolMethods.sessionEvents,
        {
          sessionId: params.sessionId,
          afterSeq: params.afterSeq,
          limit: params.limit,
    ...
    async readWorkspacePresentation(params: CodezAgentReadWorkspacePresentationParams) {
      const startedAt = Date.now();
      logger.info(undefined, "开始请求 Codez Protocol workspace/readPresentation", {
        workspaceKey: resolveWorkspaceKey(params),
        workspacePath: params.workspacePath,
      });
      try {
        let presentation: CodezWorkspacePresentation | undefined;
        for (let attempt = 0; attempt < 2; attempt += 1) {
          ...
          try {
            ...
            presentation = await client.request(
              codezProtocolMethods.workspaceReadPresentation,
              { workspace: buildWorkspaceRef(params) },
              codezWorkspacePresentationSchema,
            );
            break;
        ...
        if (!presentation) {
          throw new Error("Codez Protocol workspace/readPresentation did not return a result");
        }
        logger.info(undefined, "Codez Protocol workspace/readPresentation 完成", {
          durationMs: Date.now() - startedAt,
        ...
        logger.warn(undefined, "Codez Protocol workspace/readPresentation 失败", {
          durationMs: Date.now() - startedAt,
          message: error instanceof Error ? error.message : String(error),
          workspaceKey: resolveWorkspaceKey(params),
          workspacePath: params.workspacePath,
    ...
    async grantWorkspaceHookTrust(params: CodezAgentGrantWorkspaceHookTrustParams) {
    ...
    async listMcpServerStatuses(params: CodezAgentListMcpServerStatusesParams) {
      ...
      try {
        ...
        if (params.mode === "status" && unrecognizedKeys.includes("mode")) {
          ...
          throw new CodezAgentMcpStatusModeUnsupportedError();
        }
        if (
        ...
        if (!isProtocolRequestTimeout(error, codezProtocolMethods.mcpList)) {
          throw error;
        }
    ...
    async listPlugins(params: CodezAgentPluginViewParams) {
      const requestPluginsList = async () => {
        ...
        return client.request(
          codezProtocolMethods.pluginsList,
          {
            workspace: buildWorkspaceRef(params),
      ...
      try {
        ...
        if (!isProtocolRequestTimeout(error, codezProtocolMethods.pluginsList)) {
          throw error;
        }
    ...
    async getSkillReferenceCatalog(params: CodezAgentSkillReferenceCatalogParams) {
      // 与 Plugin 引用一样，Session 快照只存在于 workspace agent 进程；独立管理进程
      // 没有 resident Session，不能作为对话 authority。
      const client = await getReadOnlyClient(params);
      return client.request(
        codezProtocolMethods.skillsReferenceCatalog,
        {
          workspace: buildWorkspaceRef(params),
          ...(params.sessionId ? { sessionId: params.sessionId } : {}),
        },
    ...
    // `scope` 只在有定义时下推。
    async listSavedWorkflows(params: CodezAgentListSavedWorkflowsParams) {
      const { client, workspace } = await resolveSavedWorkflowCarrier(params);
      return client.request(
        codezProtocolMethods.workflowsList,
        { workspace, ...savedWorkflowScopeParam(params) },
        codezWorkflowsListResultSchema,
      );
    },

    async getSavedWorkflow(params: CodezAgentGetSavedWorkflowParams) {
      const { client, workspace } = await resolveSavedWorkflowCarrier(params);
      return client.request(
        codezProtocolMethods.workflowsGet,
        { workspace, name: params.name, ...savedWorkflowScopeParam(params) },
        codezWorkflowsGetResultSchema,
      );
    },

    async updateSavedWorkflowMeta(params: CodezAgentUpdateSavedWorkflowMetaParams) {
      const { client, workspace } = await resolveSavedWorkflowCarrier(params);
      return client.request(
        codezProtocolMethods.workflowsUpdateMeta,
        { workspace, name: params.name, meta: params.meta, ...savedWorkflowScopeParam(params) },
        codezWorkflowsUpdateMetaResultSchema,
      );
    },

    async deleteSavedWorkflow(params: CodezAgentDeleteSavedWorkflowParams) {
      const { client, workspace } = await resolveSavedWorkflowCarrier(params);
      return client.request(
        codezProtocolMethods.workflowsDelete,
        { workspace, name: params.name, ...savedWorkflowScopeParam(params) },
        codezWorkflowsDeleteResultSchema,
      );
    },

    async listSavedWorkflowRuns(params: CodezAgentListSavedWorkflowRunsParams) {
      const { client, workspace } = await resolveSavedWorkflowCarrier(params);
      return client.request(
        codezProtocolMethods.workflowsRuns,
        {
          workspace,
    ...
    async moveSavedWorkflow(params: CodezAgentMoveSavedWorkflowParams) {
      const client = await getReadOnlyClient(params);
      return client.request(
        codezProtocolMethods.workflowsMove,
    ...
    async collectLocalRuntimeChildProcesses(signal?: AbortSignal) {
      ...
      return Promise.all(
        managed.map(async (runtime) => {
          let children: CodezProcessChildProcess[] = [];
          try {
            const result = await runtime.client.request(
              codezProtocolMethods.processChildProcesses,
              {},
              codezProcessChildProcessesResultSchema,
    ...
    async cancelPluginOperation(params: CodezAgentCancelPluginOperationParams) {
      const client = await getPluginManagementClient();
      return client.request(
        codezProtocolMethods.pluginsCancelOperation,
        { operationId: params.operationId },
        codezPluginsCancelOperationResultSchema,
    ...
    async listAutomations(params: CodezAgentWorkspaceTarget) {
    ...
    async createAutomation(params: CodezAgentCreateAutomationParams) {
      return automationService.create({
        ...
        mode: params.mode as CodezTaskMode | undefined,
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
    ...
    async updateAutomation(params: CodezAgentUpdateAutomationParams) {
      return automationService.update(
        params.automationId,
        {
          ...
          prompt: params.prompt,
          modelSelection: params.modelSelection,
          mode: params.mode === null ? null : (params.mode as CodezTaskMode | undefined),
    ...
    async deleteAutomation(params: CodezAgentAutomationIdParams) {
    ...
    async setAutomationEnabled(params: CodezAgentSetAutomationEnabledParams) {
    ...
    async restartAutomation(params: CodezAgentAutomationIdParams) {
    ...
    async runAutomationNow(params: CodezAgentAutomationIdParams) {
    ...
    async listAutomationRuns(params: CodezAgentAutomationIdParams) {
    ...
    async deleteAutomationRun(params: CodezAgentDeleteAutomationRunParams) {
    ...
    async generateWorkspaceText(params: CodezAgentGenerateWorkspaceTextParams) {
      ...
      const cancel = () => {
        ...
        void client
          .request(
            codezProtocolMethods.workspaceCancelGenerateText,
            { operationId },
            codezWorkspaceCancelGenerateTextResultSchema,
            { timeoutMs: 5_000 },
          )
      ...
      try {
        return await client.request(
          codezProtocolMethods.workspaceGenerateText,
          {
            workspace: buildWorkspaceRef(params),
    ...
    async testModelConnectivity(params: CodezAgentTestModelConnectivityParams) {
      const client = await getClient(params);
      await ensureAccountProviderConfigSynced({
        client,
      ...
      return client.request(
        codezProtocolMethods.providerTestModelConnectivity,
        {
          workspace: buildWorkspaceRef(params),
          selection: params.selection,
        },
    ...
    async sendPrompt(params: CodezAgentSendPromptParams) {
      const startedAt = Date.now();
      const client = await getClient(params);
      ...
      const protocolParams: CodezAgentSendPromptParams = {
        ...params,
        ...(browserAmbientContext ? { browserAmbientContext } : {}),
      };
      logger.info(logTraceId, "Codez Agent session/send 开始", {
      ...
      try {
        const result = await client.request(
          codezProtocolMethods.sessionSend,
          buildSessionSendParams(protocolParams),
          codezSessionSendResultSchema,
        );
        logger.info(logTraceId, "Codez Agent session/send ACK", {
          durationMs: Date.now() - startedAt,
          inputId: params.inputId,
          queryId: params.queryId ?? null,
          sessionId: params.sessionId,
        ...
        if (compatFields.length > 0) {
          logger.warn(logTraceId, "Codez Agent session/send 命中新旧协议兼容重试", {
            compatFields,
            durationMs: Date.now() - startedAt,
            sessionId: params.sessionId,
            workspaceKey: resolveWorkspaceKey(params),
        ...
        logger.warn(logTraceId, "Codez Agent session/send 失败", {
          durationMs: Date.now() - startedAt,
          error: error instanceof Error ? error.message : String(error),
          inputId: params.inputId,
          queryId: params.queryId ?? null,
    ...
    async compactSession(params: CodezAgentCompactParams) {
      const startedAt = Date.now();
      const client = await getClient(params);
      const sessionTraceId = getSessionTraceId(params);
      logger.info(sessionTraceId ?? params.inputId, "Codez Protocol session/compact 开始", {
        inputId: params.inputId,
        sessionId: params.sessionId,
        workspaceKey: resolveWorkspaceKey(params),
        workspacePath: params.workspacePath,
      ...
      try {
        const result = await client.request(
          codezProtocolMethods.sessionCompact,
        ...
        logger.info(sessionTraceId ?? params.inputId, "Codez Protocol session/compact ACK", {
          durationMs: Date.now() - startedAt,
          inputId: params.inputId,
          sessionId: params.sessionId,
          workspaceKey: resolveWorkspaceKey(params),
        ...
        logger.warn(sessionTraceId ?? params.inputId, "Codez Protocol session/compact 失败", {
          durationMs: Date.now() - startedAt,
          error: error instanceof Error ? error.message : String(error),
          inputId: params.inputId,
          sessionId: params.sessionId,
    ...
    async goalSession(params: CodezAgentGoalParams) {
      const startedAt = Date.now();
      const client = await getClient(params);
      logger.info(params.inputId, "开始请求 Codez Protocol session/goal", {
      ...
      try {
        const result = await client.request(
          codezProtocolMethods.sessionGoal,
          {
            sessionId: params.sessionId,
            inputId: params.inputId,
            action: params.action,
        ...
        logger.info(params.inputId, "Codez Protocol session/goal 完成", {
          action: params.action,
          durationMs: Date.now() - startedAt,
        ...
        logger.warn(params.inputId, "Codez Protocol session/goal 失败", {
          action: params.action,
          durationMs: Date.now() - startedAt,
          message: error instanceof Error ? error.message : String(error),
          sessionId: params.sessionId,
    ...
    async closeSession(
      params: CodezAgentSessionTarget & {
        expectedPersistence?: "deferred" | "immediate";
      },
    ): Promise<boolean> {
      const client = await getClient(params);
      const result = await client.request(
        codezProtocolMethods.sessionClose,
        {
          sessionId: params.sessionId,
    ...
    async setModel(params: CodezAgentSetModelParams) {
      const startedAt = Date.now();
      const client = await getClient(params);
      logger.info(undefined, "开始请求 Codez Protocol session/setModel", {
      ...
      try {
        const snapshot = await client.request(
          codezProtocolMethods.sessionSetModel,
          {
            sessionId: params.sessionId,
            model: params.model,
            expectedRevision: params.expectedRevision,
        ...
        logger.info(undefined, "Codez Protocol session/setModel 完成", {
          durationMs: Date.now() - startedAt,
          requestedModel: formatModelSelectionForLog(params.model),
          sessionId: params.sessionId,
        ...
        logger.warn(undefined, "Codez Protocol session/setModel 失败", {
          durationMs: Date.now() - startedAt,
          message: error instanceof Error ? error.message : String(error),
          requestedModel: formatModelSelectionForLog(params.model),
          sessionId: params.sessionId,
    ...
    async setThoughtLevel(params: CodezAgentSetThoughtLevelParams) {
      const client = await getClient(params);
      const snapshot = await client.request(
        codezProtocolMethods.sessionSetThoughtLevel,
        {
    ...
    async setMode(params: CodezAgentSetModeParams) {
      const client = await getClient(params);
      return client.request(
        codezProtocolMethods.sessionSetMode,
        {
          sessionId: params.sessionId,
          mode: params.mode,
          expectedRevision: params.expectedRevision,
    ...
    async respondSessionRuntimePreferences(
      ...
      if (!pending) {
        logger.warn(undefined, "运行时偏好响应找不到 pending 请求", {
          event: "codez_agent.runtime_preferences.response_without_pending",
          module: "services.codez_agent",
          requestId: params.requestId,
        });
        throw new Error(`Codez session runtime preferences request not found: ${params.requestId}`);
      }
      const responseContext = {
        event: "codez_agent.runtime_preferences.host_response_received",
        module: "services.codez_agent",
        requestId: params.requestId,
        scope: pending.request.scope,
        sessionId: pending.request.sessionId,
    ...
    onDynamicSessionEvent(params: CodezAgentSessionSubscribeParams) {
      ...
      return (listener) => {
        ...
        const pendingLiveEvents: CodezAgentServiceEvent[] = [];
        ...
        const deliverToListener = (event: CodezAgentServiceEvent): void => {
          if (cancelled) {
            return;
          }
        ...
        const establishSubscription = async (attempt: number): Promise<void> => {
          ...
          try {
            ...
            const result = await client.request(
              codezProtocolMethods.sessionSubscribe,
              {
                sessionId: params.sessionId,
                deliveryKind: params.deliveryKind,
                afterSeq: params.afterSeq,
            ...
            if (attempt >= SESSION_SUBSCRIBE_MAX_ATTEMPTS - 1) {
              ...
              console.warn(
                formatLogPrefix("codez-agent", process.pid),
    ...
    async setConnectionFlowStateV4(params: CodezAgentConnectionFlowParams) {
    ...
    async subscribeConversationV4(params: CodezAgentConversationSubscribeParams) {
      ...
      // Account Config 顺序屏障，不提升模型执行权限。Codez Built-in / Personal 仍由 Worker 维护。
    ...
    async unsubscribeConversationV4(params: CodezAgentConversationUnsubscribeParams) {
      await unsubscribeV4Route(params, "conversation/");
    },

    async resyncConversationV4(params: CodezAgentConversationResyncParams) {
      return resyncV4Route(params, "conversation/");
    },

    async sendConversationCommandV4(params: CodezAgentConversationCommandParams) {
      const client = await getClient(params);
      ...
      const commandClientMode =
        readTrustedCodezAgentV4Connection(params)?.clientMode ??
        params.clientMode ??
        "desktop-continuous";
      if (params.envelope.type === "createSession") {
        // V4 草稿预热直接走 command 转发；新会话创建前只需等待 Account Config，
        // Codez Built-in / Personal 已由 Worker 进程 Registry 自己装配。
        await ensureAccountProviderConfigSynced({
          client,
    ...
    async queryConversationCommandsV4(params: CodezAgentCommandsQueryParams) {
      if (!readTrustedCodezAgentV4Connection(params)) {
    ...
    async attachmentBeginV4(params: CodezAgentAttachmentBeginParams) {
      const trusted = readTrustedCodezAgentV4Connection(params);
    ...
    async attachmentChunkV4(params: CodezAgentAttachmentChunkParams) {
      const trusted = readTrustedCodezAgentV4Connection(params);
      if (!trusted) throw new Error("fault.attachment.connectionUntrusted");
      const client = await getClient(params);
      const wireParams = {
    ...
    async attachmentCommitV4(params: CodezAgentAttachmentTerminalParams) {
      const trusted = readTrustedCodezAgentV4Connection(params);
      if (!trusted) throw new Error("fault.attachment.connectionUntrusted");
      const client = await getClient(params);
      const wireParams = {
    ...
    async attachmentAbortV4(params: CodezAgentAttachmentTerminalParams) {
      const trusted = readTrustedCodezAgentV4Connection(params);
      if (!trusted) throw new Error("fault.attachment.connectionUntrusted");
      const client = await getClient(params);
      const wireParams = {
    ...
    async attachmentReadV4(params: CodezAgentAttachmentReadParams) {
      if (!readTrustedCodezAgentV4Connection(params)) {
    ...
    async conversationAttachmentReadV4(params: CodezAgentConversationAttachmentReadParams) {
      if (!readTrustedCodezAgentV4Connection(params)) {
        throw new CodezAttachmentFaultError(
          CODEZ_ATTACHMENT_FAULT_CODES.shareReadConnectionUntrusted,
        );
      }
    ...
    async conversationAttachmentStatV4(params: CodezAgentConversationAttachmentStatParams) {
      if (!readTrustedCodezAgentV4Connection(params)) {
        throw new CodezAttachmentFaultError(
          CODEZ_ATTACHMENT_FAULT_CODES.shareStatConnectionUntrusted,
        );
      }
    ...
    async attachmentPreviewSourceV4(params: CodezAgentAttachmentPreviewSourceParams) {
      const trusted = readTrustedCodezAgentV4Connection(params);
    ...
    async conversationPlansV4(params: CodezAgentConversationPlansParams) {
      const client = await getReadOnlyClient(params);
      return client.request(
    ...
    async conversationWorkflowRunEventsV4(params: CodezAgentConversationWorkflowRunEventsParams) {
      const client = await getReadOnlyClient(params);
      return client.request(
    ...
    async conversationWorkflowRunsV4(params: CodezAgentConversationWorkflowRunsParams) {
      const client = await getReadOnlyClient(params);
      return client.request(
    ...
    async backgroundBashOutputV4(params: CodezAgentBackgroundBashOutputParams) {
      if (!readTrustedCodezAgentV4Connection(params))
    ...
    async conversationFileChangesV4(params: CodezAgentConversationFileChangesParams) {
      const client = await getReadOnlyClient(params);
      return client.request(
    ...
    async conversationFileRewindPreviewV4(params: CodezAgentConversationFileRewindPreviewParams) {
      const client = await getReadOnlyClient(params);
      return client.request(
    ...
    onDynamicLocalTtftFacts(params: CodezAgentWorkspaceTarget) {
    ...
    onDynamicConversationTelemetryFact(params: CodezAgentWorkspaceTarget) {
    ...
    async subscribeSessionsIndexV4(params: CodezAgentSessionsIndexSubscribeParams) {
    ...
    async unsubscribeSessionsIndexV4(params: CodezAgentConversationUnsubscribeParams) {
      await unsubscribeV4Route(params, "sessions-index/");
    },

    async resyncSessionsIndexV4(params: CodezAgentConversationResyncParams) {
      return resyncV4Route(params, "sessions-index/");
    },

    onDynamicSessionsIndexFrame(params: CodezAgentWorkspaceTarget) {
    ...
    async subscribeWorkspaceConfigV4(params: CodezAgentWorkspaceConfigSubscribeParams) {
      const client = await getReadOnlyClient(params, params.runtimePolicy);
      const connection = resolveV4Connection(params, v4ConnectionIdFor(params.subscriberScope));
    ...
    async unsubscribeWorkspaceConfigV4(params: CodezAgentConversationUnsubscribeParams) {
      await unsubscribeV4Route(params, "workspace-config/");
    },

    async resyncWorkspaceConfigV4(params: CodezAgentConversationResyncParams) {
      return resyncV4Route(params, "workspace-config/");
    },

    onDynamicWorkspaceConfigFrame(params: CodezAgentWorkspaceTarget) {
    ...
    onAgentRuntimeLifecycle(listener: (event: CodezAgentRuntimeLifecycleEvent) => void) {
