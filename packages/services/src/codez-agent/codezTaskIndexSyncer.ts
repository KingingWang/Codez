...
import { repairSubagentTaskIndex } from "#src/codez-agent/repairSubagentTaskIndex.js";
import {
  deriveCodezTaskStatusFromSessionSnapshot,
  generateTraceId,
  getCodezUserVisibleMessages,
  isCodezGoalContinuationReminderText,
  isCodezModelOnlySyntheticUserMessage,
  resolveWorkspaceKey,
...
import { Emitter, type Event, type IDisposable } from "@codez/rpc";
import { createServiceLogger } from "#src/logger/serviceLogger.js";
import { TaskIndexRepo } from "#src/session/taskIndexRepo.js";
import type { CodezWorkspaceEventSubscriptionParams } from "#src/session/codezTaskListTypes.js";
import type {
  ICodezAgentService,
...
import { formatTaskMetaModelSelectionFromSnapshot } from "./codezConfigOptions.js";

const logger = createServiceLogger("codez-task-index-syncer");
...
export interface CodezTaskIndexReadyEvent {
  target: CodezAgentSessionTarget;
  /** v4 phase 终态映射：agent 收口后可接受下一条输入的 ready 边界。 */
  reason: "prompt_completed" | "prompt_failed";
}

export interface CodezTaskIndexSyncer {
  /**
   ...
   */
  ensureWorkspaceSubscription(target: CodezAgentWorkspaceTarget): void;
  /**
   ...
   * desktop-continuous 路径上，codezSessionService 在 createSession/resumeSession/setModel
  ...
  syncSnapshotAndBroadcast(
    ...
    options: {
      ...
      unreadSignal?: CodezWorkspaceTaskListChanged["unreadSignal"];
      /**
       ...
       */
      broadcastReason: CodezWorkspaceTaskListChanged["reason"];
    },
   ...
   */
  syncTaskModel(target: CodezAgentSessionTarget, model: string): Promise<CodezTaskMeta | null>;
  /**
  ...
  emitWorkspaceTaskListChanged(
    ...
    meta: CodezTaskMeta | undefined,
    reason: CodezWorkspaceTaskListChanged["reason"],
    options?: Pick<CodezWorkspaceTaskListChanged, "unreadSignal">,
  ): void;
  /**
...
): CodezWorkspaceTaskListChanged["unreadSignal"] {
...
function taskStatusFromSummaryPhase(phase: SessionPhase): CodezTaskMeta["status"] {
  switch (phase) {
    case "running":
    case "prewarming":
      return "running";
...
export function createCodezTaskIndexSyncer(
  options: CreateCodezTaskIndexSyncerOptions,
  ...
  const workspaceEmitters = new Map<string, Emitter<CodezWorkspaceEvent>>();
  const terminalEventEmitter = new Emitter<CodezTaskIndexTerminalEvent>();
  const readyEventEmitter = new Emitter<CodezTaskIndexReadyEvent>();
  let disposed = false;
  ...
  const isRuntimeUnavailableError = (error: unknown): boolean =>
    typeof error === "object" &&
    error !== null &&
    (error as { code?: unknown }).code === CODEZ_AGENT_RUNTIME_UNAVAILABLE_CODE;
  ...
  function getWorkspaceEmitter(workspace: WorkspaceEventInput): Emitter<CodezWorkspaceEvent> {
    ...
    if (!emitter) {
      emitter = new Emitter<CodezWorkspaceEvent>();
  ...
  function broadcastTargetFrom(target: CodezAgentSessionTarget): WorkspaceBroadcastTarget {
    return {
      workspacePath: target.workspacePath,
      workspaceIdentity: target.workspaceIdentity,
  ...
  function applyTerminalTransition(
    ...
    void taskIndexRepo
      ...
      .then((meta) => {
        ...
        // 不经过 codezSessionService 的 op 驱动 snapshot 同步，行缺失/正文搜索/lastError
  ...
  function applyTitleChange(target: CodezAgentSessionTarget, title: string): void {
    const updatedAt = Date.now();
    void taskIndexRepo
      .applyAgentPatch({
        workspacePath: target.workspacePath,
      ...
      .then((meta) => {
        if (meta) {
          ...
          // v4 createSession 不经过 codezSessionService.createSession；首个标题到达且
  ...
  function processSummary(
    ...
    if (becameVisibleTask) {
      ...
      // 不经过 codezSessionService.createSession。此处是最早且不依赖标题时序的新任务边界；
  ...
  async function seedDiscoveredTaskRows(
    ...
    // 纯 V4 UI 不经过 codezSessionService.initializeWorkspace；若把首帧
  ...
  function applyWorkspaceConfigFrame(
    ...
    if (!config || config.configOptions.length === 0) {
      // 空目录（无 live session 的订阅种子）不下发：下游 useCodezConfig 收到空
      // configOptions 会把聊天工具栏的模型目录清掉。
      completeConfigRecoveryFrame(state, deliveryKind);
      return;
    }
    // v4 载荷与 CodezConfigOption 结构对齐（shared 黄金测试背书），零映射直通，
    // 下游 workspace_config_options_update 消费面（useCodezConfig 等）不改。
    getWorkspaceEmitter({
      workspacePath: state.target.workspacePath,
      workspaceIdentity: state.target.workspaceIdentity,
    }).fire({
  ...
  function ensureWorkspaceSubscription(target: CodezAgentWorkspaceTarget): void {
    if (disposed) {
      return;
    }
...
function hasUserVisibleContent(snapshot: CodezSessionStateSnapshot): boolean {
  const title = snapshot.session.title?.trim() ?? "";
  if (title && !isCodezGoalContinuationReminderText(title)) {
    return true;
  }
  ...
  return snapshot.messages.some((message) => {
    ...
    if (isCodezModelOnlySyntheticUserMessage(message)) return false;
...
// 与 UI 默认折叠后展示的范围一致。
function buildSearchableTextFromSnapshot(snapshot: CodezSessionStateSnapshot): string {
  const parts: string[] = [];
  let total = 0;
  // /goal 自动续跑会把内部 system-reminder 作为 runtime user turn 持久化；
  // 这条内容是 model-only 输入，不是用户可见 query，不能写进侧边栏搜索正文。
  for (const message of getCodezUserVisibleMessages(snapshot.messages, {
    target: snapshot.projection.target,
  })) {
    const textParts = message.parts.filter(
      (part): part is Extract<CodezMessagePart, { type: "text" }> => part.type === "text",
    );
...
function fromCodezGoal(
  goal: NonNullable<CodezSessionStateSnapshot["projection"]["target"]>,
): CodezTaskGoal {
  return {
