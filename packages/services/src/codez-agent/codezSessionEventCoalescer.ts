...
type SessionServiceEvent = Extract<CodezAgentServiceEvent, { type: "session.event" }>;
...
interface BackgroundSessionEventCoalescer {
  accept(event: CodezAgentServiceEvent): void;
  flush(): void;
  dispose(): void;
}
...
function getBackgroundSessionEventCoalesceKey(event: CodezSessionEvent): string | null {
...
function mergeBackgroundSessionEvents(
  current: CodezSessionEvent,
  next: CodezSessionEvent,
): CodezSessionEvent {
...
export function createBackgroundSessionEventCoalescer(params: {
  emit: (event: CodezAgentServiceEvent) => void;
  flushDelayMs?: number;
  maxItems?: number;
