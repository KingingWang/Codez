...
import { codezProtocolMessageSchema } from "@codez/shared";
import { createServiceLogger } from "#src/logger/serviceLogger.js";
import type {
  CodezProtocolTransport,
  CodezProtocolTransportClosedEvent,
} from "./codezProtocolTransport.js";
import {
...
interface CodezStdioTransportOptions {
...
const processTreeLogger = createServiceLogger("codez-agent-process-tree");

export class CodezStdioTransport implements CodezProtocolTransport {
  readonly kind = "stdio" as const;

  private readonly messageEmitter = new Emitter<CodezProtocolMessage>();
  private readonly closeEmitter = new Emitter<CodezProtocolTransportClosedEvent>();
  ...
  constructor(
    ...
    private readonly options?: CodezStdioTransportOptions,
  ) {
    this.stderrCollector = new AgentStderrCollector(child.stderr, options?.onStderrLine);

    // Codez Protocol stdio 帧边界只认 LF。Node readline 会把 U+2028/U+2029
  ...
  async send(message: CodezProtocolMessage): Promise<void> {
    if (this.disposed || this.closed || this.child.killed || !this.child.stdin.writable) {
      throw new Error("Codez agent stdio transport is closed");
    }
  ...
  private async disposeAndWaitOnce(): Promise<void> {
    ...
    if (!this.hasChildExited()) {
      ...
      const configuredEofWaitMs =
        process.env.CODEZ_E2E_COVERAGE === "1"
  ...
  private handleStdoutFrame(frame: string): void {
    ...
    try {
      const parsed = codezProtocolMessageSchema.parse(JSON.parse(line));
  ...
  private handleStreamError(stream: "stdin" | "stdout", error: Error): void {
    ...
    // codez-server 会因未处理的 EPIPE 直接崩溃，UI 只能看到远端连接断开而不是协议请求失败。
    // 这里把 stream error 视为 transport 已关闭，阻止后续继续向失效 agent 写入。
    this.fireClose({ reason: `${stream}_error: ${error.message}` });
  }

  private fireClose(event: CodezProtocolTransportClosedEvent): void {
    if (this.closed) {
      return;
    }
    this.closed = true;
