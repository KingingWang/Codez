...
// to @codez/codez-cua/src/broker/ports.ts. The descriptor registration itself stays in
// services (host control plane — depends on services' createServiceDescriptor
// + @codez/shared ServiceChannels), so services internal callers (node.ts,
// accessor.ts, services/index.ts) and ui consumers (via @codez/services root
...
// at compile time; Vite never resolves @codez/codez-cua for these).
import type {
  CuaPermissionState,
  CuaPermissionStatus,
...
} from "@codez/codez-cua/broker/ports";
