...
const DEV_ROOT_ENV = "CODEZ_CUA_DEV_ROOT";
const EXPECTED_PACKAGE_NAME = "@codez/codez-cua";
...
async function requireExpectedPackage(
  ...
  try {
    ...
    const contract = pkg.codezCuaRuntime;
    if (
  ...
  throw new WindowsCuaDevRuntimeResolutionError(
    "invalid-package",
    `Windows CUA development root package.json must name ${EXPECTED_PACKAGE_NAME} and expose a valid codezCuaRuntime contract.`,
    PACKAGE_JSON,
  );
}
