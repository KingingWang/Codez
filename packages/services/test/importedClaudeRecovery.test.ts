...
for (const clientMode of ["desktop-continuous", "web-remote-replayable"] as const) {
  test(`previously imported Claude history becomes a real session for ${clientMode}`, async () => {
    const dir = await mkdtemp(join(tmpdir(), "codez-import-recovery-"));
    setDataBaseDir(dir);
    const meta = {
    ...
    type CreateInput = Parameters<Options["codezAgentService"]["createSession"]>[0];
    const created: CreateInput[] = [];
    let session: CodezSessionStateSnapshot | undefined;
    ...
    const service = createCodezTaskServiceAdapter({
      ...
      codezAgentService: {
        ...
        async createSession(input: CreateInput) {
          created.push(input);
          session = codezSessionStateSnapshotSchema.parse({
