...
import { readLegacyCodezConfigProviders } from "../src/model-provider/legacyCodezConfigProviderReader.js";
...
async function setup() {
  const dir = await mkdtemp(join(tmpdir(), "codez-provider-migration-"));
  setDataBaseDir(dir);
  ...
  const runtime = createProviderConfigRuntime({
    ...
    readLegacyProviders: async () => {
      reads += 1;
      return readLegacyCodezConfigProviders();
    },
...
test("startup migrates published Codez config into personal config without changing the source", async () => {
