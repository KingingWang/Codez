...
import { createCodezTaskServiceAdapter } from "../src/codez-agent/codezTaskServiceAdapter.js";
import {
...
test("current Project Memory catalog and files remain readable", async () => {
  const dir = await mkdtemp(join(tmpdir(), "codez-current-memory-"));
  setDataBaseDir(dir);
  try {
...
test("opening the task index leaves retired ACP IDs and user rows untouched", async () => {
  const dir = await mkdtemp(join(tmpdir(), "codez-acp-index-"));
...
test("missing sessions report the owner error even when a valid ACP snapshot exists", async () => {
  const dir = await mkdtemp(join(tmpdir(), "codez-acp-snapshot-"));
  setDataBaseDir(dir);
  ...
  type Options = Parameters<typeof createCodezTaskServiceAdapter>[0];
  const service = createCodezTaskServiceAdapter({
    codezAgentService: {
      async resumeSession() {
        throw ownerError;
      },
      disposeAll() {},
    } as unknown as Options["codezAgentService"],
    taskIndexRepo: {
      async getTaskMeta() {
        return meta;
      },
...
test("current session recovery preserves Desktop and replayable projections", async () => {
  const snapshot = codezSessionStateSnapshotSchema.parse({
    protocol: { name: CODEZ_PROTOCOL_NAME, version: CODEZ_PROTOCOL_VERSION },
    session: {
...
test("current Provider configuration starts without an old config migration callback", async () => {
  const dir = await mkdtemp(join(tmpdir(), "codez-provider-current-"));
  const runtime = createProviderConfigRuntime({
    codezBuiltinFilePath: fileURLToPath(
  ...
  try {
    ...
    assert.ok(config.codezBuiltinRevision);
