import type { HelloMessage, HelloAckMessage } from "@codez/shared";
import { CODEZ_VERSION, formatZodError, helloMessageSchema } from "@codez/shared";
 ...
 * 1. Read lines from stdout until we find a codez-hello JSON
 *    (skip SSH banner/motd lines)
 * 2. Send a codez-hello-ack to stdin
...
export function performHandshake(
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      rejectWithDiagnostics("Handshake timeout: no codez-hello received within timeout");
    }, timeoutMs);
        ...
        if (line.startsWith("{")) {
          try {
            ...
            if (
              ...
              "type" in rawValue &&
              (rawValue as { type?: unknown }).type === "codez-hello"
            ) {
              rejectWithDiagnostics(`Invalid codez-hello: ${formatZodError(result.error)}`);
              return;
            }
          } catch {
