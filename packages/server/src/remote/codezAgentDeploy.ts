...
import type { IRemoteBackend, RemoteEnvironment } from "@codez/server/remote/backend.js";
import {
  REMOTE_BASE,
  type DeployLoggers,
...
import type { RemoteAssetInstaller } from "@codez/server/remote/remoteAssetInstaller.js";
import { buildWriteLiteralFileCommand } from "@codez/server/remote/posixShell.js";
import { deployDevelopmentCodezAgentRuntime } from "@codez/server/remote/codezAgentDevDeploy.js";
import {
...
export interface DeployCodezAgentRuntimeOptions extends RemoteAssetDeployOptions {
  platformArch: string;
...
function isSelectedCodezAgentComponent(
  componentId: string,
...
async function shouldSkipCodezAgentDeploy(params: {
  ...
  params.loggers.log(
    `[codez-agent-deploy] ${CODEZ_AGENT_PROVIDER}: 制品 SHA ${params.expectedArtifactSha256} 已部署，跳过`,
  );
  return true;
}
...
/**
 * 部署 Codez Agent runtime 到远程机器。
 *
 * 生产态只用 manifest SHA 判断制品是否变化；语义版本不参与跳过决策。
 */
export async function deployCodezAgentRuntime(
  backend: IRemoteBackend,
  env: RemoteEnvironment,
  options: DeployCodezAgentRuntimeOptions,
  loggers: DeployLoggers,
): Promise<void> {
  const provider = CODEZ_AGENT_PROVIDER;
  const runtime = CODEZ_AGENT_RUNTIME;
  const componentId = provider;
  if (!isSelectedCodezAgentComponent(componentId, options.selectedResourcePackageIds)) {
    loggers.log(`[codez-agent-deploy] ${provider}: 未选择资源包 ${componentId}，跳过检查和部署`);
    return;
  }

  // binaryName 指 wrapper 可执行文件名（如 codez-agent / codez-agent.exe）——
  // 一个调用远端 node 执行 codez.cjs 的壳脚本。
  const binaryName = runtime.resolveEntrySegments(env.platform).at(-1);
  if (!binaryName) {
    loggers.logWarn(`[codez-agent-deploy] ${provider}: 无法解析 agent 入口名称，跳过部署`);
    return;
  }
  ...
  if (
    await deployDevelopmentCodezAgentRuntime(
      backend,
      {
  ...
  try {
    ...
    loggers.logWarn(
      `[codez-agent-deploy] ${provider}: 读取 manifest SHA 失败，将重新部署: ${String(error)}`,
    );
  }

  if (
    await shouldSkipCodezAgentDeploy({
      backend,
      remoteBinaryPath,
      remoteBundlePath,
      runtimeResourceDir: runtime.bundledResourceDir,
  ...
  loggers.log(`[codez-agent-deploy] ${provider}: 开始部署 v${runtime.version}...`);
  ...
  if (permissionRepairSucceeded) {
    ...
    await installOfficialPluginPackages();
    // 2) packages 替换成功后再安装编译产物 codez.cjs（跨平台同一份，glm 组件里就是它）。
    await installBundle();
  }
  // 3) 写入 wrapper（即 resolver 期望的 codez-agent），用远端已部署的 node 执行 codez.cjs。
  ...
  loggers.log(`[codez-agent-deploy] ${provider}: 部署完成 v${runtime.version}`);
}
