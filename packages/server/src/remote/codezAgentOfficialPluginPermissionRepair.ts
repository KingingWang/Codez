import { CODEZ_AGENT_PROVIDER } from "@codez/shared";
import type { IRemoteBackend } from "@codez/server/remote/backend.js";
import { type DeployLoggers, waitForClose } from "@codez/server/remote/deployShared.js";
...
export async function repairLegacyRemoteOfficialPluginDirectoryPermissions(params: {
  ...
  params.loggers.logWarn(
    `[codez-agent-deploy] ${CODEZ_AGENT_PROVIDER}: 检查并修复旧 builtin plugin 目录权限 ${params.remoteOfficialPluginDir}`,
  );
  ...
  try {
    ...
    params.loggers.logWarn(
      `[codez-agent-deploy] ${CODEZ_AGENT_PROVIDER}: 修复旧 builtin plugin 目录权限失败，将继续尝试替换 packages: ${
