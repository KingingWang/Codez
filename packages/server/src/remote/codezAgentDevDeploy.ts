...
import { CODEZ_AGENT_PROVIDER, resolveCodezRuntimeEnv } from "@codez/shared";
import { createHash, randomUUID } from "node:crypto";
import { existsSync } from "node:fs";
import { cp, lstat, mkdir, mkdtemp, readdir, readFile, readlink, rm } from "node:fs/promises";
import { tmpdir } from "node:os";
import { dirname, join, resolve } from "node:path";
import type { IRemoteBackend } from "@codez/server/remote/backend.js";
import {
...
import {
  ...
  quotePosixPathArg,
} from "@codez/server/remote/posixShell.js";
import { createTarGzArchive } from "@codez/server/remote/localTarGz.js";
import {
...
} from "@codez/server/remote/codezAgentOfficialPluginAssets.js";
import { repairLegacyRemoteOfficialPluginDirectoryPermissions } from "@codez/server/remote/codezAgentOfficialPluginPermissionRepair.js";

const DEV_AGENT_BUNDLE_RELATIVE_PATH = "apps/codez-cli/packages/cli/dist/codez.cjs";
const DEV_AGENT_BUNDLE_ENV = "CODEZ_REMOTE_DEV_AGENT_BUNDLE";
const REMOTE_DEV_AGENT_BUNDLE_NAME = REMOTE_AGENT_BUNDLE_NAME;
const REMOTE_DEV_AGENT_VERSION_FILE_NAME = ".dev-version";

interface DeployDevelopmentCodezAgentRuntimeParams {
...
function shouldUseDevelopmentAgentBundle(): boolean {
  if (resolveCodezRuntimeEnv(process.env) !== "development") {
    return false;
  }
...
async function hashDevelopmentOfficialPluginPackage(
  ...
  const manifestPath = join(packageRoot, ".codez-plugin", "plugin.json");
  if (!existsSync(manifestPath)) {
    throw new Error(`[codez-agent-deploy] missing official plugin manifest: ${manifestPath}`);
  }
...
function assertDevelopmentOfficialPluginRequiredAssets(
  ...
  for (const requiredRelativePath of REMOTE_AGENT_OFFICIAL_PLUGIN_REQUIRED_RELATIVE_PATHS) {
    ...
    if (!existsSync(requiredPath)) {
      throw new Error(
        `[codez-agent-deploy] missing official plugin required asset: ${requiredPath}`,
      );
    }
  }
}
...
async function shouldSkipDevelopmentCodezAgentDeploy(params: {
  backend: IRemoteBackend;
  remoteVersionFile: string;
  ...
  params.loggers.log(`[codez-agent-deploy] ${CODEZ_AGENT_PROVIDER}: 开发态 codez.cjs 未变化，跳过`);
  return true;
}
...
async function stageDevelopmentOfficialPluginPackages(params: {
  ...
  for (const packageName of REMOTE_AGENT_OFFICIAL_PLUGIN_PACKAGE_NAMES) {
    const sourceRoot = join(params.repoRoot, "apps", "codez-cli", "packages", packageName);
...
async function uploadDevelopmentOfficialPluginPackages(params: {
  ...
  const tempDir = await mkdtemp(join(tmpdir(), "codez-remote-agent-packages-"));
    ...
    const cleanupRemoteStaging = async (): Promise<void> => {
      try {
        ...
        params.loggers.logWarn(
          `[codez-agent-deploy] ${CODEZ_AGENT_PROVIDER}: 清理 owner staging 失败 (${ownerSuffix}): ${String(error)}`,
        );
      }
    };
    params.loggers.log(`[codez-agent-deploy] ${CODEZ_AGENT_PROVIDER}: 开发态上传官方插件资源`);
    try {
...
export async function deployDevelopmentCodezAgentRuntime(
  backend: IRemoteBackend,
  params: DeployDevelopmentCodezAgentRuntimeParams,
  loggers: DeployLoggers,
): Promise<boolean> {
  ...
  if (
    await shouldSkipDevelopmentCodezAgentDeploy({
      backend,
  ...
  // 开发态 SSH 远端过去只会部署本地 codez.cjs，不会携带 packages/*-plugin。
  // builtin plugin seed 依赖 agent 包旁边的官方插件源资源，所以 dev 部署需要同步 bundle 与插件资源。
  // 本地修改 apps/codez-cli 后，远端测试仍运行滞后的发布包。这里改为上传 dev 启动时刚构建的
  // dist/codez.cjs，并用远端已部署的 node 包一层 wrapper 启动，保证 agent 仍运行在目标机器内。
  loggers.log(
    `[codez-agent-deploy] ${CODEZ_AGENT_PROVIDER}: 开发态上传本地 codez.cjs ${devVersion.slice(0, 12)}`,
  );
  ...
  loggers.log(
    `[codez-agent-deploy] ${CODEZ_AGENT_PROVIDER}: 开发态部署完成 ${devVersion.slice(0, 12)}`,
  );
  return true;
}
