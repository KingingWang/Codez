import { SocketProtocol, ChannelClient } from "@codez/rpc";
import type { IServiceAccessor } from "@codez/services";
import { RemoteServiceAccess } from "@codez/client";
import {
...
/**
 * Connect to a remote codez server via an IRemoteBackend.
 *
 * Steps:
...
async function connectRemoteUnchecked(
  ...
  stream.stderr.on("data", (chunk: Buffer) => {
    // 远端 codez-server 的服务日志走 stderr，直接写 host stderr 时可能被结构化日志中继吞掉。
...
export function buildRemoteServerCommand(
  ...
  const envParts = [
    ...
    `CODEZ_SERVER_RUNTIME_ROOT=${quotePosixPathArg(REMOTE_RUNTIME.root)}`,
  ];
  if (REMOTE_RUNTIME.kind === "codex") {
    envParts.push(
      "CODEZ_DESKTOP_RUNTIME='codex'",
      `CODEZ_DATA_BASE_DIR=${quotePosixPathArg("~/.codez-codex")}`,
      `CODEZ_HOME=${quotePosixPathArg("~/.codez-codex/.codez")}`,
      `CODEZ_CODEX_COMMAND=${quotePosixPathArg(`${REMOTE_RUNTIME.root}/codex/codex`)}`,
      `CODEZ_CODEX_BRIDGE_PATH=${quotePosixPathArg(`${REMOTE_RUNTIME.root}/codex/bridge.cjs`)}`,
      `CODEZ_CODEX_BRIDGE_HOME=${quotePosixPathArg("~/.codez-codex/bridge")}`,
    );
  }
  ...
  if (appVersion) {
    ...
    envParts.push(`${CODEZ_APP_VERSION_ENV}=${quotePosixShellArg(appVersion)}`);
  }
  if (remoteRuntimeNetwork?.authoritative) {
    envParts.push(`${CODEZ_REMOTE_RUNTIME_NETWORK_AUTHORITY_ENV_KEY}='1'`);
    if (remoteRuntimeNetwork.httpProxy !== undefined) {
      envParts.push(
        `${CODEZ_REMOTE_HTTP_PROXY_ENV_KEY}=${quotePosixShellArg(remoteRuntimeNetwork.httpProxy)}`,
      );
    }
    if (remoteRuntimeNetwork.noProxy !== undefined) {
      envParts.push(
        `${CODEZ_REMOTE_NO_PROXY_ENV_KEY}=${quotePosixShellArg(remoteRuntimeNetwork.noProxy)}`,
      );
    }
  }
  return `${envParts.join(" ")} ${quotePosixPathArg(`${REMOTE_RUNTIME.root}/node`)} ${quotePosixPathArg(`${REMOTE_RUNTIME.root}/codez-server.cjs`)}`;
}
