...
import type { RemoteAssetInstallMode } from "@codez/shared";
...
import type {
  ...
  RemoteDownloadTool,
  RemoteSha256Tool,
} from "@codez/server/remote/remoteAssetPreflight.js";
import {
...
function buildStaleRemoteStagingCleanupCommand(parentDir: string, patterns: string[]): string {
  ...
  return [
    ...
    // 新连接只回收超过 24 小时的 Codez owner staging，避免误删当前 owner 或正常时长内的活跃部署。
...
export class LocalUploadAssetInstaller implements RemoteAssetInstaller {
  ...
  async installDirectory(params: {
    ...
    const localTarPath = join(
      tmpdir(),
      `codez-remote-${params.componentId}-${Date.now()}-${randomUUID()}.tar.gz`,
    );
