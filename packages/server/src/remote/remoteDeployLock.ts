...
import { REMOTE_BASE } from "@codez/server/remote/deployShared.js";
...
function buildRemoteDeployLockScript(lockDir: string, ownerToken: string): string {
  const acquiredMarker = `codez-deploy-lock-acquired:${ownerToken}`;
  const releaseMarker = `codez-deploy-lock-release:${ownerToken}`;
  return [
    "set -eu",
