...
import {
  ...
  CODEZ_RPC_HOST_CAPABILITY_HEADER,
  CODEZ_VERSION,
...
const log = (...args: unknown[]) =>
  console.log(formatLogPrefix("codez-server:http", process.pid), ...args);
...
function resolveServerId(options: HttpServerOptions): string {
  return (
    options.serverId?.trim() || readTrimmedEnv("CODEZ_SERVER_ID") || hostname() || "codez-server"
  );
}
...
function resolveServerWorkspaces(options: HttpServerOptions): ServerRemoteWorkspaceInfo[] {
  ...
  const workspacePath = readTrimmedEnv("CODEZ_SERVER_WORKSPACE") || process.cwd();
  return [
    {
      path: workspacePath,
...
function createServerInfo(options: HttpServerOptions): ServerRemoteInfo {
  return {
    ...
    ...(options.name?.trim() || readTrimmedEnv("CODEZ_SERVER_NAME")
      ? { name: options.name?.trim() || readTrimmedEnv("CODEZ_SERVER_NAME") }
      : {}),
    version: CODEZ_VERSION,
    protocolVersion: SERVER_REMOTE_PROTOCOL_VERSION,
    authRequired: options.authRequired ?? Boolean(readTrimmedEnv("CODEZ_SERVER_TOKEN")),
...
const codezLiteTokenCookieName = "codez_lite_token";
...
function hasValidLiteToken(c: Context, token: string): boolean {
  ...
  if (url.searchParams.get("token") === token) {
    c.header(
      "Set-Cookie",
      `${codezLiteTokenCookieName}=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Lax`,
    );
    return true;
  }
  return parseCookieHeader(c.req.header("cookie")).get(codezLiteTokenCookieName) === token;
}
...
export function createHttpServer(
  ...
  app.use("/ws/host", async (c, next) => {
    const capability = c.req.header(CODEZ_RPC_HOST_CAPABILITY_HEADER);
