...
type AgentIntl = ReturnType<typeof useCodezIntl>["intl"];
...
export function getAgentKindLabel(
  ...
  const rawName =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "codez", "agentType"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "codez", "agent_type"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "codez", "subagent_type"]);
...
export function getAgentColor(toolCall: AgentToolCall) {
  ...
  const inputRecord = isPlainRecord(toolCall.input) ? toolCall.input : null;
  const rawColor =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "codez", "color"]) ??
...
export function readBackgroundAgentInfo(toolCall: AgentToolCall) {
  ...
  const codez = meta && isPlainRecord(meta.codez) ? meta.codez : null;
  const codezBackgroundAgent =
    codez && isPlainRecord(codez.backgroundAgent) ? codez.backgroundAgent : null;
  const taskNotification =
    codez && isPlainRecord(codez.taskNotification) ? codez.taskNotification : null;
  const backgroundAgent = codezBackgroundAgent;
...
export function getAgentActivityContent(toolCall: AgentToolCall) {
  const taskNotificationResult =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "codez", "taskNotification", "result"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "codez", "taskNotification", "summary"]);
