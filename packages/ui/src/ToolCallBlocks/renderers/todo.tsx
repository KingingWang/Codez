...
import { extractPlanStepsFromToolInput, extractPlanStepsFromToolOutput } from "@codez/shared";
import type { CodezPlanStep } from "@codez/shared";
...
const todoTextClasses: Record<CodezPlanStep["status"], string> = {
  pending: "text-foreground-subtle",
  in_progress: "text-foreground",
  completed: "text-foreground-subtlest line-through",
};

function readTodoPlan(context: ToolCallBlockRenderContext): CodezPlanStep[] | null {
...
function TodoStatusIcon({ status }: { status: CodezPlanStep["status"] }) {
  if (status === "completed") {
