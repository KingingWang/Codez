...
import type { Locale, LocalePreference } from "@codez/shared";
import { DEFAULT_LOCALE } from "@codez/shared";
import type { BroadcastMessage, IBroadcastService, ISettingService } from "@codez/services";
import {
...
const LOCALE_PREFERENCE_KEY = "codez-locale-preference";
 ...
 */
export function CodezIntlProvider({
  children,
  settingService,
  broadcastService,
  initialLocale,
...
/** 获取 intl 上下文 */
export function useCodezIntl(): IntlContextValue {
  const ctx = useContext(IntlContext);
  if (!ctx) {
    throw new Error("useCodezIntl 必须在 CodezIntlProvider 内使用");
  }
  return ctx;
}
