...
export function useWorkspaceTaskNavigation({
  ...
  const setActiveTaskId = useCodezSessionStore((s) => s.setActiveTaskId);
  const taskNavHistory = useCodezSessionStore((s) => s.taskNavHistory);
  const taskNavPushAutomations = useCodezSessionStore((s) => s.taskNavPushAutomations);
  const taskNavPushPluginStore = useCodezSessionStore((s) => s.taskNavPushPluginStore);
  const taskNavGoBack = useCodezSessionStore((s) => s.taskNavGoBack);
  const taskNavGoForward = useCodezSessionStore((s) => s.taskNavGoForward);
  const removeTaskFromNavHistory = useCodezSessionStore((s) => s.removeTaskFromNavHistory);
  ...
  const handleSelectTask = useCallback(
    (
      ...
      const targetWorkspaceState = useCodezSessionStore
        .getState()
      ...
      if (shouldClearUnread) {
        ...
        if (!targetServices) {
          ...
          void targetServices.codezTaskService
            .setTaskUnread({
              ...targetTask,
              unread: false,
              expectedUnreadAt,
  ...
  const handleTaskNavBack = useCallback(() => {
    const currentWorkspaceState = useCodezSessionStore
      .getState()
    ...
    while (entry) {
      ...
      const navWorkspaceState = useCodezSessionStore
        .getState()
  ...
  const currentWorkspaceState = useCodezSessionStore.getState().getWorkspaceState(workspaceAbsPath);
