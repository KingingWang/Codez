import type { CodezGroupedTaskViewNode, CodezTaskGroupColor } from "@codez/services";
...
import { CRON_DEFAULT_GROUP_ID, OFF_PEAK_DEFAULT_GROUP_ID } from "@codez/shared";
...
type StickyGroupNode = Extract<CodezGroupedTaskViewNode, { type: "group" }>;
...
export function StickyGroupHeader({
  ...
  onUpdateGroupColor: (groupId: string, color: CodezTaskGroupColor) => void;
  ...
  const handleGroupColorChange = (color: string) => {
    onUpdateGroupColor(node.group.id, color as CodezTaskGroupColor);
  };
  const newTaskButton = (
    <Button
      type="button"
