...
import type { CodezGroupedTaskViewNode } from "@codez/services";
...
function getTopLevelNodeKey(node: CodezGroupedTaskViewNode | undefined, index: number): string {
  if (!node) {
...
function estimateTopLevelNodeSize(
  node: CodezGroupedTaskViewNode | undefined,
...
function VirtualizedGroupedTopLevelList({
  ...
  isGroupCollapsed: (groupId: string) => boolean;
  renderNode: (node: CodezGroupedTaskViewNode, index: number) => ReactNode;
}) {
