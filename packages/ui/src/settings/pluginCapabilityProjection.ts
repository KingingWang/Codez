...
import { compareDocumentPluginPriority, isPluginCommand, isUserCommand } from "@codez/shared";
...
export function selectBuiltInPlugins(
  ...
  installedPlugins: readonly CodezInstalledPluginSummary[],
): CodezPluginInfo[] {
...
interface PluginSettingsGroups {
  installed: CodezPluginInfo[];
  builtIn: CodezPluginInfo[];
}
...
export function selectPluginsForScope(
  plugins: readonly CodezPluginInfo[],
  _installedPlugins: readonly CodezInstalledPluginSummary[],
  _scope: CodezPluginScope,
): CodezPluginInfo[] {
...
export function selectSkillsForScope(
  ...
  scopedPlugins: readonly Pick<CodezPluginInfo, "id" | "name" | "enabled">[],
...
export function selectCommandsForScope(
  commands: readonly CodezCommand[],
  scopedPlugins: readonly Pick<CodezPluginInfo, "id" | "name" | "enabled">[],
  scope: CodezPluginScope,
): CodezCommand[] {
  const enabledPlugins = scopedPlugins.filter((plugin) => plugin.enabled);
