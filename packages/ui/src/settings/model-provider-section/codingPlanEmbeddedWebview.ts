...
import type { CodingPlanWebviewLocale } from "@codez/shared";
...
const CODING_PLAN_REPORT_CONTEXT_STORAGE_KEY = "codez:coding-plan:report-context";
...
export function resolveCodingPlanEmbeddedOrigin({
  ...
  if (
    ...
    return normalizeCodezEndpointOrigin(normalizedOverride);
  }
  const normalizedEndpointOrigin = normalizeCodezEndpointOrigin(endpointOrigin);
...
export function buildCodingPlanEmbeddedWebviewUrl({
  ...
  const url = new URL("/coding-plan", normalizeCodezEndpointOrigin(origin));
...
export function createCodingPlanAuthInjectionScript({
  ...
  // App 当前 locale，写入 window.__codezLang__ 供 codezBridge.getLang() 读取，
  ...
  const values: Record<string, string | null> =
    provider === "zai"
      ? {
          ...
          codezjwttoken: credentials.codezJwtToken?.trim() || null,
          "oauth:bigmodel:access_token": null,
        }
      : {
          "oauth:zai:access_token": null,
          // codezjwttoken 是 codez-plan 域通用凭证（BigModel OAuth callback 同样落盘），
  ...
  return `(() => {
  ${storageUpdates}
  const codezTheme = ${JSON.stringify(theme)};
  document.documentElement.classList.toggle("dark", codezTheme === "zai-dark");
  document.documentElement.classList.toggle("theme-zai-light", codezTheme === "zai-light");
  document.documentElement.classList.toggle("theme-zai-dark", codezTheme === "zai-dark");
  localStorage.setItem("codez-theme", codezTheme);
  localStorage.setItem("codez:coding-plan:embedded", "app");
  // 写入当前 App locale，供官网 codezBridge.getLang() 读取。
  // 注意：这是注入 webview 执行的原始 JS，不能用 TS 语法（如 as any）。
  window.__codezLang__ = ${JSON.stringify(resolvedLocale)};
  const codezReportContext = ${JSON.stringify(normalizedReportContext)};
  window.__codezReportContext__ = codezReportContext;
  localStorage.setItem(${JSON.stringify(CODING_PLAN_REPORT_CONTEXT_STORAGE_KEY)}, JSON.stringify(codezReportContext));
  window.dispatchEvent(new CustomEvent("codez-coding-plan-auth-ready", {
    detail: { ...${JSON.stringify({ provider, locale: resolvedLocale })}, reportContext: codezReportContext },
  }));
})()`;
}
...
export function createCodingPlanCredentialClearScript(): string {
  ...
  delete window.__codezReportContext__;
})()`;
}

export function createCodingPlanScrollbarHideScript(): string {
  return `(() => {
  const styleId = "codez-coding-plan-hide-scrollbar";
 ...
 * 重写 window.__codezLang__ 并派发 codez-coding-plan-lang-change 事件，
 * 官网侧（codezBridge.onLangChange 或 window 监听）据此无感切换语言。
 */
export function createCodingPlanLangInjectionScript(locale: CodingPlanWebviewLocale): string {
  ...
  window.__codezLang__ = ${JSON.stringify(resolvedLocale)};
  window.dispatchEvent(new CustomEvent("codez-coding-plan-lang-change", {
...
export function getCodingPlanCredentialKeys(provider: CodingPlanWebsiteProvider): string[] {
  // codezjwttoken 对两个 provider 都加载：它是 codez-plan 域通用凭证，
  // BigModel OAuth callback 同样落盘（见 resolveBigModelStartPlanZcodeJwt）。
  return provider === "zai"
    ? [`oauth:${ZAI_PROVIDER_ID}:access_token`, "codezjwttoken"]
    : [`oauth:${BIGMODEL_PROVIDER_ID}:access_token`, "codezjwttoken"];
}
