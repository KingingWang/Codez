import type {
  ...
  CodezPluginInfo,
  CodezPluginComponentKind,
  CodezPluginsDescribeResult,
} from "@codez/shared";
import { isPluginCommand, isUserCommand, CODEZ_COMMAND_AGENT_SOURCE } from "@codez/shared";
...
const COMPONENT_KIND_ORDER: CodezPluginComponentKind[] = [
  "agent",
  "command",
  "skill",
  "hook",
...
export function describeResultToDisplayGroups(
  result: CodezPluginsDescribeResult,
): PluginComponentDisplayGroup[] {
  const byKind = new Map<CodezPluginComponentKind, PluginComponentDisplayGroup>();
...
export interface PluginMcpServerItem {
  active: boolean;
  authorization?: CodezMcpServerStatusSnapshot["authorization"];
  error?: string;
  failureKind?: CodezMcpServerStatusSnapshot["failureKind"];
  id: string;
...
export function groupCommandsByPlugin(
  ...
  for (const command of commands) {
    if (
      ...
      command.agentSource === CODEZ_COMMAND_AGENT_SOURCE &&
...
export function filterLocalMcpServers(servers: CodezMcpServer[], query: string): CodezMcpServer[] {
  return servers.filter((server) => {
    if (server.source !== "codezagentmcp") {
      return false;
    }
...
function toPluginMcpServerDisplayName(plugin: CodezPluginInfo, serverName: string): string {
...
function resolvePluginMcpRuntimeServerName(plugin: CodezPluginInfo, displayName: string): string {
...
function mapPluginRuntimeStatus(
  snapshot: CodezMcpServerStatusSnapshot | undefined,
): Pick<
  PluginMcpServerItem,
