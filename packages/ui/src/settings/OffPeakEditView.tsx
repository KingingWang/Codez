...
interface OffPeakEditViewProps {
  ...
  onDeleteHistory?: (task: CodezOffPeakTask) => void;
...
export function OffPeakEditView({
  ...
  const thoughtLevelOption = useMemo<CodezConfigOption | null>(
    () =>
    ...
    >
      ...
      {tab === "history" ? (
        <OffPeakHistoryTab
          ...
          {...(onOpenSession
            ? {
                onOpenSession: (task: CodezOffPeakTask) =>
                  task.sessionId
                    ? onOpenSession({
