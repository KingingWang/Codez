...
import { RENDERER_CODEZ_ENDPOINT_URLS } from "@/lib/rendererCodezEndpoint.js";
...
interface CodingPlanEmbeddedWebviewDialogProps {
   ...
   * 官网页购买成功后的回调。
   *
   * 官网页通过 preload 注入的 window.codezBridge.notifyPurchaseComplete({ provider })
   * 发送 codez:coding-plan-purchase-complete 频道消息，本组件在 webview 的
...
export function CodingPlanEmbeddedWebviewDialog({
  ...
  const theme = useCodezStoreWithDefault((state) => state.theme, "zai-dark");
  const userId = useCodezStoreWithDefault((state) => state.user?.id ?? null, null);
  ...
  const computedWebviewUrl = useMemo(() => {
    ...
    const origin = resolveCodingPlanEmbeddedOrigin({
      endpointOrigin: RENDERER_CODEZ_ENDPOINT_URLS.origin,
      e2eStoreBridgeEnabled: env.VITE_CODEZ_E2E_STORE_BRIDGE === "1",
        ...
        const credentials: CodingPlanEmbeddedCredentials =
          provider === "zai"
            ? {
                ...
                codezJwtToken: values[1],
              }
            : {
                bigmodelAccessToken: values[0],
                // BigModel OAuth callback 同样会落盘 codez JWT；官网用它在
                // codez-plan 域查 billing/balance 判定 Start Plan 状态。
                codezJwtToken: values[1],
              };
  ...
  const handleWebviewRef = useCallback(
    (element: ElectronWebviewTag | null) => {
      ...
      // ipc-message: 官网页通过 preload 的 window.codezBridge.notifyPurchaseComplete
      // 发回购买完成信号（codez:coding-plan-purchase-complete）。
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        >
          ...
          <webview
            ...
            partition="persist:codez-coding-plan"
            src={webviewUrl}
            className={cn(
