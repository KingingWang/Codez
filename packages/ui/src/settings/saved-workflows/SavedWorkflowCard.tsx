...
function SavedWorkflowLastRunBadge({
  ...
  run: CodezSavedWorkflowRun | undefined;
  now: number;
}) {
...
interface SavedWorkflowCardProps {
  ...
  lastRun: CodezSavedWorkflowRun | undefined;
  now: number;
  busy?: boolean;
  onOpen: (entry: CodezSavedWorkflowEntry) => void;
  onRun: (entry: CodezSavedWorkflowEntry) => void;
  onRevise: (entry: CodezSavedWorkflowEntry) => void;
  onCopyPath: (entry: CodezSavedWorkflowEntry) => void;
  /** 作用域动作：项目档「提升为全局」（AI 概括）/ 全局档「移到项目…」；仅在传入时出现。 */
  onMove?: (entry: CodezSavedWorkflowEntry) => void;
  onDelete: (entry: CodezSavedWorkflowEntry) => void;
}
