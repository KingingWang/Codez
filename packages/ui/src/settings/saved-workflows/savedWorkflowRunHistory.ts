...
import type { CodezSavedWorkflowRun, CodezSavedWorkflowRunStatus } from "@codez/shared";
...
): Map<string, CodezSavedWorkflowRun> {
  const byName = new Map<string, CodezSavedWorkflowRun>();
...
export function savedWorkflowRunBadgeKind(
  status: CodezSavedWorkflowRunStatus | "errored" | "stopped" | "failed" | "cancelled" | undefined,
...
export function savedWorkflowRunDurationMs(run: CodezSavedWorkflowRun, now: number): number {
