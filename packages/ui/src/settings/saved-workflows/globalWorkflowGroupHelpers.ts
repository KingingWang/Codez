...
// 也便于单测这些无 React 的判定。
import { resolveWorkspaceKey, type CodezSavedWorkflowRun } from "@codez/shared";
import type { CodezAgentSavedWorkflowTarget } from "@codez/services";
...
export const GLOBAL_SAVED_WORKFLOW_TARGET: CodezAgentSavedWorkflowTarget = { scope: "global" };
...
export function buildGlobalRunProjectResolver(
  localProjects: readonly AutomationWorkspaceOption[],
): (run: CodezSavedWorkflowRun) => SavedWorkflowRunProject | null {
  return (run) => {
