...
// max-lines 400；载体是 `useServices().codezAgentService`，RPC 一律带 `{ scope: "global" }`。
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import type { CodezSavedWorkflowEntry, CodezSavedWorkflowRun } from "@codez/shared";
...
export function useSavedWorkflowGlobalGroup({
  ...
  const { codezAgentService: agentService, fileWatcherService } = useServices();
  ...
  const [launchEntry, setLaunchEntry] = useState<CodezSavedWorkflowEntry | null>(null);
  const [moveEntry, setMoveEntry] = useState<CodezSavedWorkflowEntry | null>(null);
  ...
  // 目录监听：list 回的绝对目录（`~/.codez/workflows`）本机可直接 watch。
  ...
  const handleDelete = useCallback(
    async (entry: CodezSavedWorkflowEntry) => {
  ...
  const handleCardDelete = useCallback(
    (entry: CodezSavedWorkflowEntry) => void handleDelete(entry),
    [handleDelete],
  );
  const handleCardMove = useCallback((entry: CodezSavedWorkflowEntry) => setMoveEntry(entry), []);
  const handleOpen = useCallback(
    (entry: CodezSavedWorkflowEntry) => onOpenDetail(entry.name),
    [onOpenDetail],
  );
  // 全局档的目标项目按 `run.cwd` 反查已打开项目；没打开就两个入口都关掉
  // （实例必须开在发起它的项目）。门与实参构造与项目档共用（产物不需要 toolCallId）。
  const resolveRunTarget = useCallback(
    (run: CodezSavedWorkflowRun) => findProjectByCwd(localProjects, run.cwd) ?? null,
    [localProjects],
  );
