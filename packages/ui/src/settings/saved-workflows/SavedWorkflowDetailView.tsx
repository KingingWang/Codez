...
import {
  ...
  type CodezSavedWorkflowEntry,
  type CodezSavedWorkflowMeta,
  type CodezSavedWorkflowRun,
  type CodezWorkflowsGetResult,
} from "@codez/shared";
import type { ICodezAgentService, CodezAgentSavedWorkflowTarget } from "@codez/services";
...
type DetailLoad =
  | { status: "loading" }
  | { status: "ready"; detail: Extract<CodezWorkflowsGetResult, { ok: true }> }
  | { status: "failed"; reason: string };

type MetaDraft = SavedWorkflowMetaDraft;

function draftFromMeta(meta: CodezSavedWorkflowMeta): MetaDraft {
  return {
...
interface SavedWorkflowDetailViewProps {
  ...
  target: CodezAgentSavedWorkflowTarget;
  agentService: ICodezAgentService;
  name: string;
  /** 面包屑「自动化 › <项目名> › <工作流名>」里的项目一级。 */
  projectLabel: string;
  entry: CodezSavedWorkflowEntry | undefined;
  runs: readonly CodezSavedWorkflowRun[];
  now: number;
  busy: boolean;
  canOpenRun: boolean;
  onBack: () => void;
  ...
  onMove?: () => void;
  onDelete: () => void;
  onOpenRun: (run: CodezSavedWorkflowRun) => void;
  /** 产物 chip → `workflow-artifact` tab；缺席即 chips 只读。 */
  onOpenArtifact?: (run: CodezSavedWorkflowRun, artifactId: string) => void;
  onMetaSaved: () => void;
  /** 传入即在运行历史每行渲染项目列（全局工作流跨项目历史）。 */
  resolveRunProject?: (run: CodezSavedWorkflowRun) => SavedWorkflowRunProject | null;
...
export function SavedWorkflowDetailView({
  ...
  const save = useCallback(async () => {
    ...
    const meta: CodezSavedWorkflowMeta = {
      description,
