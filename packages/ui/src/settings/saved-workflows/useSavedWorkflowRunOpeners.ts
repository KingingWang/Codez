...
import type { CodezSavedWorkflowRun } from "@codez/shared";
import type {
...
export function useSavedWorkflowRunOpeners(options: {
  ...
  resolveTarget: (run: CodezSavedWorkflowRun) => SavedWorkflowRunOpenTarget | null;
  ...
  handleOpenRun: (run: CodezSavedWorkflowRun, workflowName: string) => void;
  handleOpenArtifact: (run: CodezSavedWorkflowRun, artifactId: string) => void;
} {
  const { onOpenWorkflowArtifact, onOpenWorkflowRun, resolveTarget } = options;

  const handleOpenRun = useCallback(
    (run: CodezSavedWorkflowRun, workflowName: string) => {
  ...
  const handleOpenArtifact = useCallback(
    (run: CodezSavedWorkflowRun, artifactId: string) => {
      if (!run.parentSessionId) return;
      const target = resolveTarget(run);
      if (!target) return;
      // 行上的 chip 载荷（`CodezSavedWorkflowRun.artifacts`）带最新版的 `contentType`：
