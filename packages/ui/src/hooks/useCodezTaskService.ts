...
import type { CodezTaskSnapshot } from "@codez/shared";
import { uiMemoryDiagnosticsRegistry } from "@/lib/memoryDiagnostics.js";

type GetTaskSnapshotParams = Parameters<ICodezTaskService["getTaskSnapshot"]>[0];
type GetTaskSnapshotResult = Promise<CodezTaskSnapshot | null>;
type GetTaskSnapshotWithEtagParams = Parameters<ICodezTaskService["getTaskSnapshotWithEtag"]>[0];

const codezTaskServiceProxyCache = new WeakMap<ICodezTaskService, ICodezTaskService>();
...
const snapshotCacheByService = new WeakMap<
  ICodezTaskService,
  Map<string, { etag: string; snapshot: CodezTaskSnapshot }>
>();
const SNAPSHOT_CACHE_STORAGE_KEY = "codez-task-snapshot-cache:v1";
...
let latestSnapshotCache: Map<string, { etag: string; snapshot: CodezTaskSnapshot }> | undefined;
...
function getOrCreateSnapshotCacheMap(
  ...
  const created = new Map<string, { etag: string; snapshot: CodezTaskSnapshot }>();
...
function writePersistedSnapshotEntry(key: string, etag: string, snapshot: CodezTaskSnapshot): void {
  ensurePersistedSnapshotCacheLoaded();
...
function createCodezTaskServiceProxy(service: ICodezTaskService): ICodezTaskService {
...
/** 获取 Codez task wrapper 服务实例 */
export function useCodezTaskService(
  workspacePath?: string,
  preferredRemoteSessionId?: string | null,
  workspaceIdentity?: string | null,
): ICodezTaskService {
  // Codez task 服务按 workspace 身份解析，保证所有 task RPC 都落到对应的 host。
  const services = workspacePath
    ? useWorkspaceServices(workspacePath, preferredRemoteSessionId, workspaceIdentity)
    : useServices();
  const rawService = services.codezTaskService;
  if (!rawService || typeof rawService !== "object") {
    return rawService;
  }
  const cachedProxy = codezTaskServiceProxyCache.get(rawService);
  if (cachedProxy) {
    return cachedProxy;
  }
  const nextProxy = createCodezTaskServiceProxy(rawService);
  codezTaskServiceProxyCache.set(rawService, nextProxy);
  return nextProxy;
}
