...
import type { CodezOffPeakTaskStatus } from "@codez/shared";
import type { IOffPeakTaskService } from "@codez/services";
...
function notifiableStatus(status: CodezOffPeakTaskStatus): "completed" | "failed" | null {
...
export function useOffPeakTaskNotifications(params: {
  ...
  const seenStatusRef = useRef(new Map<string, CodezOffPeakTaskStatus>());
