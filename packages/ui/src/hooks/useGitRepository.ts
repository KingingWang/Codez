...
function createLastTurnChange(
  workspacePath: string,
  snapshotByPath: Map<string, CodezPersistedFileSnapshot>,
  file: CodezTaskChangeSummary["files"][number],
): GitPaneFileChange {
...
function buildLastTurnDataset(options: {
  workspacePath: string;
  turnIndex: number | null;
  fileChange: CodezPersistedFileChange | null;
  summary: CodezTaskChangeSummary | null;
}): GitPaneDataset {
  const snapshotByPath = new Map<string, CodezPersistedFileSnapshot>(
  ...
  return {
    ...
    // 关键业务逻辑：上一轮更改继续优先复用 Codez Agent 已持久化的单轮文件快照，
