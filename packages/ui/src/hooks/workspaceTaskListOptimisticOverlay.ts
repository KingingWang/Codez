...
export function mergeWorkspaceTaskListItemsWithOptimistic(params: {
  items: readonly CodezTaskMeta[];
  optimisticTasks: readonly CodezTaskMeta[];
  activeTaskId: string | null;
  ...
  const sortedItems = [...taskByKey.values()].sort((left, right) =>
    compareCodezTaskListItems(left, right, params.sortBy),
  );
...
export function useWorkspaceTaskOptimisticOverlayByWorkspaceKey(
  ...
  const optimisticTaskListSignature = useCodezSessionStore((state) =>
    JSON.stringify(
  ...
  return useMemo(() => {
    const state = useCodezSessionStore.getState();
