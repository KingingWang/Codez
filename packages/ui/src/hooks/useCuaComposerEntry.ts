...
import { isRemoteWorkspaceIdentity, CODEZ_CUA_OFFICIAL_PLUGIN_ID } from "@codez/shared";
...
import { getVisibleTaskMetas, getWorkspaceState } from "@/store/codezSessionStoreSelectors.js";
import { useOptionalTabStore } from "@/store/TabStoreProvider.js";

/** 与 codezSessionStoreTaskSlice 的 isRunningStatus 同口径。 */
...
export function useCuaComposerEntry({
  ...
  // 失败都写）。只有失败操作的目标是 codez-cua 时才映射为本按钮的错误态，
  ...
  const workspaceSessionBusy = useCodezSessionStore((state) => {
  ...
  const view = useMemo(
    () =>
      resolveCuaComposerEntryView({
        ...
        pluginEnabled,
        pluginToggling: togglingPluginId === CODEZ_CUA_OFFICIAL_PLUGIN_ID,
        pluginError:
          Boolean(pluginStoreError) && lastFailedPluginId === CODEZ_CUA_OFFICIAL_PLUGIN_ID,
