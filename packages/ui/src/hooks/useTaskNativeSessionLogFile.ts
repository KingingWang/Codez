...
function supportsTaskNativeSessionLogFile(_provider: CodezProvider | null | undefined): boolean {
 ...
 *
 * 路径规则统一通过 codezTaskService 解析，避免 UI 层猜 provider 自己的目录结构。
 */
export function useTaskNativeSessionLogFile(
  workspacePath: string,
  taskId: string | null,
  providerHint?: CodezProvider | null,
  workspaceIdentity?: string,
  options: { enabled?: boolean } = {},
) {
  const codezTaskService = useCodezTaskService(workspacePath, undefined, workspaceIdentity);
  ...
  }, [enabled, codezTaskService, providerHint, taskId, workspaceIdentity, workspacePath]);
