...
import type { CodezSessionEndedSubagent, CodezSessionSubagentsResult } from "@codez/shared";
...
interface SessionSubagentsViewState {
  revision: number;
  ended: CodezSessionSubagentsResult["ended"];
  error: string | null;
  loading: boolean;
}
...
function mergeEndedSubagentPages(
  current: readonly CodezSessionEndedSubagent[],
  incoming: readonly CodezSessionEndedSubagent[],
): CodezSessionEndedSubagent[] {
...
export function useSessionSubagents(options: {
  ...
  const codezAgentService = services.codezAgentService;
  ...
  const requestPage = useCallback(
    (endedLimit: number, endedCursor?: string): Promise<CodezSessionSubagentsResult> => {
      if (!options.sessionId) return Promise.reject(new Error("session_id_missing"));
      if (typeof codezAgentService?.listSessionSubagents !== "function") {
      ...
      return codezAgentService.listSessionSubagents({
