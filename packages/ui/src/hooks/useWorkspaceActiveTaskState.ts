...
function areResolvedTaskMetasEqual(left: CodezTaskMeta | null, right: CodezTaskMeta | null) {
  if (left === right) {
    return true;
  }
...
function useStableResolvedActiveTaskMeta(taskMeta: CodezTaskMeta | null) {
  const stableTaskMetaRef = useRef<CodezTaskMeta | null>(null);
...
export function useWorkspaceActiveTaskState({
  ...
  const workspaceState = useCodezSessionStore((state) =>
    selectWorkspaceCodezState(state, workspaceAbsPath, workspaceIdentity),
  );
  ...
  const activeTaskMeta = useMemo(() => {
    ...
    // App 以前依赖 codezTaskMetaMerge 同时拉普通列表和 pinned 列表，只是为了给当前激活 task
