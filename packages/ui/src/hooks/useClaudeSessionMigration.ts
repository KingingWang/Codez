...
import type { CodezImportSessionsResult, CodezImportableSessionCandidate } from "@codez/shared";
import { logger } from "@/logger.js";
import { useCodezTaskService } from "@/hooks/useCodezTaskService.js";
...
export function useClaudeSessionMigration(params: {
  ...
  const codezTaskService = useCodezTaskService(
  ...
  const [candidates, setCandidates] = useState<CodezImportableSessionCandidate[]>([]);
  const [selectedSessionIds, setSelectedSessionIds] = useState<string[]>([]);
  const [scanError, setScanError] = useState<string | null>(null);
  const [importError, setImportError] = useState<string | null>(null);
  const [lastImportResult, setLastImportResult] = useState<CodezImportSessionsResult | null>(null);
  ...
  const scan = useCallback(async () => {
    ...
    try {
      ...
      const nextCandidates = await codezTaskService.scanImportableClaudeSessions({
  ...
  const importSessions = useCallback(
    async (sessionIds: string[]) => {
      ...
      try {
        ...
        const result = await codezTaskService.importClaudeSessions({
          workspacePath: effectiveWorkspacePath,
          ...(effectiveWorkspaceIdentity ? { workspaceIdentity: effectiveWorkspaceIdentity } : {}),
          sessionIds,
        });
