...
import { CODEZ_MODE_OPTION_DESCRIPTION_IDS, CODEZ_MODE_OPTION_LABEL_IDS } from "./display-help.js";
...
function isConfigSelectValueInOptions(option: CodezConfigOption, value: string): boolean {
  return option.options?.some((entry) => String(entry.value) === value) ?? false;
}

function getConfigSelectTriggerTestId(option: CodezConfigOption): string | undefined {
...
export function ChatApiRetryStatus({
  ...
  const retryLabel = useMemo(() => {
    ...
    // 当前 Codez Agent 只会推送某一刻的 retryDelayMs 快照，不会每秒递减。
...
export function getModeOptionDisplayLabel(
  ...
  entry: Pick<CodezConfigSelectValue, "name" | "value">,
): string {
...
function getModeOptionLabelMessageId(
  provider: CodezProvider | undefined,
  entry: Pick<CodezConfigSelectValue, "value">,
): string | null {
  if (!provider) {
    return null;
  }

  return CODEZ_MODE_OPTION_LABEL_IDS[provider]?.[entry.value] ?? null;
}
...
export function getModeOptionDescriptionMessageId(
  ...
  return CODEZ_MODE_OPTION_DESCRIPTION_IDS[provider]?.[entry.value] ?? null;
}
