import type { SessionCreateSource } from "@codez/shared";
...
} from "@/store/codezSessionStoreTypes.js";
import {
  getTaskMeta,
  getWorkspaceState,
...
type SetFn = (
  partial:
    | CodezSessionStoreState
    | Partial<CodezSessionStoreState>
    | ((state: CodezSessionStoreState) => CodezSessionStoreState | Partial<CodezSessionStoreState>),
) => void;
...
function normalizeThoughtLevelConfigOption(option: CodezConfigOption): CodezConfigOption {
  if (
    option.type !== "select" ||
...
function normalizeConfigOptions(options: CodezConfigOption[]): CodezConfigOption[] {
  return options.map(normalizeThoughtLevelConfigOption);
}

function cloneConfigOptions(options: readonly CodezConfigOption[]): CodezConfigOption[] {
...
function isModeConfigOption(option: CodezConfigOption): boolean {
  return option.category === "mode" && option.type === "select";
}

function createFallbackModeOption(params: {
  currentModeId: string;
  options?: NonNullable<CodezConfigOption["options"]>;
}): CodezConfigOption {
...
function replaceModeConfigOption(
  currentOptions: CodezConfigOption[] | null,
  nextModeOption: CodezConfigOption | null,
): CodezConfigOption[] | null {
...
function updateActiveTaskConfigOptions(
  current: WorkspaceCodezUIState,
  updater: (options: CodezConfigOption[] | null) => CodezConfigOption[] | null,
): Pick<WorkspaceCodezUIState, "taskConfigOptionsByTaskId"> | null {
...
function resolveActiveTaskConfigOptionsOnSwitch(
  current: WorkspaceCodezUIState,
  taskId: string,
): { configOptions: CodezConfigOption[]; status: ConfigOptionsStatus } | null {
  ...
  return {
    ...
    promoteGroupedDraftTask: (
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          workspacePath,
          (current) => {
            const optimisticTask: CodezTaskMeta = {
              taskId,
              traceId: `session-${taskId}` as CodezTaskMeta["traceId"],
              title: "",
              workspacePath,
    ...
    startDraft: (
      ...
      const normalizedProvider = provider
        ? normalizeAgentProviderToCodezAgent(provider)
        : undefined;
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          workspacePath,
          (current) => {
            const nextSelectedProvider = normalizeAgentProviderToCodezAgent(
    ...
    bindRuntimeProvider: (
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          (current) => ({
            ...current,
            selectedProvider: normalizeAgentProviderToCodezAgent(provider),
          }),
          workspaceIdentity,
        ),
      ),
