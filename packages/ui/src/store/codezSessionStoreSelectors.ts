/**
 * Codez Session Store 选择器与内部辅助函数
 *
 * 从 codezSessionStore.ts 拆分出来，包含 workspace 状态读取/更新辅助函数，
 * 以及所有按 task 粒度的只读访问器和独立选择器。
 */
import type { CodezTaskRuntimeStatus, CodezTaskMeta } from "@codez/shared";
...
import {
  ...
  type CodezSessionStoreState,
  type WorkspaceInitState,
  type TaskRuntimeState,
  type WorkspaceCodezUIState,
} from "./codezSessionStoreTypes.js";
...
function collectIdentityTaskIds(
  baseState: WorkspaceCodezUIState,
  workspaceIdentity: string,
): Set<string> {
...
function createIdentityWorkspaceStateSeed(
  baseState: WorkspaceCodezUIState | undefined,
  workspaceIdentity?: string,
...
export function updateWorkspaceState(
  state: CodezSessionStoreState,
  workspacePath: string,
  updater: (current: WorkspaceCodezUIState) => WorkspaceCodezUIState,
  workspaceIdentity?: string,
): Pick<CodezSessionStoreState, "workspaces"> {
  const workspaceKey = resolveWorkspaceStateKey(workspacePath, workspaceIdentity);
  const current =
  ...
  if (nextWorkspaceState === current) {
    // 单 Codez Agent 迁移后旧 provider 选择都会归一为 glm，很多调用实际不会改变状态。
...
interface WorkspaceDisplayedTaskState {
  taskStatus: CodezTaskRuntimeStatus;
...
export function getTaskUiState(workspaceState: WorkspaceCodezUIState, taskId: string) {
...
export function getTaskMeta(
  workspaceState:
    | WorkspaceCodezUIState
    | Partial<Pick<WorkspaceCodezUIState, "optimisticTaskListByTaskId" | "taskListCache">>,
  taskId: string,
): CodezTaskMeta | null {
...
export function getVisibleTaskMetas(
  ...
  const taskById = new Map<string, CodezTaskMeta>();
...
export function getTaskUnreadIndicator(
  workspaceState:
    ...
    | Partial<
      ...
      >,
  taskId: string,
  fallbackTask?: Pick<CodezTaskMeta, "unreadAt">,
): boolean {
...
export function selectWorkspaceCodezState(
  state: CodezSessionStoreState,
  workspacePath: string,
  workspaceIdentity?: string,
) {
...
export function getWorkspaceInitState(
  workspaceState: Pick<WorkspaceCodezUIState, "workspaceInit">,
): WorkspaceInitState {
