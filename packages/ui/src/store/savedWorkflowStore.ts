...
import {
  ...
  type CodezSavedWorkflowInvalidEntry,
...
export interface SavedWorkflowWorkspaceState {
  entries: CodezSavedWorkflowEntry[];
  invalid: CodezSavedWorkflowInvalidEntry[];
  runs: CodezSavedWorkflowRun[];
 ...
 */
function savedWorkflowStoreKey(target: CodezAgentSavedWorkflowTarget): string {
...
async function fetchWorkspace(
  target: CodezAgentSavedWorkflowTarget,
  agentService: ICodezAgentService,
): Promise<{ list: CodezWorkflowsListResult; runs: CodezSavedWorkflowRun[] }> {
  ...
  const [list, runs] = await Promise.all([
    ...
    agentService
      ...
      .catch((error: unknown) => {
        ...
        return [] as CodezSavedWorkflowRun[];
      }),
  ]);
  return { list, runs };
}
...
export function selectSavedWorkflowState(
  ...
  target: CodezAgentSavedWorkflowTarget | null,
