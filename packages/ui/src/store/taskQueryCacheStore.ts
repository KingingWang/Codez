...
import type { CodezTaskListItem } from "@codez/services";
...
interface TaskQueryCacheState {
  ...
  upsertTaskMeta: (task: CodezTaskMeta) => void;
  updateTaskMetaPreservingMembership: (task: CodezTaskMeta) => void;
  applyTaskMutation: (params: {
    previousTask: CodezTaskMeta;
    nextTask: CodezTaskMeta;
    previousState: TaskListMembershipState;
    nextState: TaskListMembershipState;
  }) => void;
  setTaskUnreadOverlay: (
    task: Pick<CodezTaskMeta, "taskId" | "workspacePath" | "workspaceIdentity">,
...
type CachedTaskListItem = CodezTaskListItem & { searchSnippets?: string[] };
...
function matchesTaskSearch(descriptor: TaskListCacheDescriptor, task: CodezTaskMeta): boolean {
...
function sortTaskKeysByDescriptor(params: {
  taskKeys: TaskEntityKey[];
  taskMetaByEntityKey: Record<TaskEntityKey, CodezTaskListItem>;
  descriptor: TaskListCacheDescriptor;
}): TaskEntityKey[] {
  ...
  return uniqueTaskKeys.sort((leftKey, rightKey) => {
    ...
    return compareCodezTaskListItems(leftTask, rightTask, params.descriptor.sortBy);
  });
}
