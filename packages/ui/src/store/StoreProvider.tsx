...
import type { IBroadcastService } from "@codez/services";
import { createCodezStore, type CodezStore, type CodezState } from "./index.js";

// 导出 Context 供测试直接注入已构造的 store 实例（如跨窗口广播抑制用例）。
const StoreContext = createContext<CodezStore | null>(null);
...
export function StoreProvider({
  ...
  const storeRef = useRef<CodezStore | null>(null);
  if (!storeRef.current) {
    storeRef.current = createCodezStore(broadcastService, {
 ...
 */
export function useCodezStore<T>(selector: (state: CodezState) => T): T {
  const store = useContext(StoreContext);
  if (!store) {
    throw new Error("useCodezStore 必须在 StoreProvider 内使用");
  }
  return useStore(store, selector);
}

/**
 * 带默认值的容错版 useCodezStore（store 耦合剥离配套）。
 *
 ...
 */
export function useCodezStoreWithDefault<T>(
  selector: (state: CodezState) => T,
  defaultValue: T,
): T {
  const store = useContext(StoreContext);
