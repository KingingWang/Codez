...
interface RemotePinnedTaskState {
  ...
  refreshWorkspace: (params: {
    ...
    codezTaskService: Pick<ICodezTaskService, "listPinnedTasks">;
  }) => Promise<void>;
  upsertTask: (task: CodezTaskMeta) => void;
...
function sortPinnedTasks(tasks: CodezTaskMeta[]): CodezTaskMeta[] {
...
export const useRemotePinnedTaskStore = create<RemotePinnedTaskState>()((set) => ({
  ...
  errorByWorkspaceKey: {},
  async refreshWorkspace({ workspacePath, workspaceIdentity, codezTaskService }) {
    ...
    try {
      const items = await codezTaskService.listPinnedTasks({
        workspacePath,
