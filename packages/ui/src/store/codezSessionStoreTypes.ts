/* oxlint-disable eslint(max-lines) -- Codez Agent store 类型和默认状态集中导出，避免切片间重复定义共享结构。 */
/**
 * Codez Session Store 类型定义、接口、常量与默认值工厂
 *
 * 从 codezSessionStore.ts 拆分出来，供 store 本体和 selectors / navigation 等子模块共享。
 */
import {
  ...
  type CodezWorkspaceInitStatus,
  type CodezTaskRuntimeStatus,
  type CodezProvider,
  type CodezTaskMeta,
  type CodezContextCacheUsage,
  ...
  type CodezSessionActiveTurnKind,
  type CodezContextUsageBreakdownItem,
  type InputId,
  type SessionCreateSource,
} from "@codez/shared";
import type { CodezUiError } from "@/lib/codezUiError.js";
import type {
...
export interface WorkspaceInitState {
  status: CodezWorkspaceInitStatus;
  error: string | null;
  attempts: number;
}
...
export interface TaskRuntimeState {
  status: CodezTaskRuntimeStatus;
  error: string | null;
  /** 该 task 当前运行态绑定的 Codez Agent 进程 provider，用于 workspace 级进程重建 busy lock。 */
  ...
  backgroundTaskControls: CodezBackgroundTaskControlItem[];
...
export interface TaskUiState {
  permissionRequest: CodezPermissionRequest | null;
  pendingPermissionRequests: CodezPermissionRequest[];
  elicitationRequest: CodezElicitationRequest | null;
  pendingElicitationRequests: CodezElicitationRequest[];
  elicitationFormDraftsByRequestId: Record<string, ElicitationFormDraft>;
  error: CodezUiError | null;
}
...
export interface WorkspaceCodezUIState {
  /** 当前 workspace 激活中的 task */
  activeTaskId: string | null;
  /**
   * Codez Agent 的 workspace 初始化状态。
   *
   * 单 Codez Agent 迁移后继续按 provider 分桶会保留多份已经不会再被真实运行时更新的旧状态，
  ...
  taskConfigOptionsByTaskId: Record<string, CodezConfigOption[]>;
  ...
  optimisticTaskListByTaskId: Record<string, CodezTaskMeta>;
  ...
  /** 当前 workspace 选中的 Codez Agent provider */
  ...
  /** Codez Agent configOptions（模型、模式、思考级别等） */
  configOptions: CodezConfigOption[] | null;
  /** configOptions 加载状态 */
  ...
  taskListCache: CodezTaskMeta[] | null;
  /** 新建草稿时递增，驱动输入框在切到草稿态后主动聚焦 */
  draftFocusVersion: number;
}

export interface CodezSessionStoreState {
  /** 按 workspace 维护聊天相关 UI 状态 */
  workspaces: Record<string, WorkspaceCodezUIState>;
  getWorkspaceState: (workspacePath: string, workspaceIdentity?: string) => WorkspaceCodezUIState;
  ...
  setTaskPermissionRequest: (
    workspacePath: string,
    taskId: string,
    request: CodezPermissionRequest | null,
    workspaceIdentity?: string,
  ) => void;
  ...
  setTaskElicitationRequest: (
    workspacePath: string,
    taskId: string,
    request: CodezElicitationRequest | null,
    workspaceIdentity?: string,
  ) => void;
  ...
  /** 任务列表变更时调用（创建/删除任务），驱动 codezTaskMetaMerge 重新拉取列表 */
...
const FALLBACK_PROVIDER: CodezProvider = CODEZ_AGENT_PROVIDER;
...
export function getDefaultWorkspaceState(): WorkspaceCodezUIState {
  // 单 Codez Agent 迁移后默认 provider 必须收敛到 glm。
