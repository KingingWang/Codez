...
import type { CodezTaskListSortBy, ICodezTaskService } from "@codez/services";
...
interface RemoteTimelineTaskState {
  itemsByWorkspaceKey: Record<string, CodezTaskMeta[]>;
  ...
  refreshWorkspace: (params: {
    ...
    codezTaskService: Pick<ICodezTaskService, "listTaskList">;
    sortBy: CodezTaskListSortBy;
    limit?: number;
  }) => Promise<void>;
  upsertTask: (task: CodezTaskMeta, sortBy?: CodezTaskListSortBy) => void;
...
function sortTimelineTasks(
  tasks: CodezTaskMeta[],
  sortBy: CodezTaskListSortBy = "updated",
): CodezTaskMeta[] {
...
export const useRemoteTimelineTaskStore = create<RemoteTimelineTaskState>()((set) => ({
  ...
  errorByWorkspaceKey: {},
  async refreshWorkspace({ workspacePath, workspaceIdentity, codezTaskService, sortBy, limit }) {
    ...
    try {
      const result = await codezTaskService.listTaskList({
        kind: "timeline",
        workspaceScopes: [
          {
            workspacePath,
...
export async function refreshRemoteTimelineTasksForSession({
  ...
  await useRemoteTimelineTaskStore.getState().refreshWorkspace({
    workspacePath,
    workspaceIdentity,
    codezTaskService: session.services.codezTaskService,
    sortBy,
    limit,
  });
}
