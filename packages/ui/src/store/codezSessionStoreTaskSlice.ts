...
function sortTasksByUpdatedAt(tasks: readonly CodezTaskMeta[]): CodezTaskMeta[] {
...
function arePermissionOptionsEqual(
  left: CodezPermissionRequest["options"],
  right: CodezPermissionRequest["options"],
): boolean {
...
function arePermissionRequestsEquivalent(
  left: CodezPermissionRequest,
  right: CodezPermissionRequest,
): boolean {
  if (left === right) {
    return true;
  }
          ...
          (current) => ({
            ...
            taskUiByTaskId: {
              ...
              [taskId]: {
                ...
                // 完整错误对象之前只放在 ChatView/useCodezChat 的本地 state。
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          (current) => {
            ...
            return {
              ...current,
              selectedProvider: normalizeAgentProviderToCodezAgent(params.provider),
