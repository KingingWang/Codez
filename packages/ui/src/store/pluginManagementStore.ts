...
export interface PluginDescribeEntry {
  ...
  data?: CodezPluginsDescribeResult;
  error?: string;
}

// 设置页「插件管理」的数据源: 经 IPluginManagementService 薄服务由 codez-cli 提供 (list + enable/disable)。
// UI 不再直触 ICodezAgentService，plugins/* 旧协议词的消费收拢到服务实现一处。
...
export interface PluginManagementState {
  ...
  diagnostics: CodezPluginDiagnostic[];
  loading: boolean;
  error: string | null;
  /**
...
function throwForErrorDiagnostic(diagnostics: CodezPluginDiagnostic[]): void {
