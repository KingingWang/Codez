import type {
  ...
  CodezTaskChangedFileSummary,
  CodezTaskMeta,
} from "@codez/shared";
import { computeLineChangeStat } from "@codez/shared";
...
export function getTaskChangeSummary(
  task: Pick<CodezTaskMeta, "changeSummary"> | null | undefined,
): CodezTaskChangeSummary | null {
...
function formatTaskChangeStats(
  summary: CodezTaskChangeSummary,
...
export function buildTaskChangeSummary(
  fileChanges: readonly CodezPersistedFileChange[] | undefined,
): CodezTaskChangeSummary | null {
  ...
  let added = 0;
  let removed = 0;
  const files: CodezTaskChangedFileSummary[] = Array.from(changedFileMap.values())
    .map((file) => {
...
export function buildTurnChangeSummary(
  turn: CodezPersistedFileChange | null | undefined,
): CodezTaskChangeSummary | null {
  ...
  let added = 0;
  let removed = 0;
  const files: CodezTaskChangedFileSummary[] = Array.from(filesByPath.entries())
    .map(([path, snapshot]) => {
...
export function buildPerTurnChangeSummaries(
  fileChanges: readonly CodezPersistedFileChange[] | undefined,
): Map<number, CodezTaskChangeSummary> {
  const result = new Map<number, CodezTaskChangeSummary>();
  if (!fileChanges || fileChanges.length === 0) {
    return result;
  }
