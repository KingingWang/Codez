...
interface FinalizePromptTelemetryInput {
  ...
  status: string;
  finishedAt: number;
  fileChanges?: readonly CodezPersistedFileChange[];
  usage?: CodezUsage;
  errorType?: string;
  errorMsg?: string;
...
function resolvePromptTelemetryModelProvider(params: {
  ...
  // ghost supplier 代表尚未解析到稳定 custom provider 的临时隔离态，provider 维度统一回退到当前 Codez Agent provider。
...
export function buildPromptTelemetryExtraDetail(params: {
  ...
  return {
    ...
    // agent 字段取 Codez Agent provider；本仓库没有独立 session.agentId。
...
const COMPACTION_TERMINAL_STATUSES: readonly CodezTimelineStatus[] = [
  "completed",
  "failed",
  "interrupted",
];

function isCompactionTerminalStatus(status: CodezTimelineStatus): boolean {
...
export function buildCompactionTelemetryExtraDetail(params: {
  timeline: CodezContextCompactionTimelineMeta;
  provider?: CodezProvider;
...
function buildPromptUsageTelemetryExtraDetail(
  ...
  return {
    ...
    cache_write_input_tokens: String(usage.cachedWriteInputTokens ?? 0),
    // Codez Agent 链路未透出 tool use prompt token，成功态显式补 0 保持 extraDetail 字段集合完整。
...
function mergePromptUsage(left: CodezUsage | null, right: CodezUsage): CodezUsage {
  if (!left) {
    return {
...
function collectFileChangeMetrics(fileChanges: readonly CodezPersistedFileChange[] | undefined) {
...
export function recordPromptModelRequestStarted(
  taskId: string,
  event: Extract<CodezStreamEvent, { type: "task_network_debug_status" }>,
