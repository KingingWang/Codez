...
// sessions-index 只补实时 activity/detail，最终拼出旧 CodezGroupedTaskView 同形结果。
...
function taskKeyOf(task: CodezTaskMeta): string {
...
function taskOrderMapKey(task: CodezTaskMeta): string {
...
function nodeMapKey(node: CodezGroupedTaskViewNode): string {
...
function sortGroupTasks(
  ...
  membersByTaskKey: Map<string, CodezGroupedTaskViewStructureMember>,
): CodezTaskListItem[] {
  const resolvedOrderByTaskKey = new Map<string, number>();
  let maxOrder = 0;
  const missing: Array<{ task: CodezTaskListItem; addedAt: number; key: string }> = [];
...
export function buildGroupedTaskViewFromSessions(
  ...
  const activeTaskByKey = new Map<string, CodezTaskListItem>();
  ...
  const membersByGroupId = new Map<string, CodezGroupedTaskViewStructureMember[]>();
  ...
  const nodes: CodezGroupedTaskViewNode[] = structure.groups.map((group) => {
