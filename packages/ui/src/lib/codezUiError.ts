import type { CodezError, TraceId } from "@codez/shared";
import { errorAttributionSchema, type ErrorAttribution } from "@codez/shared/codez-protocol-v4";

export interface CodezUiError extends CodezError {
...
interface NormalizeCodezUiErrorOptions {
  fallbackCode?: string;
...
const GENERIC_CODEZ_UI_ERROR_MESSAGES = new Set([
  "Internal error",
  "Turn execution failed",
  "Compact failed",
  "Rewind failed",
  "Codez session failed",
]);
...
function collectMessageCandidatesFromRecord(record: Record<string, unknown>): string[] {
  ...
  const messagePaths: Array<readonly string[]> = [
    ...
    ["data", "message"],
    ["data", "detail"],
    // Codez Agent 常把可读原因放在 data.details（复数）里；之前只识别 detail，
    ...
    ["data", "error", "detail"],
    ["data", "error", "details"],
    // codez-cli 会把模型/网络错误摘要放在 data.codez.error 下。
    // 之前 UI 只读 data.error，导致已经结构化好的 provider 根因仍被 “Internal error” 盖住。
    ["data", "codez", "error", "message"],
    ["data", "codez", "error", "detail"],
    ["data", "codez", "error", "details"],
  ];
...
export function normalizeCodezUiError(
  error: unknown,
  options: NormalizeCodezUiErrorOptions = {},
): CodezUiError {
  const candidates = collectMessageCandidates(error);
  // codez-cli 已经把 provider/network 根因放进 detail 或 data.codez.error，
  // 外层仍可能保留 "Internal error" 这类包装文案。主提示优先选非泛化候选，避免根因被盖住。
  const primaryMessage =
    candidates.find((candidate) => !GENERIC_CODEZ_UI_ERROR_MESSAGES.has(candidate)) ??
    candidates[0] ??
    options.fallbackMessage ??
    "Internal error";
  const detailMessage = candidates.find(
    (candidate) => candidate !== primaryMessage && !GENERIC_CODEZ_UI_ERROR_MESSAGES.has(candidate),
  );
  ...
  const underlyingErrorMessage = readFirstStringFromPaths(error, [
    ...
    ["data", "codez", "error", "underlyingErrorMessage"],
  ]);
  const underlyingErrorDetail = readFirstStringFromPaths(error, [
    ...
    ["data", "codez", "error", "underlyingErrorDetail"],
  ]);
