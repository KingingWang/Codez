// re-home 产物（为删除 codezChatMessages 铺路，模式同 codez-task-types-core）。
// 本文件承载旧消息转换层（codezChatMessages/codezChatMessageHelpers）中仍被存活栈
...
// 旧消息拼装运行时（codezChatMessages.ts）删除后，这里是该类型面的唯一事实源。
import type {
  CodezAssistantMessagePart,
  CodezAssistantMessageFeedback,
  CodezPromptAttachment,
  CodezTimelineMeta,
  CodezTaskSnapshotBodyRef,
  CodezTaskSnapshotToolFieldRef,
  CodezTaskSnapshotToolSlice,
} from "@codez/shared";
...
export interface TaskChatToolCall {
  ...
  /** Codez 固定工具名；kind 仍保留给旧 Codez Agent 快照和 UI 聚合分类兼容。 */
  toolName?: string;
  kind: string;
  title?: string;
  input: unknown;
...
export type TaskChatMessagePart = CodezAssistantMessagePart;
