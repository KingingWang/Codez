...
> &
  Partial<Pick<ICodezTaskService, "listDeletedTaskIds">>;
...
interface TaskListMembershipSets {
  ...
  terminalStatusByTaskId: Map<string, Extract<CodezTaskMeta["status"], "completed" | "error">>;
  /**
...
function collectUnreadAt(unreadAtByTaskId: Map<string, number>, tasks: CodezTaskMeta[]): void {
...
function mergeTaskIndexItems(lists: CodezTaskMeta[][]): CodezTaskMeta[] {
  const itemByEntityKey = new Map<string, CodezTaskMeta>();
...
async function fetchTaskListMembershipSetsForEndpoints(
  ...
  const taskIndexItemByEntityKey = new Map<string, CodezTaskMeta>();
