...
import { extractCodezFileCitations } from "@/lib/codezFileCitation.js";
...
function projectCitationTextNode(
  ...
  const citations = extractCodezFileCitations(value);
...
export function createCodezFileCitationRemarkPlugin(
  workspacePath: string,
  homePath?: string,
): Plugin {
  return function codezFileCitationRemarkPlugin() {
    return (tree: unknown) => {
