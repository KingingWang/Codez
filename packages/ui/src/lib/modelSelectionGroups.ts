...
function supportsRegistryApiFormat(
  ...
  // 仅剩 glm（Codez Agent）provider；三方 CLI 的 api format 差异已随 provider 下线。
  return isCodezAgentProvider(selectedProvider);
}
...
export function buildRegistryModelSelectGroups(
  ...
  return view.providers.flatMap((provider) => {
    ...
    const accountAccess = codezProviderAccountAccessSchema.safeParse(provider.config.access);
