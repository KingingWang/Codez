...
import { extractConversationPreviewFileReferences } from "@codez/shared";
import { decodeFilePathUriEscapes, joinFilePath } from "@/lib/path.js";
import { MEDIA_PREVIEW_FORMATS } from "@codez/shared";
import {
  extractCodezFileCitationDirectives,
  resolveCodezFileCitationPreviewKind,
} from "@/lib/codezFileCitation.js";
import {
...
export function extractAssistantFileReferences(
  ...
  for (const citation of extractCodezFileCitationDirectives(content)) {
  ...
  for (const match of content.matchAll(FILE_PATH_RE)) {
    ...
    // 明确的 codez-file-citation 仍在上面的专用分支处理。
    if (/^~[\\/]/.test(raw)) {
      protectedRanges.push([start, end]);
      continue;
    }
