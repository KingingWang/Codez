type CodezUserQuestionAnswers = Record<string, unknown>;
...
interface AskUserQuestionData {
  ...
  answers?: CodezUserQuestionAnswers;
}
...
function readQuestions(input: unknown): unknown[] {
  ...
  // Codez Agent 的单题输入与交互请求的多题输入共用展示管线。
  return typeof input.question === "string" && Array.isArray(input.options) ? [input] : [];
}

function readAnswers(input: unknown): CodezUserQuestionAnswers | undefined {
  if (!isPlainRecord(input)) {
    return undefined;
  }
...
function readNestedAskUserQuestionAnswers(input: unknown): CodezUserQuestionAnswers | undefined {
...
function parseCodezAskUserQuestionOutput(
  output: unknown,
  input: unknown,
): CodezUserQuestionAnswers | undefined {
...
export function readAskUserQuestionAnswers(value: {
  ...
  const parsedCodezOutputAnswers = parseCodezAskUserQuestionOutput(
    value.output,
    readAskUserQuestionInput(value),
  );
  if (parsedCodezOutputAnswers) {
    return parsedCodezOutputAnswers;
  }
  ...
  if (isPlainRecord(value.raw)) {
    ...
    const parsedRawCodezOutputAnswers = parseCodezAskUserQuestionOutput(
      value.raw.rawOutput,
      readAskUserQuestionInput(value),
    );
    if (parsedRawCodezOutputAnswers) {
      return parsedRawCodezOutputAnswers;
    }
    const nestedRawAnswers = readNestedAskUserQuestionAnswers(value.raw.output);
    if (nestedRawAnswers) {
      return nestedRawAnswers;
    }
    const parsedRawCodezAnswers = parseCodezAskUserQuestionOutput(
      value.raw.output,
      readAskUserQuestionInput(value),
    );
    if (parsedRawCodezAnswers) {
      return parsedRawCodezAnswers;
    }
...
export function getAskUserQuestionAnswerText(
  ...
  answers: CodezUserQuestionAnswers | undefined,
  noAnswerText: string,
) {
