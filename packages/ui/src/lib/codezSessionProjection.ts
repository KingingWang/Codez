/* oxlint-disable eslint(max-lines) -- Codez session 到当前聊天 projection 的迁移桥需要同时保持 snapshot 和 event 映射一致。 */
import {
...
export function parseModelPickerValue(value: string): ModelSelection {
  ...
  if (customModel?.providerId && customModel.modelName) {
    ...
    // Codez Protocol 必须收到严格的 providerId/modelId 结构。
    return {
...
export function codezSessionSettingsToConfigOptions(
...
export function codezWorkspacePresentationToConfigOptions(
  ...
  return [
    {
      ...
      category: "mode",
      type: "select",
      currentValue: normalizeAvailableCodezMode(mode),
      options: getCodezAgentModeSelectOptions(),
    },
  ];
}
...
function resolveSettingsThoughtLevelCurrentValue(
  thoughtLevel: CodezSessionSettingsState["thoughtLevel"],
): string | undefined {
  ...
  const defaultThoughtLevel =
    thoughtLevel.defaultLevel && thoughtLevelValues.has(thoughtLevel.defaultLevel)
      ...
      : undefined;
  // Codez Protocol 的 defaultLevel 是模型事实，current 为空时表示用户尚未显式修改。
...
export function codezSessionSnapshotToTaskMeta(snapshot: CodezSessionStateSnapshot): CodezTaskMeta {
  return {
...
function normalizeAvailableCodezMode(mode: CodezSessionMode): string {
  return CODEZ_AGENT_MODE_ID_SET.has(mode) ? mode : "build";
}

function getCodezAgentModeSelectOptions(): NonNullable<CodezConfigOption["options"]> {
