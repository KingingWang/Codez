import type { CodezModelTrajectoryMessage } from "@codez/services";

export function trajectoryToolOutputs(message: CodezModelTrajectoryMessage): string[] {
  return message.parts.flatMap((part) =>
    part.kind === "tool-result" ? [formatTrajectoryToolPayload(part.output)] : [],
  );
}

export function trajectoryToolCallInputs(message: CodezModelTrajectoryMessage): string[] {
  return message.parts.flatMap((part) =>
    part.kind === "tool-call" ? [formatTrajectoryToolPayload(part.input)] : [],
  );
}

export function trajectoryToolHasError(message: CodezModelTrajectoryMessage): boolean {
...
export function trajectoryToolMetadata(message: CodezModelTrajectoryMessage): {
  names: string;
  ids: string;
} {
