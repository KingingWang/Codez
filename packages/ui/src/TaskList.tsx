...
import { TID_TASK_LIST, TID_TASK_EMPTY, TID_TASK_SETTINGS_BUTTON } from "@codez/shared";
...
export const TaskList = memo(function TaskList({
  ...
  hasMore?: boolean;
  onShowMore?: () => void;
  onRenameTask: (taskId: string, title: string) => Promise<CodezTaskMeta | null>;
  onSetTaskPinned: (taskId: string, pinned: boolean) => Promise<CodezTaskMeta | null>;
  onArchiveTask: (taskId: string) => Promise<CodezTaskMeta | null>;
  onSetTaskUnread: (taskId: string, unread: boolean) => Promise<CodezTaskMeta | null>;
  ...
  const startDraft = useCodezSessionStore((state) => state.startDraft);
  ...
  const handleSubmitRenameTask = useCallback(
    async (taskId: string) => {
      ...
      let renamedTask: CodezTaskMeta | null;
      try {
  ...
  const visibleSourceTasks = useMemo(() => {
    const orderedTasks =
      sortBy === "manual"
        ? tasks
        : [...tasks].sort((left, right) => compareCodezTaskListItems(left, right, sortBy));
    return orderedTasks;
  }, [sortBy, tasks]);
  useEffect(() => {
