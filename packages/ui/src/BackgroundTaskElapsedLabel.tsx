...
function createElapsedBaseline(job: CodezBackgroundTaskControlItem) {
  const mountedAt = Date.now();
  return {
    elapsedMs: getCodezBackgroundTaskControlItemElapsedMs(job, mountedAt),
...
function elapsedMsForClock(input: {
  ...
  const elapsedFromJob = getCodezBackgroundTaskControlItemElapsedMs(input.job, input.now);
