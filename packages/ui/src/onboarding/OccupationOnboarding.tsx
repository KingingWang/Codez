...
import type { OnboardingRecordEntry } from "@codez/shared";
...
export function OccupationOnboarding({
  ...
  const requested = useCodezStore((state) => state.newUserOnboardingOpen);
  const setRequested = useCodezStore((state) => state.setNewUserOnboardingOpen);
  // 登录态变化（useRootOAuthEffects 登录成功后 setUser）时按 userId 重新判定是否触发引导。
  const userId = useCodezStore((state) => state.user?.id) ?? null;
  ...
  const savedInterfaceMode = useCodezStore((state) => state.interfaceMode);
  ...
  const requestOnboardingDialog = useCodezStore((state) => state.requestOnboardingDialog);
