...
import type { CodezTaskPendingInteraction } from "@codez/shared";
import { cn } from "@/components/lib/utils.js";

interface TaskInteractionBadgeProps {
  interaction?: CodezTaskPendingInteraction;
...
function isAskUserQuestionInteraction(
  interaction: CodezTaskPendingInteraction | undefined,
): boolean {
...
export function TaskInteractionBadge({
  ...
  const style =
    progress !== undefined
      ? ({
          "--codez-interaction-progress": String(progress),
          "--codez-interaction-remaining-ms": `${remainingMs}ms`,
        } as CSSProperties)
      : undefined;
  ...
  const badgeContent = (
    <>
      {progress !== undefined ? (
        <span
          aria-hidden="true"
          className="codez-task-interaction-countdown-fill absolute inset-0 origin-left bg-interaction-ask-fill transition-opacity group-hover/interaction-badge:opacity-0 group-focus-visible/interaction-badge:opacity-0 motion-reduce:transition-none"
        />
      ) : null}
      {canSnooze ? (
