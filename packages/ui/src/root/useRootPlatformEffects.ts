...
export function useRootPlatformEffects({
  ...
  initialWorkspacePurpose?: import("@codez/shared").WorkspacePurpose;
  ...
  isDesktop?: boolean;
  locale: ReturnType<typeof import("@/i18n/IntlProvider.js").useCodezIntl>["locale"];
  tabs: WindowTabState[];
  ...
  useEffect(() => {
    ...
    const disposeNotificationClick = platform.onTaskNotificationClick((taskId: string) => {
      ...
      const workspaces = useCodezSessionStore.getState().workspaces;
  ...
  useEffect(() => {
    ...
    syncActiveSession();
    return useCodezSessionStore.subscribe(syncActiveSession);
