const CODEZ_JWT_INVALID_RESTART_MARKER_KEY = "codez:auth:jwt-invalid-restart";
...
export function markZcodeJwtInvalidRestart(storage?: RestartMarkerStorage): void {
  resolveStorage(storage)?.setItem(CODEZ_JWT_INVALID_RESTART_MARKER_KEY, "1");
}

export function consumeZcodeJwtInvalidRestartMarker(storage?: RestartMarkerStorage): boolean {
  const resolved = resolveStorage(storage);
  if (!resolved || resolved.getItem(CODEZ_JWT_INVALID_RESTART_MARKER_KEY) !== "1") {
    return false;
  }
  // 该标记只服务于本次重启；如果不在读取时删除，后续正常启动仍会被强制带回登录页。
  resolved.removeItem(CODEZ_JWT_INVALID_RESTART_MARKER_KEY);
  return true;
}
