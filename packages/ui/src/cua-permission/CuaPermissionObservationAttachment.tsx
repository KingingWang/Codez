...
import type { CuaAccessibilitySettingsResult, CuaPermissionKind } from "@codez/shared";
import { requiredCuaPermissionsForRequestAccessStatus } from "@codez/shared/codez-protocol-v4";
import {
  isCuaPermissionStatusAvailable,
  type CuaPermissionRestartOptions,
  type CuaPermissionStatusResult,
  type CodezAgentCuaPermissionObservation,
} from "@codez/services";
...
function cuaPermissionObservationKey(observation: CodezAgentCuaPermissionObservation): string {
...
export function CuaPermissionObservationAttachment() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      typeof services.codezAgentService.onDynamicCuaPermissionObservation !== "function"
    ) {
      return;
    }
    ...
    let disposed = false;
    const subscription = services.codezAgentService.onDynamicCuaPermissionObservation()(
      (observation) => {
        const missing = requiredCuaPermissionsForRequestAccessStatus(observation.permissionStatus);
