import { BufferWriter, serialize } from "@codez/rpc";
...
interface AttachmentUploadAgent {
  ...
  attachmentCommitV4(params: CodezAgentAttachmentTerminalParams): Promise<V4AttachmentPutResult>;
...
function measureAttachmentChannelRequestBytes(method: string, params: unknown): number {
  ...
  serialize(writer, [100, 2_147_483_647, ServiceChannels.CodezAgent, method]);
...
export async function uploadAttachmentTransaction(
  ...
  const beginParams: CodezAgentAttachmentBeginParams = {
    ...common,
  ...
  try {
    ...
    for (let chunkIndex = begin.nextChunkIndex; chunkIndex < totalChunks; chunkIndex += 1) {
      ...
      const chunkParams: CodezAgentAttachmentChunkParams = {
        ...common,
        chunkIndex,
    ...
    const terminal = common satisfies CodezAgentAttachmentTerminalParams;
