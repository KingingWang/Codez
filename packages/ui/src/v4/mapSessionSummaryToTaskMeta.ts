// sessions-index 的 SessionSummary → 侧栏实时 detail CodezTaskMeta 映射。
...
import type { TraceId, CodezProvider, CodezTaskMeta } from "@codez/shared";
...
function phaseToStatus(phase: SessionSummary["phase"]): CodezTaskMeta["status"] {
  switch (phase) {
    case "running":
    case "prewarming":
      return "running";
...
interface MapSessionSummaryOptions {
  ...
  previous?: CodezTaskMeta;
}

/**
 * SessionSummary → 实时 detail CodezTaskMeta。sessions-index 不携带的字段
  ...
  return attachTaskListRowActivity(
    {
      ...
      ...(previous?.provider ? { provider: previous.provider as CodezProvider } : {}),
      ...(status ? { status } : {}),
