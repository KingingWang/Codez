...
import { resolveWorkspaceTelemetryDetail, type IPlatformService } from "@codez/shared";
import { createConversationTelemetryService, type IServiceAccessor } from "@codez/services";
...
function acquireSupervisor(
  ...
  const key = attachmentScopeKey(scope, services.codezAgentService);
...
function ensureSupervisorSubscription(
  ...
  const telemetryService = createConversationTelemetryService(services.codezAgentService);
...
export function ConversationTelemetryWorkspaceAttachment({
  ...
  const lease = useMemo(() => {
    const agentService = services.codezAgentService as object | null | undefined;
