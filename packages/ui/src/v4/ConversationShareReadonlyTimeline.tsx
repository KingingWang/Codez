 ...
 * 公开页 bundle 体积与无 Desktop 宿主（window.codez / PlatformProvider / tab store）的运行环境；
...
import { getCompactToolCallStatusMessageId, type Locale } from "@codez/shared";
import type {
  ArtifactRow,
  AssistantTextRow,
  ConversationRow,
...
function resolveImportedArtifactPath(
  ...
  // 取舍：这里只校验形状（.codez-share/<dir>/shared-artifacts/<file> 四段），刻意不把
  ...
  // .codez-share/*/shared-artifacts/ 内，无越权读放大。
  ...
  const labels: ReadonlyLabels =
    locale === "zh-CN"
      ? {
          ...
          unsupportedRows: "部分内容需要更新 Codez 查看",
        }
      : {
          history: "Reasoning",
          ...
          unsupportedRows: "Some content requires a newer version of Codez",
        };
