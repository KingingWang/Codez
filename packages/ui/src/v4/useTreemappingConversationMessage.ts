...
// 复用 treemappingActivity 的既有解析规则，不复活 codezChatMessages。
//
  ...
  const layer = useMemo(
    () =>
      new SessionDataLayer({
        transport: createAgentConversationTransport(codezAgentService, {
          workspacePath,
          workspaceIdentity,
        }),
      }),
    [codezAgentService, workspacePath, workspaceIdentity],
  );
  useEffect(() => {
