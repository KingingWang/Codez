...
const TASK_LIST_ROW_ACTIVITY_FIELD = "__codezSessionActivity" as const;
...
export type TaskListMetaWithActivity = CodezTaskMeta & {
  [TASK_LIST_ROW_ACTIVITY_FIELD]: TaskListRowActivity;
};

export function attachTaskListRowActivity<T extends CodezTaskMeta>(
  task: T,
...
export function getTaskListRowActivity(task: CodezTaskMeta): TaskListRowActivity | null {
...
function isTaskListRowRunning(task: CodezTaskMeta): boolean {
 ...
 */
export function isTaskListRowActive(task: CodezTaskMeta): boolean {
...
export function mergeTaskListMembershipFields(
  activityTask: CodezTaskMeta,
  membershipTask: CodezTaskMeta,
): CodezTaskMeta {
