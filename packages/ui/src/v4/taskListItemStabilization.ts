...
//
// sessions-index / Controller tasks-index 每个内容帧都会全量重建 CodezTaskMeta[]，
...
export function buildTaskListItemIdentityKey(meta: CodezTaskMeta): string {
...
export function areTaskListItemsEquivalent(left: CodezTaskMeta, right: CodezTaskMeta): boolean {
  return areStabilizedValuesEquivalent(left, right);
}

/** 引用稳定化：等价条目复用旧对象；顺序与内容全等时复用整个旧数组。 */
export function stabilizeTaskListItems<T extends CodezTaskMeta>(previous: T[], next: T[]): T[] {
