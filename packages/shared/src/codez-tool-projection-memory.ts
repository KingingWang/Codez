import type { CodezStreamingToolInputState } from "./streaming-tool-input-preview.js";

export interface CodezToolProjectionMemory {
  completeToolInputById?: Map<string, unknown>;
  streamingToolInputById?: Map<string, CodezStreamingToolInputState>;
  toolNameById?: Map<string, string>;
}

export interface CodezToolProjectionMetadata {
  hasInput: boolean;
  input?: unknown;
  toolName?: string;
}

export function createCodezToolProjectionMemory(): CodezToolProjectionMemory {
  return {
    completeToolInputById: new Map<string, unknown>(),
    streamingToolInputById: new Map<string, CodezStreamingToolInputState>(),
    toolNameById: new Map<string, string>(),
  };
}

export function ensureCodezToolProjectionMemory(
  memory: CodezToolProjectionMemory,
): CodezToolProjectionMemory {
  memory.completeToolInputById ??= new Map<string, unknown>();
  memory.streamingToolInputById ??= new Map<string, CodezStreamingToolInputState>();
  memory.toolNameById ??= new Map<string, string>();
  return memory;
}

export function resolveCodezToolProjectionMetadata(
  payload: Record<string, unknown>,
  toolId: string,
  memory: CodezToolProjectionMemory,
): CodezToolProjectionMetadata {
...
export function finalizeCodezToolProjectionInput(
  toolId: string,
  input: unknown,
  memory: CodezToolProjectionMemory,
): void {
...
export function forgetCodezToolProjectionMetadata(
  toolId: string,
  memory: CodezToolProjectionMemory,
): void {
