import type { CodezApiRetryStatus } from "./codez-task-types-core.js";

export function normalizeCodezApiRetryStatus(
  value: unknown,
...
export function codezApiRetryFromModelNetworkStatusPayload(
  payload: Record<string, unknown>,
): CodezApiRetryStatus | null | undefined {
  const type = stringValue(payload.type);
  if (type === "model_retry_scheduled") {
    return normalizeCodezApiRetryStatus(payload);
  }
  if (type === "model_request_started") {
    const streamRecoveryRetry = codezApiRetryFromStreamRecoveryPayload(payload.streamRecovery);
...
export function isCodezModelRetryRecoveryProgressPayload(
  payload: Record<string, unknown>,
): boolean {
...
export function codezApiRetryFromStreamRecoveryPayload(
  value: unknown,
): CodezApiRetryStatus | undefined {
  const record = asRecord(value);
