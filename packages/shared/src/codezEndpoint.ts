import type { CodezEnv } from "./env.js";

export const DEFAULT_CODEZ_ENDPOINT_ORIGIN = "https://zcode.z.ai";
...
declare const __CODEZ_ENDPOINT_ENV__: Record<string, string | undefined> | undefined;
export function pickProductEndpointEnv(
  env: Record<string, string | undefined>,
): Record<string, string> {
  const keys = [
    "CODEZ_BASE_URL",
    "CODEZ_ENDPOINT_ORIGIN",
...
export function readProductEndpointEnv(): Record<string, string | undefined> {
  return {
    ...(typeof __CODEZ_ENDPOINT_ENV__ === "undefined" ? {} : __CODEZ_ENDPOINT_ENV__),
    ...pickProductEndpointEnv(typeof process === "undefined" ? {} : process.env),
  };
}

export interface CodezEndpointUrls {
  origin: string;
  apiBaseUrl: string;
  webShareCallbackUrl: string;
  codezPlanOpenAiBaseUrl: string;
  codezPlanAnthropicBaseUrl: string;
  codezPlanBillingCurrentUrl: string;
  codezPlanBillingBalanceUrl: string;
}

export interface RuntimeCodezEndpointEnv {
  [key: string]: string | undefined;
  CODEZ_ENV?: string;
  CODEZ_BASE_URL?: string;
  CODEZ_ENDPOINT_ORIGIN?: string;
}
...
export interface RuntimeProductEndpointEnv
  extends RuntimeCodezEndpointEnv, RuntimeBigModelApiEnv, RuntimeZaiEndpointEnv {}

export interface RuntimeProductEndpointConfig {
  codezEnv: CodezEnv;
  codezEndpointOrigin: string;
  codezEndpointUrls: CodezEndpointUrls;
...
export function normalizeCodezEndpointOrigin(value: string): string {
  const trimmed = value.trim();
  if (!trimmed) {
    throw new Error("Codez endpoint origin is empty");
  }

  const parsed = new URL(trimmed);
  if (parsed.protocol !== "https:" && parsed.protocol !== "http:") {
    throw new Error("Codez endpoint origin must use http or https");
  }
...
export function isTrustedCodingPlanWebviewOrigin(
  ...
  try {
    const origin = normalizeCodezEndpointOrigin(value);
    if (
      origin === DEFAULT_CODEZ_ENDPOINT_ORIGIN ||
      origin === resolveRuntimeCodezEndpointOrigin()
    ) {
      return true;
    }
...
export function resolveCodezEndpointOrigin(options?: {
  env?: CodezEnv;
  envBaseOrigin?: string | null;
  overrideOrigin?: string | null;
}): string {
  const origin = options?.overrideOrigin?.trim() || options?.envBaseOrigin?.trim();
  return origin ? normalizeCodezEndpointOrigin(origin) : DEFAULT_CODEZ_ENDPOINT_ORIGIN;
}

export function resolveRuntimeCodezEnv(
  env: RuntimeCodezEndpointEnv = readProductEndpointEnv(),
): CodezEnv {
  // 产品身份仅用于既有展示与安装标识，不参与地址解析。
  return env.CODEZ_ENV?.trim().toLowerCase() === "test" ? "test" : "production";
}

export function resolveRuntimeCodezEndpointOrigin(
  env: RuntimeCodezEndpointEnv = readProductEndpointEnv(),
  options?: { overrideOrigin?: string | null },
): string {
  return resolveCodezEndpointOrigin({
    envBaseOrigin:
      readRuntimeEnvValue(env, "CODEZ_BASE_URL") ??
      readRuntimeEnvValue(env, "CODEZ_ENDPOINT_ORIGIN"),
    overrideOrigin: options?.overrideOrigin,
  });
}

export function buildRuntimeCodezEndpointUrls(
  env: RuntimeCodezEndpointEnv = readProductEndpointEnv(),
): CodezEndpointUrls {
  return buildCodezEndpointUrls(resolveRuntimeCodezEndpointOrigin(env));
}

export function buildRuntimeCodezApiUrl(
  env: RuntimeCodezEndpointEnv = readProductEndpointEnv(),
  path: string,
): string {
  const normalizedPath = path.startsWith("/") ? path : `/${path}`;
  return `${resolveRuntimeCodezEndpointOrigin(env)}${normalizedPath}`;
}

export function resolveBigModelApiOrigin(
  env: RuntimeBigModelApiEnv = readProductEndpointEnv(),
): string {
  return normalizeCodezEndpointOrigin(
...
export function buildZaiOAuthUrl(origin: string, path: string): string {
  const normalizedPath = path.startsWith("/") ? path : `/${path}`;
  return `${normalizeCodezEndpointOrigin(origin)}${normalizedPath}`;
}
...
export function resolveRuntimeProductEndpointConfig(
  ...
  const codezEnv = resolveRuntimeCodezEnv(env);
  const codezEndpointOrigin = resolveRuntimeCodezEndpointOrigin(env);

  return {
    codezEnv,
    codezEndpointOrigin,
    codezEndpointUrls: buildCodezEndpointUrls(codezEndpointOrigin),
...
export function buildCodezEndpointUrls(origin: string): CodezEndpointUrls {
  const normalizedOrigin = normalizeCodezEndpointOrigin(origin);
  return {
    origin: normalizedOrigin,
    apiBaseUrl: `${normalizedOrigin}/api/v1`,
    webShareCallbackUrl: `${normalizedOrigin}/cn/share/callback`,
    codezPlanOpenAiBaseUrl: `${normalizedOrigin}/api/v1/zcode-plan`,
    codezPlanAnthropicBaseUrl: `${normalizedOrigin}/api/v1/zcode-plan/anthropic`,
    codezPlanBillingCurrentUrl: `${normalizedOrigin}/api/v1/zcode-plan/billing/current`,
    codezPlanBillingBalanceUrl: `${normalizedOrigin}/api/v1/zcode-plan/billing/balance`,
  };
}

export function rewriteCodezEndpointUrl(input: string | URL, endpointOrigin: string): string | URL {
  ...
  const sourceOrigin = DEFAULT_CODEZ_ENDPOINT_ORIGIN;
  if (parsed.origin !== sourceOrigin) {
    return input;
  }

  const targetOrigin = normalizeCodezEndpointOrigin(endpointOrigin);
