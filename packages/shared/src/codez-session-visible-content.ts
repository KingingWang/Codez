...
// 消费者：codezTaskServiceAdapter/codezSessionProjection/codezTaskIndexSyncer、
// CLI bootstrap session-mapper。textFromCodezMessageParts 已迁
// codez-protocol-legacy-types.ts（幸存面）。本文件与旧投影栈同生命周期。
import type { CodezSessionGoal } from "./codez-protocol/index.js";
...
export function isCodezGoalContinuationReminderText(text: string): boolean {
...
export function isCodezGoalContinuationReminderMessage(message: CodezMessageWithParts): boolean {
  return (
    message.info.role === "user" &&
    (message.info.source === "goal-continuation" ||
      String(message.info.metadata?.["source"] ?? "") === "goal-continuation" ||
      isCodezGoalContinuationReminderText(textFromCodezMessageParts(message.parts)))
  );
}

export function isCodezGoalStateReminderText(text: string): boolean {
  const normalized = text.trimStart();
  return normalized.startsWith("<system-reminder>") && normalized.includes(GOAL_STATE_TEXT_MARKER);
}

export function isCodezGoalModelOnlyReminderMessage(message: CodezMessageWithParts): boolean {
  return (
    message.info.role === "user" &&
    (isCodezGoalContinuationReminderMessage(message) ||
      isCodezGoalStateReminderText(textFromCodezMessageParts(message.parts)))
  );
}

export function isCodezModelOnlySyntheticUserMessage(message: CodezMessageWithParts): boolean {
  if (
    message.info.role === "user" &&
...
export function isCodezCompactSummaryMessage(message: CodezMessageWithParts): boolean {
  return (
    message.info.role === "user" &&
...
export function getCodezUserVisibleMessages(
  messages: readonly CodezMessageWithParts[],
  _options: { target?: CodezSessionGoal | null } = {},
): CodezMessageWithParts[] {
  const visibleMessages: CodezMessageWithParts[] = [];
  for (const message of messages) {
    if (isCodezModelOnlySyntheticUserMessage(message) || isCodezCompactSummaryMessage(message)) {
...
export function getCodezGoalIterationByAssistantMessageId(
  messages: readonly CodezMessageWithParts[],
  options: { maxGoalIteration?: number; target?: CodezSessionGoal | null } = {},
): Map<string, number> {
  ...
  const sortedMessages = [...messages].sort(compareCodezMessagesByCreatedTime);
  ...
  for (const message of sortedMessages) {
    if (message.info.role === "user") {
      if (isCodezGoalContinuationReminderMessage(message)) {
...
export interface CodezGoalIterationCountTimelineItem {
  goalIteration?: number;
  status: "started" | "completed" | "failed_closed" | "cancelled";
  verification?: { passed?: boolean | null } | null;
}

export function getCodezGoalActiveIterationCount(input: {
  targetStatus?: string | null;
  timeline?: readonly CodezGoalIterationCountTimelineItem[] | null;
}): number {
...
function compareCodezMessagesByCreatedTime(
  left: CodezMessageWithParts,
  right: CodezMessageWithParts,
) {
...
function isVisibleRealGoalUserMessage(message: CodezMessageWithParts, target: CodezSessionGoal) {
  if (
    message.info.role !== "user" ||
...
export function resolveCodezVisibleSessionTitle(input: {
  title?: string;
  messages: readonly CodezMessageWithParts[];
  target?: CodezSessionGoal | null;
  fallback?: string;
}): string {
  const normalizedTitle = input.title?.trim() ?? "";
  if (
    normalizedTitle &&
    !isCodezGoalContinuationReminderText(normalizedTitle) &&
    !isCodezGoalStateReminderText(normalizedTitle)
  ) {
  ...
  const firstUser = input.messages.find(
    (message) =>
      message.info.role === "user" &&
      getConversationMessageProjectionPolicy(message) === "realUserInput" &&
      !isCodezGoalModelOnlyReminderMessage(message),
  );
  const firstUserText = textFromCodezMessageParts(firstUser?.parts ?? []).trim();
  if (firstUserText) {
