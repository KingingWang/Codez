...
import { z } from "zod";
import { codezProcessDiagnosticSchema } from "./process-diagnostic.js";
...
import { codezProviderSchema } from "./providers.js";
import { codezAgentProviderSchema } from "./codez-agent-policy.js";
...
import { sessionWorkflowActivitySchema } from "./codez-protocol-v4/sessions-index-workflow-activity.js";
import {
...
export { codezTaskModeSchema } from "./codez-task-mode-schema.js";
...
export const helloAckMessageSchema = z.object({
  type: z.literal("codez-hello-ack"),
  version: z.string(),
...
export const hostInitLocalMessageSchema = z.object({
  ...
  codezBuiltinProviderConfigFilePath: nonEmptyStringSchema,
...
export const hostOffPeakRunMessageSchema = z.object({
  ...
  workspaceIdentity: z.string().optional(),
  prompt: nonEmptyStringSchema,
  // 权限四档映射现有 CodezTaskMode；与 cron-run 的 mode 同样按宽松 string 传输
...
export { codezProviderSchema };

export const codezTaskMigrationSourceSchema = z.enum(["claudeCode"]);
...
export const hostAgentProcessExceptionResponseSchema = z
  .object({
    ...
    runtimeGeneration: z.number().int().positive(),
    runtimeInstanceId: nonEmptyStringSchema,
    diagnostic: codezProcessDiagnosticSchema,
  })
  .strict();
...
export const agentLaneResourceSampleSchema = codezProcessResourceSampleSchema
...
export const nodeSelfResourceSampleSchema = z
  .object({
     ...
     * 上限刻意放宽（与 CLI 的 `codezProcessResourceSampleSchema` 同口径）：读数异常时宁可让
...
export const hostMcpResourceSamplesResponseSchema = z
  .object({
    ...
    runtimeSurface: z.enum(["local", "remote"]),
    environmentKey: resourceTelemetryEnvironmentKeySchema.optional(),
    samples: codezMcpResourceSamplesSchema,
  })
  .strict();
...
export const hostToolExecResourceResponseSchema = z
  .object({
    ...
    runtimeSurface: z.enum(["local", "remote"]),
    sample: codezToolExecResourceSchema,
  })
  .strict();
...
export const hostMcpTelemetryResponseSchema = z
  .object({
    ...
    runtimeSurface: z.enum(["local", "remote"]),
    event: codezMcpTelemetryEventSchema,
  })
  .strict();
...
export const codezTaskPersistStatusSchema = z.enum(["running", "completed", "error"]);

export const codezPromptImageAttachmentSchema = z.object({
...
export const codezPromptVideoAttachmentSchema = z.object({
...
export const codezPromptPdfAttachmentSchema = z.object({
  kind: z.literal("pdf"),
  filename: z.string(),
  mimeType: z.string(),
  sizeBytes: z.number().int().nonnegative().optional(),
...
export const codezPromptFileAttachmentSchema = z.object({
  kind: z.literal("file"),
  filename: z.string(),
  mimeType: z.string(),
...
export const codezPromptAttachmentSchema = z.discriminatedUnion("kind", [
  codezPromptImageAttachmentSchema,
  codezPromptVideoAttachmentSchema,
  codezPromptPdfAttachmentSchema,
  codezPromptFileAttachmentSchema,
]);

export const codezPersistedToolCallSchema = z.object({
...
const codezPersistedMessagePartSchema = z.discriminatedUnion("type", [
...
export const codezPersistedMessageSchema = z.object({
  ...
  attachments: z.array(codezPromptAttachmentSchema).optional(),
  tools: z.array(codezPersistedToolCallSchema).optional(),
  thought: z.string().optional(),
  parts: z.array(codezPersistedMessagePartSchema).optional(),
...
export const codezTaskGoalStatusSchema = z.enum(["active", "paused", "budget_limited", "complete"]);

export const codezTaskTargetChangedActionSchema = z.enum([
  "set",
  "status_updated",
  "cleared",
  "usage_accounted",
...
export const codezTaskTargetChangedSourceSchema = z.enum(["command", "tool", "runtime"]);

export const codezTaskGoalSchema = z.object({
  sessionID: nonEmptyStringSchema,
  ...
  status: codezTaskGoalStatusSchema,
...
export const codezTaskGoalChangedPatchSchema = z.object({
  action: codezTaskTargetChangedActionSchema,
  source: codezTaskTargetChangedSourceSchema,
  target: codezTaskGoalSchema.nullable(),
  previousTarget: codezTaskGoalSchema.nullable().optional(),
});

export const codezTaskTargetStatusSchema = codezTaskGoalStatusSchema;
export const codezTaskTargetSchema = codezTaskGoalSchema;
export const codezTaskTargetChangedPatchSchema = codezTaskGoalChangedPatchSchema;

export const codezTaskMetaSchema = z.object({
  taskId: nonEmptyStringSchema,
  ...
  mode: codezTaskModeSchema,
  model: z.string().optional(),
  thoughtLevel: nonEmptyStringSchema.optional(),
  runtimeEpoch: z.number().int().nonnegative().optional(),
  provider: codezAgentProviderSchema.optional(),
  migrationSource: codezTaskMigrationSourceSchema.optional(),
  ...
  status: codezTaskPersistStatusSchema.optional(),
  lastError: z
    .object({
  ...
  changeSummary: z
    ...
    .optional(),
  target: codezTaskGoalSchema.nullable().optional(),
});

export const codezTaskIndexEntrySchema = z.object({
  workspaceHash: nonEmptyStringSchema,
  taskId: nonEmptyStringSchema,
});

export const codezPinnedTasksFileSchema = z.object({
  version: z.literal("1"),
  tasks: z.array(codezTaskIndexEntrySchema),
});

const codezPersistedFileSnapshotSchema = z.object({
  path: z.string(),
...
const codezPersistedFileChangeSchema = z.object({
  turnIndex: z.number().int().nonnegative(),
  snapshots: z.array(codezPersistedFileSnapshotSchema),
  fileState: z.enum(["applied", "reverted"]).optional(),
});

const codezPersistedTurnCheckpointSchema = z.object({
  turnIndex: z.number().int().nonnegative(),
  baseFileCheckpointId: nonEmptyStringSchema,
  resultFileCheckpointId: nonEmptyStringSchema.optional(),
});

export const codezSessionFileSchema = z.object({
  meta: codezTaskMetaSchema,
  messages: z.array(codezPersistedMessageSchema),
  fileChanges: z.array(codezPersistedFileChangeSchema).optional(),
  turnCheckpoints: z.array(codezPersistedTurnCheckpointSchema).optional(),
});
