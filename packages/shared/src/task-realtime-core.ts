...
import { z } from "zod";
import type { CodezTaskMigrationSource, CodezTaskMode } from "./codez-task-types-core.js";
import { codezAgentProviderSchema } from "./codez-agent-policy.js";
import { codezPermissionResponseSchema } from "./codez-protocol-legacy-types.js";
// merge 冲突解决：两侧分别在相邻行新增独立 import（本分支 hook trust review
// 决策 schema、staging telemetry error attribution schema），二者无语义交集，均保留。
import { workspaceHookReviewDecisionSchema } from "./codez-protocol-v4/workspace-hook-review.js";
import { errorAttributionSchema } from "./codez-protocol-v4/snapshot.js";

const nonEmptyString = z.string().trim().min(1);
const codezTaskModeRealtimeValues = [
  "yolo",
  "plan",
  "edit",
  "auto",
  "autoEdit",
  "build",
] as const satisfies readonly CodezTaskMode[];
const codezTaskMigrationSourceRealtimeValues = [
  "claudeCode",
] as const satisfies readonly CodezTaskMigrationSource[];
const codezTaskChangeSummaryRealtimeSchema = z
  .object({
...
const taskMetaRealtimeSchema = z.object({
  ...
  // schema 推导类型因此无法回到 CodezTaskMeta，host typecheck 也就无法覆盖这条链路。
  mode: z.enum(codezTaskModeRealtimeValues),
  model: z.string().optional(),
  runtimeEpoch: z.number().int().nonnegative().optional(),
  provider: codezAgentProviderSchema.optional(),
  migrationSource: z.enum(codezTaskMigrationSourceRealtimeValues).optional(),
  ...
  lastError: z
    ...
    .optional(),
  changeSummary: codezTaskChangeSummaryRealtimeSchema.optional(),
});
...
export const workspaceTaskListInvalidatedEventSchema = taskRealtimeInvalidationBaseEventSchema
  ...
  .strict();
const codezPromptAttachmentSchema = z.discriminatedUnion("kind", [
  z
    .object({
...
const codezTaskRuntimeCommandBaseSchema = z
  .object({
  ...
  .strict();
const codezTaskRuntimeCommandSchema = z.discriminatedUnion("type", [
  codezTaskRuntimeCommandBaseSchema
    .extend({
...
const taskEnqueueCommandOwnerCommandRequestSchema = taskOwnerCommandBaseSchema
  .extend({
    ...
    taskCommand: codezTaskRuntimeCommandSchema,
  })
  .strict();
...
export const taskOwnerCommandResultSchema = z.discriminatedUnion("success", [
  z
    .object({
      ...
      taskCommand: codezTaskRuntimeCommandSchema.optional(),
    })
    .strict(),
  z
    .object({
