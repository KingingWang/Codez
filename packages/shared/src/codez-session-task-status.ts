// ── 旧协议兼容面（过渡期）──────────────────────────────
// 剩余 1 个导出：deriveCodezTaskStatusFromSessionSnapshot。
// 消费者：codezTaskServiceAdapter/codezTaskIndexSyncer/codezSessionProjection（旧投影栈）。
import type { CodezSessionStateSnapshot } from "./codez-protocol/index.js";
import { getCodezUserVisibleMessages } from "./codez-session-visible-content.js";
import type { CodezTaskMeta } from "./codez-task-types-core.js";
type CodezTaskStatus = CodezTaskMeta["status"];

function statusFromCodezSession(
  status: CodezSessionStateSnapshot["session"]["status"],
): CodezTaskStatus {
...
function hasBlockingActiveSnapshotRuntime(snapshot: CodezSessionStateSnapshot): boolean {
...
function hasActiveSnapshotRuntime(snapshot: CodezSessionStateSnapshot): boolean {
...
function hasCompletedVisibleAssistantTurn(snapshot: CodezSessionStateSnapshot): boolean {
  const visibleMessages = getCodezUserVisibleMessages(snapshot.messages, {
...
export function deriveCodezTaskStatusFromSessionSnapshot(
  ...
  const status = statusFromCodezSession(snapshot.session.status);
