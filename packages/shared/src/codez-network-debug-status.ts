import type {
  ...
  TraceId,
  CodezStreamEvent,
  CodezTaskNetworkDebugStatusType,
} from "./codez-task-types-core.js";

const MODEL_NETWORK_STATUS_TYPES = new Set<CodezTaskNetworkDebugStatusType>([
...
export function codezTaskNetworkDebugStatusFromPayload(params: {
  taskId: string;
  traceId: TraceId;
  inputId?: InputId;
  queryId?: QueryId;
...
}): Extract<CodezStreamEvent, { type: "task_network_debug_status" }> | null {
...
function networkStatusTypeValue(value: unknown): CodezTaskNetworkDebugStatusType | undefined {
  return typeof value === "string" && MODEL_NETWORK_STATUS_TYPES.has(value as never)
    ? (value as CodezTaskNetworkDebugStatusType)
    : undefined;
}
