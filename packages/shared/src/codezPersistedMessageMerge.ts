import type { CodezPersistedMessage, CodezPersistedMessagePart } from "./codez-task-types-core.js";

// Codez runtime 把同一个 user turn 里**每一轮 LLM 调用**落成独立的 assistant 消息（各带 time.created/completed）。
...
export function coalesceConsecutiveCodezAssistants(
  messages: readonly CodezPersistedMessage[],
): CodezPersistedMessage[] {
  const merged: CodezPersistedMessage[] = [];
...
function mergeAssistantPair(
  first: CodezPersistedMessage,
  next: CodezPersistedMessage,
): CodezPersistedMessage {
  ...
  const shiftedNextParts: CodezPersistedMessagePart[] = (next.parts ?? []).map((part) =>
    part.type === "tool-call"
