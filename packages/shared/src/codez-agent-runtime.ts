export type CodezAgentBinaryKind = "native-binary";

export interface CodezAgentRuntimeDescriptor {
  binaryKind: CodezAgentBinaryKind;
  binaryEnvVar: string;
  ...
  /**
   * 桌面端把 agent 的 JS bundle（codez.cjs）打进 resources/glm，由 app 内置的 Electron Node runtime
...
export const CODEZ_AGENT_RUNTIME: CodezAgentRuntimeDescriptor = {
  binaryKind: "native-binary",
  binaryEnvVar: "GLM_BINARY_PATH",
  bundledResourceDir: "glm",
  version: "0.13.3",
  spawnArgs: ["app-server", "--stdio"],
  nativeConfigDir: ".codez/cli",
  nativeConfigFileName: "config.json",
  missingBinaryMessage:
    "[Codez Agent] glm binary 未找到，请设置 GLM_BINARY_PATH 或先准备 GLM 运行时资源",
  resolveEntrySegments: (platform) => [resolvePlatformBinaryName("codez-agent", platform)],
  nodeBundleEntryFile: "codez.cjs",
  resolveNodeBundleSegments() {
    return [this.nodeBundleEntryFile];
  },
};

export function getCodezAgentRuntime(): CodezAgentRuntimeDescriptor {
  return CODEZ_AGENT_RUNTIME;
}
