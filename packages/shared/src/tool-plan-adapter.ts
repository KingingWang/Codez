import type { CodezPlanStep } from "./codez-task-types-core.js";
...
function normalizePlanStatus(value: unknown): CodezPlanStep["status"] | null {
...
function parsePlanStep(value: unknown, index: number): CodezPlanStep | null {
...
function extractPlanStepsFromValue(value: unknown): CodezPlanStep[] | null {
  ...
  const steps = collection
    ...
    .filter((step): step is CodezPlanStep => step !== null);
