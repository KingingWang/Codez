...
import { ICodezAgentService } from "@codez/services";
...
export async function runServerCore(generation: number): Promise<void> {
  ...
  const explicitCodezBuiltinProviderConfigFilePath =
    process.env[CODEZ_BUILTIN_PROVIDER_CONFIG_FILE_ENV]?.trim();
  const codezBuiltinProviderConfigFilePath = explicitCodezBuiltinProviderConfigFilePath
    ? explicitCodezBuiltinProviderConfigFilePath
    : typeof __CODEZ_BUILTIN_PROVIDER_CONFIG_JSON__ === "string"
      ? await materializeCodezBuiltinProviderConfig({
          environmentConfigRoot: getAppConfigDir(),
          content: __CODEZ_BUILTIN_PROVIDER_CONFIG_JSON__,
        })
      : undefined;
  if (!codezBuiltinProviderConfigFilePath) {
    throw new Error(
      `当前构建未嵌入 Codez Built-in Provider Config，且未设置 ${CODEZ_BUILTIN_PROVIDER_CONFIG_FILE_ENV}`,
    );
  }
  ...
  const taskActivityTracker = createTaskActivityTracker(services.getOptional(ICodezAgentService));
