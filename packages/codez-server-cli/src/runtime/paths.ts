...
function getDefaultServerDataRoot(): string {
  const configured = process.env.CODEZ_DATA_BASE_DIR?.trim();
  return join(configured || homedir(), ".codez", "server");
}
  ...
  return {
    ...
    controlEndpoint:
      process.platform === "win32"
        ? `\\\\.\\pipe\\codez-server-${stablePathId(root)}`
...
function inferDataBaseDir(serverRoot: string): string {
  ...
  if (basename(serverRoot) === "server" && basename(parent) === ".codez") {
