...
const desktopProductIdentity = resolveDesktopProductIdentity({
  ...process.env,
  CODEZ_ENV: builtinProviderConfig.environment,
});
...
const shouldEnableMacSigning =
  process.env.CODEZ_ENABLE_MAC_SIGN === "1" && Boolean(macSigningIdentity);
const codexSigned = isCodexBuild && process.env.CODEZ_CODEX_SIGNED === "1";
if (codexSigned) {
  if (
...
const desktopDistDir = process.env.CODEZ_DESKTOP_DIST_DIR || "dist";
...
const REQUIRED_ASAR_RUNTIME_MODULES = [
  ...
  "pngjs",
  // @codez/services 的代理连通性探测会动态 require("undici") 取 ProxyAgent。
...
const WINDOWS_INSTALL_MANIFEST_NAME = ".codez-install-manifest";
...
function resolveElectronDownloadMirror(env = process.env) {
  const existingMirror =
    env.CODEZ_ELECTRON_RUNTIME_MIRROR ||
...
const desktopArtifactEnvSuffix = `${resolveDesktopArtifactSuffix(process.env)}${isCodexBuild && process.env.CODEZ_CODEX_SIGNED !== "1" ? "-unsigned" : ""}`;

// Preview 是内部签名测试包。CI 明确打开 macOS 签名时若没有身份，必须在生成未签名包前失败，
// 避免“产物存在”被误认为已经走完和生产版相同的签名链路。
if (
  desktopProductIdentity.flavor === "preview" &&
  process.env.CODEZ_ENABLE_MAC_SIGN === "1" &&
  !macSigningIdentity
) {
  throw new Error(
    "Codez Preview macOS packaging requires APPLE_SIGNING_IDENTITY or CSC_NAME when CODEZ_ENABLE_MAC_SIGN=1",
  );
}
...
async function injectHoistedRuntimeModulesIntoAsar(context) {
  ...
  const stagingDir = mkdtempSync(resolve(tmpdir(), "codez-app-asar-"));
  try {
...
export default {
  ...
  extraResources: [
      ...
      ? [
          {
            ...
            // 只是不吸附，所以这里不做存在性断言。
            from: "resources/macos-window-bounds/codez-window-bounds",
            to: "macos-window-bounds/codez-window-bounds",
          },
        ]
      : []),
    {
    ...
    {
      // Provider Registry 的 Codez Built-in Config 是静态 Provider/Model 事实的唯一内置来源。
      // 显式随包发布，避免正式 Host 回退到旧 Catalog/Preset hardcode。
      from: builtinProviderConfig.sourcePath,
      to: "config/provider/codez-builtin.json",
    },
    {
      ...
      : [
          {
            ...
            // 桌面端内置的是 agent 的 JS bundle（glm/codez.cjs，由 prepare:agent-bundle 生成），
            // Host 进程用 app 自带的 Electron Node runtime（ELECTRON_RUN_AS_NODE）执行 `codez.cjs app-server --stdio`，
  ...
  protocols: [
    {
      ...
      // 展示名跟随安装包身份；scheme 仍保持 codez，因此两个应用中最后注册者会成为默认 handler。
      name: desktopProductIdentity.productName,
      schemes: [isCodexBuild ? "codez-codex" : "codez"],
    },
  ],
  mac: {
    target: ["dmg", "zip"],
  ...
  linux: {
    ...
    // desktop 包名是 scoped package（@codez/desktop），electron-builder 默认会把
    // Linux executable/Icon 推成 @codezdesktop。部分桌面环境无法按这个 icon name 命中
    // hicolor 图标，最终回退成系统齿轮。这里固定成稳定的小写名称，让 Icon=codez
    // 与 /usr/share/icons/hicolor/*/apps/codez.png 保持一致。
