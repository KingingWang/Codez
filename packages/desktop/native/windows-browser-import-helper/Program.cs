...
{
    private const string Protocol = "CODEZ_BROWSER_IMPORT_V1";
        ...
        try
        {
            if (HasArgument(args, "--version"))
            {
                ...
                Console.Out.WriteLine(
                    "CODEZ_BROWSER_IMPORT_HELPER\t" + HelperVersion + "\t" + GetProcessArchitecture() +
    ...
    private static void RunBrokerCore()
    {
        ...
        try
        {
            ...
            pipeName = "codez-browser-import-broker-" + Guid.NewGuid().ToString("N");
            token = CreateToken();
    ...
    private static void RunTemporarySystemService(
        ...
        int brokerPid)
    {
        string serviceName = "CodezBrowserImport_" + Guid.NewGuid().ToString("N");
        string pipeName = "codez-browser-import-system-" + Guid.NewGuid().ToString("N");
