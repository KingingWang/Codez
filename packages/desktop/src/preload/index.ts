...
contextBridge.exposeInMainWorld("__CODEZ_DEVICE_ID__", parseDeviceIdFromArgs());
...
import {
  InternalChannels,
  PlatformChannels,
  formatCodezRendererProcessName,
...
if (shouldEnableE2ETestBridge(process.env)) {
  contextBridge.exposeInMainWorld("__codezFinalArmsCustomEventsE2E", {
...
function updateRendererProcessTitle(): void {
  process.title = formatCodezRendererProcessName(document.title);
}
...
// 这里在 preload 阶段补上 codez-* title，便于按窗口角色筛选。
 ...
 */
contextBridge.exposeInMainWorld("codez", {
  connectRemote: (
    ...
    context?: {
      ...
      connectTrigger?: import("@codez/shared").RemoteWorkspaceConnectTrigger;
    },
  ) =>
  ...
  reportLocalTtftBatch: (batch: import("@codez/shared").LocalTtftBatch): void =>
  ...
  /** 导出日志：打包 ~/.codez/v2 及外部 agent 日志为 zip 并在 Finder 中显示 */
  exportLogs: (): Promise<{
    success: boolean;
    path?: string;
    error?: string;
  ...
  importChromeBrowserData: (options?: import("@codez/shared").ChromeBrowserDataImportOptions) =>
  ...
  getCodezStdioTapDevState: (): Promise<CodezStdioTapDevState> =>
    ipcRenderer.invoke(PlatformChannels.GetCodezStdioTapDevState),
