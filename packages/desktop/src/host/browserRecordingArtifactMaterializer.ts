...
import type { IRemoteBackend } from "@codez/server/remote";
import type { BrowserRecordingArtifact } from "@codez/shared";
...
export async function materializeBrowserRecordingArtifact(input: {
  ...
  const stagingPath = `${targetPath}.codez-recording-${randomUUID()}.tmp`;
  try {
