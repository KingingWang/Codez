...
const CODEZ_CLIENT_CONFIG_API_PATH = "/api/v1/client/configs";
...
function resolveForceUpdateClientConfigUrl(endpointOrigin = DEFAULT_CODEZ_ENDPOINT_ORIGIN): string {
  const url = new URL(
    `${buildCodezEndpointUrls(endpointOrigin).origin}${CODEZ_CLIENT_CONFIG_API_PATH}`,
  );
...
function getForceUpdateMinimalVersionFromClientConfig(config: unknown): string | undefined {
  ...
  if (typeof envelope.code === "number" && envelope.code !== 0) {
    ...
    throw new Error(`Codez client config failed: ${envelope.code}`);
  }
