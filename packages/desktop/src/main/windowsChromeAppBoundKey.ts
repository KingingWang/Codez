...
import { CODEZ_COMMIT, CODEZ_VERSION } from "@codez/shared";
import {
...
const HELPER_PROTOCOL = "CODEZ_BROWSER_IMPORT_V1";
const HELPER_VERSION_PROTOCOL = "CODEZ_BROWSER_IMPORT_HELPER";
const HELPER_VERSION = "2";
const HELPER_TIMEOUT_MS = 90_000;
const HELPER_MAX_OUTPUT_BYTES = 128 * 1024;
const HELPER_FILENAME = "codez-browser-import-helper.exe";
...
async function verifyAuthenticodePair(
  ...
  const script = [
    "$stream=[IO.File]::Open($codezArg0,[IO.FileMode]::Open,[IO.FileAccess]::Read,[IO.FileShare]::Read);",
    "try{",
    "$helper=Microsoft.PowerShell.Security\\Get-AuthenticodeSignature -LiteralPath $codezArg0;",
    "$app=Microsoft.PowerShell.Security\\Get-AuthenticodeSignature -LiteralPath $codezArg1;",
...
async function runLockedPackagedHelper(options: {
  ...
  const invocation =
    options.mode === "broker"
      ? "$request=[Console]::In.ReadToEnd();$response=$request | & $codezArg0 --broker --parent-pid $codezArg2;"
      : "$response=& $codezArg0 --version;";
  const script = [
    "$stream=[IO.File]::Open($codezArg0,[IO.FileMode]::Open,[IO.FileAccess]::Read,[IO.FileShare]::Read);",
    "try{",
    "$sha=[Security.Cryptography.SHA256]::Create();",
    ...
    "finally{$sha.Dispose()};",
    `if($digest -ne $codezArg1){${hashMismatchResponse}};`,
    invocation,
...
async function verifyHelper(helperPath: string, options: ReadAppBoundKeyOptions): Promise<string> {
  ...
  try {
    ...
    const expectedAppVersion = options.expectedAppVersion ?? CODEZ_VERSION;
    const expectedBuildCommit = options.expectedBuildCommit ?? CODEZ_COMMIT;
    if (
      version.exitCode !== 0 ||
      fields.length !== 5 ||
