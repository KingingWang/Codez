...
import { formatCodezRendererProcessName } from "@codez/shared";
...
export function buildAuxiliaryRendererName(contents: WebContents): string {
  ...
  if (url?.startsWith("devtools://")) {
    return formatCodezRendererProcessName(
      `devtools-${contentsId}-${hostTitle ?? ownerLabel ?? "unknown"}`,
    );
  }

  if (type === "window") {
    return formatCodezRendererProcessName(title ?? `window-${contentsId}`);
  }

  return formatCodezRendererProcessName(`${type}-${contentsId}-${pageLabel}`);
}
