...
export type CodezDataSizePartialReason = "file_limit" | "io_error" | "time_limit";

export type CodezDataSizeScanResult = {
  bytes: number;
  ...
  | { status: "partial"; partialReason: CodezDataSizePartialReason }
);

export interface CodezDataSizeScanRequest {
  rootPath: string;
  maxDurationMs: number;
  maxFiles: number;
}

interface CodezDataSizeScanOptions extends CodezDataSizeScanRequest {
  signal?: AbortSignal;
...
function createAbortError(): DOMException {
  return new DOMException("Codez data size scan aborted", "AbortError");
}
 ...
 */
export async function scanCodezDataDirectory(
  options: CodezDataSizeScanOptions,
  ...
  let terminalPartialReason: Exclude<CodezDataSizePartialReason, "io_error"> | null = null;
...
export function isCodezDataSizeScanResult(value: unknown): value is CodezDataSizeScanResult {
  if (typeof value !== "object" || value === null) {
    return false;
  }
  const candidate = value as Partial<CodezDataSizeScanResult>;
  const finiteNumbers = [
    candidate.bytes,
