...
import type { SaveFileRequest, SaveFileResult } from "@codez/shared";
import { PlatformChannels } from "@codez/shared";
...
async function downloadRemoteFile(sourceUrl: URL, destinationPath: string): Promise<void> {
  ...
  const temporaryDirectory = await mkdtemp(join(tmpdir(), "codez-save-file-"));
