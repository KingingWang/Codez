...
import { LOCAL_TTFT_BUCKETS_MS, localTtftBatchSchema, type LocalTtftBatch } from "@codez/shared";
import {
...
export function createLocalTtftExporter(options: {
  ...
  const resource = resourceFromAttributes({
    "service.name": "codez-local-ttft",
    "service.version": options.version,
    "os.type": process.platform,
    "codez.telemetry.schema_version": 1,
  });
  const meterProvider = new MeterProvider({
    ...
    views: [
      {
        instrumentName: "codez.local_ttft.*",
        aggregation: {
  ...
  const meter = meterProvider.getMeter("@codez/local-ttft", "1");
  const duration = meter.createHistogram("codez.local_ttft.duration", { unit: "ms" });
  const stageDuration = meter.createHistogram("codez.local_ttft.stage.duration", { unit: "ms" });
  const stageObservation = meter.createHistogram("codez.local_ttft.stage.observation.duration", {
    unit: "ms",
  });
  const preparationObservation = meter.createHistogram(
    "codez.local_ttft.preparation.observation.duration",
    { unit: "ms" },
  );
  const systemDuration = meter.createHistogram("codez.local_ttft.system.duration", { unit: "ms" });
  const preparationDuration = meter.createHistogram("codez.local_ttft.preparation.duration", {
    unit: "ms",
  });
  const attemptDuration = meter.createHistogram("codez.local_ttft.attempt.duration", {
    unit: "ms",
  });
  const executionDuration = meter.createHistogram("codez.local_ttft.execution.duration", {
    unit: "ms",
  });
  const failureWait = meter.createHistogram("codez.local_ttft.no_output.wait", { unit: "ms" });
  const textDuration = meter.createHistogram("codez.local_ttft.first_text.duration", {
    unit: "ms",
  });
  const outcomes = meter.createCounter("codez.local_ttft.records");
  const drops = meter.createCounter("codez.local_ttft.dropped");
