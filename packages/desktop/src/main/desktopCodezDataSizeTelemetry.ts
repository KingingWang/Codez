...
import type { ArmsRumEnv, FinalArmsCustomEventPayload } from "@codez/shared";

import type { CodezDataSizeScanResult } from "./codezDataSizeScanner.js";
import {
  readCodezDataSizeTelemetryState,
  writeCodezDataSizeTelemetryState,
  type CodezDataSizeTelemetryState,
} from "./codezDataSizeTelemetryState.js";
import { scanCodezDataDirectoryInWorker } from "./codezDataSizeWorkerClient.js";

export type { CodezDataSizeTelemetryState } from "./codezDataSizeTelemetryState.js";

const CODEZ_DATA_SIZE_SCAN_MAX_DURATION_MS = 30_000;
const CODEZ_DATA_SIZE_SCAN_MAX_FILES = 200_000;
const CODEZ_DATA_SIZE_DAILY_INTERVAL_MS = 24 * 60 * 60 * 1000;
const CODEZ_DATA_SIZE_IDLE_POLL_MS = 5 * 60 * 1000;
const CODEZ_DATA_SIZE_IDLE_WAIT_FALLBACK_MS = 6 * 60 * 60 * 1000;
const CODEZ_DATA_SIZE_MINIMUM_IDLE_SECONDS = 5 * 60;
const CODEZ_DATA_SIZE_STARTUP_MIN_DELAY_MS = 2 * 60 * 1000;
const CODEZ_DATA_SIZE_STARTUP_JITTER_MAX_MS = 8 * 60 * 1000;
const CODEZ_DATA_SIZE_DAILY_JITTER_MAX_MS = 60 * 60 * 1000;
const CODEZ_DATA_SIZE_ABORTED_RETRY_MS = 30 * 60 * 1000;
const CODEZ_DATA_SIZE_FAILURE_RETRY_MS = 6 * 60 * 60 * 1000;
const CODEZ_DATA_SIZE_ACTIVITY_POLL_MS = 1_000;

interface CodezDataSizeTelemetryTiming {
...
const DEFAULT_TIMING: CodezDataSizeTelemetryTiming = {
  abortedRetryMs: CODEZ_DATA_SIZE_ABORTED_RETRY_MS,
  activityPollMs: CODEZ_DATA_SIZE_ACTIVITY_POLL_MS,
  dailyIntervalMs: CODEZ_DATA_SIZE_DAILY_INTERVAL_MS,
  dailyJitterMaxMs: CODEZ_DATA_SIZE_DAILY_JITTER_MAX_MS,
  failureRetryMs: CODEZ_DATA_SIZE_FAILURE_RETRY_MS,
  idlePollMs: CODEZ_DATA_SIZE_IDLE_POLL_MS,
  idleWaitFallbackMs: CODEZ_DATA_SIZE_IDLE_WAIT_FALLBACK_MS,
  minimumIdleSeconds: CODEZ_DATA_SIZE_MINIMUM_IDLE_SECONDS,
  startupJitterMaxMs: CODEZ_DATA_SIZE_STARTUP_JITTER_MAX_MS,
  startupMinDelayMs: CODEZ_DATA_SIZE_STARTUP_MIN_DELAY_MS,
};

interface CodezDataSizeTelemetryLogger {
  info: (...args: unknown[]) => void;
  warn: (...args: unknown[]) => void;
}

interface CodezDataSizeTelemetrySchedulerDependencies {
  deviceMid: string;
  getSystemIdleTimeSeconds: () => number;
  isAppBackground: () => boolean;
  isCodezBusy: () => boolean;
  logger: CodezDataSizeTelemetryLogger;
  readState: () => Promise<CodezDataSizeTelemetryState | null>;
  report: (result: CodezDataSizeScanResult) => Promise<void> | void;
  scan: (options: { signal: AbortSignal }) => Promise<CodezDataSizeScanResult>;
  timing?: Partial<CodezDataSizeTelemetryTiming>;
  writeState: (state: CodezDataSizeTelemetryState) => Promise<void>;
}
...
function buildCodezDataSizeArmsPayload(params: {
  context: {
    appVersion: string;
    armsEnv: ArmsRumEnv;
  ...
  result: CodezDataSizeScanResult;
}): FinalArmsCustomEventPayload {
  const eventName = "perf_resource_codez_data_size";
  return {
    group: "resource",
    name: eventName,
    properties: stringifyProperties({
      ...
      schema_version: 1,
      codez_data_bytes: params.result.bytes,
    }),
    type: "custom",
...
function stableJitterMs(deviceMid: string, bucketAt: number, maxMs: number): number {
  ...
  const key = `${deviceMid}:${Math.floor(bucketAt / CODEZ_DATA_SIZE_DAILY_INTERVAL_MS)}`;
...
function createCodezDataSizeTelemetryScheduler(
  dependencies: CodezDataSizeTelemetrySchedulerDependencies,
  ...
  let persistedState: CodezDataSizeTelemetryState | null = null;
  let stopped = true;
  ...
  function collectionEligibilityLost(relaxedIdle: boolean): boolean {
    try {
      return dependencies.isCodezBusy() || (!relaxedIdle && !isPreferredIdleWindow());
    } catch {
      return true;
    }
  }
  ...
  async function persistSuccess(reportedAt: number): Promise<void> {
    try {
      ...
      dependencies.logger.warn("[codez-data-size] failed to persist report state", error);
    }
  }

  async function reserveReport(reservedAt: number): Promise<CodezDataSizeTelemetryState | null> {
  ...
  async function rollbackReportReservation(
    previousState: CodezDataSizeTelemetryState | null,
  ): Promise<void> {
    try {
      ...
      dependencies.logger.warn("[codez-data-size] failed to roll back report reservation", error);
    }
  }
  ...
  async function collectAndReport(relaxedIdle: boolean): Promise<void> {
    ...
    try {
      ...
      if (abortController.signal.aborted || collectionEligibilityLost(relaxedIdle)) {
        ...
        abortController.abort();
        throw new DOMException("Codez data size scan eligibility lost", "AbortError");
      }
      ...
      if (abortController.signal.aborted || collectionEligibilityLost(relaxedIdle)) {
        await rollbackReportReservation(previousState);
        abortController.abort();
        throw new DOMException("Codez data size report eligibility lost", "AbortError");
      }
      try {
      ...
      dependencies.logger.info(
        `[codez-data-size] reported status=${result.status} bytes=${result.bytes}`,
      );
      schedule(
      ...
      dependencies.logger.warn(
        `[codez-data-size] ${aborted ? "scan aborted" : "scan/report failed"}`,
        error,
      );
  ...
  async function initializeFromPersistedState(): Promise<void> {
    let state: CodezDataSizeTelemetryState | null;
    try {
      state = await dependencies.readState();
    } catch (error) {
      // Bug 根因：状态读取失败不等于没有历史状态；按首次启动继续采集会绕过持久化限流。
      // 无法确认配额时保持 fail-closed，并且只重试状态读取，不进入采集调度。
      dependencies.logger.warn("[codez-data-size] failed to read report state", error);
...
let desktopScheduler: ReturnType<typeof createCodezDataSizeTelemetryScheduler> | null = null;

export function registerDesktopCodezDataSizeTelemetry(options: {
  context: Parameters<typeof buildCodezDataSizeArmsPayload>[0]["context"];
  getSystemIdleTimeSeconds: () => number;
  isAppBackground: () => boolean;
  isCodezBusy: () => boolean;
  logger: CodezDataSizeTelemetryLogger;
  ...
  desktopScheduler = createCodezDataSizeTelemetryScheduler({
    ...
    isCodezBusy: options.isCodezBusy,
    logger: options.logger,
    readState: () => readCodezDataSizeTelemetryState(options.stateFile),
    report: (result) => {
      const payload = buildCodezDataSizeArmsPayload({ context: options.context, result });
      armsRum.sendCustom({
        group: payload.group,
        name: payload.name,
    ...
    scan: ({ signal }) =>
      scanCodezDataDirectoryInWorker(
        {
          maxDurationMs: CODEZ_DATA_SIZE_SCAN_MAX_DURATION_MS,
          maxFiles: CODEZ_DATA_SIZE_SCAN_MAX_FILES,
          rootPath: options.rootPath,
        },
        signal,
      ),
    writeState: (state) => writeCodezDataSizeTelemetryState(options.stateFile, state),
  });
  void desktopScheduler.start().catch((error) => {
    options.logger.warn("[codez-data-size] scheduler start failed", error);
  });
}

export function stopDesktopCodezDataSizeTelemetry(): void {
