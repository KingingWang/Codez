...
function purgeExpired(now: number): void {
  for (const [key, entry] of latest) {
    if (now - entry.receivedAt > CODEZ_MCP_RESOURCE_SAMPLE_INTERVAL_MS * 2) latest.delete(key);
  }
}
...
export function ingestMcpResourceSamples(
  ...
  const parsed = codezMcpResourceSamplesSchema.safeParse(raw);
  ...
  sample(context) {
    ...
    for (const [key, group] of groupSamples()) {
      recordExternalAppResourceSample({
        ...
        intervalMs: CODEZ_MCP_RESOURCE_SAMPLE_INTERVAL_MS,
