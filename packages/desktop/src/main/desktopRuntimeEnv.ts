...
import type { ConnectOptions } from "@codez/server/remote";
import { listSSHConfigAliasesFromLocalConfig } from "@codez/services/node";
import { DEV_HELPER_APP_NAME, HELPER_APP_NAME } from "@codez/codez-cua/broker/helperConstants";
import {
  CODEZ_APP_VERSION_ENV,
  CODEZ_AGENT_RUNTIME,
  CODEZ_DYNAMIC_WORKFLOW_MODE_ENV,
  CODEZ_ENV,
  CODEZ_PRODUCT_FLAVOR,
  CODEZ_RUNTIME_ENV_KEY,
  CODEZ_VERSION,
  buildCodezToolEnvPassthroughEnv,
...
import {
  getAppConfigDir,
  getDataBaseDir,
  CODEZ_CUA_BUNDLED_HELPER_APP_PATH_ENV,
...
export const desktopRuntimeEnv: CodezRuntimeEnv = isLocalDevelopmentRuntime
  ? "development"
  : "production";
// 身份看编译期 flavor 而不是 CODEZ_ENV：CODEZ_PREVIEW_IDENTITY=1 的生产后端构建同样是 Preview，
// 需要独立的应用名、Electron 数据目录和 Helper 安装子目录才能与正式版并排运行。
const isPreviewPackagedRuntime = !isLocalDevelopmentRuntime && CODEZ_PRODUCT_FLAVOR === "preview";
...
// e2e 运行的是生产构建，默认会和本机正式版 Codez 共用 app name / userData，
// 触发 Electron 单实例锁后只激活已有窗口，Chromedriver 无法接管测试进程。
// 这里允许测试显式隔离运行时身份，正常桌面/远控路径保持原来的默认值。
export const runtimeApplicationName =
  readRuntimeEnvOverride("CODEZ_DESKTOP_APPLICATION_NAME") ??
  resolveDesktopApplicationName(CODEZ_PRODUCT_FLAVOR, !isLocalDevelopmentRuntime);
// Electron 的 app.getPath("home") 不一定跟随测试进程里的 HOME 覆盖。
// e2e 默认工作区依赖 home 路径，因此提供显式覆盖，避免测试写到开发者真实 ~/CodezProject。
export const runtimeHomePath = readRuntimeEnvOverride("CODEZ_DESKTOP_HOME_DIR");
...
export const shouldUseElectronDefaultUserDataPath = isTruthyRuntimeEnvOverride(
  "CODEZ_DESKTOP_USE_ELECTRON_DEFAULT_USER_DATA",
);
export const runtimeUserDataPath =
  readRuntimeEnvOverride("CODEZ_DESKTOP_USER_DATA_DIR") ??
...
export const runtimeSessionDataPath =
  readRuntimeEnvOverride("CODEZ_DESKTOP_SESSION_DATA_DIR") ??
...
export async function isDockerDaemonAvailable(): Promise<boolean> {
  const { isDockerAvailable } = await import("@codez/server/remote");
  return isDockerAvailable();
}

export async function listAvailableWSLDistros() {
  const { listWSLDistros } = await import("@codez/server/remote");
  return listWSLDistros();
}

export async function listAvailableDockerContainers() {
  const { listDockerContainers } = await import("@codez/server/remote");
...
export function loadHostProcessEnvFromLocalFiles(): Record<string, string> {
  if (isElectronAppPackaged()) {
    ...
    return { CODEZ_TELEMETRY_RUNTIME_DISTRIBUTION: "packaged" };
  }
  ...
  return applySelectedCodezEnvLinks(merged);
}
...
function resolveAvailableDevelopmentMockCdnDir(): string | undefined {
  ...
  const releaseDir = join(mockCdnDir, "releases", CODEZ_VERSION);
...
function shouldUseRemoteCdnInDevelopment(localEnv: LocalRuntimeEnv = {}): boolean {
  return isTruthyEnvFlag(resolveEnvValue("CODEZ_DEV_REMOTE_ASSET_USE_CDN", localEnv));
}
...
function resolveRemoteCdnBaseUrls(
  ...
  const raw = resolveEnvValue("CODEZ_REMOTE_ASSET_CDN_BASE_URL", localEnv);
...
export function resolveCodezEndpointEnvBaseOrigin(
  localEnv: LocalRuntimeEnv = {},
): string | undefined {
  const buildEnv = readProductEndpointEnv();
  // main 进程临时验证更新服务时不会重新写 .env，命令行传入的 endpoint 必须优先于本地文件。
  return (
    process.env["CODEZ_BASE_URL"]?.trim() ||
    process.env["CODEZ_ENDPOINT_ORIGIN"]?.trim() ||
    localEnv.CODEZ_BASE_URL?.trim() ||
    localEnv.CODEZ_ENDPOINT_ORIGIN?.trim() ||
    buildEnv.CODEZ_BASE_URL?.trim() ||
    buildEnv.CODEZ_ENDPOINT_ORIGIN?.trim() ||
    undefined
  );
}
...
function applySelectedCodezEnvLinks(env: Record<string, string>): Record<string, string> {
  const endpointEnv = {
    ...readProductEndpointEnv(),
    ...env,
    CODEZ_ENV,
  ...
  return {
    ...
    ...env,
    CODEZ_BASE_URL: env.CODEZ_BASE_URL ?? resolveRuntimeCodezEndpointOrigin(endpointEnv),
...
function resolveHostProcessNodeEnv(): CodezRuntimeEnv {
  return desktopRuntimeEnv;
}

function resolveRemoteAssetCacheDir(localEnv: LocalRuntimeEnv = {}): string {
  const overrideCacheDir = resolveEnvValue("CODEZ_REMOTE_ASSET_CACHE_DIR", localEnv);
  if (overrideCacheDir) {
...
function resolveBundledCodezAgentBinaryPath(): string | undefined {
  const runtime = CODEZ_AGENT_RUNTIME;
...
export function resolveBundledGlmBinaryPath(): string | undefined {
  return resolveBundledCodezAgentBinaryPath();
}
...
function resolveHostProcessBinaryEnv(
  ...
  // Codez Agent 与 app 协议适配强绑定版本，生产包必须优先使用随包携带的固定 runtime。
...
function resolveDynamicWorkflowModeHostEnv(options: {
  ...
  if (!options.isPackaged) {
    ...
    return mode ? { [CODEZ_DYNAMIC_WORKFLOW_MODE_ENV]: mode } : {};
  }
  if (options.isPreview) {
    return { [CODEZ_DYNAMIC_WORKFLOW_MODE_ENV]: "alwaysOn" };
  }
  return {};
}
...
export function buildHostProcessEnv(hostProcessLocalEnv: Record<string, string>) {
    ...
    process.platform !== "darwin"
      ...
      : packagedDesktop
        ...
        : // Truthy grammar must match the producer's isUnsignedHelperLocalDevRequested
          ...
          ["1", "true", "on"].includes(
              rawInheritedEnv.CODEZ_CUA_HELPER_ALLOW_UNSIGNED_LOCAL?.trim().toLowerCase() ?? "",
            )
          ? rawInheritedEnv.CODEZ_CUA_BUNDLED_HELPER_APP_PATH?.trim() ||
            join(
              rawInheritedEnv.CODEZ_HOME?.trim() || join(homedir(), ".codez"),
              "computer-use",
              "dev",
              DEV_HELPER_APP_NAME,
            )
  ...
  const agentTelemetryEnv = readCodezAgentTelemetryEnv(rawInheritedEnv);
  ...
  const inheritedEnv = applySelectedCodezEnvLinks({
    ...sanitizeCodezRuntimeEnv(rawInheritedEnv),
    ...buildCodezToolEnvPassthroughEnv(rawInheritedEnv),
  });
  ...
  if (packagedDesktop) {
    delete inheritedEnv.CODEZ_CUA_HELPER_ALLOW_UNSIGNED_LOCAL;
  }
  const dynamicWorkflowModeHostEnv = resolveDynamicWorkflowModeHostEnv({
    inheritedValue: rawInheritedEnv[CODEZ_DYNAMIC_WORKFLOW_MODE_ENV],
  ...
  delete inheritedEnv[CODEZ_DYNAMIC_WORKFLOW_MODE_ENV];
  ...
  return {
    ...
    ...agentTelemetryEnv,
    // Codez 运行时不再使用 NODE_ENV；它会被用户 shell、包管理器和测试框架复用。
    // 这里显式下发 CODEZ_RUNTIME_ENV，并在继承环境里清掉 NODE_ENV，避免 host/agent/Bash 被污染。
    [CODEZ_RUNTIME_ENV_KEY]: resolveHostProcessNodeEnv(),
    // 显式注入编译期产品身份，保证主进程与 host 的身份语义一致；地址独立解析。
    // inheritedEnv 从 .env 通用变量补齐 Codez/ZAI 链接，未覆盖时统一使用线上默认值。
    CODEZ_ENV,
    ...(isCodexDesktop
      ? {
          CODEZ_DESKTOP_RUNTIME: "codex",
          CODEZ_CODEX_BRIDGE_HOME: join(dataBaseDir, "bridge"),
          CODEZ_HOME: join(dataBaseDir, ".codez"),
          CODEZ_CUA_HELPER_INSTALL_VARIANT: "codex",
        }
      : {}),
    // Preview 与生产版共享任务、配置和凭据，但不同版本的 Helper 不能互相覆盖或触发降级保护。
    // 只隔离 computer-use 下的运行组件，不改写 CODEZ_HOME / CODEZ_DATA_BASE_DIR 业务数据根。
    ...(isPreviewPackagedRuntime ? { CODEZ_CUA_HELPER_INSTALL_VARIANT: "preview" } : {}),
    ...
    [CODEZ_APP_VERSION_ENV]: CODEZ_VERSION,
    ...(dataBaseDir !== homedir() ? { CODEZ_DATA_BASE_DIR: dataBaseDir } : {}),
    ...(windowsAppInstallDir ? { [CODEZ_WINDOWS_APP_INSTALL_DIR_ENV]: windowsAppInstallDir } : {}),
    ...(bundledCuaHelperAppPath
      ? { [CODEZ_CUA_BUNDLED_HELPER_APP_PATH_ENV]: bundledCuaHelperAppPath }
      : {}),
    ...(!isCodexDesktop && resolvedGlmBinaryPath ? { GLM_BINARY_PATH: resolvedGlmBinaryPath } : {}),
    ...(resolvedLarkCliBinaryPath ? { CODEZ_LARK_CLI_BINARY: resolvedLarkCliBinaryPath } : {}),
  };
}
