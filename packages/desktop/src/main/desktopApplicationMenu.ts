...
import { readCodezStdioTapDevState } from "@codez/services/node";
...
import {
  HELP_TOGGLE_CODEZ_STDIO_TAP_MENU_ID,
  HELP_TOGGLE_DEV_TOOLS_MENU_ID,
} from "./desktopCommandHandlers.js";

const HELP_CODEZ_ENDPOINT_PRODUCTION_MENU_ID = "help.codez-endpoint.production";
...
export function updateCodezStdioTapDevMenuState() {
  const menu = Menu.getApplicationMenu();
  const item = menu?.getMenuItemById(HELP_TOGGLE_CODEZ_STDIO_TAP_MENU_ID);
  if (!item) {
    return;
  }
  const state = readCodezStdioTapDevState();
...
function buildApplicationMenuTemplate(options: {
  ...
  codezEndpointSelection?: "production" | "test" | "custom";
  ...
  const stdioTapState = readCodezStdioTapDevState();
    ...
    },
    {
      ...
      submenu: [
        ...
        ...(isLocalDevelopmentRuntime && stdioTapState.visible
          ? [
              {
                id: HELP_TOGGLE_CODEZ_STDIO_TAP_MENU_ID,
                label: getLabel(desktopMenuMessageIds.helpToggleCodezStdioTap),
                type: "checkbox" as const,
                checked: stdioTapState.enabled,
                click: () =>
                  void options.executeDesktopCommand(DesktopCommandIds.ToggleCodezStdioTapDevProxy),
              },
              { type: "separator" as const },
            ]
          : []),
        ...(CODEZ_ENV === "test"
          ? [
              {
                label: getLabel(desktopMenuMessageIds.helpCodezEndpoint),
                submenu: [
                  {
                    id: HELP_CODEZ_ENDPOINT_PRODUCTION_MENU_ID,
                    label: getLabel(desktopMenuMessageIds.helpCodezEndpointProduction),
                    type: "radio" as const,
                    checked: (options.codezEndpointSelection ?? "production") === "production",
                    click: () =>
                      void options.executeDesktopCommand(
                        DesktopCommandIds.SetCodezEndpointProduction,
                      ),
                  },
                  { type: "separator" as const },
                  {
                    label: getLabel(desktopMenuMessageIds.helpCodezEndpointCustom),
                    click: () =>
                      void options.executeDesktopCommand(DesktopCommandIds.SetCodezEndpointCustom),
                  },
                  {
                    label: getLabel(desktopMenuMessageIds.helpCodezEndpointReset),
                    click: () =>
                      void options.executeDesktopCommand(DesktopCommandIds.ResetCodezEndpoint),
                  },
                ],
              },
              { type: "separator" as const },
  ...
  Menu.setApplicationMenu(
    Menu.buildFromTemplate(
      buildApplicationMenuTemplate({
        ...
        codezEndpointSelection: options.codezEndpointSelection,
