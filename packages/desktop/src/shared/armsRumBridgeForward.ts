...
type PatchedIpcRenderer = IpcRenderer & { __codezArmsIpcPatched?: boolean };
...
type ArmsEventBridgeLike = {
  ...
  __codezArmsBridgeForwardPatched?: boolean;
};
...
function patchArmsEventBridgeSend(bridge: ArmsEventBridgeLike): void {
  if (bridge.__codezArmsBridgeForwardPatched) {
    return;
  }
  ...
  bridge.__codezArmsBridgeForwardPatched = true;
}
...
export function installArmsRumBridgeIpcForward(ipc: IpcRenderer): void {
  ...
  if (patched.__codezArmsIpcPatched) {
    return;
  }
  ...
  patched.__codezArmsIpcPatched = true;
}
