...
function resolveScheduledAt(automation: CodezAutomation, now: number): number {
...
async function handleClaimed(automation: CodezAutomation, now: number): Promise<void> {
...
function postDispatchRequest(
  ...
  runId: string,
  fixedSelection?: CodezAutomationRun["modelSelection"],
): void {
 ...
 */
async function handleOffPeakClaimed(task: CodezOffPeakTask, now: number): Promise<void> {
...
async function settleDispatchResult(
  ...
  const trigger: CodezAutomationTrigger =
