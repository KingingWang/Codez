...
import { resolveCodezEndpointOrigin, pickProductEndpointEnv } from "@codez/shared/codezEndpoint";
...
function resolveCodezEnv(value: string | undefined): "test" | "production" {
...
function createE2EUIRendererCoveragePlugin(repoRoot: string): Plugin {
  ...
  return {
    name: "codez:e2e-ui-source-coverage",
    enforce: "pre",
...
export default defineConfig(({ mode }) => {
  // `.env*` 只提供链接常量；当前产品环境由启动脚本或 CI 注入 CODEZ_ENV。
  const env = { ...loadEnv(mode, "../..", ""), ...process.env };
  const repoRoot = resolve(__dirname, "../..");
  const codezEnv = resolveCodezEnv(env.CODEZ_ENV);
  // 安装包身份与后端环境分轴；renderer 用它决定是否展示更新入口。
  const codezProductFlavor = resolveDesktopProductFlavor({
    ...process.env,
    ...env,
    CODEZ_ENV: codezEnv,
  });
  const e2eCoverageEnabled =
    env.CODEZ_E2E_COVERAGE === "1" || process.env.CODEZ_E2E_COVERAGE === "1";
  const e2eStoreBridgeEnabled =
    env.VITE_CODEZ_E2E_STORE_BRIDGE === "1" || process.env.VITE_CODEZ_E2E_STORE_BRIDGE === "1";
  const codezEndpointOrigin = resolveCodezEndpointOrigin({
    env: codezEnv,
    envBaseOrigin: env.CODEZ_BASE_URL ?? env.CODEZ_ENDPOINT_ORIGIN,
  });
  ...
  return {
    ...
    define: {
      ...
      __CODEZ_LOCAL_DEVELOPMENT_RUNTIME__: JSON.stringify(mode !== "production"),
      "import.meta.env.VITE_CODEZ_BASE_URL": JSON.stringify(codezEndpointOrigin),
      // 兼容旧 renderer 读取名；新代码统一读 VITE_CODEZ_BASE_URL。
      "import.meta.env.VITE_CODEZ_ENDPOINT_ORIGIN": JSON.stringify(codezEndpointOrigin),
      ...
      // E2E store bridge 只能由 WDIO 专用变量打开，避免把 CODEZ_ENV=test 产品环境误当成测试运行态。
      "import.meta.env.VITE_CODEZ_E2E_STORE_BRIDGE": JSON.stringify(
