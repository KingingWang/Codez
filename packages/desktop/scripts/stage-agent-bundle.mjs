// Agent bundle 的暂存动作：把 apps/codez-cli/packages/cli/dist/codez.cjs 放进
...
// desktopRuntimeEnv.ts 的 resolveBundledCodezAgentBinaryPath() 解析，候选**只有**
...
export const AGENT_BUNDLE_SOURCE_RELATIVE = "apps/codez-cli/packages/cli/dist/codez.cjs";
...
export function resolveAgentBundlePaths({ repoRoot, platformKey }) {
  ...
  return {
    ...
    glmDir,
    stagedBundlePath: resolve(glmDir, "codez.cjs"),
 ...
 * （codez-agent / codez-acp 等）和旧 meta 会被一并打进安装包（CI 干净检出不会有，本地会）。
 */
