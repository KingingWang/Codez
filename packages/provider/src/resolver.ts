...
import type { completeModelConfigDataSchema } from "@codez/shared/model-config";
import type {
  ...
  resolve(input: ProviderConfigResolverInput): ProviderConfigResolution {
    const accountProviders = new ProviderConfigMap(
      input.accountProviders
        .entries()
        .filter(([providerId]) => input.codezBuiltinProviders.has(providerId))
        .map(([providerId, config]) => [providerId, config.withoutGroup()] as const),
    );
    const concreteBuiltinProviders = input.codezBuiltinProviders.overlay(accountProviders);
    const providerTemplates = input.codezBuiltinProviderTemplates;
    ...
    for (const providerId of resolveProviderOrder(input, effectiveProviders)) {
      ...
      const models = orderedModelIds.map((modelId): ResolvedProviderModel => {
        ...
        const effectiveBuiltinConfig = input.codezBuiltinModelRules.resolve({
          providerId,
          templateId,
          modelId,
          apiType: config.api?.type,
...
function resolveProviderOrder(
  ...
  const builtinIds = input.codezBuiltinProviders
    .keys()
