...
export class ProviderSettingsFacade {
  ...
  getView(): ProviderSettingsView {
    ...
    return createProviderSettingsView({
      ...
      codezBuiltinProviders: snapshot.config.codezBuiltinProviders,
      codezBuiltinProviderTemplates: snapshot.config.codezBuiltinProviderTemplates,
  ...
  resolveModelConfig(input: ResolveModelConfigInput): ModelConfigResolution {
    ...
    if (!("personalConfig" in input)) {
      const modelRules = ModelConfigRules.composeEffective(
        snapshot.config.codezBuiltinModelRules,
    ...
    const inheritedConfig = snapshot.config.codezBuiltinModelRules.resolve({
      providerId: input.providerId,
      templateId: provider.templateId,
      modelId: input.modelId,
      apiType: provider.config.api?.type,
...
function createProviderSettingsView(input: {
  revision: number;
  codezBuiltinProviders: ProviderRegistryServiceSnapshot["config"]["codezBuiltinProviders"];
  codezBuiltinProviderTemplates: ProviderRegistryServiceSnapshot["config"]["codezBuiltinProviderTemplates"];
  ...
  return Object.freeze({
    ...
    providerTemplates: Object.freeze(
      (input.codezBuiltinProviderTemplates ?? ProviderTemplateMap.empty())
        .entries()
