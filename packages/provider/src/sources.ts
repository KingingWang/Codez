...
export interface ProviderConfigSnapshot {
  ...
  readonly codezBuiltinProviders: ProviderConfigMap;
  readonly codezBuiltinProviderTemplates: ProviderTemplateMap;
  readonly personalProviders: ProviderConfigMap;
  readonly codezBuiltinModelRules: ModelConfigRules;
...
export function createFailClosedAccountProviderConfigSnapshot(
  ...
  const unentitledProviders = new ProviderConfigMap(
    config.codezBuiltinProviders.entries().flatMap(([providerId, provider]) =>
  ...
  return createAccountProviderConfigSnapshot(config.codezBuiltinRevision, unentitledProviders);
}

export function createAccountProviderConfigSnapshot(
  basedOnCodezBuiltinRevision: string,
  providers: ProviderConfigMap,
  states?: AccountProviderStates,
): AccountProviderConfigSnapshot {
  return Object.freeze({
    revision: `account:${JSON.stringify([basedOnCodezBuiltinRevision, providers.toJSON(), states])}`,
...
const EMPTY_ACCOUNT_PROVIDER_CONFIG_SNAPSHOT: AccountProviderConfigSnapshot = Object.freeze({
  ...
  basedOnCodezBuiltinRevision: "uninitialized",
