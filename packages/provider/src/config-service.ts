...
import type { ModelSelection } from "@codez/shared/model-selection";
...
export interface ProviderConfigServiceDependencies {
  readonly codezBuiltinSource: ProviderSource<ProviderConfigLayerSnapshot>;
...
export class ProviderConfigService implements ProviderSource<ProviderConfigSnapshot> {
  ...
  constructor(dependencies: ProviderConfigServiceDependencies) {
    this.#codezBuiltinSource = dependencies.codezBuiltinSource;
    this.#personalRepository = dependencies.personalRepository;
    this.#sourceDisposers = [
      this.#codezBuiltinSource.onDidChange((reason) => this.#emit(`codezBuiltin:${reason}`)),
  ...
  async read(): Promise<ProviderConfigSnapshot> {
    ...
    const [codezBuiltin, personal] = await Promise.all([
      this.#codezBuiltinSource.read(),
      this.#personalRepository.read(),
    ]);
    return Object.freeze({
      revision: JSON.stringify([codezBuiltin.revision, personal.revision]),
      codezBuiltinRevision: codezBuiltin.revision,
      personalRevision: personal.revision,
      codezBuiltinProviders: codezBuiltin.providers,
      codezBuiltinProviderTemplates: codezBuiltin.providerTemplates ?? ProviderTemplateMap.empty(),
      personalProviders: personal.providers,
      codezBuiltinModelRules: codezBuiltin.models,
  ...
  async savePersonalProviderOverlay(
    ...
    const codezBuiltin = await this.#codezBuiltinSource.read();
    return this.#updatePersonal((current) => {
      assertMembershipCurrent(membership, providerId, current);
      const builtin = codezBuiltin.providers.get(providerId);
      const currentPersonal = current.providers.get(providerId);
      const currentEffectiveProviders = codezBuiltin.providers.overlay(current.providers);
      ...
      if (builtin?.access?.type === "zhipu-account" && config.access !== undefined) {
        ...
        throw new Error(
          `固定 Account Provider 的 Access 只能由 Codez Built-in Config 声明: ${providerId}`,
        );
      }
      ...
      const membershipBaseline =
        builtin ?? resolveTemplateBaseline(codezBuiltin, current.providers, providerId);
      ...
      const nextEffectiveProviders = codezBuiltin.providers.overlay(providers);
  ...
  async createPersonalProvider(
    ...
    const template = templateId ? codezBuiltin.providerTemplates?.get(templateId) : undefined;
    ...
    await this.#updatePersonal((current) => {
      const occupied = new Set([...codezBuiltin.providers.keys(), ...current.providers.keys()]);
      const providerId = nextPersonalProviderId(occupied, templateId);
      createdProviderId = providerId;
      const effectiveProviders = resolvePersonalProviderBaselines(codezBuiltin, current.providers);
  ...
  async reorderPersonalProviders(
    ...
    return this.#updatePersonal((current) => ({
      ...
      models: current.models,
      providerOrder: normalizeProviderOrder(codezBuiltin.providers, current.providers, providerIds),
    }));
  }
  ...
  async reorderPersonalModels(
    ...
    return this.#updatePersonal((current) => {
      ...
      const builtinProvider =
        codezBuiltin.providers.get(providerId) ??
        resolveTemplateBaseline(codezBuiltin, current.providers, providerId);
      const provider = writableProviderOverlay(codezBuiltin, current, providerId);
  ...
  async addPersonalModel(
    ...
    return this.#updatePersonal((current) => {
      ...
      const provider = writableProviderOverlay(codezBuiltin, current, normalizedProviderId);
      const builtinModelIds =
        membership?.inheritedModelIds ??
        resolveProviderBuiltinModelIds(codezBuiltin, current.providers, normalizedProviderId);
  ...
  async setPersonalModelEnabled(
    ...
    const builtin = await this.#codezBuiltinSource.read();
    return this.#updatePersonal((current) => {
...
function normalizeProviderOrder(
  _codezBuiltinProviders: ProviderConfigMap,
...
function appendCurrentProviderOrder(
  codezBuiltinProviders: ProviderConfigMap,
  personalProviders: ProviderConfigMap,
  currentOrder: readonly ProviderId[] | undefined,
  addedProviderId: ProviderId,
): ProviderId[] {
  const current = normalizeProviderOrder(
    codezBuiltinProviders,
    personalProviders,
    currentOrder ?? [],
  );
  return normalizeProviderOrder(codezBuiltinProviders, personalProviders, [
...
function resolvePersonalProviderBaselines(
  ...
  const personal = personalProviders.mapConfigs((config, _id, rule) => {
    const template = rule.templateId
      ? codezBuiltin.providerTemplates?.get(rule.templateId)
      : undefined;
    return template ? template.config.overlay(config) : config;
  });
  return codezBuiltin.providers.overlay(personal);
}
