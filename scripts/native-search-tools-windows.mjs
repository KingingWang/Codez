...
export function buildNativeSearchToolsWindows({
  ...
  const workDir = mkdtempSync(join(tmpdir(), "codez-native-search-build-"));
  ...
  console.log("==> Codez native search build");
  ...
  try {
    ...
    run(
      ...
      [
        ...
        `-DCODEZ_BUILD_PATH=${toCmakePath(workDir)}`,
        `-DCODEZ_UGREP_OUTPUT_DIR=${toCmakePath(cmakeOutputDir)}`,
        `-DCODEZ_UGREP_SOURCE_DIR=${toCmakePath(sources.ugrep)}`,
        `-DCODEZ_PCRE2_SOURCE_DIR=${toCmakePath(sources.pcre2)}`,
        `-DCODEZ_ZLIB_SOURCE_DIR=${toCmakePath(sources.zlib)}`,
        `-DCODEZ_BZIP2_SOURCE_DIR=${toCmakePath(sources.bzip2)}`,
        `-DCODEZ_ZSTD_SOURCE_DIR=${toCmakePath(sources.zstd)}`,
        `-DCODEZ_BROTLI_SOURCE_DIR=${toCmakePath(sources.brotli)}`,
      ],
      { env, quiet },
    );
    run(
