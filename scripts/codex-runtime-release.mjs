...
export function releaseIdentity(env) {
  if (env.GITHUB_REPOSITORY !== "KingingWang/ZCode")
  ...
  return {
    ...
    sha: env.GITHUB_SHA,
    tag: `zcode-codex-build-${env.GITHUB_RUN_ID}-${env.GITHUB_SHA.slice(0, 12)}`,
  ...
  for (const [os, extensions] of Object.entries(formats)) {
    for (const arch of ["x64", "arm64"]) {
      ...
      const matches = folders.filter((entry) =>
        new RegExp(`^zcode-codex-${key}-(un)?signed$`).test(entry.name),
      );
      ...
      for (const line of lines) {
        const match = /^([a-f0-9]{64})  (ZCode[ .]Codex-[A-Za-z0-9 ._+-]+)$/.exec(line);
...
export async function publishCodexRelease({
  ...
  if (!release) {
    const notes = [
      "Independent community ZCode Codex build; not an official OpenAI product.",
