...
function printUsageAndExit() {
  process.stderr.write(
    [
      "Usage: codez-stdio-tap.mjs --workspace-key <key> [--log-dir <dir>] -- <command> [...args]",
      "",
    ].join("\n"),
  );
  process.exit(2);
...
function parseArgs(argv) {
  ...
  let workspaceKey = "";
  let logDir = process.env.CODEZ_STDIO_TAP_LOG_DIR?.trim() || "";
  ...
  return {
    ...
    logDir: logDir || join(homedir(), ".codez", "v2", "dev", "stdio-traffic"),
    workspaceKey,
  };
}
...
child.on("error", (error) => {
  ...
  process.stderr.write(`[codez-stdio-tap] spawn failed: ${error.message}\n`);
});
