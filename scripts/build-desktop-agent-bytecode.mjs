...
const defaultEntryPath = join(repoRoot, "apps/codez-cli/packages/cli/dist/codez.cjs");
...
export async function buildDesktopAgentBytecode({
  ...
  if (env.CODEZ_E2E_COVERAGE === "1") throw new Error("coverage 构建不能启用字节码试验");
  const directory = dirname(entryPath);
  const temporary = join(directory, `.bytecode-${randomUUID()}`);
  const loaderPath = join(directory, "codez.bytecode.cjs");
  try {
    ...
    const bytecodeFile = `codez.bytecode-${metadata.bytecodeSha256}.jsc`;
    ...
    const runtimeFile = `codez.bytecode-runtime-${runtimeHash}.cjs`;
