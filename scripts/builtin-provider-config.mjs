...
export async function resolveBuiltinProviderBuildEnvironment({
  ...
  if (!value?.trim()) {
    ...
    for (const file of files) {
      ...
      if (parsed.CODEZ_ENV !== undefined) value = parsed.CODEZ_ENV;
    }
  }
  const normalized = value?.trim().toLowerCase() || "test";
  if (normalized !== "test" && normalized !== "production") {
    throw new Error(`Invalid CODEZ_ENV for Built-in Provider build: ${normalized}`);
  }
  return normalized;
}
...
export async function loadBuiltinProviderConfig({ root = repositoryRoot, env = process.env } = {}) {
  ...
  const sourcePath = resolve(
    root,
    env.CODEZ_BUILTIN_PROVIDER_CONFIG_FILE?.trim() || "config/provider/codez-builtin.json",
  );
  try {
    ...
    const { decodeCodezBuiltinRelease } = await tsImport(
      pathToFileURL(resolve(repositoryRoot, "packages/provider-node/src/codez-builtin-release.ts"))
        .href,
      import.meta.url,
    );
    decodeCodezBuiltinRelease(JSON.parse(content));
...
export async function stageBuiltinProviderConfig({ directory, ...options }) {
  ...
  await writeFile(resolve(directory, "codez-builtin.json"), config.content, "utf8");
  return config;
}
