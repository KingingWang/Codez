...
assert.ok(archive, "Usage: node scripts/codez-distribution-smoke.mjs <archive.tar.gz>");
const directory = await realpath(await mkdtemp(join(tmpdir(), "codez-release-smoke-")));
const root = join(directory, "codez");
const runner = join(root, "bin/codez.mjs");
...
try {
  ...
  await writeFile(
    runtimeCheck,
    'import { runTui } from "@codez/tui"; if (typeof runTui !== "function") throw new Error("Missing TUI export"); console.log("tui-runtime-ok");',
  );
  ...
  await until(
    () => /Codez/.test(screen) && /(?:登录|\/login|输入提示词|Type a prompt)/i.test(screen),
