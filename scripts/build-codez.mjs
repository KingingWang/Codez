...
import {
  ...
  stageTuiRuntime,
} from "./codez-distribution/assets.mjs";
import { installScriptSource } from "./codez-distribution/installer.mjs";

const root = resolve(import.meta.dirname, "..");
const defaultOutDir = resolve(root, "dist", "codez");
const defaultBaseUrl = (await loadEndpointEnv()).CODEZ_DIST_BASE_URL?.trim() || "";
const packageDirName = "codez";
const usage = `Usage:
  pnpm build:codez
  node scripts/build-codez.mjs --skip-build
  node scripts/build-codez.mjs --version 3.3.3-dev.1
  node scripts/build-codez.mjs --out-dir dist/codez
  node scripts/build-codez.mjs --base-url http://host/codez/deps/codez/
...
Options:
  ...
  --out-dir <path>    Output directory. Defaults to dist/codez.
...
function run(command, args, options = {}) {
  console.log(`[codez] ${commandText(command, args)}`);
...
async function buildOutputs(skipBuild) {
  if (skipBuild) {
    console.log("[codez] skipping build; reusing existing outputs");
    return;
  }

  run("pnpm", ["--filter", "@codez/cli...", "build"]);
  await rm(resolve(root, "packages", "server", "dist"), {
    force: true,
    recursive: true,
  });
  run("pnpm", ["--filter", "@codez/server", "build"]);
  run("pnpm", ["--filter", "@codez/web", "build"]);
}

async function stageCodezPackage({ packageRoot, version }) {
  const webDist = resolve(root, "packages", "web", "dist");
  const serverDist = resolve(root, "packages", "server", "dist");
  const agentBundle = resolve(root, "apps", "codez-cli", "packages", "cli", "dist", "codez.cjs");
  const agentProvider = resolve(root, "apps/codez-cli/packages/cli/dist/provider");
  ...
  await assertFile(resolve(agentProvider, "codez-builtin.json"), "Agent provider config");
  ...
  await cp(agentBundle, resolve(packageRoot, "agent", "codez.cjs"));
  // TUI 入口通过真正的 CLI 路径定位伴随配置；只复制 JS 会在仓库外启动失败。
  await cp(agentProvider, resolve(packageRoot, "agent/provider"), { recursive: true });
  await cp(
    resolve(root, "apps/codez-cli/packages/cli/dist/THIRD-PARTY-NOTICES.md"),
    resolve(packageRoot, "agent/THIRD-PARTY-NOTICES.md"),
  );
  await chmod(resolve(packageRoot, "agent", "codez.cjs"), 0o755);
  ...
  const runner = resolve(packageRoot, "bin", "codez.mjs");
  await cp(resolve(root, "scripts/codez-distribution/runner.mjs"), runner);
...
async function main() {
  ...
  if (!options.help && !options.baseUrl)
    throw new Error("Configure CODEZ_DIST_BASE_URL in .env or pass --base-url");
  if (options.help) {
    console.log(usage);
    return;
  }
  ...
  if (!version || typeof version !== "string") {
    throw new Error("Unable to resolve Codez version.");
  }
  ...
  console.log(`[codez] release directory: ${outDir}`);
  console.log(`[codez] tarball: ${tarball}`);
  console.log(`[codez] sha256: ${sha256}`);
}
