...
const agentEntry = join(root, "agent", "codez.cjs");

function usage() {
  return `Usage:
  codez --web [--host <host>] [--port <port>] [--workspace <path>] [--open|--no-open] [--token <token>|--no-token]
  codez --version
`;
}
...
async function serve(options) {
  ...
  const child = spawn(process.execPath, [serverEntry], {
    ...
    env: {
      ...process.env,
      PORT: String(port),
      CODEZ_AGENT_SERVER_ARGS_JSON: JSON.stringify([agentEntry, "app-server", "--stdio"]),
      CODEZ_AGENT_SERVER_COMMAND: process.execPath,
      CODEZ_SERVER_HOST: options.host,
      CODEZ_SERVER_WORKSPACE: options.workspace,
      CODEZ_WEB_STATIC_ROOT: webRoot,
      // 显式关闭 token 时必须清空继承值，否则 --no-token 仍会开启后端鉴权。
      CODEZ_SERVER_AUTH_TOKEN: token,
    },
  ...
  console.log("");
  console.log("Codez Web is running");
...
try {
  ...
  if (argv.length === 1 && ["--version", "-v"].includes(argv[0])) {
    ...
    if (argv.length === 1 && ["--help", "-h"].includes(argv[0])) {
      console.log("Web mode: codez --web [options] (codez --web --help for details)\n");
    }
